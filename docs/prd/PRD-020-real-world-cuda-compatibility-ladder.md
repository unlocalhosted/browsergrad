...
- `NVIDIA/cuda-samples` at `b7c5481`: `357` kernel definitions, `328`
  compile/codegen-ok (`327` strict direct WGSL plus `1` host-orchestrated
  ...
  pointer/local-array `printf` debug arguments, with `26` hard gaps.
  Main failures:
...
- The current live aggregate gate is `1002/1038` compile/codegen-ok across the four
  pinned corpora: CUDA-120 `240/240`, `cuda-samples` `328/357`, `llm.c`
...
- `NVIDIA/cuda-samples` at `b7c5481` remains `357` total kernel definitions,
  `>=328` compile/codegen-ok, and `<=26` hard gaps.
