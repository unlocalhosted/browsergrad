...
| **Eager Grad dtype/view semantics** | `bf16`/`bfloat16` reject before allocation rather than renaming float32 storage; `torch.bfloat16` stays distinct and unsupported. `nn.Linear`/`nn.Embedding` enforce their exact float32 parameter profile. `Tensor.contiguous()` preserves identity only for C-contiguous storage and otherwise creates an owning C-order copy with dtype-preserving backward. `Tensor.detach()` returns a distinct tensor sharing exact storage, dtype, and layout while severing autograd history. |
