...
| Evidence quality | Strong for narrow authorities; release incomplete | Bounded build records, exact ABI reviews, two-build and full-distribution reproducibility, separate Worker identities, strict source observations, artifacts, semantic candidates, shared lowering, real-device evidence, and opaque backend/release compositions remain distinct. External trust, legal approval, and actual production authority issuance remain false. | Run both compositions only with real external authorities; do not collapse build, verifier, compiler-Worker, legal, producer-trust, backend, or release evidence. |
| Feedback speed | Strong engineering loop; intentionally expensive proof lane | The fast gate passes 100 files/801 tests in 15.97 seconds; the complete compiler unit suite passes 106 files/1,655 tests in 16.37 seconds. Two cache-free clean builds remain a concurrent release-evidence lane, while strict and exact eight-case browser matrices take about four minutes each. | Keep ordinary edits on focused/fast paths; run clean builds and complete matrices only when output identities or producer semantics change. |
...
| Delivery truthfulness | Strong | Package admission authenticates exact reproducibility, header, compiler-Worker, verifier-Worker, source, artifact, candidate, shared lowering, CPU, and actual-WebGPU identities. Backend and release compositions are fixture-proved, while current production external trust, legal approval, backend issuance, and release status remain false. | Preserve the separated authority model while installing external evidence and issuing actual authorities. |
