...
Last updated: 2026-07-04T17:51:54Z
...
| Active work item | Fixed active-lane conditional helper-call RHS laziness; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects outside and inside active-lane return-before-barrier lowering; keep probing nested conditional/vector combinations
...
- Perf/tooling:
  ...
  - WebGPU smoke after active-lane conditional helper-call RHS fix is `337/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for helper pointer-array fixtures; hot repeat for the new active-lane conditional helper RHS fixtures is `1.8-4.4ms`, so steady-state remains small
