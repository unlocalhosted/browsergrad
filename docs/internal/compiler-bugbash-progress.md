...
Last updated: 2026-07-04T17:33:38Z
...
| Active work item | Added active-lane pointer-handle rebind left/right/all-inactive probes; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after active-lane pointer-handle rebind probes is `327/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for helper pointer-array fixtures; hot repeat for the new pointer-handle fixtures is `3.8-4.5ms`, so steady-state remains small
