...
Last updated: 2026-07-04T17:04:45Z
...
| Active work item | Added surf3D helper multi-surface guarded-RHS all-inactive probe; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after surf3D multi-surface guarded-RHS all-inactive probe is `312/0/0`; slowest cases are still pre-existing texture/surface compound pointer-array probes, not the new surf3D guarded-RHS cases; tiny probes run around `3-5ms`, so correctness repeat is useful but hard speedup thresholds can be timing-floor noisy
