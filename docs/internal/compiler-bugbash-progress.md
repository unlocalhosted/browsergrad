...
Last updated: 2026-07-04T17:47:20Z
...
| Active work item | Fixed conditional helper-call RHS laziness; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects outside active-lane lowering; keep probing nested conditional/active-lane combinations
...
- Perf/tooling:
  ...
  - WebGPU smoke after conditional helper-call RHS fix is `333/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for helper pointer-array fixtures; hot repeat for the new conditional helper RHS fixtures is `4.1-4.2ms`, so steady-state remains small
