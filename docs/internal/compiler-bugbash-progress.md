...
Last updated: 2026-07-04T17:57:16Z
...
| Active work item | Fixed conditional helper-call var-init laziness; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects in assignment RHS and local var initializers, outside and inside active-lane return-before-barrier lowering; keep probing nested conditional/vector combinations
...
- Perf/tooling:
  ...
  - WebGPU smoke after conditional helper-call var-init fix is `341/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for helper pointer-array fixtures; hot repeat for the new conditional var-init fixtures is `1.9-4.6ms`, so steady-state remains small
