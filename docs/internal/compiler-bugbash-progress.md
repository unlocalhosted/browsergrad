...
Last updated: 2026-07-04T17:28:03Z
...
| Active work item | Fixed active-lane pointer-array assignment lowering found by texture helper multi-object pointer-array guarded-RHS probes; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after texture helper multi-object pointer-array guarded-RHS probes is `324/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for the new helper pointer-array fixture; hot repeat for the new fixture is `5.1-5.7ms`, so steady-state remains small
