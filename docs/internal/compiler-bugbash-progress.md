...
Last updated: 2026-07-04T17:41:56Z
...
| Active work item | Fixed active-lane loop predication for helper-call compound RHS; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after active-lane pointer-handle loop rebind fix is `330/0/0`; slowest cases are still pre-existing volume/compound pointer-array probes plus cold first-run cost for helper pointer-array fixtures; hot repeat for the new pointer-handle loop fixtures is `4.2-4.5ms`, so steady-state remains small
