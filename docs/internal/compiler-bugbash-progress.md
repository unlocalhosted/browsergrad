...
Last updated: 2026-07-04T18:16:33Z
...
| Active work item | Fixed nested conditional helper-call laziness in call arguments; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects in assignment RHS, local var initializers, device return expressions, and standalone call arguments, including nested expression contexts, outside and inside active-lane return-before-barrier lowering; keep probing vector-lane and condition contexts
...
- Perf/tooling:
  ...
  - WebGPU smoke after nested conditional helper-call argument fix is `349/0/0`; hot repeat for the nested conditional call-argument fixtures is `4.4-4.6ms`, so steady-state remains small
