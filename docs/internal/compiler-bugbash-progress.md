...
Last updated: 2026-07-04T17:14:22Z
...
| Active work item | Added surf1D helper multi-surface guarded-RHS true/false/all-inactive probes; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after surf1D helper multi-surface guarded-RHS probes is `318/0/0`; slowest cases are still pre-existing texture/surface compound pointer-array probes, not the new surf1D/layered/surf3D guarded-RHS cases; tiny probes run around `3-5ms`, so correctness repeat is useful but hard speedup thresholds can be timing-floor noisy
