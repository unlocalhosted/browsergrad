...
Last updated: 2026-07-04T17:00:57Z
...
| Active work item | Added surf3D helper multi-surface guarded-RHS true/false source-selection probes; continue next corpus-shaped probe only if we keep bugbash open |
...
- Perf/tooling:
  ...
  - WebGPU smoke after surf3D multi-surface guarded-RHS probes is `311/0/0`; slowest cases are still pre-existing texture/surface compound pointer-array probes, not the new surf3D guarded-RHS cases; tiny probes run around `3-4ms`, so correctness repeat is useful but hard speedup thresholds can be timing-floor noisy
