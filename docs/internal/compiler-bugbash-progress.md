...
Last updated: 2026-07-02T17:52:45Z
...
| Fixed failure movement | Started from 87 failing real-world/audit cases; current verifier gate is green at src `430/0/0`, dist `430/0/0`; cuda-samples compile/codegen audit now has `0` hard fails; unreachable helper feature/shared/atomic/texture/constant/device-global pollution fixes are green |
| Current focus | Pointer/vector storage correctness, texture/vector conversion, active-lane/control semantics, and hot-loop test speed |
| Active work item | Unreachable helper device-global binding is now scoped to selected-kernel reachability |
...
- Pointer/vector family:
  - shared-byte reinterpret local pointers, typed aliases over byte roots, direct packed-byte shared writes, unaligned typed byte-storage overlays, cuda-samples WMMA shared-memory aliases, and selected-kernel reachability filtering for unused constants/textures/device globals are now green; mixed local pointer-param + generic storage pointer helper still has explicit diagnostic and implementation support remains future work
