...
Last updated: 2026-07-04T18:10:35Z
...
| Active work item | Fixed nested conditional helper-call laziness in device return expressions; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects in assignment RHS, local var initializers, and device return expressions, including nested expression contexts, outside and inside active-lane return-before-barrier lowering; keep probing vector-lane and call-argument combinations
...
- Perf/tooling:
  ...
  - WebGPU smoke after nested conditional helper-call return fix is `347/0/0`; hot repeat for the nested conditional return fixtures is `2.0-4.6ms`, so steady-state remains small
