...
Last updated: 2026-07-04T18:05:10Z
...
| Active work item | Fixed nested conditional helper-call laziness in var initializers; continue next corpus-shaped probe only if we keep bugbash open |
...
- Active-lane/control family:
  ...
  - conditional helper-call RHS branch laziness is now green for pointer/atomic helper side effects in assignment RHS and local var initializers, including nested expression contexts, outside and inside active-lane return-before-barrier lowering; keep probing vector-lane and call-argument combinations
...
- Perf/tooling:
  ...
  - WebGPU smoke after nested conditional helper-call var-init fix is `345/0/0`; hot repeat for the nested conditional var-init fixtures is `3.5-4.5ms`, so steady-state remains small
