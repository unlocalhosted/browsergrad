...
- **Current implementation slice:** Gate 5 tiled-attention semantic baseline and schedule contract
...
| Gate 5 — tiled attention flagship | `partial` | Semantic-core owns the closed frontend-neutral `browsergrad.kernel.attention-forward@1` artifact and canonical dense constructor. It binds verified static rank-4 f32 Q/K/V/output views, exact inverse-square-root scale bits, non-causal or upper-left causal masking, stable-softmax phase meaning, finite-score/state preconditions, a named abs-or-relative comparison policy, disjoint effects, and explicit VJP refusal. Schedule and backend vocabulary are rejected. | Define and verify the separate tiled-attention schedule, CPU oracle, block-tiled K/V-staged WebGPU lowering, causal/non-causal numerical evidence, truthful baseline naming, and correctness/performance records on named devices/browsers. | Semantic-core 17 files/147 tests; build, typecheck, lint, architecture, focused hostile mutations, and packed-release consumer checks pass locally. No CPU, WebGPU, performance, FlashAttention, frontend convergence, or Gate 5 exit claim. |
...
current slice is the independent tiled-attention schedule contract.
...
- Gate 5 is therefore `partial`. The next capability is the separate verified
  online K/V-tile schedule binding this exact logical semantic hash.
...
contract and keeps the row-wise online-softmax path named as a baseline. The
next software transition is the verified tiled-attention schedule: it must bind
the exact logical hash and own physical query/key tiling, cooperative K/V
staging, boundary participation, uniform barriers, and online-softmax tile
recurrence without adding backend or FlashAttention claims.
