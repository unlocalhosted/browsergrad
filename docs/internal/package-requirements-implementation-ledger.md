...
- **Current implementation slice:** Gate 3 canonical Clang-WASM runtime ABI and profile binding
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. Closed AOT/browser profiles, deployment-versus-compilation identity, a producer-neutral local-source request, source-independent available-header profiles, a canonical design-only compiler-runtime ABI manifest, strict browser asset/VFS-pack authorities, common lowering authorization, and an explicit CUDA device-extraction/host-validation contract now exist. Profile wire 2.3 binds the exact ABI resource while semantic compilation contract 1.0 is independently versioned and deployment-neutral. Optional AOT composes around the same request/artifact and owns no second source-intent authority. Same-origin redirect-free acquisition, exact length/hash verification, cache rehash/admission, manifest-complete pack binding, collision-free mount construction, retained-pack accounting, cancellation, and typed failure mint separate opaque host authorities. One input-only build lock pins LLVM/CUTLASS/emsdk/OCI inputs and a deterministic planner materializes its recipe without execution or release authority. Existing source-spelling CuTe adapters remain frozen. All browser assets remain synthetic: the ABI deliberately reports incomplete first-build interface review; no tracked Clang-WASM build, raw-WASM conformance authority, ABI asset binding, worker-owned aggregate lazy-VFS session, worker invocation/result/evidence, browser authorization adapter, browser-produced artifact, real CuTe source proof, dynamic tensor/storage meaning, or browser-local source-compatibility evidence exists. Docker/native contracts remain optional build/CI/reference infrastructure only and never enter the portable browser execution graph. | Bind the ABI resource into the asset/build locks; implement bounded raw-WASM inspection and independently repin the complete generated-import/support-export/table/global/custom-section projection from the first build; build/pin the extractor and reviewed header packs; implement worker-owned aggregate lazy-VFS and worker evidence; emit the canonical artifact from unmodified source in-browser; then prove Gate 2 semantic convergence, dynamic tensor copy, and required real WebGPU execution. | Gate 2 exact-source baseline at `aa605421`; runtime ABI 1 file/34 tests; profile/runtime focused 3 files/70 tests; zero browser-local C++ producer executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.3, producer-neutral request, artifact v3, host-derived request binding, asset-manifest, independently versioned compilation contract 1.0, common authorization, and explicit CUDA semantic-pass authorities exist. Runtime ABI v1 now has one canonical strict-decoded design manifest for exact C/VFS calls, wasm32 memory, framing, status, result, lifecycle, cancellation, closed extension policy, checked VFS memory access, and aggregate opened-file memory; it grants neither observed-WASM nor release authority. Optional AOT composes around the same request and binding; no second source-intent authority remains. Closed manifest-bound VFS packs plus host acquisition/cache/verification/collision-free installation close the pre-worker asset chain with opaque authority and exact resident-pack accounting. Worker file opening remains absent until one worker-owned session meters aggregate WASM-resident copies. Docker may build or compare release assets but is never browser runtime. All current binary/pack identities remain fixtures. | Add the ABI resource as an exact acquired asset; implement bounded raw-WASM inspection and complete independently reviewed interface allowlists; close build-provenance/license authority; implement aggregate lazy-VFS worker session, worker invocation/evidence/browser authorization, real build assets, and one unmodified browser-local fixture. | Runtime ABI 1 file/34 tests; profile/runtime focused 3 files/70 tests; asset acquisition/install 3 files/30 tests; optional Docker 2 files/96 tests; zero worker spawns or browser-local C++ producer executions. |
| Gate 3 Clang-WASM build and distribution | `partial` | Exact input selection pins LLVM 22.1.8, CUTLASS 3.7.0, emsdk 6.0.3, one `linux/amd64` OCI closure, deterministic native-TableGen/Emscripten stages, narrowed link policy, planned outputs, and reviewed upstream notices. The input lock remains permanently non-release authority; its pure planner performs no acquisition, execution, Docker, runtime, or artifact authorization. Canonical VFS-pack writing, host asset acquisition/install, and the canonical design-only runtime ABI are landed. The ABI intentionally leaves generated Emscripten imports, support exports, tables/globals, and custom-section projection empty and release-blocked until independent first-build review. | Bind the ABI resource/hash into build and asset manifests; resolve exact extractor source, CUDA-header redistribution, Linux-sysroot redistribution, distributed-file license mapping, raw-WASM interface evidence with reviewed complete allowlists, and two-clean-build signed reproducibility evidence. Then build/package the extractor, Worker, real VFS packs, and browser lane. | Build-lock 1 file/21 tests; planner 1 file/25 tests; ABI 1 file/34 tests. No real pack, build execution, binary, complete license closure, reproducibility result, Worker, or browser execution exists. |
...
Build-input selection, deterministic command materialization, the canonical
design-only runtime ABI, and profile binding are closed. They are not build or
observed-WASM provenance. Keep the lock release-blocked until exact extractor
source, CUDA/sysroot redistribution, file-level license closure, complete
independently reviewed raw-WASM interface allowlists, and two distinct clean
builds produce signed evidence. Keep optional AOT frozen.

Next add the exact canonical ABI bytes to asset-manifest 1.1 and build-lock
closure, then decode those acquired bytes before Worker authorization. Implement
a bounded linear-time raw-WASM inspector; browser import/export reflection is
not enough. The inspector must prove signatures, memory limits, section/order
framing, table/global/tag/start policy, custom sections, `target_features`, and
forbidden feature opcodes, and it must compare observations to the independent
ABI authority rather than derive permission from the binary.
