...
| Gate 6 — framework convergence | `in-progress` | Public `Tensor.expand` is retired from `CUSTOM` and now emits typed `BROADCAST_TO`. Its shared validator is re-run at construction, CPU realization, symbolic VJP, vmap, ONNX, and tensor-plan scheduling. Closure and symbolic gradients, owning CPU materialization, typed plan admission, and materializing/resident bridge routes are covered. Grad eager expand now shares the same valid/invalid shape and dtype conformance fixture, preserves float16/integer dtypes, and retains its explicitly owning contiguous result without claiming view aliasing. JIT's bounded v1 executable operation registry now binds typed records one-to-one with validators and generates detached public support reporting from those same records. | Migrate the remaining advertised opaque operations through the registry; reconcile the remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate the cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0003; JIT build/typecheck/lint, 4 unit files/24 tests, and 25 integration files/240 tests; Grad build/typecheck/lint/unit plus 34 integration files/324 tests; architecture and release-package gates; architecture freeze narrowed from 36/39 to 35/38 while preserving the exact original-ID partition. The existing kernels actual-device lane proves the `BROADCAST_TO` primitive, but this slice does not claim a new public-surface actual-device record. |
...
Gate 6 is active. `Tensor.expand` is the first advertised operation retired
from the opaque callback inventory and now has typed CPU, VJP, transform,
export, plan, and residency decisions. The executable v1 framework-operation
registry now generates its public support record from that same validator
binding and preserves the original opaque-ID partition. The next software
transition is the pure unary elementwise family. Each operation must enter the
registry with explicit CPU, VJP/refusal, transform/export, backend-profile,
residency, and materialization decisions; support must not be inferred from
method presence or duplicated in hand-maintained tables.
