...
`Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.prod`, `Tensor.repeat`, and
...
registry generates all ten public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.gather`, with strict index/axis semantics, typed
scatter-add VJP, eager conformance, and explicit transform/export/device
