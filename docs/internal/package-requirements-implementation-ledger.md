...
Audit Gate 3 against its pinned real C++/CuTe source, current compiler frontend,
shared layout semantics, CPU reference, WGSL backend, corpus gates, and native
companion evidence. Record the exact supported profile and first tracer before
implementation. Keep the three stale direct-publish guidance edits isolated
because their files contain overlapping user-owned work.
