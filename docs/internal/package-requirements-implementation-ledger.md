...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, artifact, canonical artifact resource, runner receipt, provenance, authorization, and static lowering authorities are distinct. The receipt binds pre-run intent to reported opened inputs, deterministic invocation, pinned toolchain/container/sandbox facts, exact canonical output, resolved-or-rejected frontend outcome, and every extraction/process ceiling. Detached DSSE/in-toto provenance now requires the exact strict-decoded receipt resource and derives its source/toolchain/sandbox/run facts from that authority. Authorization derives the exact artifact/profile from the attestation rather than caller input. No real producer, real receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Implement the offline fail-closed container runner boundary; pin/build the real native extractor, image, policy, profile, source/header closure, and external signing workflow; then prove dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 60 focused profile/job/receipt/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 41 files/1066 tests plus typecheck/lint and architecture/corpus gates; existing synthetic receipt/attestation tests prove contracts, not producer execution. |
...
| Gate 3 AOT runner receipt contract | `verified` | `browsergrad.compiler.cpp-cute.aot-runner-receipt@1.0` is a closed content-addressed report binding exact prepared job/profile, deterministic output-independent invocation, pinned execution projections, opened job inputs, resolved or rejected selection, exact output, and all 24 extraction/process observations. Artifact-visible counts are independently recomputed. Structural and strict-byte authorities are separate. Detached provenance accepts only the latter and signs receipt ID, raw digest/length, job, invocation, and output. | The real runner must snapshot/hash/mount the same source blobs and decode the same bounded output/receipt buffers. | 7 receipt and 11 provenance tests; all ceilings, counters, byte snapshots, accepted/rejected outcomes, structural-resource rejection, and signed receipt/job/invocation/output mutations. |
