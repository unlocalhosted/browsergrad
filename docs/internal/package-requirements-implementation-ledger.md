...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. The canonical runtime ABI, asset/build binding, bounded raw-Wasm inspector, BrowserGrad-owned LibTooling tracer source/build integration, exact offline header-pack selection/assembly, lock-derived source-snapshot/sidecar materialization observations, Worker-owned VFS, memoized canonical input frame, complete artifact/decode verification contract, host-controller lifecycle boundary, source-level C++ imported-VFS adapter, local Wasm memory/allocator metrics observer, and package-owned same-realm runtime binding now exist as bounded components. The C++ extractor is split into bounded runtime/lifecycle, imported-VFS, Clang-action, and wired fail-closed artifact units. Runtime ABI 1.1 pins one exact nonzero stable metrics pointer and 72-byte record, including zero-byte and realloc accounting; the current C++ extractor remains ABI 1.0 and is explicitly blocked until it implements and behaviorally proves the producer. The VFS now has a memory-independent same-realm mount and one-time memory binding, but production must still reconstruct the opaque installation/request/ABI authorities inside the real Worker from verified transferred bytes. The source-level C++ adapter is lazy, fail-stop, and exact-close, but has not been compiled. The tracer still cannot return `artifact-ready` before CUDA dual-pass/artifact-v3 integration. Header-pack licensing, build execution, output identity, ABI conformance, reproducibility, provenance, browser execution, and release authority remain false. Docker/native stays optional build/CI/reference infrastructure outside the portable graph. | Implement and behaviorally prove the ABI 1.1 metrics producer; add the canonical transfer record and Worker-local authority reconstruction; implement device-first/host-second tracing and artifact-v3 output; acquire/review exact headers and notices; build and internally pin the Worker/factory; produce and inspect two builds; then prove one unmodified browser-local source through shared semantics and real WebGPU. | VFS/Worker focused boundary: 2 files/24 tests. Build-plan/source: 3 files/51 tests plus lock 24 tests. Full compiler: 62 files/1,356 tests plus optional Docker 96 tests. Release gate: 19 Python and 35 Node security tests plus packed/fresh consumers. Zero real Clang-Wasm builds, Worker spawns, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact BrowserGrad-owned LibTooling tracer and deterministic Emscripten plan. The Node executor verifies and stages its exact source closure and can atomically install generated Wasm bytes. The checked-in C++ extractor now separates runtime/lifecycle, imported VFS, Clang action, and wired fail-closed artifact ownership while the top-level file contains only the eight ABI 1.0 exports. Runtime ABI 1.1 and the build lock explicitly block conformance until the ninth metrics export and its zero-size/reallocation producer behavior exist. No empty metrics translation unit is retained; it will enter the locked source closure only with real producer behavior. No pinned LLVM/Emscripten compile has occurred. | Implement the ABI 1.1 metrics producer and behavior fixtures; then add frame decode, dual CUDA passes, artifact-v3 output, two locked builds, observed projection review, licensing, and provenance. | Build-plan/source 3 files/51 tests plus lock 24 tests; native strict syntax checks for the non-LLVM units; full compiler 62 files/1,356 tests. No complete LLVM/Emscripten compile, process, build output authority, licensed distribution, reproducibility proof, or release authority exists. |
| Gate 3 Worker-owned VFS session | `partial` | One exact unshared Wasm memory serves canonical source snapshots and verified pack ranges through the six runtime-ABI imports. VFS preparation is now split into a memory-independent opaque mount and one-time binding to an exact exported memory. The mount/session authorities are same-realm only, cloned projections are rejected, imports are stable, and destructive cleanup settles terminal state even under hostile mutable intrinsics. This closes the bind-order seam, not the Worker boundary: production still must reconstruct verified manifest/ABI/request/pack authorities inside the Worker from exact transferred bytes. Neither JavaScript VFS nor the source-level C++ adapter has run in a Worker. | Define the canonical transferable launch record; reconstruct every opaque authority locally inside the Worker; bind only `instance.exports.memory`; compare artifact `openedFileIds` with terminal VFS observations. | VFS plus runtime focused 2 files/24 tests; typecheck/lint/build; hostile intrinsic tests; full compiler 62 files/1,356 tests. No Worker, C++ execution, or semantic-contribution evidence. |
| Gate 3 Wasm runtime metrics | `partial` | Runtime ABI 1.1 now owns the exact ninth pointer export, nonzero/stable/read-only pointer contract, 72-byte little-endian record, synchronous snapshot rules, requested-byte equations, exclusions, overflow behavior, and exact malloc/free/realloc/zero-byte event semantics. The local observer derives its layout from that authority, records exact pages/capacity and local phase time, and claims neither Worker evidence nor JS heap/resident/CPU/process metrics. The current ABI 1.0 C++ extractor has no export or instrumented producer, so the build lock keeps a behavioral-conformance blocker. | Implement module-global allocator hooks and the ABI 1.1 export in the split C++ metrics unit; add zero-byte/realloc behavioral fixtures; validate between synchronous calls in an executed Worker. | Focused 6 files/144 tests; current full compiler 62 files/1,356 tests; final review closed ABI-version, impossible-linker, and realloc/zero-byte P1s. No producer evidence. |
...
exact metrics record, but the C++ producer is deliberately missing. The
extractor is already split into runtime/lifecycle, imported-VFS, Clang-action,
and wired fail-closed artifact units; no empty metrics unit remains. Next add
the metrics export and module-global allocator behavior as one real owned unit,
with exact zero-byte/realloc conformance fixtures; only then clear its build
blocker.

In parallel, extend the completed same-realm memory-independent mount/one-time
bind seam with a canonical transfer record and actual Worker-local authority
reconstruction. Transfer only exact canonical bytes and ArrayBuffers,
reconstruct every opaque installation/request/runtime-ABI/pack authority in the
Worker, instantiate only host-verified Clang-Wasm, and bind exactly
`instance.exports.memory`. Do not bundle the generated factory before this
ownership seam is closed. Afterward add device-first/host-second CUDA actions
and the canonical artifact-v3 writer, then build and internally
