...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers now preserve exact input closure, spelling/expansion provenance, resolved types/declarations/templates/overloads/ABI, structured bodies, hierarchical layouts, dynamic integer expressions, tensor engines, and typed target intrinsics. Artifact integrity remains separate from producer trust. No real-source compatibility claim has been made. | Add detached attestation authorization; pin real source/toolchain/header inputs; lower the layout tracer into shared semantics; prove CPU and WebGPU; add dynamic tensor binding; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; 27 focused profile/artifact tests; compiler typecheck/lint and architecture guards; existing corpus gates remain audit inputs, not Gate 3 proof. |
