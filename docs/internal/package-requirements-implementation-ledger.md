...
- **Current implementation slice:** Gate 7 host-graph performance evidence
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, version-1.1 whole-allocation per-rank copies, version-1.2 terminal fail-stop materialization, version-1.3 dependency-ordered completion events, version-1.4 bounded fixed-count sequential repetition, version-1.5 bounded captured-input u32 conditionals, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors snapshot inputs, execute dispatches, copies, collectives, fixed repeats, and one selected conditional branch in private rank-local storage under explicit ceilings, report completed events/repeats/conditionals only with whole-graph success, and expose only verified materialized outputs. The WebGPU adapter expands verified dispatches, raw copies, arbitrary-participant reductions, and exact fixed repeats into bounded canonical WGSL sequences, pre-lowers both conditional branches, and selects only from the captured external-input predicate while events/materialization add no GPU command; complete f32/i32/u32 collective, repeated f32 sum, event-marked raw-u8 materialization, and both conditional-branch actual-device parity pass. A separate required real-device record compares fixed repetition with an equal-work, bit-exact statically unrolled graph without asserting superiority. | Runtime/backend-derived control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles. Event timestamps, external waits, and cross-queue/cross-worker synchronization require separately named future profiles. | Semantic-core 23 files/208 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/179 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; eight complete-output CPU/WebGPU parity cases plus non-finite refusal and actual device-loss refusal on Apple Metal 3; separate two-rank 65,536-element f32 host-API performance observation with 8 warmups and 12 paired samples on the same device; root/compiler architecture checks with zero cycles/leaks. |
...
also implemented. Version-1.5 bounded conditionals select only from a captured
external-input u32 and do not claim timestamps, external waits, GPU/backend-
derived predicates, mid-graph feedback, or runtime-derived loop/launch control.
Pipeline authority and transport remain open.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies. Version 1.5 adds bounded captured-input u32 conditionals with equal branch node-kind structure, globally unique node IDs, conservative merged effects, guaranteed-write intersection, and exact one-branch work counts. Versions 1.0–1.4 retain exact decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. | Semantic-core passes 23 files/208 tests, including canonical older-version retention plus repeat and conditional version/mode/predicate/identity/body/shape/effect rejection and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, exact fixed repeats, and one captured-input-selected conditional branch with cancellation/time checks at every admitted boundary. Completed event IDs, repeat records, conditional branch records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, exact one-branch element or copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller/predicate-mutation isolation, copied-byte budget refusal, terminal output publication, successful completion reporting, exact three-iteration f32 sum to `[16,24]`, both conditional branches, and pre-admission expanded-operation refusal. Semantic-core passes 23 files/208 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed-repeat/input-conditional execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and exact fixed repeat bodies into bounded canonical WGSL sequences, and pre-lowers both conditional branches through those same paths. Equal branch execution shapes retain exact public counts; the union of resources/modules is conservatively bound, while execution selects from the captured input predicate before device work and binds branch identity into specialization evidence. Events and materialization contribute no GPU step; completed event IDs, repeat records, and conditional branch records appear only in the successful terminal trace. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the current portable DAG/fixed-repeat/input-conditional profile. Non-word-sized copies fail explicitly; runtime/backend-derived control, pipeline authority, transport/topology, native companions, and stronger event profiles require new contracts. | Eight complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions of f32 sum, and both conditional branches after caller-input mutation; separate checks reject non-finite f32 and a lost device. Kernels passes 21 files/179 tests and packed/fresh consumers prepare compiler-owned 1.0 dispatch graphs plus artifact-only 1.5 repeat/conditional/event/materialization. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. It measures the complete production host API through readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime/backend-derived control, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.90/2.80 ms and baseline median/p95 2.60/4.30 ms for two ranks, 65,536 f32 elements, and three reductions. Terminal artifact `a6660fc3a64390d2b753f8e9b1b2473e317dc2bfe1a64a70df185e8e0c799aa2`; device profile `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
runtime-derived loop/launch control. The next software checkpoint is pipeline
authority or a separately versioned runtime-derived-control slice; it must not
widen the graph implicitly. A separate required fixed-repeat/unrolled
performance record now captures equal-work production-host-API behavior
without a superiority or regression claim. Transport/topology, worker meshes,
native companion execution, and stronger event profiles remain later
