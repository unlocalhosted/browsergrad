...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, and the fixed fail-stop/no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors snapshot inputs, execute dispatches and collectives in private rank-local storage under explicit ceilings, and expose outputs only after whole-graph success. The WebGPU adapter expands verified dispatches per rank and arbitrary-participant reductions into bounded canonical WGSL sequences with complete f32/i32/u32 actual-device parity. | Copy/readback/event/materialization nodes; bounded conditional/repetition control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles; graph performance evidence. | Semantic-core 23 files/192 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels strict/unit/browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; four complete-output CPU/WebGPU parity cases plus non-finite refusal and actual device-loss refusal on Apple Metal 3; root/compiler architecture checks with zero cycles/leaks. |
...
| Gate 7 host-graph semantic foundation | `verified` | The new `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived view-copy effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Compiler's `createCudaLiteViewCopyHostGraph` is the first concrete cross-package consumer and derives a linear multi-dispatch pipeline solely from opaque prepared view-copy bindings. | None for the semantic/construction foundation. | Semantic-core graph verifier 9 tests; compiler 106 files/1,658 tests and focused 14-test pipeline suite; strict typecheck/lint/build; packed fresh-consumer gate; zero-cycle/zero-leak root and compiler architecture checks. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input, runs topologically ordered view-copy dispatches and finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, and returns outputs only after whole-graph success. Fixed configurable ceilings bound working memory including collective scratch, element operations, preparation time, execution time, and native cancellation. | None for the initial CPU-reference profile; the portable WebGPU executor is separately verified below. | Seven focused CPU tests cover multi-dispatch execution, all three dtypes, collective policy, input snapshotting, non-finite refusal, binding/alignment/shared-memory rejection, authority, budgets, and cancellation. Semantic-core passes 23 files/192 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU static-DAG execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches per rank and arbitrary-participant collectives into bounded canonical WGSL sequences, retains private rank/resource buffers, validates f32 numerical status, and publishes no output until complete readback. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the initial portable static-DAG profile. Dynamic control, explicit copy/event/materialization nodes, pipeline authority, transport/topology, native companions, and performance require new profiles. | Four complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, and exact u32 max after caller-input mutation; separate checks reject non-finite f32 and a lost device. Focused Node tests cover authority, hostile/revoked inputs, budgets, numerical WGSL, issue scopes, and publish evidence. The packed fresh consumer prepares the compiler-owned two-dispatch graph through the public tarball surface. |
...
- This closes portable execution for the current static graph profile only.
  Gate 7 remains `in progress`: copy/readback/event semantic nodes, bounded
