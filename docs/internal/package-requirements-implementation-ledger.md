...
|---|---|---|---|---|
| Frozen-adapter inventory | `verified` | Baselines are machine-enforced for compiler pointer/scalar memory, `cute_static_layout`, the 44-op shape/f32 `TensorGpuPlan`, and the original 36-constructor/39-operation JIT set partitioned into 35 typed retirements, 2 current opaque identities, and 2 removed unsupported surfaces, plus runtime requirement mapping and 17 Grad dtype/view/materialization behaviors. |
...
The next software transition resolves `flash_attention` through typed
semantics or an explicitly non-portable contract while retaining the
intentional `user` kernel extension boundary. Remaining Grad dtype/view
