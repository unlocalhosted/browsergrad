...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict producer, runtime ABI 1.9, exact reproducible factory, zero-import verifier/compiler Workers, production controller, retained result chain, hardened Artifact V3 verifier, and bounded evidence authorities remain intact. The configured five-pack/5,768-file header universe is materialized and reproducible. A no-rebuild Chromium lane verifies reviewed Wasm and all five packs, runs unchanged CuTe layout source through the exact verifier and compiler Workers, and reports the deepest authenticated failure in seconds. Fixed native first-cause codes now distinguish producer, Artifact V3, runtime, and allocator-integrity failures. The cached build workflow also fails on any raw-Wasm ABI mismatch instead of uploading a nonconforming report as a successful validation. `productionBrowserCompileObserved`, lowering, backend, distribution, and release remain false until the newly locked source is rebuilt and the live blocker is removed. Existing synthetic layout/view-copy authorities still prove canonical CPU and actual-WebGPU convergence only; they are not substituted for production compilation. External license/distribution approval, full release-output reproducibility, and a real externally rooted producer also remain false. Docker stays outside the portable runtime graph. | Rebuild the exact current lock through the cached lane, then use its native first-cause signal to make the strict real-browser lane produce and authenticate Artifact V3 for unchanged source. Independently obtain external approval for the exact file map/notices and an externally issued exact-build statement under the production signer policy. Then bind the production result to shared lowering and required real WebGPU without merging those authority transitions. | Node 25 passes the 96-file/1,613-test compiler suite and the 82-file/725-test fast Clang-Wasm harness in 10.55 seconds. The preceding 31,649,344-byte diagnostic Wasm at SHA-256 `77dcdb63a5a3c2a0732ecee6b460b021af3752dde23aa71d8d2efc609e32cb73` independently passes ABI 1.9 raw review with zero mismatches, but its embedded request-manifest hash predates the current lock and therefore grants no current execution authority. Current build lock is `bg.cpp.browser-build-input-lock.sha256.27b4735cf1ca2970e0acbcc2c0f9b7b73bc35aeab84007b1a9ae4ca9ef9855a9`; header-distribution reproducibility ID is `bg.cpp.browser-header-distribution-reproducibility.sha256.00ec67e05c5c950557e3963502e3bc772d36978467398c063e91a285dcaa05e2`. Zero valid production compiler-Worker compiles or browser-local C++ executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact ABI/semantic-adapter binding, raw-Wasm inspection, Worker frame/VFS contracts, canonical transfer, one-shot entry, and typed controller terminal protocol share one closed contract. C ABI 1.2 adds exact fail-closed frontend-work accounting. Runtime ABI 1.8 independently pins the current generated exception bridges and fixed table. The package owns the exact reproducible 27,125-byte factory, a deterministic 575,477-byte compiler Worker graph, and a separate 151,555-byte raw-Wasm verifier Worker graph; both are self-contained ES modules with no static or dynamic imports. A dedicated post-build lane admits the exact Wasm and five packs, verifies the raw interface in a separate Worker, and runs unchanged CuTe layout source without rebuilding. An explicit untrusted-diagnostic mode permits exact locally hashed fast-build Wasm while refusing clean/reproducible and strict-compile authority. Worker protocol v2 failures preserve bounded frontend-work, allocator, and VFS diagnostics. | Resolve the one-pass diagnostic/Artifact V3 or second-pass internal error, then require the strict lane to receive one authenticated Artifact V3 terminal from the package-pinned clean/reproducible Wasm. Keep external producer trust, distribution approval, lowering, backend execution, and release as separate false authorities. | Package-pinned reproducible Wasm SHA-256 `5fc425bbc051a2f5be588c2acbb164efb5e43f949afb48a373f3ed022c3b8758`, 31,641,377 bytes; current untrusted diagnostic Wasm SHA-256 `a8678f9c035e79bd1974da8ccf0ced8e4b61ba837d13fd96b8a0ff8436d1c6d8`, 31,641,128 bytes; factory SHA-256 `796a548237420df7f5eca0c0260d3cbe752aeca155d9c7182c6ad0f5491dfb12`; compiler Worker SHA-256 `500356a0cb2b6ac1df23124b7bcbe4043c302409f5195d1e847e935804c62bea`; verifier Worker SHA-256 `df514398f671b2124d0df58babc49f03ed86d70504227d33acac1e4802abbb65`. Chromium observation completes in about 7.8 seconds and now records 56,150 macro expansions, 238,041 tokens, 49,690 AST nodes, and one completed CUDA semantic pass before `BG-COMPILER-CPP-CUTE-BROWSER-WASM-COMPILER-COMPILE-STATUS` at `$.runtime.compile`. Strict verification refuses the untrusted diagnostic Wasm and the blocked outcome. Zero valid production compiler-Worker compiles or browser-local C++ executions. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact extractor closure, LLVM 22.1.8 archive, Emscripten image, four-step recipe, ABI, and distribution plan. Clean builds seal the exact Node runtime closure; cached diagnostics restore only untrusted content-addressed toolchain directories inside the isolated container and grant no clean/release authority. Current admission verifies the production-shaped configure/build working directories, rereads exact Wasm, reruns the independent inspector, and requires byte-identical reports. Runtime ABI 1.9 pins the diagnostic build's 15,167-entry table with the same 72 imports, and every CI build review now requires exact interface conformance before the build job can succeed. Reproducibility v3 retains both raw linker-map identities but compares a strict path-canonical semantic projection; the exact two-build record is package-pinned. The separate 17-file header-distribution subset is rehashed and reproducible across two distinct roots, and its path-independent observation is package-pinned and independently rederived. Manifest v1.4 binds the browser build-signature policy into the asset-set identity; the signature verifier proves exact build-subject binding and deliberately returns `producerTrusted=false`, while the separate host-only trust transition can independently admit it. The comprehensive verifier retains three serial prerequisites and executes the remaining exact command DAG in four bounded fail-fast lanes; CI separately shards source/dist corpus and native/WebGPU/Node surfaces. The fast Vitest set discovers every browser and C++/CuTe lowering test through bounded family globs rather than a manually maintained per-file browser allowlist. | Preserve the narrow reproducibility authorities while closing external license approval, the complete release-output set, production policy/external signer evidence, valid Worker execution, and release separately. | Current lock `bg.cpp.browser-build-input-lock.sha256.27b4735cf1ca2970e0acbcc2c0f9b7b73bc35aeab84007b1a9ae4ca9ef9855a9`, resource SHA-256 `3dadc3dcec2d846f3c359c5cc798eee8e18ee031bbc75b01ccccf8389abb1ffe`. Header-distribution reproducibility resource SHA-256 is `8802767f3a1f6ec66031762a0f4a86be01595bab7e2f6d3e74208c57b844c79d`. Focused fast harness: 82 files/725 tests in 10.55 seconds; full compiler suite: 96 files/1,613 tests. A fresh two-root header materialization/reverification reproduced all 17 outputs. Remote cached validation of the current lock is pending; the preceding diagnostic build completed in 4 minutes 40 seconds. No licensed distribution, real externally rooted producer, valid Worker execution, or release authority yet. |
