...
| Gate 6 — framework convergence | `in-progress` | Thirty public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1, Smooth L1, probability BCE, BCE with logits, and KL divergence own strict same-shape floating promotion, bounded reductions, both-input closure/symbolic derivatives, and nested per-example vmap shared with Grad. Probability BCE owns fail-closed domains and endpoint policy; logits BCE owns overflow-safe finite-extreme math and stable ONNX export; KL owns exact probability/log-target algebra and native batchmean/zero-target behavior. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 9 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0028; architecture freeze narrowed from 36/39 to 9/9 while preserving the exact original-ID partition. KL verification passes focused JIT 5 files/52 tests, focused Grad 3 files/29 tests, full JIT 49 files/319 tests, full Grad 54 files/350 tests, the 96-file/1,602-test compiler gate, release-package security and packed-consumer gates, and the seven-package Node 25 build in 9.77 seconds. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.kl_div` are retired from the opaque callback inventory and
have typed CPU, VJP, transform, export/refusal, backend, and residency
decisions. The executable v1 framework-operation registry generates all thirty
public support records from their validator bindings and preserves the original
opaque-ID partition. The next software transition is `nll_loss`, followed by
its coupled `cross_entropy` consumer. Each operation must enter the registry with
