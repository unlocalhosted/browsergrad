...
| Gate 6 — framework convergence | `in-progress` | Public `Tensor.expand`, `Tensor.abs`, `Tensor.sign`, `Tensor.sin`, `Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.repeat`, and `Tensor.repeat_interleave` are retired from `CUSTOM` and emit typed `BROADCAST_TO`, `ABS`, `SIGN`, `SIN`, `COS`, `CLAMP`, `FLIP`, `REPEAT`, and `REPEAT_INTERLEAVE`. Expand retains shared eager/lazy conformance and materializing/resident plan routes. Abs/sign preserve real-numeric shape/dtype; sin/cos preserve floating shape/dtype and reject integer drift. Clamp owns finite normalized optional bounds, floating dtype preservation, an inclusive-mask VJP, vmap, and ONNX `Clip`. Flip owns strict single-axis reversal, involutive VJP, batch-axis shifting, and ONNX `Slice`. Repeat owns bounded tile multipliers, block-sum VJP, batch-safe vmap, ONNX `Tile`, and shared eager/lazy dtype conformance. Repeat-interleave owns strict axis/count semantics, selected-axis block-sum VJP, batch-safe vmap, exact ONNX decomposition, and shared eager/lazy dtype conformance. Host-only typed operations explicitly refuse tensor-plan/WebGPU. JIT's bounded v1 executable operation registry binds all nine typed records one-to-one with validators and generates detached public support reporting from those same records. | Migrate the remaining advertised opaque operations through the registry; reconcile the remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate the cross-framework/platform support views from executable contracts. | ADR-0002 through ADR-0009; focused JIT operation/registry/VJP tests; architecture freeze narrowed from 36/39 to 27/30 while preserving the exact original-ID partition. Full regression counts are recorded in the dated verification entries. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.repeat`, and
...
registry generates all nine public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.prod`, with strict reduction-axis/dtype semantics, typed
VJP, scalar-result normalization, eager conformance, and explicit device
