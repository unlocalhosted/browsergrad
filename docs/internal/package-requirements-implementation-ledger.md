...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT 4-constructor-call/4-operation opaque matrix have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0033 retire 35 typed operation identities—including paired `torch.sort`/`torch.topk` outputs, overwrite `torch.scatter`, general `torch.einsum`, the typed loss family through full class-axis cross entropy, keyed dropout, ordered stateful BatchNorm1d, and typed spatial interpolation—from the original 36-constructor/39-operation baseline without widening any remaining caller or label. Grad parameterized-layer initialization obeys `torch.manual_seed` across Linear, convolution, transposed-convolution, and Embedding families. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check; 96-file/1,602-test compiler gate; 1-case JIT Gate 0 contract; full JIT and Grad integration; Node 20/24/25 and real Chromium/WebGPU evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-five public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, general string-equation `torch.einsum`, keyed dropout, stateful BatchNorm1d, and typed nearest/bilinear spatial interpolation. No framework-owned NumPy callback remains in the opaque inventory. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching partitions equal-shaped graphs by session, bypasses mutable module buffers, and excludes effectful/random graphs. | Resolve or accurately retire the three advertised accelerator/constructor-only compatibility surfaces while retaining the intentional user-kernel extension boundary; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0033; architecture freeze narrowed from 36/39 to 4/4 while preserving the exact original-ID partition. Node 25.9.0 passes the 10.86-second seven-package build; JIT build/typecheck/browser-typecheck/lint/24-unit/54-file 335-integration-test gates; Grad build/typecheck/lint/30-unit/59-file 361-integration-test gates; semantic architecture and mutation checks; and the full 96-file/1,602-test compiler gate plus its parallel build-plan/shell lanes. Focused interpolation/registry/Gate 0/IR/VJP/Grad parity passes 61 tests in seconds. No typed interpolation actual-device claim is made. |
...
remains. The next software transition resolves `flash_attention`,
`transformer_block`, and `webnn_matmul` through typed semantics, accurate
surface removal, or an explicitly non-portable contract while retaining the
