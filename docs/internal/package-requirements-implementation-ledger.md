...
- **Current implementation slice:** Gate 7 GPU-produced multidimensional launch
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, and version-1.12 request-time rank-2/rank-3 rectangular prefix dispatch. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Rectangular WebGPU changes only true 2D/3D launch geometry plus a 16-byte guarded uniform in one maximum-prewarmed canonical view-copy slot; every axis guard precedes semantic evaluation and device admission covers x/y/z. | GPU-produced multidimensional launch, rank-4-or-higher dynamic domains, repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Semantic-core 23 files/235 tests; kernels 21 files/195 tests plus strict browser/build gates; packed fresh-consumer execution; 32 complete-output CPU/WebGPU parity cases through explicit pipeline authority on Apple Metal 3; correctness artifact `d396265cee8b4d90a70334b5325c182efc4cd30f21f807dcc55a22ddba02bde3`; separate backend-1.15 prewarmed performance observation; root/compiler architecture checks with zero cycles/leaks. The full compiler verifier's native matrix passes after commit `c5c3799b`; its independent exact reviewed-bsdtar identity check currently refuses installed tool drift and was not repinned. |
...
repeated/device-side feedback, GPU-produced multidimensional launch,
rank-4-or-higher dynamic domains, nested/device-side branching, or transport.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Version 1.12 adds request-time rank-2/rank-3 rectangular prefix dispatch: one positive bounded u32 control per verified logical axis, maximum-product work admission, canonical control normalization, exact older-version decoding, and logical-extent completion records. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | GPU-produced rectangular launch, rank-4-or-higher dynamic domains, repeated/device-side feedback, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/235 tests, including rank-2/rank-3 normalization, exact partial/full rectangle execution, pre-version/rank/axis/control/shape/bound rejection, hostile extent admission, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Version 1.12 executes the selected rank-2/rank-3 dense rectangle against the full semantic row-major domain, reserves the artifact-maximum product, and reports exact logical extents, product, and rank-multiplied actual work. | None for the current CPU-reference profile; portable WebGPU remains separate. GPU-produced rectangular launch and rank-4-or-higher dynamic domains are not inferred. | Focused CPU coverage includes exact partial/full `[3,4]` and `[2,3,4]` rectangles, caller mutation isolation, missing/zero/above-bound/rank/accessor refusal before input observation, and the existing copy/collective/repeat/conditional/linear-dynamic matrix. Semantic-core passes 23 files/235 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, repetition, conditional, feedback, copy, collective, fail-stop, and exact-slot lifecycle. Backend 1.15 adds version-1.12 rank-2/rank-3 rectangular dispatch through one maximum-prewarmed canonical view-copy program: a 16-byte axis guard precedes semantic evaluation, physical x/y/z map trailing-to-leading logical axes, runtime changes only true 2D/3D geometry plus uniform bytes, and graph/pipeline identity remains stable across extents. Transient budgets charge 16 GPU and 16 host bytes per rectangular rank step; device admission checks all three workgroup dimensions plus total/storage/uniform bindings before allocation. | GPU-produced multidimensional launch, rank-4-or-higher dynamic domains, repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Thirty-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Four added small/full rank-2/rank-3 cases retain pairwise pipeline identity, distinct specialization, exact extent/product completion, and caller-control snapshot isolation. Correctness artifact `d396265cee8b4d90a70334b5325c182efc4cd30f21f807dcc55a22ddba02bde3`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. Kernels passes 21 files/195 tests plus strict typecheck/lint/build, browser typecheck, packed fresh-consumer execution, and the existing authority/device-loss/resource-limit refusals. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.30/3.10 ms and baseline median/p95 1.30/3.40 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.15.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
GPU-produced multidimensional launch or a separately bounded higher-rank
dynamic domain; repeated/device-side feedback, transport/topology, worker
meshes, native companion execution, and stronger event profiles remain later
