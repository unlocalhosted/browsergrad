...
- **Current implementation slice:** Gate 6 runtime requirement resolution
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 has no remaining compatibility-debt behavior: it owns a closed twelve-dtype registry, separates direct and torch constructor inference/ownership, implements zero-stride expand views, rejects fake bfloat16, fixes contiguous/detach/cast semantics, keeps all eager placement CPU-only and fail-closed, zero-copy wraps every supported writable NumPy storage dtype/layout, and emits only owning NumPy snapshots. Runtime now generates all 53 requirement definitions from the architecture vocabulary and creates complete provider-bound available/unavailable resolution environments; the frozen capability environment is derived only from explicitly available resolutions. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate run-plan/report/handoff/matrix and profile UI consumers to accept resolution environments directly; add program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0047; architecture freeze preserves the exact JIT partition, revised Grad contracts, and byte-exact generated requirement registry. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 99.46 seconds, runtime passes 12 files/129 tests, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 25-test mutation suite pass. |
