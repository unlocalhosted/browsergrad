...
- **Current implementation slice:** Gate 7 repeated-feedback contract design
...
| Gate 2 — multi-frontend, multi-backend view slice | `verified` | Semantic-core `0.2.0` owns verified view-copy meaning, shared specialization, and the sole canonical frontend construction sink. Its positive-affine word32 profiles now cover equal source/destination ranks 1 through 5, with rank 5 under a distinct identity that does not reinterpret existing profiles. Compiler L1 preserves its six-case non-padded contract; sibling L2 binds the exact verified operation and proves rank-2/rank-3 guarded padding through CPU, structured IR, WGSL, and actual WebGPU. JIT `0.9.0` emits a closed typed permutation request and executes the same canonical operation through materializing/resident production routes without recovering semantic args from the frozen plan. | None for the declared portable view-copy profile. Rank 6, signed/negative strides, 16-bit/bf16/f64 storage, overlap, and non-global memory require separately named profiles. Release CI must repeat the exact-source lanes before publication; registry publication is separate. | Required Apple Metal 3 semantic view-copy backend-2.1 evidence passes 14 complete CPU/WebGPU cases including rank-5 mixed-radix permutation; correctness artifact `50afa87acf5f404a3f6b5c515349a426237c2425ad36d15599e5693cfb4d5a41`; device profile `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`. Historical clean-source frontend convergence remains recorded at revision `aa605421410e9d4190d8939c24b1057731111231`. |
...
| Gate 2 view-family selection | `verified` | Selected typed JIT `PERMUTE` plus compiler L1 read binding and sibling L2 guarded materializing view copy. The required 14-case view-copy matrix adds a separately identified rank-5 positive-affine word32 profile with exact CPU/WebGPU mixed-radix parity; broader view families must add typed operation variants rather than reinterpret frozen plans. |
...
- Signed/negative strides, packed/16-bit/bf16/f64 storage, overlap,
  non-global memory, dynamic rank-5 host graphs, and frontend-produced rank-5
