...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The five-pack/5,788-file header universe is reproducible under a narrow stable header-input projection. Runtime ABI 1.17 pins the reviewed 15,301-entry exception-dispatch table, closes Artifact V3 output at 32 MiB, and introduces no new import, export, memory, tag, or ambient authority. The source producer recognizes unchanged C++17/CuTe rank-1 through rank-4 view copies over exact `float`, `int`, and `unsigned int` 32-bit device ABI facts. CPU and actual WebGPU share the canonical word32 view-copy seam for f32/i32/u32 rank-1 through rank-4 affine layouts. Docker stays outside the portable runtime graph. | Complete current-lock two-clean-build admission and rerun the strict eight-case browser matrix. Then admit the exact observation through externally rooted producer trust, shared lowering, and required real WebGPU. Independently obtain external approval for the exact file map/notices and an externally issued exact-build statement under the production signer policy. | Capability commits through `9479fcdf`; current lock `bg.cpp.browser-build-input-lock.sha256.5a96def9bac1db052108142dfe4c82e729f4b41f450d459406a4f3c5227daad7`; ABI manifest `bg.cpp.browser-runtime-abi.sha256.73131ff8285e0e66e4c7d74786b85c5cc565c937acb6923c1b1a91df5ae4137a`. Fast gate 86 files/748 tests; complete compiler 97 files/1,624 tests. Previous expanded-source builders emitted byte-identical 31,841,010-byte Wasm with SHA-256 `35cf6d2576875da094de39d6f226c37d49e542fb16a876973347647a85c62002`; strict ABI review correctly rejected the then-stale table pin. Current reproducibility run `30069614333` is active. `producerTrusted`, license approval, current strict observation, lowering authority, backend execution, and `releaseReady` remain false. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact semantic-adapter binding, separate zero-import raw-Wasm verifier/compiler Workers, closed VFS, canonical transfer, one-shot entry, and typed terminal protocol compile source-derived view-copy graphs. ABI 1.17 retains the independent diagnostic cap and 32 MiB result ceiling while pinning the smaller reviewed exception-dispatch table. The Clang AST producer now recognizes exact f32/i32/u32 rank-1 through rank-4 view-copy facts; it does not add a source-spelling execution path. | Package-pin current two-clean-build authority, rerun the eight-case strict Worker matrix, then keep external producer trust, distribution approval, shared lowering, backend execution, and release false until the exact observation crosses each boundary. | Expanded-source run `30067229885` produced the same 31,841,010-byte Wasm in both clean builders before the strict reviewer rejected the stale table projection. The exact bytes pass ABI 1.17 locally with zero mismatches. Current Worker SHA-256 `3d1692b959f5ce1b61cd9a1810641f7a7aabad08e56b992c9297368c875ef3b1`; verifier Worker SHA-256 `06ffb66e4e808e9df030cc3fe2981fa3adddf13d03780680abb091cbcbd4b9eb`; both retain zero static/dynamic imports. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the extractor closure, LLVM 22.1.8, Emscripten image, recipe, ABI, and distribution plan. Cached diagnostics remain untrusted and separate from clean/release authority. The exact reviewer rejects interface drift after preserving bounded evidence. The exact 17-file header subset is independently rehashed across two roots and package-pinned. Both clean builds run concurrently; each cache-free proof still spends roughly 40–47 minutes compiling LLVM/Clang, while the local fast lane stays seconds-scale. | Finish two-clean-build and detached verification for the current lock, then close external file-level license approval, full distributed-output reproducibility, and production policy/external-signer evidence independently. | Current lock `bg.cpp.browser-build-input-lock.sha256.5a96def9bac1db052108142dfe4c82e729f4b41f450d459406a4f3c5227daad7`; runtime ABI manifest `bg.cpp.browser-runtime-abi.sha256.73131ff8285e0e66e4c7d74786b85c5cc565c937acb6923c1b1a91df5ae4137a`; header projection `bg.cpp.browser-header-input-projection.sha256.48490ddb7b2fe655ec36824e276b90122e2f548a77b768f5978029a31129c5b7`; header reproducibility ID `bg.cpp.browser-header-distribution-reproducibility.sha256.4d4c054fd4c93dbdbdef9581eeac52b037af3425e6a1c7eff8acc585abce1e55`; current reproducibility workflow `30069614333`. |
