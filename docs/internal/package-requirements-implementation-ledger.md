...
| Gate 6 — framework convergence | `in-progress` | Twenty-one public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, replication, triangular, scan, variadic concat/stack, constant pad, and paired `torch.sort` values/indices. Sort now owns one exact axis/ordering contract, stable dtype-safe CPU permutation, owning values/int64 indices, permutation-scatter closure and symbolic VJP, vmap, ONNX `TopK`/`GatherElements`, and bounded resource/refusal semantics shared with Grad. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 18 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0020; architecture freeze narrowed from 36/39 to 15/18 while preserving the exact original-ID partition. Current full verification passes JIT 41 files/295 tests, Grad 46 files/336 tests, compiler 96 files/1,602 tests, packed release, and Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`Tensor.var`, top-level `cat` and `stack`, `F.pad`, and both `torch.sort`
outputs are retired
...
registry generates all twenty-one public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is the paired `Tensor.topk` values/indices surface, extending the
ordering seam with selected-`k`, largest, sorted, tie, derivative, transform,
