...
- **Current implementation slice:** Gate 3 browser view-copy authorization and first valid browser-Worker compile
...
reference. The next software transition is an observed browser view-copy
candidate plus independently admitted producer composition that retains the
same full Worker/profile/request/artifact lineage. Explicit allocation lengths
and offsets remain later runtime-binding facts; they must not be folded into
producer trust or inferred from CuTe `cosize`. That browser authorization must
still keep backend and release false until required real-WebGPU evidence exists.
