...
- **Current implementation slice:** Gate 7 pipeline authority
...
control. Pipeline authority and transport remain open.
...
| Gate 7 portable WebGPU DAG/fixed-repeat/conditional execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and exact fixed repeat bodies into bounded canonical WGSL sequences, and pre-lowers both conditional branches through those same paths. Equal branch execution shapes retain exact public counts; the union of resources/modules is conservatively bound, while execution selects from the captured input predicate or exact request-time runtime control before device work and binds branch identity into specialization evidence. Runtime controls are captured with inputs before device access, and invalid sets fail before device acquisition. Events and materialization contribute no GPU step; completed event IDs, repeat records, and conditional branch records appear only in the successful terminal trace. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the current portable DAG/fixed-repeat/request-control profile. Non-word-sized copies fail explicitly; GPU/backend-derived or mid-graph control, pipeline authority, transport/topology, native companions, and stronger event profiles require new contracts. | Ten complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions of f32 sum, both captured-input branches after caller-input mutation, and both runtime-control branches after caller-control mutation; separate checks reject non-finite f32 and a lost device. Kernels passes 21 files/181 tests and packed/fresh consumers prepare and execute version-1.6 runtime-control conditional/repeat/event/materialization graphs. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. It measures the complete production host API through readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.60/2.80 ms and baseline median/p95 1.90/3.90 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.6.0. Terminal artifact `a6660fc3a64390d2b753f8e9b1b2473e317dc2bfe1a64a70df185e8e0c799aa2`; device profile `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
GPU/backend-derived or mid-graph control. The next software checkpoint is
pipeline authority; it must not widen the graph implicitly. A separate
required fixed-repeat/unrolled performance record captures equal-work
production-host-API behavior without a superiority or regression claim.
