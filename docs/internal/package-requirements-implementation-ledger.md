...
`Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.gather`, `Tensor.prod`,
`Tensor.repeat`, `Tensor.repeat_interleave`, and `Tensor.var` are retired
...
registry generates all twelve public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.masked_fill`, with typed broadcast/mask semantics, stable
