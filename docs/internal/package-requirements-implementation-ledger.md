...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT 21-constructor-call/24-operation opaque matrix have pinned inventories and executable contracts. ADR-0002 retired `Tensor.expand`; ADR-0004 retired `Tensor.abs`/`Tensor.sign`; ADR-0005 retired `Tensor.sin`/`Tensor.cos`; ADR-0006 retired `Tensor.clamp`; ADR-0007 retired `Tensor.flip`; ADR-0008 retired `Tensor.repeat`; ADR-0009 retired `Tensor.repeat_interleave`; ADR-0010 retired `Tensor.prod`; ADR-0011 retired `Tensor.gather`; ADR-0012 retired `Tensor.var`; ADR-0013 retired `Tensor.masked_fill`; ADR-0014 retired `Tensor.tril`; ADR-0015 retired `Tensor.triu` from the original 36/39 baseline without widening any remaining caller or label. Grad parameterized-layer initialization now obeys `torch.manual_seed` across Linear, convolution, transposed-convolution, and Embedding families. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check; 945 compiler tests; 125 runtime tests; blocking Grad contract; 5-case JIT Gate 0 contract and full JIT integration. Exact-source CI `29660471267` passed the repaired full Pyodide/Grad lane, Node 20/24/25, and real Chromium/WebGPU. |
...
`Tensor.repeat_interleave`, `Tensor.tril`, `Tensor.triu`, and `Tensor.var` are retired
...
registry generates all fifteen public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.cumsum`, with canonical axis/dtype accumulation semantics,
stable scan VJP, eager conformance, and explicit transform/export/device
