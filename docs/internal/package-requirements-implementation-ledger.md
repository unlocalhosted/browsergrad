...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. The canonical runtime ABI, asset/build binding, bounded raw-Wasm inspector, and BrowserGrad-owned LibTooling tracer source/build integration now exist as closed design inputs. The tracer accepts the exact C ABI but deliberately cannot return `artifact-ready` before its VFS, CUDA dual-pass, and artifact-v3 integrations exist. A Worker-owned lazy VFS session composes source snapshots and verified packs through the six-import ABI. Pure Worker result validation is caller-frame consistency only and cannot claim execution, termination, timing, or lowering authority. Current empty first-build projections still prevent production Wasm conformance. Docker/native stays optional build/CI/reference infrastructure outside the portable graph. | Produce the first real extractor build and reviewed header packs; implement VFS, device-first/host-second tracing, artifact-v3 output, and the package-owned Worker/controller; independently repin complete observed Wasm projections; emit a canonical artifact from unmodified source in-browser; then prove semantic convergence, dynamic tensor copy, and required real WebGPU execution. | Extractor source/build-plan: compiler build, 29 planner/source tests, and 24 lock tests. Current Worker/VFS/ABI/profile boundary: 9 files/167 tests; latest full compiler baseline 57 files/1,286 tests. Zero real Clang-Wasm builds, Worker spawns, or browser-local C++ executions. |
