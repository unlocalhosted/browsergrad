...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native compile session, closed policy materialization, pass-scoped VFS observation, sealed CUDA device-first/host-second actions, diagnostic normalization, and canonical production Artifact V3 writer now execute as one native producer. The pinned build boundary executes exactly four no-shell steps in a networkless, read-only, capability-free container and records bounded immutable logs, generated bytes, source/tool identities, and isolation observations. Separate authorities now compare two distinct clean builds and emit a raw-Wasm ABI projection without granting release or Worker authority. A distinct cached diagnostic lane accelerates ordinary extractor edits but explicitly cannot satisfy clean-build, reproducibility, provenance, or release authority. Production remains capability-blocked before Worker creation because no completed build output has been reviewed or bundled. Header licensing, completed clean-build execution, ABI conformance, full-output reproducibility, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Complete and inspect the first clean build; run the two-clean-build lane; review and repin the exact Wasm projection; bundle the generated factory into package-owned Worker bytes; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Harness, target, and feedback fixes are on `main` through `09dc21a2`; the required-native boundary passes 26 files with 166 passed and 9 platform skips in 89.31 seconds locally under Clang 22. Current clean runs mount only a sealed 26-file runtime dependency closure plus the 37 lock-listed extractor files, and build/reproducibility evidence v2 binds that closure. Run `29658164083` compiled every BrowserGrad translation unit and then failed the final Wasm link on RTTI-disabled LLVM/Clang libraries and unsupported POSIX host services; it admitted no factory/Wasm. Validation `29661850267` stopped before acquisition/build on Linux POSIX non-null contract violations; `60a3d9f9` closed that defect. Run `29662148150` was cancelled before acquisition to prioritize iteration speed. The lock now pins four build jobs; the exact-key diagnostic lane reuses only toolchain build directories and emits non-clean, cache-untrusted evidence. Cold provisioning `29663494490` is active. Zero completed pinned clean Clang-Wasm builds, valid Worker compile launches, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact 37-file BrowserGrad LibTooling extractor closure, LLVM 22.1.8 archive, Emscripten image manifest/config, four-step recipe, ABI, and distribution plan. The executor stages only locked snapshots, executes exact argv/cwd/environment without a shell, enforces per-step time/output/cancellation limits, binds inputs against inode-version drift, hashes native TableGen tools, seals the factory/Wasm/link map and logs, validates UTF-8/WebAssembly, and atomically materializes only the Wasm sidecar. The manual workflow verifies acquisition and image identities, then runs with no network, read-only root/input mounts, zero capabilities, no-new-privileges, and a private writable work mount. Clean runs additionally stage, seal, enumerate, and hash the exact 26-file Node runtime closure; build-execution evidence v2 records it and the v2 two-build comparator requires it to match. Cached fast validation restores only exact-key native-TableGen and Wasm-LLVM/Clang directories, treats them as untrusted, and emits a separate diagnostic observation with no clean/reproducibility authority. The raw-Wasm reviewer emits discovery without self-authorizing an ABI. Target preflight requires JavaScript exceptions, RTTI-enabled LLVM/Clang libraries, both canonical Clang include roots, and the exact locked factory output before the expensive Wasm compile. Unsupported POSIX host operations resolve inside the module with deterministic identity-free or `ENOSYS` behavior, and the linker reports every unresolved symbol. Failure receipts preserve a bounded typed cause chain without granting success authority. | Finish the first clean build, review its logs/factory/Wasm/link closure, then run the two-job clean-build lane under the exact runtime boundary. Repin the independently reviewed ABI projection before any Worker or distribution authority. | Build lock `bg.cpp.browser-build-input-lock.sha256.15a71eedf31da3ec752332f90ce74c3d2f7308bb81d3eeba698fc466b683fe14`; clean compilation now pins four jobs. Required-native boundary: 26 files, 166 passed, 9 platform skips in 89.31 seconds locally. Run `29658164083` compiled every BrowserGrad translation unit and failed at final link on missing library RTTI plus unsupported POSIX symbols, with no admitted outputs. Validation `29661850267` stopped before acquisition/build on Linux-only POSIX contract violations. Run `29662148150` was cancelled before acquisition. Cold cache provisioning `29663494490` targets fast-feedback commit `09dc21a2`; no warm timing, completed clean build, ABI conformance, licensed distribution, authenticated provenance, or release authority yet. |
