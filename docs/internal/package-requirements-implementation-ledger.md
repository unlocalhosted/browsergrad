...
- **Current implementation slice:** Gate 6 Grad view/materialization convergence
...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0037 classify the original JIT baseline and the revised Grad dtype contract without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad keeps `torch.bfloat16` distinct and rejects it before allocation rather than naming float32 storage bfloat16. Grad parameterized-layer initialization obeys `torch.manual_seed`, and Linear/Embedding enforce their exact float32 parameter profile. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention and bfloat16-refusal integration; full package and actual-device evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, general string-equation `torch.einsum`, keyed dropout, stateful BatchNorm1d, typed nearest/bilinear spatial interpolation, and bounded rank-4 float32 attention forward. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 rejects fake bfloat16 before allocation and parameter constructors enforce their exact float32 profile. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Reconcile remaining Grad view/materialization and conversion fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0037; architecture freeze preserves the exact JIT partition and revised Grad refusal contract. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 128.58 seconds, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 24-test mutation suite pass. |
...
The next Gate 6 software transition is remaining Grad view/materialization and
conversion convergence, beginning with `Tensor.contiguous`, followed by
generated runtime/profile support consumption. Support must not be inferred
