...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict producer, runtime ABI 1.8, exact reproducible factory, zero-import package Worker, production controller, and bounded evidence authorities remain intact. The ordinary harness completes in 35.32 seconds; direct one-run distribution materialization completes in 23.1 to 25.4 seconds, and exact two-root subset reproducibility completes in 44 to 47 seconds. The package-pinned normalization path extracts 5,769 source files plus eight exact license/copyright evidence files, emits five independently inspected VFS packs containing 5,768 files/67,470,214 bytes, admits the exact CUDA 12.6.3 redistribution index, rechecks all ten package notices, writes one canonical per-file review input, materializes all ten component-license files, and creates the deterministic aggregate notice. All 17 header-distribution outputs are reproducible across distinct roots. The configured header universe and complete package-owned external-review input are materialized; external license/distribution approval, full release-output reproducibility, signed provenance, valid browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Obtain external approval for the exact prepared file map, CUDA index, Linux/CUDA evidence, and notice identities; bind the approved pack/notice identities into the asset/provenance chain; then prove one unmodified browser-local source through authenticated Worker execution, shared semantics, and real WebGPU. | Capability commits `3acf5b65` through `e2fdbaeb`. The fast gate passes 50 files/437 tests; its focused test phase is 24.55 seconds. Header-distribution reproducibility ID is `bg.cpp.browser-header-distribution-reproducibility.sha256.986bcd7b462b1bac0653f33e61dba69403295fd69df4fdb1780bb27092de9337` for 17 files/68,899,643 bytes. Zero valid production Worker compiles or browser-local C++ executions. |
...
| Gate 3 deterministic header-pack selection and assembly | `partial` | The exact plan binds seven archives, eight header subtrees, and eight upstream license/copyright files. Admission streams all 252,406,685 archive bytes. The release-shaped pipeline accepts only the package-reviewed Darwin arm64 `/usr/bin/bsdtar` and Node 25.9.0/Zstandard closure, preserves case-distinct Linux paths in collision-free storage, verifies the configured Clang resource-header output, and writes five canonical packs containing 5,768 files/67,470,214 bytes. It also admits the exact 49,142-byte CUDA index, writes the deterministic complete `license-inventory.json`, copies all ten verifier-retained component notices, and constructs the exact aggregate notice. Every output is no-clobber, independently reread, and checked against the exact final file/directory tree. All 17 outputs/68,899,643 bytes are independently rehashed and identical across two distinct roots. Policy remains non-authoritative: the prepared map still requires external review, and full release-output reproducibility/release facts remain false. | Obtain external review of the exact prepared input, then bind independently approved pack/notice identities into the asset and provenance chain and mount only those exact packs inside the package Worker. | Capability commits `6351bcd4` through `e2fdbaeb`; fast gate 50 files/437 tests with strict typecheck/lint/architecture. Repeated direct exact materialization completes in 23.1 to 25.4 seconds; two-root verification completes in 44.45 to 46.84 seconds with reproducibility ID `bg.cpp.browser-header-distribution-reproducibility.sha256.986bcd7b462b1bac0653f33e61dba69403295fd69df4fdb1780bb27092de9337`. No license approval, production Worker compile, Docker, or browser execution occurred. |
...
header review input, exact notice output materialization, and two-root
header-subset reproducibility are now independent narrow authorities. Preserve
them while closing the external distribution/license decision and complete
asset/provenance closure. Neither reproducible bytes nor a package-generated
file map or notice bundle can substitute for external approval.
