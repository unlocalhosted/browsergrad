...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. Closed AOT/browser profiles, deployment-versus-compilation identity, a strict browser asset manifest, common lowering authorization, and a regular-file-only canonical VFS-pack verifier now exist. Existing source-spelling CuTe adapters remain frozen. All browser hashes/assets remain synthetic: no tracked Clang-WASM build, verified fetch/cache or mounted VFS authority, producer-neutral source request, worker invocation/result/evidence, browser authorization adapter, browser-produced artifact, real CuTe source proof, dynamic tensor/storage meaning, or browser-local source-compatibility evidence exists. Docker/native contracts remain optional synthetic CI/reference infrastructure only. | Replace AOT/test-oracle assumptions with a producer-neutral request, detached provenance/conformance expectations, source-independent available-header profile, and distinct compiler-runtime ABI; build/pin the BrowserGrad LibTooling extractor and full declared header packs; implement asset acquisition/mount and worker evidence; emit the canonical artifact from unmodified source in-browser; then prove Gate 2 semantic convergence, dynamic tensor copy, and required real WebGPU execution. | Gate 2 exact-source baseline at `aa605421`; focused profile/asset/VFS/artifact/authorization tests; zero browser-local C++ producer executions. Existing Docker shell/lifecycle tests prove only optional synthetic-adapter contracts. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile, asset-manifest, compilation-identity, and common-authorization authorities exist. A closed `application/vnd.browsergrad.vfs-pack.v1` structural inspector plus prepared-manifest/asset-bound authority now replace generic tar: fixed binary header/version, portable-ASCII regular-file index, implicit contiguous data, exact index/file/content-set/outer hashes, independent limits, and exact include-root/mount ownership. Identity transfer/unpacked bytes retain one meaning; mounted file-content bytes have separate per-asset/aggregate fields and ceilings. Tar/zip, links, sparse entries, offsets, metadata, Unicode aliases, and compression are unrepresentable. The selected implementation direction is a BrowserGrad-owned LibTooling AST extractor cross-built with pinned LLVM/Clang and Emscripten; Docker may reproduce release assets but is not browser runtime. All current identities/assets remain fixtures. | Correct the profile/request generality gaps before freezing the worker: producer-neutral local-source request, detached source provenance/conformance assertions, available-header universe versus observed opened closure, compiler-runtime ABI, worker byte length/integrity, and coherent VFS/heap budgets. Then implement verified fetch/cache/mount, worker evidence/browser authorization, real build assets, and one unmodified browser-local fixture. | Focused profile/asset/VFS/authorization contracts; real-byte manifest-binding fixture passes; zero asset fetches, mounts, worker spawns, or browser-local C++ producer executions. |
...
Correct the producer-neutral request and browser runtime profile before adding
more Docker producer work. Remove mandatory repository/revision and expected
opened-header/input-closure hashes from the common compile request; keep them
as detached provenance/conformance records. Split available VFS content from
request-observed opened inputs. Add the compiler runtime ABI, exact worker byte
length/interface, host-backed versus in-WASM VFS storage model, and coherent
cache/VFS/stack/heap/output ceilings.
