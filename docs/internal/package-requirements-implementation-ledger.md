...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact ABI/semantic-adapter binding, raw-Wasm inspection, Worker frame/VFS contracts, canonical transfer, one-shot entry, and typed controller terminal protocol share one closed contract. C ABI 1.2 adds exact fail-closed frontend-work accounting. Runtime ABI 1.8 independently pins the current generated exception bridges and fixed table. The package owns the exact reproducible 27,125-byte factory, a deterministic 572,534-byte compiler Worker graph, and a separate 151,555-byte raw-Wasm verifier Worker graph; both are self-contained ES modules with no static or dynamic imports. A dedicated post-build lane now admits the exact Wasm and five packs, verifies the raw interface in a separate Worker, and runs unchanged CuTe layout source in the compiler Worker without rebuilding. Worker failures preserve the deepest bounded authenticated cause, while VFS misses and module logs remain bounded diagnostics. | Resolve the current producer-or-rejected-Artifact-V3 internal error, then require the strict lane to receive one authenticated Artifact V3 terminal. Keep external producer trust, distribution approval, lowering, backend execution, and release as separate false authorities. | Exact reproducible Wasm SHA-256 `5fc425bbc051a2f5be588c2acbb164efb5e43f949afb48a373f3ed022c3b8758`, 31,641,377 bytes; factory SHA-256 `796a548237420df7f5eca0c0260d3cbe752aeca155d9c7182c6ad0f5491dfb12`; compiler Worker SHA-256 `25e42408771a2ccd3e0c232082022ba2c9c02ede8b5fc6a2618058a4ffad970f`; verifier Worker SHA-256 `df514398f671b2124d0df58babc49f03ed86d70504227d33acac1e4802abbb65`. Exact current preflight completes in under one second; Chromium observation completes in about seven seconds and reports `BG-COMPILER-CPP-CUTE-BROWSER-WASM-COMPILER-COMPILE-STATUS` at `$.runtime.compile`. The strict gate rejects this blocked observation. Zero valid production compiler-Worker compiles or browser-local C++ executions. |
