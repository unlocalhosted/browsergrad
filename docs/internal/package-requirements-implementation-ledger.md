...
Its fixture is synthetic; actual production backend issuance still requires
the package-controlled external producer policy and exact signed statement.
...
transition over the exact promoted build subject. Exact-payload CPU and
required real-WebGPU convergence and the separate backend composition are
implemented. Run that composition only after the package-controlled external
producer policy admits the externally issued exact-build statement. The
external file-level redistribution decision has a software admission seam but
no production evidence. Externally rooted producer trust, external legal
approval, actual production backend issuance, and final release composition
remain separate prerequisites; reproducibility, local execution, or synthetic
composition fixtures cannot satisfy them.
