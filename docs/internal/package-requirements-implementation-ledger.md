...
- **Current implementation slice:** Gate 3 signed-source browser compilation
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The current extractor is package-pinned from two byte-identical cache-free builds, and the strict eight-case Chromium matrix uses one package-owned exact compile profile. The five-pack/5,788-file header universe, Runtime ABI 1.17, both zero-import Workers, unchanged C++17/CuTe f32/i32/u32 rank-1-through-rank-4 sources, accepted Artifact V3 outputs, and distinct semantic view-copy candidates are content-bound. A deterministic materializer writes the exact 24-subject tree, and a producer-gated finalizer adds only the detached DSSE envelope. Two live exact 25-output roots are byte-reproducible and package-pinned. The exact-payload lane re-fetches that distribution in the browser, re-verifies the local producer, compiles all eight sources, authorizes each candidate through the shared seam, and proves complete bit-exact CPU/required-WebGPU convergence with nonzero-offset canaries. The hardened no-clobber exchange now re-verifies both external responses and composes opaque backend plus final-release authority in one process without serializing reusable authority. Docker stays outside the portable runtime graph. | Install the production policies, externally controlled builder and reviewer keys, and externally issued exact statements, then run the implemented unified production-release operation against those immutable responses. | Capability implementation commits `8d7f27eb`, `625244b6`, and `4b67ef09`; current lock `bg.cpp.browser-build-input-lock.sha256.fa21cfe45dec6b4869662cd613a7a300848657518f375c04f7f2193f3a874ad4`; complete-distribution reproducibility `bg.cpp.browser-full-distribution-reproducibility.sha256.64cc7401523b6026aba9430e2f081d708bc62cfcbe6fc343bf58ec0798aeec7b`; package evidence SHA-256 `285a73cc67ec4196104389f127f6728e531aa41c44fba8a79d61e89533f2b76e`. Workflow `30069614333` produced identical 31,841,008-byte Wasm at `19edd5622461b2308e83f10fb90f9f029241a5ba706e4c1741b194cb52a82138`. The 26,213-byte strict resource is `38fcfae4d0b9c11314ec90a50f3bbb17b34c46a2c407ca475b9f8b44311833e1`; the 13,224-byte convergence resource is `c60b3dd770b3267633cda4ad6fe10995b6a237b83b4e9bd72fb0d1c5796759ea` with matrix `bg.cpp.browser-exact-distribution-convergence.sha256.12665a3d1f38689f9439f0aba2a6b4c87f42d28e021c5804a528b294f8f7fe31`. All eight headed Chromium/Apple Metal 3 cases passed real browser compile, authorization, CPU, and actual WebGPU convergence. Unified release composition passes with ephemeral signer/reviewer fixtures, but external trust/approval, actual production authority issuance, and current `releaseReady` status remain false. |
...
| Gate 3 authorized CuTe view-copy lowering | `partial` | One exact authorized frontend artifact plus a closed entry/storage request lowers through semantic-core's sole `createVerifiedViewCopyArtifacts` constructor. The current artifact profile accepts same-dtype f32/i32/u32 with exact 32-bit device ABI/alignment, two distinct non-null global parameter pointers with source-only const qualification, one synchronous portable 32-bit copy with exact read/write effects, and equal flat static ranks 1 through 4. Source strides may be positive, zero/broadcast, or signed; the destination remains positive-affine and dense. Allocation lengths and view offsets are explicit host facts. Version-2 lowering derives exact minimum/maximum affine element offsets and proves both sides of the rebased root-allocation range; CuTe `cosize` never grants pointer capacity. Semantic-core still owns IDs, alias sets, effects, canonical hashes, reject-invalid-source semantics, and forbid-overlap. Focused signed reversals at ranks 1 through 4 converge through the canonical CPU and WGSL preparation paths with untouched canaries. Existing real Worker candidates still reach this same seam without folding storage facts into candidate or authorization identity. | Add an unchanged negative-stride C++/CuTe source to the strict real-browser compile/authorization/CPU/required-WebGPU matrix. Then replace the synthetic producer/reviewer material with valid externally rooted evidence. Dynamic layouts, rank 5 frontend facts, signed destinations, and broader tensor-engine profiles remain unsupported. | Commits `85631464`, `a8a861e2`, and `dca33be5`; this slice adds ten focused lowering cases, including four signed ranks and exact lower/upper storage refusal. Compiler passes 106 files/1,659 tests, strict typecheck/lint, and both architecture checks. Required actual WebGPU independently reconfirms all 20 backend cases on Apple Metal 3. The current eight-case real-browser matrix remains positive/broadcast only and retains `productionBrowserCompileObserved=false` until the next source-evidence slice. |
...
reproducible extractor. The exact distribution convergence lane now compiles,
authorizes, lowers, and executes that lineage through the shared CPU and
required real-WebGPU seam without a source-shaped bypass. The distinct backend
composition seam now binds that result and current reproducibility to an exact
independently admitted producer through the stable payload build subject.
