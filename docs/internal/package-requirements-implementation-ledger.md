...
| Gate 6 — framework convergence | `in-progress` | Public `Tensor.expand`, `Tensor.abs`, and `Tensor.sign` are retired from `CUSTOM` and emit typed `BROADCAST_TO`, `ABS`, and `SIGN`. Expand retains its shared eager/lazy conformance and materializing/resident plan routes. Abs/sign preserve real-numeric shape/dtype, reject bool, provide owning CPU realization, closure and symbolic VJP, functional grad, vmap, and direct ONNX mappings; tensor-plan/WebGPU remain explicit refusals. JIT's bounded v1 executable operation registry binds all three typed records one-to-one with validators and generates detached public support reporting from those same records. | Migrate the remaining advertised opaque operations through the registry; reconcile the remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate the cross-framework/platform support views from executable contracts. | ADR-0002 through ADR-0004; focused JIT unary/registry/VJP tests; architecture freeze narrowed from 36/39 to 33/36 while preserving the exact original-ID partition. Full regression counts are recorded in the dated verification entry. The existing kernels actual-device lane proves `BROADCAST_TO`; abs/sign make no actual-device claim. |
...
Gate 6 is active. `Tensor.expand`, `Tensor.abs`, and `Tensor.sign` are retired
...
registry generates all three public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is the coupled `Tensor.sin`/`Tensor.cos` family. Each operation must
enter the registry with explicit CPU, VJP/refusal, transform/export,
backend-profile, residency, and materialization decisions; support must not be
inferred from method presence or duplicated in hand-maintained tables.
