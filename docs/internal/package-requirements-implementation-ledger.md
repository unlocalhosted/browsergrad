...
| Gate 6 — framework convergence | `in-progress` | Thirty-one public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1, Smooth L1, probability BCE, BCE with logits, and KL divergence own bounded floating reduction contracts and nested per-example vmap shared with Grad. NLL owns unbatched/spatial class-axis semantics, int64 range-checked targets, optional class weights, ignored targets, exact weighted denominators, selected-class input VJP, mapped/captured-weight vmap, and unmapped ONNX `NegativeLogLikelihoodLoss`. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching now partitions equal-shaped graphs by session and bypasses mutable local/descendant buffers. | Migrate the remaining 8 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0029; architecture freeze narrowed from 36/39 to 8/8 while preserving the exact original-ID partition. NLL verification passes full JIT 50 files/324 tests, full Grad 55 files/352 tests, package codegen/build/typecheck/lint/unit gates, the 96-file/1,602-test compiler gate, 19 Python archive and 35 Node release-security tests plus packed/fresh consumers, and the seven-package Node 25.9.0 build in 8.10 seconds. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.kl_div` and `torch.nn.functional.nll_loss` are retired
from the opaque callback inventory and have typed CPU, VJP, transform,
export/refusal, backend, and residency decisions. The executable v1
framework-operation registry generates all thirty-one public support records
from their validator bindings and preserves the original opaque-ID partition.
The next software transition is the coupled `cross_entropy` consumer. Each
