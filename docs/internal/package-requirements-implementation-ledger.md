...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict producer, runtime ABI 1.8, exact reproducible factory, zero-import package Worker, production controller, retained result authority chain, hardened Artifact V3 verifier, and bounded evidence authorities remain intact. The controller now re-unwraps the exact protocol-issued terminal authority and cross-binds invocation, request, profile, request-binding, and artifact identity immediately before minting execution evidence. Manifest v1.4 and the strict DSSE verifier bind a synthetic P-256 signature to the exact build subject and profile-pinned asset identity without claiming producer trust. The local fast harness is a tens-of-seconds loop; cached remote validation is a parallel sub-five-minute lane, and the comprehensive harness uses four bounded post-prerequisite lanes. Direct one-run distribution materialization completes in 23.1 to 25.4 seconds, and exact two-root subset reproducibility completes in 44 to 47 seconds. All 17 header-distribution outputs are reproducible across distinct roots. The configured header universe and complete package-owned external-review input are materialized; external license/distribution approval, full release-output reproducibility, independent producer trust, valid browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Obtain external approval for the exact prepared file map, CUDA index, Linux/CUDA evidence, and notice identities; admit the browser-build signer policy through an independent package-owned trust root and obtain a real external exact-build statement; bind approved pack/notice identities into the asset chain; then prove one unmodified browser-local source through authenticated Worker execution, shared semantics, and real WebGPU. | Capability commits through `32fd4c47`. The Node 25 focused Vitest phase passes 56 files/510 tests in 11.19 seconds; the full compiler suite passes 80 files/1,501 tests in 12.51 seconds after build. Comprehensive local verification passed in 2 minutes 6.39 seconds. CI `29697264202` passed the sharded graph in about 4 minutes 15 seconds; exact signature-binding CI `29698350889` passed in about 4 minutes 12 seconds. Header-distribution reproducibility ID is `bg.cpp.browser-header-distribution-reproducibility.sha256.986bcd7b462b1bac0653f33e61dba69403295fd69df4fdb1780bb27092de9337` for 17 files/68,899,643 bytes. Zero valid production Worker compiles or browser-local C++ executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact ABI/semantic-adapter binding, raw-Wasm inspection, Worker frame/VFS contracts, canonical transfer, one-shot entry, and typed controller terminal protocol share one closed contract. C ABI 1.2 adds exact fail-closed frontend-work accounting. Runtime ABI 1.8 independently pins the current generated exception bridges and fixed table. The package owns the exact reproducible 27,125-byte factory and one deterministic 584,954-byte self-contained ES-module graph with no static or dynamic imports. The production controller captures platform effects before input inspection and consumes one package-owned invocation with failure-safe terminal cleanup. | Acquire and mount licensed exact header packs, then execute one unmodified browser-local fixture through the production controller. Keep lowering authority false until the valid terminal frame and Artifact V3 are authenticated. | Exact reproducible Wasm SHA-256 `5fc425bbc051a2f5be588c2acbb164efb5e43f949afb48a373f3ed022c3b8758`, 31,641,377 bytes; factory SHA-256 `796a548237420df7f5eca0c0260d3cbe752aeca155d9c7182c6ad0f5491dfb12`; package bundle SHA-256 `d9bd0eea4b9084eb7dd0768b35fadd0f14667b3a09dd4662a3bc052fb331c4e9`. Two-build v3 evidence is package-pinned, and real Chromium exact-bundle smoke passes 1 file/3 tests. Zero valid production Worker compiles or browser-local C++ executions. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact extractor closure, LLVM 22.1.8 archive, Emscripten image, four-step recipe, ABI, and distribution plan. Clean builds seal the exact Node runtime closure; cached diagnostics restore only untrusted content-addressed toolchain directories inside the isolated container and grant no clean/release authority. Current admission verifies the production-shaped configure/build working directories, rereads exact Wasm, reruns the independent inspector, and requires byte-identical reports. Reproducibility v3 retains both raw linker-map identities but compares a strict path-canonical semantic projection; the exact two-build record is package-pinned. The separate 17-file header-distribution subset is rehashed and reproducible across two distinct roots, and its path-independent observation is package-pinned and independently rederived. Manifest v1.4 binds the browser build-signature policy into the asset-set identity; the signature verifier proves exact build-subject binding but deliberately returns `producerTrusted=false`. The comprehensive verifier retains three serial prerequisites and executes the remaining exact command DAG in four bounded fail-fast lanes; CI separately shards source/dist corpus and native/WebGPU/Node surfaces. | Preserve the narrow reproducibility authorities while closing external license approval, the complete release-output set, independently admitted producer trust, Worker execution, and release separately. | Current lock `bg.cpp.browser-build-input-lock.sha256.bf62353c9421b955cd1a07e14e13c5e3417b5431e2be4555283acdacc0ee7def`. Extractor v3 resource SHA-256 is `6c7aebe1376edf0f9a526b55bacd930e7e9e0fd454a95213d97931117548f31a`; header-distribution resource SHA-256 is `eb85b8fb54d1bad1b932bd4bfb3ffc7aff8d66d6185c9926ddd3c9f6084d918a`. Focused fast-harness Vitest phase: 56 files/509 tests in 11.39 seconds; full local `verify:compiler`: 2 minutes 6.39 seconds; sharded CI `29697264202`: about 4 minutes 15 seconds. No licensed distribution, independently trusted producer, Worker execution, or release authority yet. |
| Gate 3 browser build signature binding | `partial` | Manifest v1.4 binds the exact predicate type, trust-store digest, and canonical builder allowlist into the profile-pinned asset-set identity. One strict canonical DSSE/in-toto verifier authenticates P-256 P1363 bytes and rebinds the statement to the exact profile/compilation contract, asset manifest, build-input lock/recipe, package Worker/factory, and cycle-free build subject. Its opaque result is narrowly a `VerifiedCppCuteBrowserBuildSignatureBinding`; `manifestSignaturePolicyMatched=true` while `producerTrusted=false`, exact asset bytes, full reproducibility, legal/distribution approval, Worker execution, and release remain false. | Add a separate host-only package-owned trust-policy authority that independently admits the manifest policy; then obtain and verify one externally issued statement for the exact current build subject. Synthetic or package-generated keys cannot satisfy producer trust. | Commit `332ddf7a`; ephemeral P-256 fixture only; compiler typecheck, focused signature tests, full 80-file/1,500-test compiler suite, 56-file/509-test fast harness, strict lint, Worker-bundle check, and architecture check passed locally. Exact-source CI `29698350889` passed all Node 20/24/25, required-native, source/dist corpus, real WebGPU, and Pyodide lanes in about 4 minutes 12 seconds. |
...
| Gate 3 Worker invocation/result protocol | `partial` | Exact profile/asset/VFS/request/ABI/raw-Wasm identities bind one invocation. Host transfer remains single-reservation/single-materialization with no network authority. Caller input cannot provide Worker module bytes. The package composer injects a fresh exact bundle copy, and the production controller captures Blob/Worker/URL/timer/listener operations at module evaluation, rechecks bytes before Blob construction, validates one terminal frame, and closes every effect before minting an opaque execution observation. The controller now re-unwraps the exact protocol-issued frame and cross-binds its retained invocation/request/profile/request-binding/artifact chain at that minting boundary; structural copies fail closed. Validated results retain the exact observed execution plus invocation/request/profile/manifest/VFS/runtime-ABI/package-Worker chain. Caller validation remains `caller-frame-consistency-only`; no lowering authority is minted. | Execute and authenticate one valid package invocation using the reviewed factory, current ABI Wasm, licensed packs, and independently admitted producer trust. Only then add a separate authority transition from observed execution to the verified semantic artifact seam. | Commits `5733afe2`, `bc429d5e`, `4f4e5814`, `f758eecc`, and `32fd4c47`; current fast/full coverage is 56 files/510 tests and 80 files/1,501 tests. Real Chromium bundle/platform smoke passes 1 file/3 tests. No valid browser Worker compile or lowering authority exists. |
...
| Work item | Status | Notes | Remaining | Evidence |
|---|---|---|---|---|
| Gate 3 canonical artifact resource | `verified` | Artifact v3 has one authority-bound canonical byte representation with independent raw SHA-256 and wire-u64 length. Source declarations require exact identifier/declarator `identitySpanId`; exact input ownership distinguishes source, compiler resource, and pinned dependency files; forced includes are profile-bound. View-copy control graphs now use bounded non-recursive traversal with target-intrinsic cycle detection, including cycles that do not return through the first operand. Optional AOT receipt and in-toto contracts bind the raw resource; browser worker evidence remains missing. | Real browser-produced bytes and browser authorization remain unproved. | Commit `912195fa`; artifact suite 33 tests plus request-binding/AOT mutation coverage; pinned v3 semantic hash `bbb05a48...`, byte SHA-256 `02e00b21...`, and byte length `15081`. |
...
The manifest-pinned build-signature verifier is now complete as a narrow
cryptographic binding. The next trust transition must be a separate host-only,
package-owned trust-policy authority that independently admits the exact
predicate, trust-store hash, builder IDs, key IDs, and policy version before
minting producer authority. It must consume the existing opaque signature
binding rather than changing its `producerTrusted=false` claim. A real status
claim additionally requires an externally controlled key and an externally
issued statement over the exact current build subject; fixture or
package-generated signatures remain synthetic.

The controller and package invocation are wired, and the execution-evidence
boundary now re-authenticates the exact protocol-issued result authority rather
than accepting a structural copy. The next Worker proof must use them to
instantiate only host-verified current-ABI Wasm bytes, preserve the stable
pre-bind VFS imports, bind exactly the exported unshared memory, exercise the
allocator/result metrics synchronously, and keep lowering authority false until
valid module observations satisfy the independent manifest and independently
admitted producer trust.
