...
- **Current implementation slice:** Gate 3 pinned native producer and live execution evidence
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, job, logical plan, artifact/resource, receipt, provenance, authorization, OCI metadata, image observation, and completed-run authorities remain distinct. One private Node session now performs `version -> info -> image-inspect -> stage -> create -> inspect-created -> start-attached -> inspect-terminal -> remove -> prove-absent`, then removes staging before decoding the existing bounded result frame and minting authority. Container identity recovery is label/name/plan-bound; ambiguity preserves staging and fails stop. Created-container checks bind the selected manifest, config/argv, daemon-recorded security/resource options, registered mounts, state, and exact CID file. Production and fake issuers are disjoint. This is still fake-adapter contract evidence: inspect proves daemon-recorded request/state, not kernel enforcement. No live image, extractor, supervisor, receipt, cgroup record, external signing, dynamic tensor/storage meaning, backend execution, or source-compatibility claim exists. | Build and pin the real extractor/supervisor OCI image and execution-environment manifest; exercise the lifecycle on the exact live Linux/Docker/runc environment; authenticate the resulting receipt externally; then implement dynamic tensor copy and exact Gate 2 semantic/backend convergence. | Gate 2 exact-source baseline at `aa605421`; Docker shell/lifecycle 92 tests; focused policy/runner/OCI 21 tests; all Docker lifecycle evidence remains synthetic until the pinned live environment is exercised. |
...
| Gate 3 logical AOT execution plan | `verified` | Checked policy `1.1` defines one exact local Docker endpoint and client/engine/API/store tuple, authorized-manifest creation with pull forbidden, fixed native supervisor/hostname/argv, Docker-injected versus supervisor-cleared environment, non-root identity, no network/IPC, private remaining namespaces, read-only image rootfs plus explicit Docker-managed mounts, no added capabilities/new privileges/host-device passthrough/socket, bounded tmpfs/frame/stderr, profile-derived limits, complete lifecycle/decode budgets, fail-stop cleanup, and a dedicated single-job host trust boundary requiring external environment attestation. Plan hashing re-verifies the current policy. | CPU/peak resource facts, seccomp/runc/kernel enforcement, dedicated UID/core-dump configuration, and the actual supervisor environment still require the real execution-environment manifest and live evidence; daemon inspect cannot prove them. | Checked JSON/TypeScript equality; policy `86d5ccd7ca20ebdc72431358b9e02c581381a6191cea161a15635cc04c08cb78`; fixture execution plan `8b8b3809df6e20dde842a3488553b2f84093426aca0c95f7b1bcfb53b04091c7`; nested immutability; 92 Docker tests; strict typecheck/lint. |
| Gate 3 offline-runner I/O authority | `verified` | The only caller inputs are one prepared-job authority plus exact `{ fileId, bytes }` blobs. Captured descriptors establish membership/kind/length before allocation; hostile/shared/detached/non-byte arrays fail closed. Bytes are copied before the first await, checked by digest/anchor, and retained privately. The container lifecycle stages canonical profile/job bytes plus those exact snapshots with exclusive no-follow files, read-only bind trees, readback hashes, and a closed path inventory. Output accepts one bounded framed canonical artifact+receipt only after container absence and staging removal. | No contract gap. Real producer output remains unproved until live execution. | Runner-plan tests plus lifecycle happy/failure staging checks; 0444 files, 0555 bind trees, descriptor/proxy/TOCTOU/frame/copy isolation coverage. |
| Gate 3 OCI manifest/config metadata authority | `verified` | One cacheable opaque authority snapshots and strict-decodes exact raw OCI manifest/config bytes, derives their SHA-256 descriptors, requires a closed OCI leaf manifest and self-contained distributable layer descriptors, binds exact config bytes, requires `linux/amd64`, exact layer/diff-ID correspondence, bounded history/annotations, and empty image execution config. Separate authorities bind metadata to one plan, observe the selected local image, and verify the created container still names that manifest/config closure. Layer blobs and actual execution remain outside synthetic evidence. | Exercise the exact contract against the pinned live image/environment; current adapter tests prove verifier behavior only. | OCI contract tests plus 92 Docker observation/lifecycle/strict-decode/adversarial tests. No live-daemon claim. |
| Gate 3 local Docker runtime/image observation contract | `verified` | Fixed `/usr/bin/docker` probes run in strict order `version -> info -> image-inspect` against one Unix socket with a closed environment, private empty mode-0700 config/HOME, exact Go templates, and independent ceilings. Exact Docker 29.6.1 client/engine/API/containerd store precedes image inspection. The selected manifest/config/rootfs/platform is cross-bound to plan-authorized OCI metadata. The same private session is consumed once by the lifecycle; neither public observation nor test authority can mint production completed-run authority. | Live invocation and binary/execution-environment provenance remain required. | 79 observation/process tests plus 13 lifecycle tests; timeout/abort/reaping, hostile shapes, config/HOME mutation, issuer separation, and structural-copy rejection. |
...
synthetic lifecycle contract. Add the extractor and supervisor sources,
hermetic Dockerfile/build inputs, exact Clang/CuTe/header closure, execution-
environment manifest, supervisor environment clearing and cgroup/resource
receipt fields, production trust/signing workflow, and at least one real CuTe
fixture. Build the digest-qualified `linux/amd64` image, exercise the exact
Linux Docker 29.6.1/runc lifecycle, retain authenticated artifact/receipt
evidence, and keep daemon-recorded request state separate from kernel/runtime
enforcement evidence. Only then continue to dynamic tensor copy, exact Gate 2
semantic-hash convergence, and required WebGPU execution.
