...
- **Current implementation slice:** Gate 3 static rank-3 CuTe view-copy lowering through the existing canonical path
...
| Gate 3 authorized CuTe view-copy lowering | `partial` | One exact authorized frontend artifact plus a closed entry/storage request lowers through semantic-core's sole `createVerifiedViewCopyArtifacts` constructor. The initial profile requires f32 with exact 32-bit device ABI/alignment, two distinct non-null global parameter pointers with source-only const qualification, synchronous portable 32-bit copy with exact read/write effects, and positive flat static rank-2 source/destination layouts. Allocation byte lengths and view offsets are explicit host facts, aligned and range-checked against the verified affine address spans; CuTe `cosize` never grants pointer capacity. Semantic-core owns IDs, alias sets, reject policy, forbid-overlap policy, and canonical hashes. The canonical CPU reference executes the pinned transpose bit-for-bit with nonzero offsets and untouched canaries. The exact observed Worker candidate plus independent producer now reaches this same canonical seam without folding storage facts into candidate or authorization identity. | Prove the same canonical payload and bits through required real WebGPU. Replace the synthetic browser authority fixture and producer material with a valid production Worker compile and externally rooted producer. Dynamic/rank-3/tensor-engine extensions remain later profiles. | Commits `85631464` and `a8a861e2`; pinned layout hash `5ade6e06...`, kernel hash `64dc9d67...`; 80-file/700-test fast harness in 11.54 seconds, build/typecheck/lint/architecture/whitespace checks, and exact-source CI `29807986545`. Current browser authority remains synthetic, not production Worker execution. |
...
- This closes exact canonical fixture-payload CPU/WebGPU convergence only. It
  ...
  backend, distribution, or release authority. The next software capability
  is static positive-affine rank-3 view-copy lowering through the same seam.
...
The producer-neutral authorized artifact seam now lowers one exact static
rank-2 f32 view-copy into the canonical Gate 2 layout/kernel artifacts and CPU
reference. An observed browser view-copy candidate plus the independently
admitted exact producer converge into that same canonical authorization while
retaining the full Worker/profile/request/artifact lineage. Candidate and
...
from CuTe `cosize`. The exact canonical payload now also passes required actual
WebGPU on Apple Metal 3 and CI SwiftShader without minting backend,
distribution, or release authority. The next software transition is static
positive-affine rank-3 lowering through the same canonical path. The synthetic
browser authority fixture still does not substitute for a real production
Worker compile.
