...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The five-pack/5,788-file header universe is reproducible under a narrow stable header-input projection. The native producer now emits a complete source-derived pointer/Tensor/`cute::copy` Artifact V3 graph. Runtime ABI 1.14 pins the current 75-function import surface and 15,304-entry table; detached inspection of the diagnostic Wasm reports exact conformance with zero mismatches. The package Worker and verifier bundles were regenerated deterministically. The prior layout-only Chromium observation remains historical and is deliberately rejected by the current exact-Worker verifier. Docker stays outside the portable runtime graph. | Obtain two-clean-build and detached package evidence for the current ABI/build lock, then run the exact source-derived view-copy payload through the current real browser Worker. Admit that exact observed candidate through externally rooted producer trust, shared lowering, and required real WebGPU. Independently obtain external approval for the exact file map/notices and an externally issued exact-build statement under the production signer policy. Broader dtype/layout profiles remain separate capabilities. | Diagnostic build `30053246445` completed isolated compile/link in about 3 minutes 18 seconds and produced a 31,835,141-byte Wasm with SHA-256 `7939cb244b1d02346dbd511c61b4bdaf4d58c9321dfd62dba20bf7c57558cc8f`; ABI 1.14 detached review reports zero mismatches. Current Worker SHA-256 is `c69678649bd3c152b4381cadec5b570a239d9e7ac8c4f302e897cea47d2bab50` at 583,032 bytes. The historical strict observation remains pinned but no longer mints current authority; `producerTrusted`, license approval, lowering, backend execution, and `releaseReady` remain false. |
