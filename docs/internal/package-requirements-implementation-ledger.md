...
- **Current implementation slice:** Gate 7 GPU/backend-derived control contract
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, whole-allocation copies, terminal fail-stop materialization, dependency-ordered completion events, bounded fixed-count repetition, captured-input and request-time runtime-control u32 conditionals, one ordered produced-resource u32 conditional, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors run dispatches, copies, collectives, fixed repeats, and one selected branch in private rank-local storage under explicit ceilings, report completion only with whole-graph success, and expose only verified materialized outputs. The WebGPU adapter pre-lowers every branch, selects input/runtime controls before device work, and executes the produced-resource predicate through one exact prefix submission, four-byte readback, and selected branch-plus-suffix submission over persistent private buffers. Its separate device-bound pipeline authority prewarms every exact step-slot program and alternative, authorizes only complete or contiguous exact slot ranges, binds graph/backend/WGSL/schedule/features/limits/numerical-policy identity into the cache namespace, rejects forged/destroyed/cross-device/invalid-offset/mismatched use, and is reused by explicit hot execution and the convenience path. Complete f32/i32/u32 collective, repeated f32 sum, event-marked raw-u8 materialization, and both branches of all three predicate sources have actual-device parity. A separate required real-device record compares prewarmed fixed repetition with an equal-work, bit-exact statically unrolled graph without asserting superiority. | General runtime-derived loop/launch control and nested/device-side branching; browser-worker transport/topology/failure evidence; named native companion profiles. Event timestamps, external waits, and cross-queue/cross-worker synchronization require separately named future profiles. | Semantic-core 23 files/215 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/182 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; twelve complete-output CPU/WebGPU parity cases through explicit pipeline authority plus non-finite refusal and actual device-loss/authority-lifecycle refusal on Apple Metal 3; generic real-device contiguous-slot authority coverage; separate prewarmed two-rank 65,536-element f32 authority-reuse performance observation with 8 warmups and 12 paired samples on the same device; root/compiler architecture checks with zero cycles/leaks. |
...
stage over persistent private buffers. These profiles do not claim timestamps,
external waits, general runtime-derived loop/launch control, nested/device-side
branching, or transport. Separate device-bound pipeline authority is
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies. Versions 1.5 and 1.6 add bounded captured-input and request-time runtime-control u32 conditionals. Version 1.7 adds at most one produced-resource u32 conditional whose exact rank-local aligned four-byte temporary predicate requires a dominating ordered writer. All conditional profiles retain equal branch node-kind structure, globally unique node IDs, conservative merged effects, guaranteed-write intersection, and exact one-branch work counts. Versions 1.0–1.6 retain exact decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. General runtime-derived loop/launch control and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/215 tests, including canonical older-version retention plus repeat and conditional version/mode/predicate/control/identity/body/shape/effect/order/count rejection and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and required runtime control before its first await, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, exact fixed repeats, and one input-, runtime-control-, or produced-resource-selected conditional branch with cancellation/time checks at every admitted boundary. Resource predicates are read from private graph storage only after ordered producer execution. Missing, duplicate, unknown, or greater-than-u32 runtime controls fail before execution. Completed event IDs, repeat records, conditional branch records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, exact one-branch element or copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller input/control mutation isolation, strict runtime-control admission, copied-byte budget refusal, terminal output publication, successful completion reporting, exact three-iteration f32 sum to `[16,24]`, both branches from all three predicate sources, ordered resource production, and pre-admission expanded-operation refusal. Semantic-core passes 23 files/215 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed-repeat/conditional execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and exact fixed repeat bodies into bounded canonical WGSL sequences, and pre-lowers every conditional branch through those same paths. `browsergrad.host-graph.webgpu-pipeline@1` separately binds the exact prepared graph to one admitted device, prewarms every unique program at each exact step slot including conditional alternatives, and records graph/backend/WGSL/schedule/features/limits/numerical-policy identity. Explicit hot execution reuses that authority; the convenience API captures caller inputs/controls and then delegates through the same ephemeral path. Complete sequences and validated contiguous slot ranges reject copied, destroyed, cross-device, reordered, invalid-offset, or unadmitted program use before request resource allocation. Input/runtime controls select before device work. The version-1.7 resource predicate keeps private graph buffers resident across an exact prefix submission, four-byte predicate readback, and selected branch-plus-suffix submission; the same timeout, cancellation, in-flight owner, device-loss, cleanup, numerical-status, and terminal-publication contract spans both stages. Events and materialization contribute no GPU step; completed event IDs, repeat records, and conditional branch records appear only in the successful terminal trace. | None for the current portable DAG/fixed-repeat/three-predicate/pipeline-authority profile. Non-word-sized copies fail explicitly; general runtime-derived loop/launch control, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Twelve complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities and cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions of f32 sum, and both branches of captured-input, runtime-control, and produced-resource predicates after caller mutation. Branch pairs retain one request-independent pipeline identity and distinct branch-specialization identities; resource branches report one feedback stage. Separate checks reject non-finite f32, a lost device, copied authority, and post-destroy reuse. Generic real-browser coverage also rejects cross-device, wrong-offset, and program-mismatched pipeline-set use. Kernels passes 21 files/182 tests; packed/fresh-consumer coverage prepares and executes graph control through the public package seam. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 2.00/2.70 ms and baseline median/p95 1.90/3.20 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.8.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
- The separate observational performance lane remains semantically unchanged
  ...
  2.00/2.70 ms and unrolled median/p95 is 1.90/3.20 ms; artifact
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`,
...
buffers. The separate device-bound pipeline authority prewarms every exact
...
supports exact contiguous range execution and explicit hot reuse without
...
general runtime-derived control/launch beyond the single bounded resource
predicate; transport/topology, worker meshes, native companion execution, and
