...
- **Current implementation slice:** Gate 3 pre-run AOT producer request and evidence authority
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers preserve exact resolved facts. A separate pre-run job authority now pins exact source identity/revision, content-addressed virtual files, one AST declaration-token anchor and expected entry, profile/header/input/output closure, and contains no command, flag, environment, host-path, or output-path escape hatch. Detached provenance uses DSSE/in-toto with an instance-bound authorized wrapper, and the static lowerer consumes only that wrapper. No real producer, observed sandbox receipt, canonical artifact-byte subject, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists yet. | Bind provenance to exact canonical artifact bytes and an observed runner receipt; implement the offline fail-closed container runner; pin/build the real native extractor, image, policy, profile, source/header closure, and external signing workflow; then prove dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 53 focused profile/job/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 40 files/1059 tests plus typecheck/lint and architecture/corpus gates; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Ahead-of-time profile, pre-run producer request, and structural artifact verification are implemented as separate instance-authorized records. The request is closed, content-addressed, profile/header-bound, and pins exact source Git identity, source blobs, VFS paths, selected declaration anchor/expected entry, and expected source/header/input/output closure before execution. The artifact remains one-TU, exact-version, bounded, canonical, content-addressed, opaque, and browser-safe, with resolved source/type/template/layout/intrinsic facts. Producer metadata changes transport identity only; no field can self-assert trust. | Canonical artifact-byte authority, observed runner receipt, offline sandbox runner, real native extractor/image/profile/fixture/signing workflow, dynamic tensor copy, exact Gate 2 hash equality, and required WebGPU evidence. | 33 focused profile/job/artifact tests; pinned synthetic request/profile/artifact IDs; compiler typecheck/lint; compiler and semantic architecture checks. |
...
| Gate 3 detached provenance authorization | `verified` | A profile pins extractor, runner, resolved `linux/amd64` OCI manifest, sandbox policy, allowed builders, and exact P-256 SPKI trust store. Verification authenticates one canonical DSSE envelope before evaluating its in-toto Statement v1 claims, binds full artifact/profile/toolchain/sandbox/runner/invocation/output identity, accepts tagged Git SHA-1/SHA-256 revisions, enforces source/include VFS roots, and mints opaque attestation and authorization authority. Rejected artifacts, structural copies, object-instance substitution, unpinned profiles/sources, hostile trust stores, malformed P1363 signatures, noncanonical payloads, and signed policy drift fail closed. The static layout lowerer has no raw/verified artifact overload and consumes only this authority. The custom predicate is explicitly not SLSA or Sigstore. Trust-key rotation creates a new authorization lineage while lowered semantic hashes remain provenance-neutral. | Real producer and Sigstore-backed external evidence remain separate and are not claimed. | 11 provenance tests plus mutation matrices; authorized-only lowering substitution tests; pinned trust/profile/artifact/source hashes; strict typecheck and lint. |
...
- Provenance audit found the current in-toto subject binds the semantic artifact
  hash rather than SHA-256 of exact emitted canonical bytes, and signs declared
  sandbox facts without an observed runner receipt. Those are now explicit
  pre-producer blockers. No real artifact may be authorized as producer evidence
  until raw-byte and receipt authority are implemented and mutation-tested.
...
Make canonical normalized artifact bytes an explicit verified resource: expose
their SHA-256 and byte length, reject noncanonical producer files, use that raw
digest as the in-toto subject, and bind it in the output manifest. Then add a
versioned observed runner receipt and require its digest for attestation. Only
after both authorities exist should the offline Docker runner shell be added.
