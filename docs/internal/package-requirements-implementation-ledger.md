...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native producer and pinned four-step no-shell build execute in a networkless, read-only, capability-free container with bounded immutable evidence. The canonical local harness loop is 29.98 seconds and exact-source warm validation is 5 minutes. C ABI 1.2 records exact frontend-work metrics. Runtime ABI 1.8 closes the observed 66-function generated-import surface and 15,166-entry exception table. Two distinct cache-free builds now prove exact extractor Wasm/factory reproducibility. The package binds their canonical 3,470-byte evidence and includes the factory inside one exact 577,876-byte zero-import Worker graph. Production uses captured platform effects and package-owned one-shot invocation composition, but mints no lowering authority. Exact inventory/materialization boundaries exist for all five required header trees, and all ten approved locked notice resources are packaged and reverified. Real header packs, two remaining distribution reviews, external file-level license review, full-distribution reproducibility, provenance, valid browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Acquire and materialize the real exact header trees; close CUDA 12.6.3 and Linux-sysroot distribution policy plus the external per-file license map; then prove one unmodified browser-local source through authenticated Worker execution, shared semantics, and real WebGPU. | Commits `c01b5f25`, `c1a79c0d`, and `2d4a6fa5` close extractor reproducibility without rebuilding completed artifacts. Local `verify:browser-clang-wasm:fast` passes 37 files/400 tests in 29.98 seconds. Build run `29683677087` produced matching 31,641,377-byte Wasm and 27,125-byte factories; verifier-only run `29685632925` admitted them in 1 minute 7 seconds. The v3 evidence SHA-256 is `6c7aebe1376edf0f9a526b55bacd930e7e9e0fd454a95213d97931117548f31a`. Zero real distributed header packs, valid production Worker compiles, or browser-local C++ executions. |
...
- The current Node 25 fast gate passes 37 files/400 tests in 29.98 seconds end
  to end, including a 21.72-second Vitest phase. Strict typecheck, source lint,
...
- `2d4a6fa5` package-pins the exact v3 evidence, verifies it byte for byte behind
  ...
  The fast gate passes 37 files/400 tests in 29.98 seconds, and real Chromium
