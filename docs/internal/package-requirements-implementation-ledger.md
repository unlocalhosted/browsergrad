...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. The exact canonical runtime-ABI resource is now an acquired asset, a deterministic distribution output, and a strict-decoded authority bound to the same verified asset set as the VFS installation. A bounded raw-WASM inspector separates standards validity, exact byte identity, observed structure, and ABI comparison; it accepts spec-valid padded LEB encodings, validates the complete module, closes imports/exports/memory/tables/globals/custom sections and extension opcodes, and cannot let observed bytes widen the independent manifest. The current ABI deliberately keeps first-build projections empty, so no production conformance authority can mint until a real build is independently reviewed and the ABI is repinned. Existing request/artifact/CUDA-pass/common-lowering seams and optional AOT composition remain unchanged. Docker/native contracts remain optional build/CI/reference infrastructure only and never enter the portable browser execution graph. | Build the real extractor and reviewed header packs; independently repin complete generated-import/support-export/table/global/custom-section projections; finish worker-owned aggregate lazy VFS, actual Worker controller/evidence, and browser authorization; emit a canonical artifact from unmodified source in-browser; then prove semantic convergence, dynamic tensor copy, and required real WebGPU execution. | ABI/assets/build-lock/inspector focused 5 files/111 tests; strict compiler typecheck and focused lint; zero real Clang-WASM builds, Worker spawns, or browser-local C++ executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.3, producer-neutral request, artifact v3, request binding, compilation contract 1.0, common lowering, and explicit CUDA pass authorities remain the shared seams. Asset manifest 1.1 now requires the exact canonical runtime-ABI bytes; acquisition strict-decodes them into an opaque authority bound to the same verified asset set as the installed VFS. The bounded raw-WASM inspector proves binary framing, standards validation, exact signatures, memory, tables/globals/tags/start, custom metadata, and closed extension use without deriving permission from observation. Current empty first-build projections make positive production conformance intentionally impossible. | Finish aggregate lazy-VFS execution, actual Worker-controller evidence, browser authorization, first real build/ABI repin, licensed packs, and one unmodified browser-local fixture. | Focused ABI/asset/build/inspection 5 files/111 tests; typecheck/lint; zero Worker spawns or browser-local C++ executions. |
...
The dedicated-Worker protocol draft binds the right profile/request/asset/ABI
facts, but its public result verifier is not execution evidence: caller-supplied
frames and wall time can self-attest without a Worker instance. Keep that path
non-authoritative. First complete the Worker-owned aggregate VFS session and
retain only compact result-verification state. Then add a host-owned controller
that creates the exact verified Worker, owns the fresh invocation nonce and
event source, enforces one terminal result, and performs terminate-and-replace
cleanup. Only that controller may mint browser execution evidence or feed the
browser branch of common lowering authorization.
