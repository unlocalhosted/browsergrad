...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 rejects fake bfloat16 before allocation, parameter constructors enforce their exact float32 profile, and `contiguous()` now owns conditional identity/C-order-copy semantics with dtype-preserving backward. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Reconcile remaining Grad detach, conversion, and NumPy-interop fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0038; architecture freeze preserves the exact JIT partition and revised Grad contracts. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 137.16 seconds, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 24-test mutation suite pass. |
...
The next Gate 6 software transition is remaining Grad materialization and
conversion convergence, beginning with `Tensor.detach`, followed by
cross-dtype `to`, NumPy interop, and generated runtime/profile support
