...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native compile session, closed policy materialization, pass-scoped VFS observation, sealed CUDA device-first/host-second actions, diagnostic normalization, and canonical production Artifact V3 writer execute as one native producer. The pinned build boundary executes exactly four no-shell steps in a networkless, read-only, capability-free container and records bounded immutable logs, generated bytes, source/tool identities, and isolation observations. Separate authorities compare two distinct clean builds and inspect raw Wasm without granting release or Worker authority. The cached diagnostic lane now completes an ordinary extractor validation in under five minutes and every admitted build emits an independent ABI report, but that lane cannot satisfy clean-build, reproducibility, provenance, or release authority. A real 31,327,956-byte diagnostic artifact was inspected and failed conformance on ten exact differences, including 92 generated imports with forbidden clock, random, process, environment, and ambient-filesystem capabilities. Production therefore remains capability-blocked before Worker construction. Header licensing, completed clean-build execution, ABI conformance, full-output reproducibility, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Close forbidden generated imports; complete and inspect a clean build; run the two-clean-build lane; independently review and repin only a conforming Wasm projection; bundle the generated factory into package-owned Worker bytes; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Harness and iteration fixes are on `main` through `056aaf02`. Full compiler verification passes 69 files/1,446 tests; its build-plan boundary passes 169 tests with 9 intentional platform skips and the optional Docker contract passes 96 tests. Cold run `29658164083` spent 97 minutes 5 seconds in isolated execution before failing closed. Final stable-cache run `29668822793` completed in 4 minutes 39 seconds, including a 27-second JavaScript boundary, 2-second cache restore, 3-minute-14-second executor, and 3-second ABI review; exact-source CI `29668819938` passed at the same revision. Its toolchain cache is content-addressed and untrusted; clean and reproducibility lanes do not restore it. ABI conformance remains false. Zero completed pinned clean Clang-Wasm builds, valid Worker compile launches, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact BrowserGrad LibTooling extractor closure, LLVM 22.1.8 archive, Emscripten image manifest/config, four-step recipe, ABI, and distribution plan. The executor stages only locked snapshots, executes exact argv/cwd/environment without a shell, enforces per-step time/output/cancellation limits, binds inputs against inode-version drift, hashes native TableGen tools, seals the factory/Wasm/link map and logs, validates UTF-8/WebAssembly, and atomically materializes only the Wasm sidecar. The manual workflow verifies acquisition and image identities, then runs with no network, read-only root/input mounts, zero capabilities, no-new-privileges, and a private writable work mount. Clean runs additionally seal the exact Node runtime closure; evidence v2 records it and the two-build comparator requires it to match. Cached validation restores only the exact content-addressed native-TableGen and Wasm-LLVM/Clang directories, treats them as untrusted, and emits no clean/reproducibility authority. Link-only policy is excluded from that expensive cache key, while compiler flags, CMake, selected libraries, LLVM, Emscripten, and the recipe remain bound. The fast lane runs focused non-native harness checks before container execution; clean/reproducibility and ordinary CI retain the full native sanitizer boundary. Every admitted build runs the bounded raw-Wasm reviewer, which emits discovery without self-authorizing an ABI. Temporary legacy/broad cache migration paths have been removed. | Close the forbidden generated Emscripten imports, finish and review one clean build, then run the two-job clean-build lane under the exact runtime boundary. Independently repin only a conforming projection before any Worker or distribution authority. | Build lock `bg.cpp.browser-build-input-lock.sha256.fe4e9fe4816e107d532322d3a81ebe457400ad70ae07d64f84cf072f5bab4a69`; diagnostic toolchain key `ba2867dd05bdc99485e0de219510c5f87ec5c5a459d2ee607bca40be549de41a`. Final run `29668822793` completed in 4 minutes 39 seconds at current `main` with a primary-key hit, skipped cache staging/saving, and no migration code. The independent review of its 31,327,956-byte Wasm completed in about 3 seconds and found ten mismatches. Exact-source CI `29668819938` passed at the same revision. Full compiler verification passes 1,446 tests; build-plan passes 169 with 9 intentional skips; optional Docker contracts pass 96. No completed clean build, ABI conformance, licensed distribution, authenticated provenance, or release authority yet. |
...
- Cached run `29667784478` first proved a stable no-migration diagnostic path in
  ...
  `29668822793` at current `main` completed in 4 minutes 39 seconds: JavaScript
...
Use the now-bounded cached loop to remove forbidden generated clock, random,
process, environment, and ambient-filesystem imports without weakening the
browser capability contract. Preserve an independent raw-Wasm observation after
each build. Classify the remaining Emscripten `invoke_*`, exception, allocator,
table, and support surfaces explicitly; do not repin the runtime manifest until
the observed module conforms to a reviewed least-authority projection.

Then run clean validation without a restored diagnostic cache and inspect its
