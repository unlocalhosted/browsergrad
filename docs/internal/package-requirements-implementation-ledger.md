...
- **Current implementation slice:** Gate 5 authority-bound WebGPU execution
...
| Gate 5 — tiled attention flagship | `partial` | Semantic-core owns the closed frontend-neutral `browsergrad.kernel.attention-forward@1` artifact, separate `browsergrad.schedule.attention-online-kv-tile@1` artifact, schedule-independent CPU oracle, and authority-bound schedule specialization. Logical meaning binds static rank-4 f32 Q/K/V/output views, exact scale, masks, stable-softmax phases, finite-domain requirements, comparison policy, effects, and VJP refusal. The schedule independently binds physical tiles, cooperative K/V staging, increasing traversal, tile-wise rescaling, uniform barriers, scalar vectors, and pre-state-update masks. The CPU path snapshots admitted inputs, rounds every declared f32 step, fails atomically, and implements the comparator. Schedule specialization derives bounded workgroup, staging, private-state, key-tile, and dispatch geometry from the exact logical/schedule authorities. Kernels now prepares exact schedule-bound scalar WGSL with cooperative K/V staging, private Q/output state, causal/tail exclusion before recurrence, and uniform barriers. | Implement authority-bound finite input snapshot/upload and WebGPU execution; add required causal/non-causal two-schedule real-device numerical evidence, truthful baseline naming, and separate correctness/performance records on named devices/browsers. | Semantic-core 19 files/166 tests. Kernels preparation passes 19 files/151 tests, build, typecheck, lint, architecture, hostile request/resource/schedule tests, two-schedule codegen separation, causal/non-causal mask placement, and packed-release consumption locally. Commits `4cfacd75`, `1ccd541e`, `de80103d`, and `fb438082` passed CI runs `29820268289`, `29820944140`, `29822236114`, and `29822619976`. No actual WebGPU execution, numerical-preservation, performance, FlashAttention-v2, frontend convergence, or Gate 5 exit claim. |
...
implemented. Kernels WGSL preparation is implemented without claiming device
execution. The current slice is authority-bound host upload/WebGPU execution.
