...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT 5-constructor-call/5-operation opaque matrix have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0032 retire 34 typed operation identities—including paired `torch.sort`/`torch.topk` outputs, overwrite `torch.scatter`, general `torch.einsum`, the typed loss family through full class-axis cross entropy, keyed dropout, and ordered stateful BatchNorm1d—from the original 36/39 baseline without widening any remaining caller or label. Grad parameterized-layer initialization obeys `torch.manual_seed` across Linear, convolution, transposed-convolution, and Embedding families. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check; 96-file/1,602-test compiler gate; 4-case JIT Gate 0 contract; full JIT and Grad integration; Node 20/24/25 and real Chromium/WebGPU evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-four public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, general string-equation `torch.einsum`, keyed dropout, and stateful BatchNorm1d. BatchNorm owns ordered exactly-once running-state effects, biased batch normalization, unbiased persistent variance, counting/cumulative momentum, immutable eval snapshots, closure/symbolic VJP, functional grad, and checkpoint replay. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching partitions equal-shaped graphs by session, bypasses mutable module buffers, and excludes effectful/random graphs. | Migrate the remaining 5 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0032; architecture freeze narrowed from 36/39 to 5/5 while preserving the exact original-ID partition. Node 25.9.0 verification passes the 11.22-second seven-package build, JIT build/typecheck/browser-typecheck/lint/24-unit/53-file 335-integration-test gates, Grad build/typecheck/lint/30-unit/58-file 359-integration-test gates, architecture mutation suite, and full 96-file/1,602-test compiler gate. Focused BatchNorm, replay, lifecycle, checkpoint, registry, Gate 0, IR, VJP, wrong-target, and malformed-contract checks pass in 2–4 seconds. The preceding dropout commit retains the latest complete release-body evidence. No typed BatchNorm actual-device claim is made. |
...
`nn.BatchNorm1d` are retired from the opaque callback
...
all thirty-four public support records from their validator bindings and
preserves the original opaque-ID partition. The next software transition is
`torch.nn.functional.interpolate`, including exact nearest/bilinear forward and
backward geometry. Each
