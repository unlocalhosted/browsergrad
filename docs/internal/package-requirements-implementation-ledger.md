...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.12/1.13 request-time/produced-resource rank-2/rank-3 rectangles, version-1.14/1.15 rank-4 rectangles, version-1.16/1.17 rank-5 rectangles, and version-1.18 request-time rank-6 rectangles. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Rank-6 WebGPU keeps axes 5/4 on x/y, flattens axes 0 through 3 into z, and reconstructs them behind a 32-byte eight-u32 guard in one maximum-prewarmed canonical view-copy slot; produced-resource rectangles retain the rank-5/single-feedback bound. | Produced-resource rank 6, rank 7+, repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Semantic-core 23 files/241 tests; kernels 21 files/197 tests plus strict actual-device gate; 46 complete-output CPU/WebGPU parity cases through explicit pipeline authority on Apple Metal 3; correctness artifact `39d99833cfa0e11597cb7a30f91121ed4e4fac4ad5296962c2ba05f7b2e8103a`; separate backend-1.20 prewarmed performance observation. |
...
and flattens axes 0 through 3 into z behind the same 32-byte guard.
These profiles do not claim timestamps, external waits,
repeated/device-side feedback, produced-resource rank 6, rank-7-and-higher domains,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Versions 1.12/1.13 add request-time/produced-resource rank-2/rank-3 rectangles, 1.14/1.15 add rank 4, 1.16/1.17 add rank 5, and 1.18 adds request-time rank 6 while preserving all older exact rank limits. All retain maximum-product admission, canonical axis normalization, exact older-version decoding, and logical-extent completion. | Produced-resource rank 6, rank 7+, repeated/device-side feedback, and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact request-time rank-2-through-rank-6 CPU rectangles, produced-resource rank-2-through-rank-5 rectangles, pre-1.18 rank-6 refusal, and version-1.18 produced-resource rank-6 refusal. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.12 through 1.18 execute request-time rank-2-through-rank-6 and produced-resource rank-2-through-rank-5 dense rectangles against the full semantic row-major domain, reserve the artifact-maximum product, and report exact logical extents, product, and rank-multiplied actual work. | None for the current CPU-reference profile; portable WebGPU remains separate. Produced-resource rank 6 and rank 7+ are not inferred. | Focused CPU coverage includes exact partial/full `[3,4]`, `[2,3,4]`, `[2,2,3,4]`, `[2,2,2,3,4]`, and request-time `[2,2,2,2,3,4]` rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.21 adds version-1.18 request-time rank-6 rectangles through the same maximum-prewarmed canonical view-copy seam. It maps axes 5/4 to x/y, flattens axes 0 through 3 into z, and reconstructs them behind the existing 32-byte guard while preserving the produced-resource rank-5/single-feedback bound. | Produced-resource rank 6, rank 7+, repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Forty-six complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added small/full request-time rank-6 cases retain pipeline identity, distinct specialization, exact completion, and snapshot isolation. Correctness artifact `39d99833cfa0e11597cb7a30f91121ed4e4fac4ad5296962c2ba05f7b2e8103a`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
0 through 3 into z, and reconstructs them behind the same 32-byte guard. The separate
device-bound pipeline authority prewarms every exact
...
a separately bounded produced-resource rank-6 contract,
repeated/device-side feedback contract, rank-7+ domain, or transport profile;
