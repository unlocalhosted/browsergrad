...
- **Current implementation slice:** Gate 6 program-scoped capability decisions
...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0049 classify the original JIT baseline and revised Grad/runtime contracts without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad owns a closed twelve-dtype eager registry, separates direct and torch constructor semantics, implements zero-stride expand views, rejects fake bfloat16 before allocation, fixes contiguous/detach/cast semantics, keeps every eager placement entrypoint CPU-only and fail-closed, zero-copy wraps all twelve supported writable NumPy storage dtypes/layouts, and exports only owning NumPy snapshots. Runtime generates all 53 registered assignment definitions and the one capability/three backend static registry, requires provider-bound resolution before requirement availability, preserves relevant records through every public readiness carrier, and requires program/artifact-scoped lowering decisions before constructing a support view. No Grad behavior remains marked compatibility debt. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention, dtype/view/cast/device, expand-view, NumPy integration, runtime requirement-resolution, and program-capability tests; full package and actual-device evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 has no remaining compatibility-debt behavior: it owns a closed twelve-dtype registry, separates direct and torch constructor inference/ownership, implements zero-stride expand views, rejects fake bfloat16, fixes contiguous/detach/cast semantics, keeps all eager placement CPU-only and fail-closed, zero-copy wraps every supported writable NumPy storage dtype/layout, and emits only owning NumPy snapshots. Runtime generates all 53 requirement definitions from the architecture vocabulary, creates complete provider-bound environments, accepts them directly in run-plan/preflight/matrix/profile-runner APIs, and preserves relevant canonical records through plans, reports, handoffs, external requests, and benchmark rows. Semantic-core and runtime now also distinguish static capability/backend definitions from program/artifact-scoped lowering decisions; runtime support views can be constructed only from actual decisions for one exact subject. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Generate cross-framework/platform support views from executable framework and program contracts; keep terminal execution evidence separate. | ADR-0002 and ADR-0004 through ADR-0049; architecture freeze preserves the exact JIT partition, revised Grad contracts, byte-exact generated requirement and program-capability registries, six direct resolution consumers, five resolution carriers, and architecture implementation/declaration parity. Node 25.9.0 passes the seven-package build in 8.50 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 99.46 seconds, semantic-core passes 21 files/174 tests, runtime passes 13 files/134 tests, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 28-test mutation suite pass. |
...
The next Gate 6 software transition is a generated cross-framework/platform
view that consumes the executable JIT operation registry and exact
program/artifact-scoped lowering decisions. Support must not be inferred from
method presence, static backend registration, requirement availability, or
duplicated in hand-maintained tables.
