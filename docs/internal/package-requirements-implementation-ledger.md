...
| Gate 3 — real C++/CuTe frontend slice | `audit` | Existing CUDA-lite/CuTe source handling, semantic lowering, backend reuse, and corpus gates are being inventoried against the pinned Gate 3 source and exit criteria. No Gate 3 compatibility claim has been made. | Pin the source revision and supported language/profile boundary; prove a real source through frontend, shared layout semantics, CPU, and WebGPU; add strict unsupported diagnostics; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; existing compiler corpus and `verify:real-world-cuda` gates are audit inputs, not Gate 3 proof. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Audit selected an ahead-of-time resolved frontend artifact as the first deployment mode. It will preserve source/header/profile provenance and resolved compiler facts, then lower only through semantic-core layout/kernel construction. | Closed artifact/profile verifier, hermetic producer, layout tracer, dynamic tensor copy, exact Gate 2 hash equality, required WebGPU evidence. | Three independent read-only audits agree current regex/motif handling is not CUTE-002 evidence. |
...
Implement the closed AOT C++/CuTe frontend artifact/profile verifier, then
lower a pinned layout-only resolved fact into semantic-core. Keep the frozen
