...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, logical execution plan, artifact, canonical artifact resource, runner receipt, provenance, authorization, and static lowering authorities are distinct. The profile pins both OCI platform-manifest and image-config digests plus a closed execution-environment manifest. The receipt binds pre-run intent to a deterministic built-in sandbox policy and exact output-independent execution-plan hash, reported opened inputs/resources, exact canonical output, and resolved-or-rejected frontend outcome. Detached DSSE/in-toto provenance authenticates that strict receipt and plan/environment lineage. No real producer, real receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Implement opaque source snapshots, OCI evidence verification, fixed Docker create/inspect/start/cleanup, and the bounded artifact/receipt frame; then pin/build the real native extractor, image, environment, source/header closure, and external signing workflow before dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 61 focused policy/profile/job/receipt/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 42 files/1067 tests plus typecheck/lint and architecture/corpus gates; synthetic evidence proves contracts only. |
...
| Gate 3 logical AOT execution plan | `verified` | One checked-in closed policy defines the output-independent Docker contract: local-only digest-pinned image use, fixed native supervisor, non-root identity, closed environment, private namespaces, read-only root/snapshots, no added capabilities/new privileges, no image volumes/devices/socket, bounded tmpfs/result/stderr, profile-derived limits, and external attestation after receipt. Its canonical policy SHA and a deterministic plan SHA over the exact job/profile/toolchain/VFS/limits are independently pinned. Profile and receipt also bind the OCI image-config digest and closed execution-environment manifest. | Node runner must prove its actual image/config/container/mount/argv state implements this logical plan. CPU/peak resource facts require the real pinned in-image supervisor/cgroup observation; fake Docker tests cannot establish them. | Checked-in JSON/TypeScript equality, canonical policy hash `314957ce...`, execution plan `18d13939...`, receipt/provenance plan/environment/config mutation tests, strict typecheck/lint. |
...
Implement the offline runner boundary without claiming a real producer: a pure
planner plus fake-process tests must require an already-local digest-pinned
`linux/amd64` image, snapshot and hash source blobs before mounting the same
snapshot, derive the fixed invocation from profile/job only, enforce read-only
root/source, no network/capabilities/new privileges, non-root identity and all
limits, accept exactly one bounded output, then strict-decode artifact and
receipt bytes. Keep real toolchain/image/extractor/signing evidence blocked.
