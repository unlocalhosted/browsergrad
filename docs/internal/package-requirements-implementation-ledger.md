...
- **Current implementation slice:** Gate 3 real C++/CuTe frontend audit and first source tracer
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers preserve exact resolved facts. Detached provenance now uses standard DSSE PAE around an in-toto Statement v1 with a BrowserGrad-specific predicate, profile-pinned trust root, exact runner/container/invocation projections, authenticated subject/policy checks, and an instance-bound authorized wrapper. Semantic-core now has an allocation-free verified layout-expression value and coordinate tracer, so pure CuTe layout lowering need not fabricate tensor storage. This is not SLSA/Sigstore evidence, the compiler lowerer does not yet consume the wrapper, and no real-source compatibility claim has been made. | Pin a real producer profile/source/header closure; implement and sandbox the hermetic producer; make layout lowering consume only authorized authority; bind real tensor/storage meaning; prove exact shared-semantic CPU/WebGPU parity; add dynamic tensor binding; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; 38 focused profile/artifact/provenance tests; semantic-core 12 files/107 tests; compiler typecheck/lint and architecture guards; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Ahead-of-time profile and structural artifact verification are implemented. The artifact is one-TU, exact-version, closed, bounded, canonical, content-addressed, opaque, and browser-safe. It recomputes source/header/include-closure hashes; validates source ranges, macro/template/declaration/type/ABI/control-flow ownership graphs, static layout queries, dynamic integer expressions, tensor engines, typed target intrinsics, diagnostics, and tier-neutral frontend outcomes. Producer metadata changes transport identity only; no field can self-assert trust. Architecture checks keep these modules out of frozen CUDA-lite parser, IR, layout-binding, and source-shaped lowering paths. | Hermetic producer, authorized layout tracer, dynamic tensor copy, exact Gate 2 hash equality, required WebGPU evidence. | 27 focused profile/artifact tests after shared fixture extraction; pinned artifact/source/header/closure hashes; compiler typecheck/lint; compiler and semantic architecture checks. |
| Gate 3 detached provenance authorization | `verified` | A profile pins extractor, runner, resolved `linux/amd64` OCI manifest, sandbox policy, allowed builders, and exact P-256 SPKI trust store. Verification authenticates one canonical DSSE envelope before evaluating its in-toto Statement v1 claims, binds full artifact/profile/toolchain/sandbox/invocation/output identity, accepts tagged Git SHA-1/SHA-256 revisions, enforces source/include VFS roots, and mints opaque attestation and authorization authority. Rejected artifacts, structural copies, object-instance substitution, unpinned profiles/sources, hostile trust stores, malformed P1363 signatures, noncanonical payloads, and signed policy drift fail closed. The custom predicate is explicitly not SLSA or Sigstore. Trust-key rotation intentionally creates a new profile/artifact authorization lineage; lowered semantic hashes remain provenance-neutral. | Lowering must consume only this authorized wrapper. Real producer/Sigstore evidence remains separate and is not claimed. | 11 provenance tests plus mutation matrices; pinned trust/profile/artifact/source hashes; strict typecheck and lint. |
| Gate 3 allocation-free layout semantics | `verified` | Semantic-core snapshots, normalizes, verifies, hashes, and authority-binds one standalone layout expression as a `browsergrad.layout@1` artifact containing exactly one index map and zero allocations/views. Coordinate traces expose element location plus logical/predicate bounds only; no dtype, byte range, alias, effect, tensor, or backend meaning is implied. The pinned `(3,2):(1,3)` fixture maps to `[0,3,1,4,2,5]` with semantic hash `4e1fa226...`; dynamic bindings, ordering, resource limits, hostile input, transport-neutral identity, and structural-copy rejection are covered. | The compiler must lower the exact authorized CuTe fact through this API. Hierarchy and dynamic CuTe expression coverage remain compiler work. CPU copy and WebGPU proof wait for real tensor/storage facts. | Semantic-core typecheck/lint; 12 files/107 tests, including 8 standalone-layout tests. |
...
Implement a compiler lowerer that consumes only the authorized artifact
wrapper, explicitly selects the pinned layout entry, verifies exact root/fact
ownership, and lowers its static hierarchy into the allocation-free
semantic-core API. Keep the frozen CUDA-lite CuTe normalizers unchanged. Keep
the three stale direct-publish guidance edits isolated because their files
