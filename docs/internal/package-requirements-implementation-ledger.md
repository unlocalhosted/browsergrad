...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native producer and pinned four-step no-shell build execute in a networkless, read-only, capability-free container with bounded immutable evidence. The canonical local harness loop is 24.7 seconds and exact-primary isolated validation is 4 minutes 17 seconds. C ABI 1.2 records exact frontend-work metrics. Runtime ABI 1.8 closes the observed 66-function generated-import surface and 15,166-entry exception table. One exact 574,770-byte zero-import Worker graph is package-owned and hash-verified; production remains blocked on a captured platform adapter and package-owned invocation composition. Header licensing, current-source clean/reproducibility evidence, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Rebuild the repinned ABI through the warm lane, then run one current-source cache-free clean build and the repaired two-clean-build lane; wire the captured production controller to the pinned bundle; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Capability commits are on `main` through `9d108b9e`, plus the current ABI-review slice. Local `verify:browser-clang-wasm:fast` passes 30 files/338 tests in 24.7 seconds. Exact-primary run `29680831101` completed in 4 minutes 17 seconds with 2 minutes 25 seconds of isolated compile/link. Its 31,641,378-byte Wasm is raw-verified and locally conforms exactly after the ABI 1.8 review update. Historical clean run `29674887505` completed in 39 minutes 29 seconds but predates the current source. Zero valid production Worker launches, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact ABI/semantic-adapter binding, raw-Wasm inspection, Worker frame/VFS contracts, canonical transfer, one-shot entry, and typed controller terminal protocol share one closed contract. C ABI 1.2 adds exact fail-closed frontend-work accounting. Runtime ABI 1.8 independently pins the current generated exception bridges and fixed table. The Worker-local runtime adopts one reconstructed realm input, executes the pinned generated factory and C ABI, verifies runtime/frontend/VFS projections, and emits canonical control plus artifact bytes. The package owns one deterministic 574,770-byte self-contained ES-module graph with no static or dynamic imports. | Add the captured production platform adapter and package-owned invocation composition; replace the historical packaged factory inside the bundle with reviewed current-ABI output and licensed packs; then execute one unmodified browser-local fixture. | Exact-primary Wasm SHA-256 `20cf9ee448af03cd91395eb098e29a9c04be741097f4fb8c7d41fc602b68fc0a`, 31,641,378 bytes, zero local ABI-review mismatches after the ABI 1.8 update. Exact bundle SHA-256 `4b1565e4ca8df2332cc6faabb6643226aa5cb26bcd5d9a8ac6d97d94972797a5` still contains the historical factory and proves module load and one-shot pre-identity rejection only. Zero valid production Worker launches or browser-local C++ executions. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact extractor closure, LLVM 22.1.8 archive, Emscripten image, four-step recipe, ABI, and distribution plan. Clean builds seal the exact Node runtime closure; cached diagnostics restore only untrusted content-addressed toolchain directories inside the isolated container and grant no clean/release authority. Every admitted build runs the bounded raw-Wasm reviewer. The configured-target reviewer requires public source, generated binary, and private AST source include roots before compilation. The cache key now preserves the reusable toolchain across extractor CMake edits, and compatible restores are admitted through the pinned action's matched-key output. | Rebuild the ABI 1.8 source through exact-primary validation, then run and independently review one current-source clean build before rerunning the two-job clean-build lane. Preserve ABI conformance as separate authority before Worker or distribution authority. | Current lock `bg.cpp.browser-build-input-lock.sha256.bf62353c9421b955cd1a07e14e13c5e3417b5431e2be4555283acdacc0ee7def`. Migration run `29680686426` completed in 4 minutes 14 seconds; exact-primary run `29680831101` completed in 4 minutes 17 seconds with 2 minutes 25 seconds of isolated compile/link. Historical clean run `29674887505` succeeded in 39 minutes 29 seconds. No current-source clean/reproducibility, licensed distribution, authenticated provenance, Worker execution, or release authority yet. |
...
- Vite/Rolldown authoring now builds the package Worker entry in two separate
  ...
  checked package resource. The current module is 574,770 UTF-8 bytes with
...
- The build lock advances to
  ...
  because the native extractor embeds the runtime-ABI resource SHA. A new warm
  validation must therefore compile this repinned source before any clean run.
...
Run cached validation once for the ABI 1.8/build-lock source and require exact
raw-Wasm interface conformance. If it succeeds, run one cache-free clean
validation and inspect its factory, Wasm, link map, allocator call closure, and
raw ABI projection. Only after that review should the two-clean-build workflow
run at the same exact `main`.
...
only host-verified current-ABI Wasm bytes, preserve the stable pre-bind VFS
imports, bind exactly the exported unshared memory, exercise the
allocator/result metrics synchronously, and keep all current evidence blockers
