...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0036 classify the original 36-constructor/39-operation baseline into 36 typed retirements, one current intentional opaque identity, and two removed unsupported surfaces without widening the remaining caller or label. The typed set includes paired `torch.sort`/`torch.topk` outputs, overwrite `torch.scatter`, general `torch.einsum`, the loss family through full class-axis cross entropy, keyed dropout, ordered stateful BatchNorm1d, spatial interpolation, and bounded attention forward. Grad parameterized-layer initialization obeys `torch.manual_seed` across Linear, convolution, transposed-convolution, and Embedding families. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; 1-case JIT Gate 0 contract; focused typed-attention integration; full JIT/Grad and Node 20/24/25/real Chromium-WebGPU evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, general string-equation `torch.einsum`, keyed dropout, stateful BatchNorm1d, typed nearest/bilinear spatial interpolation, and bounded rank-4 float32 attention forward. The non-executable `bg.experimental.webnn.matmul` and `bg.kernels.transformer_block` constructors are removed rather than silently falling back to different work or implying nonexistent backends. No advertised framework operation and no framework-owned NumPy callback remains in the opaque inventory; only the intentional user-authored WGSL extension stays opaque. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching partitions equal-shaped graphs by session, bypasses mutable module buffers, and excludes effectful/random graphs. | Reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0036; architecture freeze narrowed from 36/39 to one intentional opaque identity plus 36 typed and two removed unsupported identities while preserving the exact original-ID partition. Node 25.9.0 passes the seven-package build in 15.49 seconds; focused attention/registry/Gate 0/plan/WebGPU integration passes 5 files/47 tests, full JIT integration passes 55 files/336 tests in 135.00 seconds, the full compiler gate passes 96 files/1,602 tests plus its parallel build-plan/shell lanes, and semantic architecture plus its 24-test mutation suite pass. The direct attention WebGPU route is legacy row-wise online softmax, not Gate 5 block-tiled execution or FlashAttention-v2. |
...
The next Gate 6 software transition is remaining Grad dtype/view convergence
and generated runtime/profile support consumption. Support must not be inferred
