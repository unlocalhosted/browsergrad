...
| Gate 6 — framework convergence | `in-progress` | Twenty-nine public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1, Smooth L1, probability BCE, and BCE with logits own strict same-shape floating promotion, bounded reductions, both-input closure/symbolic derivatives, and nested per-example vmap shared with Grad. Probability BCE owns fail-closed domains and endpoint policy; logits BCE owns overflow-safe finite-extreme math and stable ONNX export. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 10 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0027; architecture freeze narrowed from 36/39 to 10/10 while preserving the exact original-ID partition. BCE-with-logits verification passes focused JIT 5 files/52 tests, focused Grad 2 files/20 tests, full JIT 48 files/316 tests, full Grad 53 files/348 tests, the 96-file/1,602-test compiler gate, release-package security and packed-consumer gates, and the seven-package Node 25 build in 9.83 seconds. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.binary_cross_entropy_with_logits` are retired from the
opaque callback inventory and have typed CPU, VJP, transform, export/refusal,
backend, and residency decisions. The executable v1 framework-operation
registry generates all twenty-nine public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `kl_div`, reusing the closed reduction/resource seam while adding
exact `log_target`, `batchmean`, zero-target, and both-derivative semantics. Each
