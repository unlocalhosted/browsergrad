...
- **Current implementation slice:** Gate 7 authority-bound CPU reference executor
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, version-1.1 whole-allocation per-rank copies, version-1.2 terminal fail-stop materialization, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors snapshot inputs, execute dispatches, copies, and collectives in private rank-local storage under explicit ceilings, and expose only verified materialized outputs after whole-graph success. The WebGPU adapter expands verified dispatches, raw copies, and arbitrary-participant reductions into bounded canonical WGSL sequences while materialization selects terminal readback without another dispatch; complete f32/i32/u32 collective plus raw-u8 materialization actual-device parity passes. | Event nodes; bounded conditional/repetition control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles; graph performance evidence. | Semantic-core 23 files/198 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/176 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; five complete-output CPU/WebGPU parity cases plus non-finite refusal and actual device-loss refusal on Apple Metal 3; root/compiler architecture checks with zero cycles/leaks. |
...
Events, bounded control, pipeline authority, and transport remain open.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies between distinct same-dtype, same-size per-rank resources. Version 1.2 requires exactly one terminal ordered materialization read for every declared output; it cannot expose inputs/temporaries, be duplicated, precede its final writer, or have dependents. Versions 1.0/1.1 retain exact decoding and implicit-output behavior. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. | Semantic-core passes 23 files/198 tests, including canonical older-version retention plus materialization version/mode/resource/coverage/ordering/terminal rejection, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, and zero-work materialization nodes, and returns only selected outputs after whole-graph success. Fixed configurable ceilings bound working memory including collective scratch, element or copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller-mutation isolation, copied-byte budget refusal, and version-1.2 terminal output publication. Semantic-core passes 23 files/198 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU static-DAG execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, and arbitrary-participant collectives into bounded canonical WGSL sequences, retains private rank/resource buffers, validates f32 numerical status, and publishes only explicitly selected outputs after complete readback. Materialization contributes no GPU step. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the current portable static-DAG profile. Non-word-sized copies fail explicitly; dynamic control, events, pipeline authority, transport/topology, native companions, and performance require new profiles. | Five complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, and explicitly materialized u8 copy after caller-input mutation; separate checks reject non-finite f32 and a lost device. Kernels passes 21 files/176 tests and packed/fresh consumers prepare both compiler-owned 1.0 dispatch graphs and artifact-only 1.2 materialization. |
...
and verified. The next software checkpoint is an explicit event profile or one
separately versioned bounded-control semantic slice; it must not widen the
static DAG implicitly. Pipeline authority, transport/topology, worker meshes,
native companion execution, and graph performance evidence remain later
