...
- **Current implementation slice:** Gate 7 sequential conditional-to-repeat feedback
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exactly two top-level produced-resource linear/rectangular dispatches only for an exact shared selection. Versions 1.26/1.27/1.28 permit exact two-, three-, or four-stage linear producer chains. Version 1.29 permits exactly one produced-u32 conditional and one repeat sharing the exact source and rank. Version 1.30 permits distinct sources only when both conditional branches guarantee-write the repeat source. Earlier versions retain their feedback bounds and rectangle-rank limits. | Broader mixed or device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/64 tests, including zero/two sequential conditional-to-repeat CPU execution, pre-version and missing-branch-write refusal, shared conditional/repeat, exact shared-count/shared-rectangle fanout, two-/three-/four-stage producer chains, and request-time/produced-resource rank-2-through-rank-8 rectangles. Full semantic-core passes 24 files/254 tests. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.24/1.25 execute both dispatches from one shared selection; versions 1.26/1.27/1.28 read every distinct selection only after its preceding selected dispatch executes; version 1.29 reads one selection for both consumers; version 1.30 reads the predicate, executes the branch, then reads its guaranteed distinct repeat count. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves zero/two sequential conditional-to-repeat selection plus shared fanout, two-/three-/four-stage linear chains, refusal, and rank-2-through-rank-8 rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.34 adds version 1.30's sequential conditional-to-repeat profile: it reads the private predicate, executes one prewarmed branch, reads its distinct repeat count, and executes the bounded repeat over the same resident buffers without recompilation. Public preparation/trace evidence separately reports two feedback nodes and two host-mediated stages. | Broader mixed or device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Eighty-four complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Zero/two sequential cases retain pipeline identity, distinct specialization, exact branch/repeat completions, snapshot isolation, two feedback nodes, and two stages. Correctness artifact `1960f84defe847f3ee35b925d18ff28d3561ff8d6322e5b72445b09ea53972f1`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
