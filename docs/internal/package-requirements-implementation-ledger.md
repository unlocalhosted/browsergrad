...
export, plan, and residency decisions. The next software transition is a
single executable framework-operation contract registry consumed by public
support reporting, followed by another bounded opaque operation family. It
must not infer support from method presence or duplicate backend capability
facts in hand-maintained documentation.
