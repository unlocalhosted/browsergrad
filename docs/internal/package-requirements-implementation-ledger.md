...
- **Current implementation slice:** Gate 5 tiled-attention schedule specialization and WebGPU lowering
...
| Gate 5 — tiled attention flagship | `partial` | Semantic-core owns the closed frontend-neutral `browsergrad.kernel.attention-forward@1` artifact, separate `browsergrad.schedule.attention-online-kv-tile@1` artifact, and schedule-independent CPU oracle. Logical meaning binds static rank-4 f32 Q/K/V/output views, exact scale, non-causal or upper-left causal masks, stable-softmax phases, finite-domain requirements, comparison policy, effects, and VJP refusal. The schedule independently binds physical tiles, cooperative K/V staging, increasing traversal, tile-wise rescaling, uniform barriers, scalar vectors, and pre-state-update masks. The CPU path proves dense addresses under resource bounds, snapshots fixed unshared inputs, rounds every declared f32 step, fails atomically on domain violations, and implements the named comparator. | Implement schedule specialization, block-tiled K/V-staged WebGPU lowering, required causal/non-causal real-device numerical evidence, truthful baseline naming, and separate correctness/performance records on named devices/browsers. | Semantic-core 19 files/164 tests; build, typecheck, lint, architecture, hostile binding/domain tests, canonical round-trip, two-schedule separation, causal/non-causal CPU values/work traces, atomic failure, comparison-policy checks, intrinsic-poison resistance, and packed-release consumption pass locally. Commits `4cfacd75` and `1ccd541e` passed CI runs `29820268289` and `29820944140`. No WebGPU, performance, named fused-attention, frontend convergence, or Gate 5 exit claim. |
...
CPU reference and numerical conformance contract is implemented. The current
slice is schedule specialization followed by block-tiled WebGPU lowering.
...
- Semantic-core now passes 19 files/164 tests plus build, typecheck, lint, and
  ...
  GEMM behavior. Packed-release execution
  is also required. Gate 5 remains `partial`; the next capability is schedule
