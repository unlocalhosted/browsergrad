...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact 36-file BrowserGrad LibTooling extractor closure, LLVM 22.1.8 archive, Emscripten image manifest/config, four-step recipe, ABI, and distribution plan. The executor stages only locked snapshots, executes exact argv/cwd/environment without a shell, enforces per-step time/output/cancellation limits, binds inputs against inode-version drift, hashes native TableGen tools, seals the factory/Wasm/link map and logs, validates UTF-8/WebAssembly, and atomically materializes only the Wasm sidecar. The manual workflow verifies acquisition and image identities, then runs with no network, read-only root/input mounts, zero capabilities, no-new-privileges, and a private writable work mount. Future runs additionally stage, seal, enumerate, and hash the exact 25-file Node runtime closure; build-execution evidence v2 records it and the v2 two-build comparator requires it to match. The raw-Wasm reviewer emits discovery without self-authorizing an ABI. Target preflight requires JavaScript exceptions, RTTI, and both canonical Clang include roots before the expensive Wasm compile. Failure receipts preserve a bounded typed cause chain without granting success authority. | Finish the first real build, review its logs/factory/Wasm/link closure, then run the two-job clean-build lane under the exact runtime boundary. Repin the independently reviewed ABI projection before any Worker or distribution authority. | Build lock `bg.cpp.browser-build-input-lock.sha256.7bae41fb6bc0d84be84035e89714fc2cd65959532854dbcffffee694721a959f`; focused boundary 24 files, 157 passed, 9 platform skips. Runs `29650211276`, `29653908647`, and `29657388282` are preserved failure observations with no admitted outputs. Replacement validation `29658164083` targets commit `cd112795`, passed the configured-target preflight, and entered the expensive compile using the prior two-package mount. No completed build, ABI conformance, licensed distribution, authenticated provenance, or release authority yet. |
