...
- **Current implementation slice:** Gate 7 sequential conditional-to-rank-3-through-rank-8 rectangle feedback
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exact shared-selection fanout; versions 1.26/1.27/1.28 permit exact two-/three-/four-stage linear chains; version 1.29 shares one conditional/repeat source; versions 1.30/1.31 require both conditional branches to guarantee-write one distinct repeat or linear-launch source; versions 1.32/1.33 require both branches to guarantee-write every distinct rank-2 or rank-3-through-rank-8 extent source. Earlier versions retain their feedback bounds and rectangle-rank limits. | Broader mixed or device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage includes small/full conditional-produced rank-2 and rank-8 rectangles plus pre-version, missing-branch-write, and shared-source refusal. Full semantic-core passes 24 files/257 tests. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed ceilings, and publishes only after success. Versions 1.30 through 1.33 read the predicate, execute the selected branch, then read its guaranteed distinct repeat, linear-launch, or rank-2-through-rank-8 extent sources. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves conditional-produced repeat/launch/rectangle selection through rank 8 plus shared fanout, two-/three-/four-stage linear chains, refusal, and rank-2-through-rank-8 rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.37 adds version 1.33's conditional-to-rank-3-through-rank-8-rectangle profile through the same shared conditional-prefix selector and generic resource-dispatch specialization used by rank 2. It reads the predicate, executes one prewarmed branch, reads every distinct extent, and specializes the maximum-prewarmed rectangle over the same resident buffers without recompilation. | Broader mixed or device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Ninety complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Small/full maximum-rank branch-produced rectangle cases retain pipeline identity, distinct specialization, exact branch/extent completions, snapshot isolation, two feedback nodes, and two stages. Correctness artifact `7aed748d3ebda3ecd49a522311fa2856fc0962cc7e52027b19fa397e465ddf77`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
