...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0043 classify the original JIT baseline and revised Grad contracts without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad rejects fake bfloat16 before allocation, fixes contiguous/detach/cast semantics, keeps every eager placement entrypoint CPU-only and fail-closed, zero-copy wraps all nine supported writable NumPy storage dtypes, and exports only owning NumPy snapshots. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention, dtype/view/cast/device, and NumPy integration; full package and actual-device evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 rejects fake bfloat16, fixes contiguous/detach/cast semantics, keeps all eager placement CPU-only and fail-closed, zero-copy wraps every supported writable NumPy storage dtype/layout, and emits only owning NumPy snapshots. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Reconcile NumPy-delegated dtype fallback, constructor-default classification, and owning expand materialization; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0043; architecture freeze preserves the exact JIT partition and revised Grad contracts. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 101.14 seconds, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 24-test mutation suite pass. |
...
- `from_numpy` now wraps writable arrays in the closed nine-dtype eager storage
...
The next Gate 6 software transition is dtype registry/default convergence,
then owning expand materialization, followed by generated runtime/profile
support consumption. Support must not be inferred from method presence or
