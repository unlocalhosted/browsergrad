...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, version-1.1 whole-allocation per-rank copies, and the fixed fail-stop/no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors snapshot inputs, execute dispatches, copies, and collectives in private rank-local storage under explicit ceilings, and expose outputs only after whole-graph success. The WebGPU adapter expands verified dispatches, raw copies, and arbitrary-participant reductions into bounded canonical WGSL sequences with complete f32/i32/u32 collective plus raw-u8 copy actual-device parity. | Readback/event/materialization nodes; bounded conditional/repetition control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles; graph performance evidence. | Semantic-core 23 files/195 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/175 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; five complete-output CPU/WebGPU parity cases plus non-finite refusal and actual device-loss refusal on Apple Metal 3; root/compiler architecture checks with zero cycles/leaks. |
...
foundation and fail-stop CPU reference are implemented; portable WebGPU graph
execution is the next backend capability before bounded control or transport.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived view-copy effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies between distinct same-dtype, same-size per-rank resources while retaining 1.0 decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. | Semantic-core passes 23 files/195 tests, including canonical 1.0 retention, 1.1 copy normalization, version/mode/resource/dtype/hazard rejection, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, and finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, and returns outputs only after whole-graph success. Fixed configurable ceilings bound working memory including collective scratch, element or copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage adds odd-sized u8 exact copy without semantic artifacts, caller-mutation isolation, and copied-byte budget refusal. Semantic-core passes 23 files/195 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU static-DAG execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, and arbitrary-participant collectives into bounded canonical WGSL sequences, retains private rank/resource buffers, validates f32 numerical status, and publishes no output until complete readback. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the current portable static-DAG profile. Non-word-sized copies fail explicitly; dynamic control, readback/event/materialization nodes, pipeline authority, transport/topology, native companions, and performance require new profiles. | Five complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, and exact u8 copy after caller-input mutation; separate checks reject non-finite f32 and a lost device. Kernels passes 21 files/175 tests and packed/fresh consumers prepare both compiler-owned 1.0 dispatch graphs and artifact-only 1.1 raw copies. |
...
f32/i32/u32 dispatch/all-reduce evidence without partial output commit. The
next software checkpoint is one separately versioned bounded-control semantic
slice or explicit copy/event graph node profile; it must not widen the static
DAG implicitly. Pipeline authority, transport/topology, worker meshes, native
companion execution, and graph performance evidence remain later independent
capabilities.
