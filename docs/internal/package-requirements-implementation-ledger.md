...
| Gate 6 — framework convergence | `in-progress` | Thirty-two public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1, Smooth L1, probability BCE, BCE with logits, KL divergence, NLL, and cross entropy own bounded reduction contracts shared with Grad. Cross entropy covers index and probability targets, optional class weights, ignore index, label smoothing, exact reduction denominators, both differentiable source VJPs, mapped/captured target and weight vmap, and exact unmapped index-target ONNX export. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching partitions equal-shaped graphs by session and bypasses mutable local/descendant buffers. | Migrate the remaining 7 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0030; architecture freeze narrowed from 36/39 to 7/7 while preserving the exact original-ID partition. Cross-entropy verification passes full JIT 51 files/327 tests, full Grad 56 files/354 tests, package codegen/build/typecheck/lint/unit gates, the 96-file/1,602-test compiler gate, 19 Python archive and 35 Node release-security tests plus packed/fresh consumers, and the seven-package Node 25.9.0 build in 9.73 seconds. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.cross_entropy` are retired from the opaque callback
...
all thirty-two public support records from their validator bindings and
preserves the original opaque-ID partition. The next software transition is
`dropout`, including explicit mask ownership and replay semantics. Each
