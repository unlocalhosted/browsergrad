...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.24 exact shared-count linear fanout, version-1.12/1.13 request-time/produced-resource rank-2/rank-3 rectangles, version-1.14/1.15 rank-4 rectangles, version-1.16/1.17 rank-5 rectangles, version-1.18/1.19 rank-6 rectangles, version-1.20/1.21 rank-7 rectangles, version-1.22/1.23 request-time/produced-resource rank-8 rectangles, version-1.25 exact shared-rectangle fanout, and version-1.26 one exact two-stage linear producer chain. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Fanout reads one shared selection once; the producer chain reads its distinct second selection only after the first selected dispatch completes. | Broader repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Focused semantic-core 2 files/62 tests and kernels 1 file/29 tests pass with strict typechecks; 62 complete-output CPU/WebGPU parity cases execute through explicit pipeline authority on Apple Metal 3; correctness artifact `959a6ada1766957d9d259e18aae6307ae02658b818761147993324983378bdb7` under backend 1.29; separate backend-1.20 prewarmed performance observation. |
...
These profiles do not claim timestamps, external waits, a third or mixed
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exactly two top-level produced-resource linear/rectangular dispatches only for an exact shared selection. Version 1.26 instead permits two distinct linear sources only when one dispatch writes the other's source. Earlier versions retain one feedback node, and versions 1.12 through 1.23 preserve every older exact rectangle-rank limit. | Broader repeated/device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact shared-count/shared-rectangle fanout, one/two-element sequential producer-chain CPU execution, pre-version, missing-producer, and mismatched-source/maximum refusal, plus request-time/produced-resource rank-2-through-rank-8 rectangles. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.24/1.25 execute both dispatches from one shared selection; version 1.26 executes the first selected dispatch before reading the distinct source it produced for the second and reports both exact completions. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves single dispatch, exact shared-count/shared-rectangle fanout, one/two-element sequential feedback, invalid selection refusal, and partial/full rank-2-through-rank-8 rectangles from both control sources. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.29 adds version-1.26 sequential linear feedback: it reads the first ordered u32, substitutes and executes its exact prepared dispatch, then reads the distinct produced u32, substitutes the dependent dispatch, and executes the suffix over the same resident buffers without recompilation. Public preparation/trace evidence separately reports feedback nodes and actual host-mediated stages. | Broader repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Sixty-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added one/two-element sequential cases retain pipeline identity, distinct specialization, two exact completions, snapshot isolation, two feedback nodes, and two feedback stages; shared fanouts report two nodes but one stage. Correctness artifact `959a6ada1766957d9d259e18aae6307ae02658b818761147993324983378bdb7`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
transport profile or a separately bounded third/mixed feedback-stage contract;
