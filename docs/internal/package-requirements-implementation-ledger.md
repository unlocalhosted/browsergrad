...
The next executable checkpoint is one unified host operation that re-verifies
both returned statements and invokes the implemented backend and final-release
compositions before process exit. Then install the package-controlled policies
and externally issued exact statements. Externally rooted producer trust,
external legal approval, and actual authority issuance remain separate
prerequisites; serialized observations, local execution, or synthetic fixtures
cannot satisfy them.
