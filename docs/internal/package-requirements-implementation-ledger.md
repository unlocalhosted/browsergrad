...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, version-1.1 whole-allocation per-rank copies, version-1.2 terminal fail-stop materialization, version-1.3 dependency-ordered completion events, version-1.4 bounded fixed-count sequential repetition, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors snapshot inputs, execute dispatches, copies, collectives, and fixed repeats in private rank-local storage under explicit ceilings, report completed events/repeats only with whole-graph success, and expose only verified materialized outputs. The WebGPU adapter expands verified dispatches, raw copies, arbitrary-participant reductions, and exact fixed repeats into bounded canonical WGSL sequences while events/materialization add no GPU command; complete f32/i32/u32 collective, repeated f32 sum, and event-marked raw-u8 materialization actual-device parity passes. | Bounded conditional/runtime-derived control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles; graph performance evidence. Event timestamps, external waits, and cross-queue/cross-worker synchronization require separately named future profiles. | Semantic-core 23 files/204 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/178 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; six complete-output CPU/WebGPU parity cases plus non-finite refusal and actual device-loss refusal on Apple Metal 3; root/compiler architecture checks with zero cycles/leaks. |
...
also implemented without claiming timestamps, external waits, runtime-derived
control, or conditionals. Pipeline authority and transport remain open.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies, globally unique node IDs, aggregate effects, and explicit iteration/expanded-node limits. Versions 1.0–1.3 retain exact decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. | Semantic-core passes 23 files/204 tests, including canonical older-version retention plus repeat version/mode/identity/body/expansion rejection and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, and exact fixed repeats with cancellation/time checks at iteration/body boundaries. Completed event IDs, repeat records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, expanded element or copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller-mutation isolation, copied-byte budget refusal, terminal output publication, successful completion reporting, exact three-iteration f32 sum to `[16,24]`, and pre-admission expanded-operation refusal. Semantic-core passes 23 files/204 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed-repeat execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and exact fixed repeat bodies into bounded canonical WGSL sequences, retains private rank/resource buffers, validates f32 numerical status, and publishes only explicitly selected outputs after complete readback. Events and materialization contribute no GPU step; completed event IDs and repeat records appear only in the successful terminal trace. One-in-flight ownership, device-loss invalidation, exact scope drainage, timeout/cancellation stale-result suppression, aggregate host/GPU memory limits, and device ceilings preserve the fail-stop contract. | None for the current portable DAG/fixed-repeat profile. Non-word-sized copies fail explicitly; conditionals, runtime-derived control, pipeline authority, transport/topology, native companions, performance, and stronger event profiles require new contracts. | Six complete-output headed-Chromium CPU/WebGPU parity cases cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, and three fixed repetitions of f32 sum after caller-input mutation; separate checks reject non-finite f32 and a lost device. Kernels passes 21 files/178 tests and packed/fresh consumers prepare compiler-owned 1.0 dispatch graphs plus artifact-only 1.4 repeat/event/materialization. |
...
repetition are also verified without claiming timestamps, external waits,
conditionals, or runtime-derived control. The next software checkpoint is a
separately versioned conditional-control slice or pipeline authority; it must
not widen the graph implicitly. Transport/topology, worker meshes, native
companion execution, graph performance, and stronger event profiles remain
