...
- **Current implementation slice:** Gate 2 rank-6 non-word32 exact-storage breadth
...
| Gate 2 — multi-frontend, multi-backend view slice | `verified` | Semantic-core `0.3.0` owns verified view-copy meaning, shared specialization, and the sole canonical frontend construction sink. Positive-affine word32 profiles cover equal source/destination ranks 1 through 7. Distinct signed-affine profiles admit negative coordinate scales in rank-1-through-rank-7 word32 sources and rank-1-through-rank-6 packed8, packed16, and word64 sources while retaining the positive-affine dense destination and every older profile identity. The separate positive and signed non-word32 profiles preserve exact same-dtype bool/i8/u8, i16/u16/f16/bf16, and f64/i64/u64 storage through raw-u32 static WGSL schedules without claiming arithmetic or conversion. Compiler L1 preserves its six-case non-padded contract; sibling L2 binds the exact verified operation and proves rank-2/rank-3 guarded padding through CPU, structured IR, WGSL, and actual WebGPU. JIT `0.9.0` emits a closed typed permutation request and executes the same canonical operation through materializing/resident production routes without recovering semantic args from the frozen plan. | None for the declared portable view-copy profiles. Rank 8+, signed destinations, integer division/modulo, dynamic non-word32 maps, non-word32 arithmetic/conversion, overlap, and non-global memory require separately named profiles. Release CI must repeat the exact-source lanes before publication; registry publication is separate. | Required Apple Metal 3 semantic view-copy backend-3.4 evidence passes 61 complete CPU/WebGPU cases including positive rank 7, signed word32 ranks 1 through 7, positive and signed packed8/packed16/word64 ranks 1 through 6, guarded negative-scale padding, all ten non-word32 dtypes, packed-tail preservation, and raw two-word 64-bit copies; correctness artifact `e784ebcafba6c7742ed076a3be8429f9709759e00580fc6c8a2c194fbb61b6e1`; case set `f311a94dcf2456b6fb96cb81ad7fa8aa1fa270ae3b9db9fdc53021da42363912`; device profile `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`. Historical clean-source frontend convergence remains recorded at revision `aa605421410e9d4190d8939c24b1057731111231`. |
...
f64/i64/u64 rank-1-through-rank-6 view copies. They deliberately preserve raw storage
...
| Gate 2 view-family selection | `verified` | Selected typed JIT `PERMUTE` plus compiler L1 read binding and sibling L2 guarded materializing view copy. The required 61-case view-copy matrix covers separately identified rank-5/rank-6/rank-7 positive-affine word32, rank-1-through-rank-7 signed-affine word32 source, and positive plus signed-affine rank-1-through-rank-6 bool/i8/u8 packed8, i16/u16/f16/bf16 packed16, and f64/i64/u64 word64 profiles with exact CPU/WebGPU parity. Packed8, packed16, and word64 prove storage movement only; broader view or arithmetic families must add typed profiles rather than reinterpret existing identities. |
...
passes 61 layout cases, including canonical rank-5-through-rank-7
...
through 6, without minting source-producer, arithmetic/conversion,
distribution, or release authority.
