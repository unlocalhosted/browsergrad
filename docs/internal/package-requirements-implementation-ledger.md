...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact extractor closure, LLVM 22.1.8 archive, Emscripten image, four-step recipe, ABI, and distribution plan. Clean builds seal the exact Node runtime closure; cached diagnostics restore only untrusted content-addressed toolchain directories inside the isolated container and grant no clean/release authority. Current admission verifies the production-shaped configure/build working directories, rereads exact Wasm, reruns the independent inspector, and requires byte-identical reports. Reproducibility v3 retains both raw linker-map identities but compares a strict path-canonical semantic projection; the exact two-build record is package-pinned. The separate 17-file header-distribution subset is rehashed and reproducible across two distinct roots, and its path-independent observation is package-pinned and independently rederived. The workflow runs JavaScript verification once in parallel with the expensive matrix while each build retains exact runtime-closure materialization; mode-scoped concurrency keeps fast feedback independent from non-cancelling evidence runs. | Preserve the two narrow reproducibility authorities while closing external license approval, the complete release-output set, signed provenance, Worker execution, and release separately. | Current lock `bg.cpp.browser-build-input-lock.sha256.bf62353c9421b955cd1a07e14e13c5e3417b5431e2be4555283acdacc0ee7def`. Extractor v3 resource SHA-256 is `6c7aebe1376edf0f9a526b55bacd930e7e9e0fd454a95213d97931117548f31a`; header-distribution resource SHA-256 is `eb85b8fb54d1bad1b932bd4bfb3ffc7aff8d66d6185c9926ddd3c9f6084d918a`. Fast gate: 51 files/442 tests in 33.89 seconds. No licensed distribution, authenticated provenance, Worker execution, or release authority yet. |
