...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT 6-constructor-call/6-operation opaque matrix have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0031 retire 33 typed operation identities—including paired `torch.sort`/`torch.topk` outputs, overwrite `torch.scatter`, general `torch.einsum`, the typed loss family through full class-axis cross entropy, and keyed dropout—from the original 36/39 baseline without widening any remaining caller or label. Grad parameterized-layer initialization obeys `torch.manual_seed` across Linear, convolution, transposed-convolution, and Embedding families. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check; 96-file/1,602-test compiler gate; 5-case JIT Gate 0 contract; full JIT and Grad integration; Node 20/24/25 and real Chromium/WebGPU evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-three public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, general string-equation `torch.einsum`, and keyed dropout. The typed loss family owns bounded reduction contracts shared with Grad. Dropout owns one replayable per-operation key across CPU, closure/symbolic VJP, functional grad, and checkpoints; identity modes consume no RNG, and stochastic cache/vmap/export/device routes fail explicitly where their contracts are absent. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. Trace caching partitions equal-shaped graphs by session, bypasses mutable module buffers, and excludes effectful/random graphs. | Migrate the remaining 6 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0031; architecture freeze narrowed from 36/39 to 6/6 while preserving the exact original-ID partition. Dropout verification passes full JIT 52 files/332 tests, full Grad 57 files/356 tests, package codegen/build/typecheck/lint/unit gates, and the 96-file/1,602-test compiler gate. Release bodies pass 19 Python archive tests, 35 Node security tests, source and packed 159-case browser corpus lanes, required real-WebGPU gates, and dogfood 5-file/44-node plus 16-file/98-browser tests with one expected failure. The seven-package build passes on Node 25.9.0. The existing kernels actual-device lane proves `BROADCAST_TO`; typed dropout makes no actual-device claim. |
...
`torch.nn.functional.cross_entropy` plus `torch.nn.functional.dropout` are
retired from the opaque callback
...
all thirty-three public support records from their validator bindings and
preserves the original opaque-ID partition. The next software transition is
`BatchNorm1d`, including explicit running-stat ownership and replay semantics. Each
