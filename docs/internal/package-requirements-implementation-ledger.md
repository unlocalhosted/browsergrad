...
- **Current implementation slice:** Gate 6 Grad request disambiguation convergence
...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0040 classify the original JIT baseline and revised Grad contracts without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad rejects fake bfloat16 before allocation, Linear/Embedding enforce exact float32 parameters, `contiguous()` conditionally preserves identity or materializes a dtype-preserving C-order copy, `detach()` shares exact storage/dtype/layout while severing autograd, and floating `to()` casts retain a source-dtype VJP. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention, bfloat16-refusal, contiguous, detach, and cast integration; full package and actual-device evidence remain separately scoped. |
...
The next Gate 6 software transition is invalid dtype/device disambiguation,
followed by NumPy interop and generated runtime/profile support consumption.
