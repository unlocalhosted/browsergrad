...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native producer and pinned four-step no-shell build execute in a networkless, read-only, capability-free container with bounded immutable evidence. The canonical local harness loop is 24.7 seconds and exact-source warm validation is 5 minutes. C ABI 1.2 records exact frontend-work metrics. Runtime ABI 1.8 closes the observed 66-function generated-import surface and 15,166-entry exception table. One exact 574,770-byte zero-import Worker graph is package-owned and hash-verified; production remains blocked on a captured platform adapter and package-owned invocation composition. Header licensing, current-source clean/reproducibility evidence, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Run one current-source cache-free clean build and the repaired two-clean-build lane; wire the captured production controller to the pinned bundle; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Capability commit `640c6d51` is on `main`. Local `verify:browser-clang-wasm:fast` passes 30 files/338 tests in 24.7 seconds. Exact-source run `29681607575` completed in 5 minutes with 3 minutes 19 seconds of isolated build; its 31,641,377-byte Wasm is raw-verified and exactly ABI-conforming. Historical clean run `29674887505` completed in 39 minutes 29 seconds but predates the current source. Zero valid production Worker launches, reviewed distributed header packs, or browser-local C++ executions. |
...
Run one cache-free clean validation for the exact ABI 1.8/build-lock source and
inspect its factory, Wasm, link map, allocator call closure, and raw ABI
projection. Only after that review should the two-clean-build workflow run at
the same exact `main`.
