...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native producer and pinned four-step no-shell build execute in a networkless, read-only, capability-free container with bounded immutable evidence. The cached diagnostic lane is stable near five minutes and the canonical local harness loop is about 30 seconds. C ABI 1.2 now records exact include depth, macro expansions, preprocessed tokens, AST nodes, classic-interpreter constexpr steps, template instantiations/depth, and completed CUDA semantic passes. The Worker-local runner executes the pinned factory/C ABI and emits canonical result control. One exact 572,755-byte zero-import Worker graph is package-owned and hash-verified; production remains blocked on a captured platform adapter and package-owned invocation composition. Header licensing, current-source clean/reproducibility evidence, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Complete and inspect the active current-source clean build; rerun the repaired two-clean-build lane; wire the captured production controller to the pinned bundle; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Capability commits are on `main` through `9945c7f6`, plus the current Worker-bundle slice. Local `verify:browser-clang-wasm:fast` passes 30 files/337 tests in 29.24 seconds; full compiler unit passes 74 files/1,467 tests; architecture checks remain green. The exact bundle hash is `669f20f8994f96e69c40d1437f5fb15efe5208732b327df4af9a886525b19e78`, and a real Chromium Blob module Worker loads those bytes and preserves the one-shot invalid-input failure boundary. Historical clean run `29674887505` completed in 39 minutes 29 seconds but predates ABI 1.2. Current clean run `29678087663` is active and is not yet evidence. Zero valid production Worker launches, reviewed distributed header packs, or browser-local C++ executions. |
...
- Vite/Rolldown authoring now builds the package Worker entry in two separate
  ...
  checked package resource. The current module is 572,755 UTF-8 bytes with
  SHA-256 `669f20f8994f96e69c40d1437f5fb15efe5208732b327df4af9a886525b19e78`.
