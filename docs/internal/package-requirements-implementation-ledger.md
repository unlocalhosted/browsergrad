...
| Gate 6 — framework convergence | `in-progress` | Twenty-eight public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1, Smooth L1, and BCE now own strict same-shape floating promotion, bounded reductions, both-input closure/symbolic derivatives, and nested per-example vmap shared with Grad; BCE additionally owns fail-closed probability-domain and endpoint numerical semantics and honestly refuses ONNX export. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 11 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0026; architecture freeze narrowed from 36/39 to 11/11 while preserving the exact original-ID partition. Current verification passes JIT 47 files/313 integration tests, Grad 52 files/346 integration tests, compiler 96 files/1,602 tests, release packages, and the seven-package Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.binary_cross_entropy` are retired from the opaque callback
...
all twenty-eight public support records from their validator bindings and
preserves the original opaque-ID partition. The next software transition is
`binary_cross_entropy_with_logits`, reusing only the closed loss-reduction
resource seam while adding stable logits-domain, weighting, and derivative
contracts. Each
