...
- **Current implementation slice:** Gate 3 production-policy handoff
...
| Gate 2 — multi-frontend, multi-backend view slice | `verified` | Semantic-core `0.3.0` owns verified view-copy meaning, shared specialization, and the sole canonical frontend construction sink. Positive-affine and distinct signed-affine profiles cover equal source/destination ranks 1 through 8 for packed8, packed16, word32, and word64 storage while retaining the positive-affine dense destination and every older profile identity. They preserve exact same-dtype bool/i8/u8, i16/u16/f16/bf16, f32/i32/u32, and f64/i64/u64 storage through raw-u32 static WGSL schedules without conflating movement with arithmetic or conversion. Compiler L1 preserves its six-case non-padded contract; sibling L2 binds the exact verified operation and proves rank-2/rank-3 guarded padding through CPU, structured IR, WGSL, and actual WebGPU. JIT `0.9.0` emits a closed typed permutation request and executes the same canonical operation through materializing/resident production routes without recovering semantic args from the frozen plan. | None for the declared portable view-copy profiles. Rank 9+, signed destinations, integer division/modulo, dynamic non-word32 maps, non-word32 arithmetic/conversion, overlap, and non-global memory require separately named profiles. Release CI must repeat the exact-source lanes before publication; registry publication is separate. | Required Apple Metal 3 semantic view-copy backend-3.7 evidence passes 75 complete CPU/WebGPU cases including positive and signed source maps for every storage width at ranks 1 through 8, guarded negative-scale padding, all ten non-word32 dtypes, packed-tail preservation, and raw two-word 64-bit copies; correctness artifact `d7c57f3ff63ba63f0b987a21eb4cabdc9c30d5539d9babb78b21d074df1f1e6b`; case set `281e050c4037981e238edee404a21c6ee6cb15e88de925468abd1e1d89c23d58`; device profile `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`. Historical clean-source frontend convergence remains recorded at revision `aa605421410e9d4190d8939c24b1057731111231`. |
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The current extractor is package-pinned from two byte-identical cache-free builds, and the strict nine-case Chromium matrix uses one package-owned exact compile profile. The five-pack/5,788-file header universe, Runtime ABI 1.17, both zero-import Workers, unchanged C++17/CuTe f32/i32/u32 rank-1-through-rank-4 plus signed-rank-2 source, accepted Artifact V3 outputs, and distinct semantic view-copy candidates are content-bound. A deterministic materializer writes the exact 24-subject tree, and a producer-gated finalizer adds only the detached DSSE envelope. Two live exact 25-output roots are byte-reproducible and package-pinned. The exact-payload lane re-fetches that distribution in the browser, re-verifies the local producer, compiles all nine sources, authorizes each candidate through the shared seam, and proves complete bit-exact CPU/required-WebGPU convergence with nonzero-offset canaries. A closed authoring check binds that convergence resource to the exact current package closure and one real non-software WebGPU device profile. A no-private-key authoring boundary now validates distinct preprovisioned P-256 public keys and canonical HTTPS principals, admits both production policies, and writes an exact no-clobber handoff while keeping every external/release claim false. Final composition independently rejects exact producer/reviewer identity or public-key reuse. The declared portable implementation exit passes; Docker stays outside the portable runtime graph. | Provision distinct externally controlled producer and reviewer keys, use the implemented authoring command to install their exact policies, obtain both externally issued statements, then run the unified production-release operation against those immutable responses. | Capability commits `c039842b` and `60d0d7e1`; current lock `bg.cpp.browser-build-input-lock.sha256.08ddf15cbd0e5600fd1ae9f935cc7e3552e0140b665138ccdcf6d0f837bcdc34`; Wasm SHA-256 `937ffe559aa5bd57db7eeecd26a590d90344457e6a6e0b06151afb6cfcc3fb6d` (31,841,971 bytes); compiler Worker SHA-256 `33db9ab814e6ac9204a147006cbca845b7917b635d3f59efb9d9ac8a796c7626` (584,969 bytes); full-distribution reproducibility `bg.cpp.browser-full-distribution-reproducibility.sha256.5543bed26d97d5b10498794ca2e1c1f47c17ebb0b7ac28d2a5814538031b80b3` with package resource SHA-256 `1c65397c68e7aec7129d4961b1891b97825468019bcd43333f49a350db119209`. The 29,393-byte strict resource is `78cd399dbd660224377eb16a0388cbd5688440e0bf9849165346a0ceaed665b0`; the 14,450-byte convergence resource is `ed887f691781b87406c2266fcb2fa5d3a2187d36a648bba82345581329cd0846` with matrix `bg.cpp.browser-exact-distribution-convergence.sha256.3e3c66fe9de9f74d8f6d0cc608cc996340a3b2aeaf66a8a580a82fe32f5f8ba3`. All nine headed Chromium/Apple Metal cases passed real browser compile, authorization, CPU, and actual WebGPU convergence under one device profile. Unified release composition passes with distinct ephemeral signer/reviewer fixtures, but external key control, trust/approval, actual production authority issuance, and current `releaseReady` status remain false. |
...
dedicated browser worker. Its declared nine-case portable implementation exit
reuses the verified artifact, layout, CPU, and WebGPU seams without a
source-shaped execution or spelling-specific lowering path. Docker/native AOT
remains an optional CI/reference parity lane and is not a browser requirement.
Gate 2 word32 exact-storage profiles now cover positive and signed-affine
f32/i32/u32 view copies at ranks 1 through 8.
Gate 2 now also owns distinct positive and signed-affine packed8, packed16, and
word64 exact-storage profiles for bool/i8/u8, i16/u16/f16/bf16, and
f64/i64/u64 rank-1-through-rank-8 view copies. They deliberately preserve raw storage
only: bool bytes are not canonicalized, values are not converted, and no
non-word32 arithmetic is claimed.
...
also implemented. Version-1.5 and 1.6 bounded conditionals select from a
captured external-input u32 or an exact request-time runtime u32 captured
before execution. Version 1.7 admits one ordered temporary-resource u32
predicate and executes it through an explicit bounded GPU readback/resubmission
stage over persistent private buffers. Version 1.8 admits zero-through-
artifact-maximum request-time u32-count repetition and selects only exact
prewarmed pipeline slots while reporting actual work. Version 1.9 admits a
positive request-time prefix element count up to an artifact maximum, derives
the same dispatch effects and geometry as static view copy, and executes the
exact prefix on CPU and arbitrary positive one-dimensional portable WebGPU
through the same maximum-prewarmed program and pipeline slot. The backend binds
the selected prefix through a four-byte uniform, submits the ceiling workgroup
count, and returns tail invocations before semantic evaluation. Version 1.10
admits one ordered
temporary-resource u32 repeat count and executes zero through the artifact
maximum through the same explicit bounded feedback authority. Version 1.11
admits a positive produced-resource prefix element count through that
authority and substitutes only the validated logical count and matching
four-byte uniform in the already-prewarmed pipeline slots.
Version 1.12 admits positive request-time rank-2/rank-3 rectangular prefix
extents, executes the exact dense rectangle on CPU, and substitutes true
2D/3D geometry plus one 16-byte guard through the same maximum-prewarmed
program and exact pipeline slots.
Version 1.13 admits one ordered rank-local produced u32 extent per axis and
reuses the same rectangle meaning, program, geometry, guard, and exact slots
through a single bounded feedback stage.
Version 1.14 extends request-time rectangles to rank 4 by flattening the two
leading selected axes into physical z and reconstructing them behind the guard.
Version 1.15 extends produced-resource rectangles to rank 4 through four
ordered private u32 extents and the same one-stage feedback lifecycle.
Version 1.16 extends request-time rectangles to rank 5 under the distinct
rank-5 view-copy profile. A 32-byte eight-u32 guard carries five extents,
physical x/y retain axes 4/3, and z flattens axes 0 through 2 before guarded
mixed-radix coordinate reconstruction. Version 1.17 extends produced-resource
rectangles to rank 5 through five ordered private u32 extents and the same
single-stage feedback lifecycle. Version 1.18 extends request-time rectangles
to rank 6 under the distinct rank-6 view-copy profile, maps axes 5/4 to x/y,
and flattens axes 0 through 3 into z behind the same 32-byte guard. Version
1.19 extends produced-resource rectangles to rank 6 through six ordered private
u32 extents and the same single-stage feedback lifecycle.
Version 1.20 extends request-time rectangles to rank 7 under the distinct
rank-7 view-copy profile, maps axes 6/5 to x/y, and generically flattens axes 0
through 4 into z behind the same 32-byte guard while retaining the version-1.19
produced-resource rank-6 limit. Version 1.21 extends produced-resource
rectangles to rank 7 through seven ordered private u32 extents and the same
single-stage feedback lifecycle.
Version 1.22 extends request-time rectangles to rank 8, maps axes 7/6 to x/y,
and generically flattens axes 0 through 5 into z behind the same 32-byte
eight-u32 guard while preserving the produced-resource rank-7 limit.
Version 1.23 reads eight ordered produced extents through the same single
feedback stage and reuses that rank-8 mapping, guard, program, and exact slots.
Version 1.24 admits exactly two linear resource dispatches sharing one source,
rank, mode, and maximum. Version 1.25 admits exactly two rectangular resource
dispatches sharing the exact ordered sources and maxima; one readback selects
both exact prepared slots in either profile.
Version 1.26 admits exactly two linear resource dispatches with distinct
sources only when one dispatch writes the other's source; execution reports
two feedback stages and preserves one fail-stop resident-buffer lifecycle.
Version 1.27 admits exactly three distinct linear sources only when their
semantic writes form one producer chain; execution reports three feedback
stages under that same lifecycle.
Version 1.28 admits exactly four distinct linear sources only when three
semantic producer relations form one connected chain; execution reports four
feedback stages under that same lifecycle and rejects disconnected
degree-equivalent cycles.
Version 1.29 separately admits exactly one produced-u32 conditional and one
bounded repeat only when both name the same source and rank; one readback
selects both semantics in one aggregate feedback stage.
Version 1.30 admits exactly one sequential conditional-to-repeat profile when
both branches guarantee-write one distinct repeat-count source; execution
reports two ordered feedback stages under one fail-stop lifecycle.
Version 1.31 admits the equivalent exact conditional-to-linear-dispatch
profile when both branches guarantee-write one distinct positive bounded
launch-count source.
Version 1.32 admits the equivalent exact conditional-to-rank-2-rectangle
profile when both branches guarantee-write both distinct positive bounded
extent sources after selecting from a third predicate resource.
Version 1.33 extends the same exact profile through ranks 3 to 8. Both branches
must guarantee-write every distinct extent source; the predicate remains
distinct from all extents, and execution reuses the generic rectangle path.
Version 1.34 raises only the exact connected linear-chain ceiling from four to
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exact shared-selection fanout; versions 1.26/1.27/1.28 permit exact two-/three-/four-stage linear chains; version 1.34 extends the same connected-chain rule through eight stages; version 1.29 shares one conditional/repeat source; versions 1.30/1.31 require both conditional branches to guarantee-write one distinct repeat or linear-launch source; versions 1.32/1.33 require both branches to guarantee-write every distinct rank-2 or rank-3-through-rank-8 extent source. Earlier versions retain their feedback bounds and rectangle-rank limits. | Broader mixed or device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage includes one/two selections through eight connected stages, pre-version ceiling refusal, small/full conditional-produced rank-2 and rank-8 rectangles, missing-branch-write, and shared-source refusal. Full semantic-core passes 24 files/257 tests. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed ceilings, and publishes only after success. Version 1.34 executes each of eight distinct positive bounded launch selections in chain order; versions 1.30 through 1.33 read the predicate, execute the selected branch, then read its guaranteed distinct repeat, linear-launch, or rank-2-through-rank-8 extent sources. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves exact eight-stage completion/output, conditional-produced repeat/launch/rectangle selection through rank 8, shared fanout, earlier two-/three-/four-stage chains, refusal, and rank-2-through-rank-8 rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.38 adds version 1.34's five-through-eight-stage connected linear chains through the existing generic staged resident-buffer executor. It reads, validates, and specializes every positive bounded selection in producer order without callbacks, recompilation, or a stage-shaped path. | Broader mixed or device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Ninety-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. One/two-element eight-stage cases retain pipeline identity, distinct specialization, exact ordered completions, snapshot isolation, eight feedback nodes, and eight stages. Correctness artifact `55c2ea89c19329ef4c522b46666402a65671abbb6b38555e09c0446a22808206`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 2.10/6.40 ms and baseline median/p95 2.10/6.20 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.20.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. CI, release, and publish retain exact-commit evidence separately from correctness. |
| Gate 2 view-family selection | `verified` | Selected typed JIT `PERMUTE` plus compiler L1 read binding and sibling L2 guarded materializing view copy. The required 75-case view-copy matrix covers separately identified positive and signed-affine source profiles at ranks 1 through 8 for bool/i8/u8 packed8, i16/u16/f16/bf16 packed16, f32/i32/u32 word32, and f64/i64/u64 word64 storage with exact CPU/WebGPU parity. Packed8, packed16, and word64 prove storage movement only; broader view or arithmetic families must add typed profiles rather than reinterpret existing identities. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact semantic-adapter binding, separate zero-import raw-Wasm verifier/compiler Workers, closed VFS, canonical transfer, one-shot entry, and typed terminal protocol compile source-derived view-copy graphs. ABI 1.17 retains the independent diagnostic cap and 32 MiB result ceiling. The package-pinned extractor recognizes exact f32/i32/u32 rank-1-through-rank-4 plus signed-rank-2 view-copy facts without adding a source-spelling execution path. The exact distribution lane carries all nine unchanged sources through authorization, canonical CPU, and required real-WebGPU execution. | Replace the local engineering producer and synthetic legal fixtures with externally rooted producer trust and distribution approval. The declared browser-local implementation path itself has no remaining software gap. | Reproducible Wasm SHA-256 `937ffe559aa5bd57db7eeecd26a590d90344457e6a6e0b06151afb6cfcc3fb6d` (31,841,971 bytes). The exact package matrix covers nine real Worker compilations at resource SHA-256 `78cd399dbd660224377eb16a0388cbd5688440e0bf9849165346a0ceaed665b0`. Compiler Worker SHA-256 `33db9ab814e6ac9204a147006cbca845b7917b635d3f59efb9d9ac8a796c7626`; verifier Worker SHA-256 `3f41d964119c0dc56e1e01ff0aca20886e93c96e7343e4d49fc646b2dc90279c`; both retain zero static/dynamic imports. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the extractor closure, LLVM 22.1.8, Emscripten image, recipe, ABI, and exact 25-path distribution plan. Cached diagnostics remain untrusted and separate from clean/release authority. The exact reviewer rejects interface drift after preserving bounded evidence. The current extractor is package-pinned from two distinct-path cache-free builds; the exact 17-file header subset is independently rehashed across two roots and package-pinned. The deterministic materializer authenticates those headers plus current package Wasm, Worker, lock, profile, and policy inputs and exclusively writes the exact 24-subject tree without a signing interface. The finalizer independently rereads that tree and profile and adds only the detached envelope after the in-process producer transition binds the same subject. Two distinct live 25-output roots passed the complete verifier; their package observation rebinds all outputs to current package authorities and grants reproducibility only. The unified no-clobber exchange issues and verifies the exact distribution-review request while persisting only a non-reusable observation. The no-private-key policy authoring command converts two fixed immutable P-256 public-key files and distinct canonical HTTPS identities into admitted producer/reviewer policies plus an exact handoff record. Final composition independently rejects exact identity or key reuse. | Provision the distinct externally controlled builder and reviewer keys, author their production policies through the implemented boundary, obtain externally issued exact statements, and retain those authorities separately from current full-distribution reproducibility and final release approval. | Current lock `bg.cpp.browser-build-input-lock.sha256.08ddf15cbd0e5600fd1ae9f935cc7e3552e0140b665138ccdcf6d0f837bcdc34`; header reproducibility `bg.cpp.browser-header-distribution-reproducibility.sha256.786197fcadb7421405943fa87c3c647b06b98aeb116554aeb7cdb058a46b0252`; full reproducibility `bg.cpp.browser-full-distribution-reproducibility.sha256.5543bed26d97d5b10498794ca2e1c1f47c17ebb0b7ac28d2a5814538031b80b3`; full package resource SHA-256 `1c65397c68e7aec7129d4961b1891b97825468019bcd43333f49a350db119209`; 25 outputs/103,638,762 bytes per root. The local engineering signer grants no external producer or release authority; production external approval remains absent. |
...
| Gate 3 authorized static layout lowering | `verified` | The internal compiler boundary accepts one instance-bound authorized artifact plus one closed explicit entry ID. It requires exactly one accepted selected layout entry, binds it to exactly one affine fact/result declaration, preserves unrelated typed facts without interpreting them, evaluates all static integer algebra through shared bounded semantics, lowers nested modes with colexicographic coordinate composition, and stores source/macro/producer origin in a compiler side table. It uses CuTe `cosize(layout) = layout(size(layout) - 1) + 1`, including signed strides; it does not manufacture storage. The exact browser distribution now reaches this seam from observed Worker candidates cross-bound to the local engineering producer. | Replace the local engineering producer with real external signer evidence. Add a dynamic layout profile only with explicit binding authority. | Nine strict Worker cases and the exact distribution convergence matrix exercise the shared authorization/lowering seam; the complete compiler suite passes 106 files/1,660 tests and both architecture checks retain zero cycles/leaks. |
| Gate 3 authorized CuTe view-copy lowering | `partial` | One exact authorized frontend artifact plus a closed entry/storage request lowers through semantic-core's sole `createVerifiedViewCopyArtifacts` constructor. The current artifact profile accepts same-dtype f32/i32/u32 with exact 32-bit device ABI/alignment, two distinct non-null global parameter pointers with source-only const qualification, one synchronous portable 32-bit copy with exact read/write effects, and equal flat static ranks 1 through 4. Source strides may be positive, zero/broadcast, or signed; the destination remains positive-affine and dense. Allocation lengths and view offsets are explicit host facts. Version-2 lowering derives exact minimum/maximum affine element offsets and proves both sides of the rebased root-allocation range; CuTe `cosize` never grants pointer capacity. Semantic-core still owns IDs, alias sets, effects, canonical hashes, reject-invalid-source semantics, and forbid-overlap. The exact signed-rank-2 Worker payload now reaches this same seam and passes complete CPU/required-WebGPU convergence without folding storage facts into candidate or authorization identity. | Replace the local engineering producer/reviewer material with valid externally rooted evidence. Dynamic layouts, rank-5 frontend facts, signed destinations, and broader tensor-engine profiles remain unsupported. | Commits `85631464`, `a8a861e2`, `dca33be5`, `c039842b`, and `60d0d7e1`; focused lowering covers signed ranks 1 through 4 and exact lower/upper storage refusal. Compiler passes 106 files/1,660 tests. The nine-case exact distributed matrix passes unchanged signed-rank-2 C++/CuTe compilation, authorization, CPU, and actual WebGPU on Apple Metal; production trust/release claims remain false. |
| L2 materializing view-copy contract and CPU reference | `verified` | `view-copy@1.0` owns effects, exact reject/fill bits, and forbid-overlap semantics. Generic L2 verification stays backend-neutral; positive-affine profiles cover f32/i32/u32 ranks 1 through 5, and distinct signed-affine profiles cover negative source-coordinate scales at ranks 1 through 5 without admitting a signed destination. Prepared CPU execution compiles maps once, proves guarded reads and dense destination writes, caches source offsets, derives binding-sensitive specialization hashes, and enforces element/step/scratch/wall-time budgets, cooperative browser yielding and abort, plus native buffer-slot, length, alignment, overlap, and shared-memory checks. |
| Kernels-owned WGSL view-copy lowering | `verified` | The lowerer consumes authority-bound immutable backend-neutral specializations, preserves whole-root 32-bit words through u32 storage, interval-proves signed i32 arithmetic, lowers canonical source/destination maps, emits structured guarded fill loads, validates device and transient-working-set limits, and derives semantic plus device-specific hashes. Backend 2.4 admits the rank-1 signed-affine source profile through the same address path as the other signed profiles. One-in-flight ownership, timeout/abort stale-result suppression, exact scope drainage, distinct error stages, and device-loss invalidation have deterministic fake-device coverage. The required headed lane emits one validated `passed` terminal record for all 20 bit-exact CPU/WebGPU cases on Apple Metal 3; headless absence remains a truthful failed environment record. |
...
| Operational release instruction convergence | `verified` | `DEVELOPMENT.md`, `RTK.md`, `RELEASING.md`, `docs/platform/agent-consumption-guide.md`, and `docs/platform/release-readiness.md` now require the same read-only preflight and protected staged-only publication path. Each rejects package-local `npm publish` and `pnpm publish`; only the immutable staged tarball reaches the credentialed workflow job with scripts disabled. The packed-release gate reads all five documents before packing and fails closed when the preflight, protected/staged terms, or any known direct-publication instruction drifts. | No implementation work remains. Registry publication is still an explicit release operation. | `pnpm test:release-packages` passes 19 tar-snapshot tests, 35 release-security tests, all seven packed/fresh-consumer checks, and the release-instruction convergence guard. |
| LLD checkpoint gate-status convergence | `verified` | The architecture check parses the bounded checkpoint and gate-ledger tables, requires exactly Gates 0 through 7 once in each, accepts only the closed status vocabulary, normalizes display spelling, and rejects any status mismatch. The LLD remains normative while the ledger remains the implementation-evidence owner; neither can silently overstate the other. | No implementation work remains. | Focused mutation tests reject missing gates and LLD/ledger drift in both directions; implementation/declaration parity and the full architecture check retain the guard. |
...
| D-165 | 2026-07-27 | accepted | Issue the initial host-graph CPU reference only from exact verifier-issued graph authority plus the exact opaque kernel/layout artifacts named by that graph. Snapshot caller inputs into private rank-local storage, admit no caller destination, execute topologically, and expose output buffers only after every node succeeds. Bound graph storage plus largest collective scratch, total element operations, preparation/execution time, and native cancellation. For collectives, use ascending participant rank, finite f32 with rounding after every sum, wrapping 32-bit integer sum, and exact integer min/max; reject non-finite f32 operands/results and shared memory. | Directly executing into caller destinations makes the declared no-partial-output-commit failure model false under later-node failure. Treating semantic verification as execution authority permits copied or cross-wired artifacts, while unbounded rank multiplication, cached offsets, or collective scratch can exhaust a browser process. One private, authority-bound reference executor gives Gate 7 a deterministic oracle without conflating it with portable GPU execution, transport/topology, or terminal evidence. |
| D-166 | 2026-07-27 | accepted | Make graph-resource initialization part of `browsergrad.host-graph@1`: every per-rank input requires `external-input`, and every temporary/output requires deterministic `zero-fill` before the first node. Derive this field in compiler-owned graph construction and reject every role/initialization mismatch during semantic verification. | A role and byte length alone do not define bytes outside a partial destination view or the result of a zero-element dispatch. Letting CPU allocation behavior accidentally define those bytes makes CPU/WebGPU convergence depend on host allocator behavior and makes no-partial-output semantics incomplete. Explicit initialization closes the observable state before adding a second backend without allowing callers to choose unsafe or backend-specific initialization. |
| D-167 | 2026-07-27 | accepted | Put the portable static host-graph adapter in kernels behind the semantic-core `/graph` seam. Require exact graph/kernel/layout authority; expand verified view-copy dispatches per rank; implement arbitrary participant counts with ordered pairwise f32/i32/u32 reduction plus raw-word replication; retain one private buffer per bound rank/resource; and commit no caller-visible output until complete readback and f32 numerical-status validation. Bound expanded steps, aggregate private host/GPU storage, preparation/execution time, and device limits; classify scoped validation/OOM/internal, shader, pipeline, device-loss, cancellation, and timeout failures separately. | Putting orchestration in compiler would create a second source-shaped execution path, while putting WebGPU in semantic-core would reverse package dependencies. Pairwise steps avoid one binding per rank and preserve canonical participant order on core WebGPU. Private storage plus stale-result suppression preserves the graph fail-stop contract; an atomic f32 status closes non-finite/overflow failures that WebGPU cannot throw from a shader. Required complete-output CPU parity is execution evidence for this closed adapter only, not dynamic-control, transport/topology, native, or performance evidence. |
| D-168 | 2026-07-27 | accepted | Add whole-allocation per-rank copy as host-graph program version 1.1 while retaining exact version-1.0 decoding and construction identity. Require distinct resources with identical dtype and allocation byte length, derive read/write effects inside the verifier, and apply the existing input-immutability, read-before-write, dependency, and unordered-hazard rules. Let the CPU oracle copy arbitrary bytes and account each copied byte; let the portable WebGPU adapter lower only whole 32-bit words to its existing raw replication WGSL and refuse other sizes explicitly. | Treating copy as a spelling of dispatch would require fake layout/kernel artifacts and distort element semantics; treating it as an unverified backend command would bypass the canonical hazard model. A minor-version semantic node gives copy one backend-neutral meaning, preserves old readers' expectations, admits all dtypes in the CPU oracle, and makes the narrower portable-device alignment contract explicit rather than silently padding or substituting semantics. |
| D-169 | 2026-07-28 | accepted | Add `host-readback-after-graph-success` as host-graph program version 1.2 terminal materialization. Require exactly one materialization for every declared output, derive an ordered read effect, require it after the output's final writer, reject input/temporary exposure and duplicates, and forbid dependents. Preserve exact 1.0/1.1 decoding and implicit-output behavior. Let CPU and portable WebGPU select results from verified materializations while retaining private storage and publishing only after whole-graph success; materialization adds no element operation or GPU dispatch. | Resource role alone made output readback implicit and left L4 unable to represent the required materialization point. Eager publication would violate fail-stop semantics, while snapshot-at-node semantics would require another allocation/copy contract. A terminal semantic read makes selection and ordering explicit, reuses the canonical hazard verifier, and lets both adapters preserve existing bounded final readback without hidden work or partial output exposure. |
| D-170 | 2026-07-28 | accepted | Add unique `completion-after-dependencies` events as host-graph program version 1.3 while retaining exact 1.0–1.2 decoding. Give an event no resource effect, timestamp, queue identity, external wait, or transport authority. Preserve it in topological execution order and report completed event IDs only in a successful whole-graph CPU/WebGPU result. Events add no element operation, allocation, or GPU command. | Reusing node IDs as implicit milestones leaves no stable event vocabulary for later pipeline or transport adapters, while claiming timestamps/fences now would invent backend semantics the static graph cannot prove. A zero-work dependency marker is the smallest honest event contract: it is observable, orders later nodes through the existing DAG, preserves fail-stop result publication, and leaves stronger timing, external-wait, and cross-queue/worker profiles explicitly open. |
| D-171 | 2026-07-28 | accepted | Add `fixed-count-sequential` repetition as host-graph program version 1.4 while retaining exact 1.0–1.3 decoding. Require one positive artifact-fixed count, a nonempty bounded linear body containing only dispatch/all-reduce/copy, globally unique top-level/body node IDs, aggregate canonical effects/hazards, and explicit iteration plus expanded-node ceilings. Execute sequentially in the CPU oracle with iteration/body cancellation points and statically expand through the existing WebGPU lowerers with per-body preparation and step checks. Report the node, count, and body IDs only after whole-graph success. | A hidden backend loop would bypass semantic hashing, hazard verification, work admission, cancellation, and trace meaning; a general loop or nested subgraph would prematurely claim runtime control-flow authority. One fixed-count linear node is the smallest honest bounded-control slice: it reuses the canonical executable nodes and failure model, proves repeated collective meaning on CPU and real WebGPU, and leaves conditionals, runtime counts/launches, pipelines, and transport explicitly open. |
| D-172 | 2026-07-28 | accepted | Add `input-u32-branch-sequential` as host-graph program version 1.5 while retaining exact 1.0–1.4 decoding. Require one exact aligned four-byte external-input u32 predicate at a fixed rank, nonempty bounded linear dispatch/all-reduce/copy branches with equal node-kind structure, globally unique IDs, conservative union effects, and guaranteed writes only where both branches write. CPU requires equal element-operation counts and selects from its private input snapshot. WebGPU pre-lowers both branches, requires equal lowered execution shapes, binds their resource/module union, selects before device work from the captured predicate, and includes branch identity in specialization and successful completion evidence. | A GPU-written or mid-graph predicate would require readback, resubmission, new failure/cancellation points, and a different authority profile; an opaque host callback would evade semantic hashing and hazard verification. Equal branch structure/work makes counts exact rather than worst-case estimates, while merged effects and guaranteed-write intersection prevent branch-dependent uninitialized reads. This is the smallest honest conditional slice and leaves runtime/backend-derived control, dynamic launches/counts, pipelines, and transport explicitly open. |
| D-173 | 2026-07-28 | accepted | Add a separate required host-graph performance observation rather than turning correctness or publication into a speed threshold. Compare version-1.4 fixed repetition with a version-1.2 statically unrolled graph only after proving bit-exact CPU outputs, equal element operations, equal expanded WebGPU steps, and an untimed actual-device correctness preflight. Measure both through the same production host API, complete readback, and queue drain with alternating paired samples; retain raw samples, exact workload, lifecycle, browser/device identity, and exact-commit evidence. Make no superiority or regression claim. | A dispatch-only microbenchmark would omit the fail-stop API's real capture, validation, readback, and cleanup costs, while unequal work would make graph-control overhead uninterpretable. A required observation catches catastrophic lifecycle regressions and records current behavior without converting noisy, machine-specific timing into semantic authority or a brittle release threshold. |
| D-174 | 2026-07-28 | accepted | Add `runtime-u32-branch-sequential` as host-graph program version 1.6 while retaining exact 1.0–1.5 decoding. Name predicates by a bounded unique control ID, require exactly one plain request binding for every graph control, admit only canonical wire u32 values, and capture the complete control set with input bytes before the first await or device access. Reuse the version-1.5 equal-shape branches, conservative effects, exact work accounting, CPU plan, and pre-lowered portable WebGPU branch path; bind selected branch identity into successful evidence. | Encoding request-time control as a fake input resource would distort resource lifetime and require caller-owned bytes for a host scalar, while an opaque callback would evade semantic hashing and exact request admission. A closed exact control set makes host-known invocation policy explicit without claiming GPU/backend-derived predicates, mid-graph feedback, runtime loop counts, or dynamic launches. Reusing the existing conditional module deepens one semantic seam and preserves its fail-stop and bounded-work guarantees. |
| D-175 | 2026-07-28 | accepted | Add a separate opaque `browsergrad.host-graph.webgpu-pipeline@1` authority without widening host-graph semantics. Bind one exact prepared graph to one `GPUDevice`; admit device limits before compilation; prewarm every unique WGSL program allowed at each exact step slot, including both alternatives of bounded conditionals; and namespace the low-level cache by graph/backend/WGSL/schedule/feature/limit/numerical-policy identity. Reject copied, destroyed, cross-device, reordered, or unadmitted programs before request-owned resource allocation. Keep one authority-bound execution implementation: the explicit API reuses a long-lived authority, while the convenience API synchronously captures request inputs/controls before creating and destroying an ephemeral authority through that same path. | Treating generic cache residency as pipeline authority would let structurally similar callers borrow compilation without proving graph, device, numerical, or lifecycle identity. Baking pipelines into the semantic graph would reverse the L4/L5 boundary, while a second hot executor would duplicate failure and output-publication behavior. Exact step-slot alternatives preserve bounded control without authorizing arbitrary reorderings; one deep backend module owns compilation, cache identity, lifecycle, and misuse rejection while request buffers remain private and per-run. |
| D-176 | 2026-07-28 | accepted | Add `resource-u32-branch-sequential` as host-graph program version 1.7 while retaining exact 1.0–1.6 decoding. Admit at most one conditional whose predicate is an exact aligned four-byte rank-local temporary u32 with an ordered writer. Reuse the existing bounded equal-shape branch semantics, conservative effects, exact work accounting, CPU plan, and prewarmed branch alternatives. For portable WebGPU, keep private graph buffers resident, submit the exact prefix, read back only the predicate, then submit the selected branch plus suffix against those same buffers. Extend generic pipeline authority only with validated contiguous slot ranges, and keep one timeout, cancellation signal, device owner, cleanup path, numerical check, and terminal publication contract across both submissions. | Selecting a GPU-produced predicate before device work would be false, while an opaque callback or source-shaped backend path would evade semantic hashing, hazard verification, and pipeline authority. Re-uploading the whole graph between stages would change resource identity and magnify feedback cost. One explicit four-byte readback/resubmission boundary is the smallest honest GPU-produced control slice: it deepens the existing graph and pipeline modules, proves CPU/real-WebGPU branch parity, prevents intermediate publication, and leaves nested/device-side control, runtime loop counts, dynamic launches, transport, and native execution separately open. |
| D-177 | 2026-07-28 | accepted | Add `runtime-u32-count-sequential` as host-graph program version 1.8 while retaining exact 1.0–1.7 decoding. Bind one required request-time u32 control and a positive artifact maximum under the existing repeat ceiling; admit zero through that maximum before input copying or device access, charge maximum semantic/CPU/WebGPU work, and treat body writes as non-guaranteed because zero iterations perform no write. CPU executes and reports the exact captured count and actual operations. WebGPU prewarms the maximum schedule once, then selects a strictly increasing exact subset of authorized step slots without compilation, reordering, duplication, or substitution; prepared budgets remain maximum while specialization and terminal traces report actual slots/work/completion. | Encoding a count as a fake resource would distort lifetime semantics, while host-side emitter loops outside the graph would evade semantic hashing, request admission, pipeline authority, and truthful work evidence. Recompiling for each count would also make runtime control part of the iteration loop and destroy the fast feedback contract. Artifact-capped request control reuses the existing graph, CPU plan, WGSL templates, and device-bound authority, including zero-work correctness, while leaving GPU/backend-derived loops, dynamic launch, nested control, transport, and native execution separately open. |
| D-178 | 2026-07-28 | accepted | Add top-level `dynamic-dispatch` as host-graph program version 1.9 while retaining exact 1.0–1.8 decoding. Bind one positive request-time `u32-prefix-element-count` and an artifact maximum within the verified semantic element domain; admit one through that maximum before input copying or device access and charge maximum work. CPU executes the exact canonical view-copy prefix and reports actual rank-multiplied operations. Portable WebGPU prewarms the maximum launch once through the existing exact pipeline slot and, for the initial profile, requires one invocation per workgroup so the admitted logical prefix is the exact x-workgroup count. Runtime substitution changes only launch geometry; actual completion enters specialization and terminal evidence. Reject zero, above-maximum, nested, and wider-workgroup use. | A real-device failure exposed that treating logical invocation count as dispatch geometry under a wider workgroup launches inactive-but-unmasked lanes and silently executes beyond the selected prefix. A masked static dispatch, host emitter loop, or per-count compilation would not prove dynamic launch and would evade or dilute graph and pipeline authority. One-lane positive prefixes are the smallest honest portable dynamic-dispatch slice: they deepen the shared semantic, CPU, WGSL, and device-bound pipeline seams while leaving wider schedules, arbitrary grids, GPU/backend-derived launch control, nested control, transport, and native execution separately open. |
| D-179 | 2026-07-28 | accepted | Add `resource-u32-count-sequential` as host-graph program version 1.10 while retaining exact 1.0–1.9 decoding. Bind one exact rank-local, aligned, four-byte, zero-filled temporary u32 with an ordered writer and a positive artifact maximum under the existing repeat ceiling. Admit zero through that maximum, charge the maximum, forbid body access to the count resource, and treat body writes as non-guaranteed. Admit at most one produced-resource feedback node total across conditional and repeat control. CPU reads the private source after its producer. Portable WebGPU prewarms the maximum schedule, submits the exact prefix, reads only the count, then selects the exact authorized body slots plus suffix over resident buffers; a zero-count repeat with no executable suffix reads outputs directly from those buffers. Above-bound counts fail without output publication, and successful specialization/trace evidence records actual completion and work. | This deepens the existing explicit feedback authority instead of introducing a fake host input, opaque callback, recompilation path, or device-side loop claim. One graph feedback point keeps ownership, timeout, cancellation, device-loss, numerical-status, cleanup, and publication bounded across both stages. Repeated/device-side feedback, GPU-produced dynamic launch, broader schedules, transport, and native execution remain separately versioned work. |
| D-180 | 2026-07-28 | accepted | Add `resource-u32-prefix-elements` as host-graph program version 1.11 while retaining exact 1.0–1.10 decoding. Bind one exact rank-local, aligned, four-byte, zero-filled temporary u32 with an ordered writer and a positive artifact maximum within the verified semantic element domain. Admit one through that maximum, charge maximum work, forbid dispatch data access to the launch source, and extend the at-most-one produced-resource feedback bound across conditional, repeat, and dynamic-dispatch control. CPU reads the private source after its producer and executes the canonical exact prefix. Portable WebGPU prewarms the maximum one-invocation-per-workgroup launch once, submits the exact producer prefix, reads only the count, substitutes launch geometry in the same authorized rank slots over resident buffers, and submits the dispatch plus suffix. Zero and above-bound counts fail without output publication; successful specialization/trace evidence records actual completion and work. | This is the smallest honest GPU-produced dynamic-launch slice and deepens the existing semantic dispatch, explicit feedback, and device-bound pipeline authority seams without a fake host control, opaque callback, source-shaped backend, or request-time recompilation. The one-invocation-per-workgroup restriction preserves exact logical-prefix meaning; wider workgroups require masking or a separately proved schedule. Repeated/device-side feedback, broader launch schedules, transport, and native execution remain separately versioned work. |
| D-181 | 2026-07-28 | accepted | Widen only the portable WebGPU backend for the existing version-1.9 and version-1.11 dynamic-dispatch semantics. Admit a wider prepared workgroup when both the artifact maximum and selected positive logical-prefix count are exact multiples of that size. Preserve the canonical invocation-count launch contract so the WGSL sequence runner submits exactly `elementCount / workgroupSize` x workgroups through the same prewarmed program and exact pipeline slots. Reject unaligned artifact maxima during preparation, request-time counts before device execution, and produced-resource counts after the bounded feedback read but before the dispatch and any output publication. Keep the semantic graph and CPU oracle unchanged. | The canonical view-copy shader already indexes by global invocation ID, so aligned prefixes need neither a mask nor a new semantic version; the existing runner converts logical dispatch counts to workgroup counts. Treating workgroup count as the public launch count would divide twice and underexecute. Exact alignment proves that every launched lane belongs to the requested prefix, while arbitrary unaligned prefixes still require an explicit count-buffer/mask schedule or another separately verified lowering. |
| D-182 | 2026-07-29 | accepted | Widen the portable WebGPU backend for the existing version-1.9 and version-1.11 dynamic-dispatch semantics to every positive bounded one-dimensional prefix. Emit one canonical runtime-prefix uniform guard in shared view-copy WGSL before coordinate, address, or resource evaluation; prewarm that program once; and let execution change only the validated logical dispatch count and matching four-byte uniform. Submit `ceil(elementCount / workgroupSize)` x workgroups. Charge 16 aligned GPU bytes and four host bytes for every dynamic rank step, and derive maximum total, storage, and uniform binding counts across all reachable programs, including `maxUniformBuffersPerShaderStage` in device identity and admission. Keep graph semantics, the CPU oracle, and the produced-resource feedback lifecycle unchanged. | The aligned-only restriction was a backend limitation. A one-lane fallback would constrain large launches and add alternative pipelines, while shader-source patching would create a second lowering path. The uniform guard preserves shared lowering, exact prefix effects, large schedules, pipeline identity, and fail-stop behavior. Multidimensional launch remains a separate semantic contract. |
| D-183 | 2026-07-29 | accepted | Add `runtime-u32-rectangular-prefix` as host-graph program version 1.12 for rank-2 and rank-3 semantic view-copy domains while retaining exact 1.0-1.11 decoding. Bind one positive request-time `u32-prefix-extent` control per axis and an artifact maximum no larger than the corresponding verified logical extent; charge the maximum product and admit the complete control set before input copying. CPU maps the selected dense rectangle into the full row-major semantic domain. Portable WebGPU emits one canonical 16-byte extent guard before semantic evaluation, prewarms the maximum program and exact pipeline slot once, maps physical x/y/z to trailing-to-leading logical axes, and changes only true 2D/3D launch geometry plus the uniform. Completion records exact logical extents and their product. Device admission checks all three workgroup dimensions. | Flattening a rectangle to a linear prefix would execute the wrong coordinates whenever an inner selected extent is smaller than the semantic shape. Shader-source patching or separate rank-shaped kernels would duplicate the canonical view-copy lowering and fragment pipeline authority. One bounded rectangular node deepens the existing graph, CPU, WGSL, and exact-slot seams while preserving request capture, maximum admission, fail-stop publication, and stable pipeline identity. GPU-produced rectangular launch, rank-4-or-higher dynamic domains, repeated feedback, transport, and native execution remain separately versioned work. |
| D-184 | 2026-07-29 | accepted | Add `resource-u32-rectangular-prefix` as host-graph program version 1.13 while retaining exact 1.0-1.12 decoding. Bind one distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 per rank-2/rank-3 semantic axis; require every writer to dominate the dispatch; forbid the dispatch from also binding an extent source as data; charge the maximum product; and keep the graph-wide at-most-one produced-resource feedback node. CPU reads all private extents after their producers and executes the exact dense rectangle. Portable WebGPU prewarms the same maximum rectangular program and exact rank slots, submits all producers in one resident-buffer prefix stage, reads exactly one u32 per axis, rejects zero and above-maximum values, then substitutes only true 2D/3D geometry plus the existing guarded 16-byte uniform before the suffix. | Packing multiple extents into a new ad hoc scalar-buffer ABI would create a second resource abstraction, while one feedback stage per axis would violate the bounded lifecycle and multiply synchronization. Distinct existing four-byte scalar resources preserve resource effects, writer dominance, exact byte accounting, and the one-stage authority. Reusing the version-1.12 rectangular plan keeps CPU/WGSL/pipeline semantics shared; only the source of validated extents changes. Rank-4-or-higher domains, repeated/device-side feedback, transport, and native execution remain separately versioned work. |
| D-185 | 2026-07-29 | accepted | Extend only request-time `runtime-u32-rectangular-prefix` to rank 4 as host-graph program version 1.14 while retaining exact 1.0-1.13 decoding and the rank-2/rank-3 produced-resource contract. Admit four bounded positive controls and maxima, preserve the same 16-byte four-u32 uniform, maximum-product work accounting, CPU rectangle semantics, and maximum-prewarmed canonical view-copy program. Portable WebGPU maps logical axes 3 and 2 to physical x and y and flattens selected axes 0 and 1 into physical z; the canonical shader guards the selected leading product before reconstructing axis 0 by quotient and axis 1 by remainder, ahead of semantic evaluation. Pipeline identity remains stable across selected rectangles and device admission checks the flattened maximum z dimension. | WebGPU exposes only three physical dispatch dimensions, so rank 4 requires one bounded semantic-to-physical mapping rather than another rank-shaped kernel or source-patched lowering. Flattening the two leading axes preserves the dense row-major rectangular domain, the existing graph/CPU/WGSL/pipeline authority seams, and constant-size runtime control. Produced-resource rank 4, rank-5-or-higher dynamic domains, repeated/device-side feedback, transport, and native execution remain separately versioned work. |
| D-186 | 2026-07-29 | accepted | Extend `resource-u32-rectangular-prefix` to rank 4 as host-graph program version 1.15 while retaining exact 1.0-1.14 decoding and all older rank limits. Bind four distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 extent sources under the existing graph-wide at-most-one produced-resource feedback-node bound. CPU reads all four private extents after their writers and executes the exact rank-4 dense rectangle. Portable WebGPU prewarms the version-1.14 maximum rank-4 program and exact slots, submits all producers in one resident-buffer prefix stage, reads and validates four u32 values together, then reuses the same flattened-z geometry, guarded 16-byte uniform, and suffix execution without recompilation or another feedback point. | Rank 4 changes the number of extent sources but not their resource semantics or the selected rectangle meaning. Reusing four existing scalar resources and one aggregate feedback stage preserves writer dominance, private storage, exact byte accounting, timeout/cancellation/device-loss ownership, stable pipeline identity, and fail-stop publication. A packed control ABI, per-axis readback stages, or a separate produced-rank-4 kernel would duplicate established authority. Rank-5-or-higher domains, repeated/device-side feedback, transport, and native execution remain separately versioned work. |
| D-187 | 2026-07-29 | accepted | Add `browsergrad.view-copy.positive-affine-rank5-word32@1` as a distinct portable profile for equal-rank, same-dtype f32/i32/u32 global-memory view copy while retaining every rank-1-through-rank-4 profile identity and rule unchanged. Reuse the canonical positive-affine verification, exact CPU evaluator, operation effects, alias policy, and guarded source semantics. Extend only the canonical WGSL coordinate lowerer with exact five-axis row-major mixed-radix reconstruction and bump its backend identity. Keep rank 6 refused. | Widening an existing rank-named profile would silently change its compatibility meaning, while a source-shaped rank-5 kernel would split the sole semantic execution path. One separately named profile honestly exposes broader layout rank through the same deep operation/evaluator/lowerer seams and lets release evidence distinguish the new backend capability. It does not claim signed strides, packed or non-32-bit storage, overlap, non-global memory, dynamic rank-5 host graphs, or frontend-produced rank-5 source compatibility. |
| D-188 | 2026-07-29 | accepted | Extend only request-time `runtime-u32-rectangular-prefix` to rank 5 as host-graph program version 1.16 while retaining exact 1.0-1.15 decoding and the version-1.15 produced-resource rank-4 limit. Bind five positive bounded controls and maxima to the distinct rank-5 view-copy profile. CPU reuses the generic exact rectangle evaluator. Portable WebGPU uses one 32-byte eight-u32 uniform with five active extent words, maps axes 4/3 to physical x/y, flattens selected axes 0 through 2 into physical z, guards their product, and reconstructs those leading coordinates by mixed radix before semantic evaluation. Preserve one maximum-prewarmed canonical program, exact pipeline slots, snapshot, budget, device-limit, and fail-stop contracts. | Rank 5 changes the semantic-to-physical schedule and uniform ABI but not rectangular meaning. A separately versioned graph contract and 32-byte uniform preserve old hashes and bindings while deepening the existing graph, canonical view-copy, WGSL, and pipeline-authority seams. A source-patched rank-5 kernel or second executor would split authority and duplicate semantics. Produced-resource rank 5, rank-6-and-higher domains, repeated/device-side feedback, transport, and native execution remain separately versioned work. |
| D-189 | 2026-07-29 | accepted | Extend `resource-u32-rectangular-prefix` to rank 5 as host-graph program version 1.17 while retaining exact 1.0-1.16 decoding and every older rank limit. Bind five distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 extent sources under the existing graph-wide at-most-one produced-resource feedback-node bound. CPU reads all five private extents after their writers and executes the exact rank-5 dense rectangle. Portable WebGPU prewarms the version-1.16 maximum rank-5 program and exact slots, submits all producers in one resident-buffer prefix stage, reads and validates five u32 values together, then reuses the same flattened-z geometry, guarded 32-byte uniform, and suffix execution without recompilation or another feedback point. | Rank 5 changes the number of produced extent sources but not their resource semantics, rectangular meaning, or established rank-5 physical mapping. Reusing five existing scalar resources and one aggregate feedback stage preserves writer dominance, private storage, exact byte accounting, timeout/cancellation/device-loss ownership, stable pipeline identity, and fail-stop publication. A packed control ABI, per-axis stages, or separate produced-rank-5 kernel would duplicate established authority. Rank-6-and-higher domains, repeated/device-side feedback, transport, and native execution remain separately versioned work. |
| D-190 | 2026-07-29 | accepted | Add `browsergrad.view-copy.signed-affine-rank2-rank3-word32@1` as a distinct portable profile for same-dtype f32/i32/u32 global-memory view copies whose fully specialized rank-2/rank-3 source location or predicate contains a negative coordinate scale. Keep the destination positive-affine and dense, preserve every existing positive-affine identity, and reuse the canonical CPU access proof plus the existing signed-i32 interval/address WGSL lowerer. Refuse signed rank 1/4/5, signed destinations, integer division/modulo, rank 6, packed storage, overlap, and non-global memory. | Exact CPU preparation and canonical WGSL already represented and proved signed addresses; only semantic profile legalization was missing. A distinct bounded profile exposes that capability without changing older compatibility meaning, coupling negative strides to unrelated division/modulo rules, or creating another executor or source-shaped kernel. |
...
| D-201 | 2026-07-30 | accepted | Add distinct positive- and signed-affine rank-6 word32 view-copy profiles while retaining every rank-1-through-rank-5 profile identity. Reuse the canonical verifier, CPU reference, specialization, signed-i32 address proof, and sole WGSL lowerer; extend only exact six-axis mixed-radix coordinate reconstruction and rectangular launch lowering. Keep rank 7+, signed destinations, packed storage, overlap, and non-global memory refused. | Rank 6 is broader layout meaning, not a source spelling. New profile identities expose it without silently widening older consumers or creating a rank-shaped executor. Required real-device evidence remains separate from semantic admission. |
| D-202 | 2026-07-30 | accepted | Extend only request-time `runtime-u32-rectangular-prefix` to rank 6 as host-graph program version 1.18 while retaining exact 1.0-1.17 decoding and the version-1.17 produced-resource rank-5 limit. Bind six positive bounded controls to the distinct rank-6 view-copy profile. CPU reuses the generic dense-rectangle evaluator. Portable WebGPU maps axes 5/4 to x/y, flattens axes 0 through 3 into z, reconstructs them by guarded mixed radix, and preserves the 32-byte uniform, maximum-prewarmed canonical program, exact pipeline slots, snapshot, budget, device-limit, and fail-stop contracts. | Rank 6 changes only the verified semantic-to-physical schedule. A separately versioned graph contract preserves older hashes and resource limits while deepening the existing graph, CPU, WGSL, and pipeline-authority seams. Produced-resource rank 6, rank 7+, repeated/device-side feedback, transport, and native execution remain separate work. |
| D-203 | 2026-07-30 | accepted | Extend `resource-u32-rectangular-prefix` to rank 6 as host-graph program version 1.19 while retaining exact 1.0-1.18 decoding and every older rank limit. Bind six distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 extent sources under the graph-wide at-most-one produced-resource feedback-node bound. CPU reads all six private extents after their writers and executes the exact rank-6 dense rectangle. Portable WebGPU prewarms the version-1.18 program and exact slots, submits all producers in one resident-buffer prefix stage, reads and validates six u32 values together, then reuses the same flattened-z geometry, guarded 32-byte uniform, and suffix execution. | Reusing six existing scalar resources and one aggregate feedback stage preserves writer dominance, private storage, exact byte accounting, lifecycle ownership, stable pipeline identity, and fail-stop publication. A packed control ABI, per-axis stages, or separate rank-6 kernel would duplicate established authority. Rank 7+, repeated/device-side feedback, transport, and native execution remain separate work. |
| D-204 | 2026-07-30 | accepted | Add distinct positive- and signed-affine rank-7 word32 view-copy profiles while retaining every rank-1-through-rank-6 profile identity. Reuse the canonical verifier, CPU reference, specialization, signed-i32 address proof, and sole WGSL lowerer; extend exact seven-axis mixed-radix coordinate reconstruction and rectangular launch lowering under the existing eight-u32 dynamic uniform. Keep rank 8+, signed destinations, packed storage, overlap, and non-global memory refused. | Rank 7 broadens layout meaning without changing the operation or storage model. New profile identities expose it without silently widening older consumers or creating a rank-shaped executor. Required real-device evidence remains separate from semantic admission. |
| D-205 | 2026-07-30 | accepted | Extend only request-time `runtime-u32-rectangular-prefix` to rank 7 as host-graph program version 1.20 while retaining exact 1.0-1.19 decoding and the version-1.19 produced-resource rank-6 limit. Bind seven positive bounded controls to the distinct rank-7 view-copy profile. CPU reuses the generic dense-rectangle evaluator. Portable WebGPU maps axes 6/5 to x/y, flattens axes 0 through 4 into z, and reconstructs them through the shared generic mixed-radix lowerer behind the existing 32-byte uniform. | The graph version changes only verified domain admission; the operation, CPU evaluator, WGSL lowerer, uniform ABI, pipeline authority, snapshots, budgets, and failure model stay shared. A separately versioned contract preserves older hashes and prevents request-time support from silently widening produced-resource rank. |
| D-206 | 2026-07-30 | accepted | Extend `resource-u32-rectangular-prefix` to rank 7 as host-graph program version 1.21 while retaining exact 1.0-1.20 decoding and every older rank limit. Bind seven distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 extent sources under the graph-wide at-most-one produced-resource feedback-node bound. CPU reads all seven private extents after their writers. Portable WebGPU submits all producers in one resident-buffer prefix stage, validates seven values together, then reuses the version-1.20 generic flattened-z mapping, 32-byte guard, maximum program, and exact suffix slots. | Reusing the established aggregate feedback stage and generic rank-7 program preserves writer dominance, private storage, exact byte accounting, lifecycle ownership, stable pipeline identity, and fail-stop publication without adding a rank-shaped execution path or packed control ABI. |
| D-207 | 2026-07-30 | accepted | Add `browsergrad.view-copy.positive-affine-rank2-rank3-packed16@1` for exact same-dtype i16/u16/f16/bf16 storage copies. Retain every word32 profile identity and the canonical verifier, CPU evaluator, specialization, index lowering, runtime binding, and evidence path. The semantic profile requires positive-affine source/destination maps, a dense destination, and reject-on-invalid-source. Its current WebGPU backend uses one static invocation per destination u32 word so adjacent halfword writes cannot race, masks the odd final element to preserve its unrelated upper half, and additionally requires a word-aligned destination and word-sized root allocations. Keep signed/dynamic packed maps, fill, arithmetic, conversion, `shader-f16`, 8-bit/f64 storage, overlap, and non-global memory outside the profile. | Raw 16-bit storage preservation is useful dtype breadth but is not f16/bf16 numerical support. A distinct semantic/backend profile prevents silent widening of word32 identities. Pair ownership makes read-modify-write race-free without atomics or a source-shaped kernel, and raw u32 packing keeps the WebGPU core path independent of optional f16 features. |
| D-208 | 2026-07-30 | accepted | Add `browsergrad.view-copy.positive-affine-rank2-rank3-packed8@1` for exact same-dtype bool/i8/u8 storage copies. Retain every word32 and packed16 profile identity plus the shared verifier, CPU evaluator, specialization, index lowering, runtime binding, and evidence path. Generalize the raw-u32 subword schedule so one static invocation owns all four bytes of one destination word, avoiding cross-invocation read-modify-write races, and masks a partial final word to preserve unrelated bytes. Require positive-affine source/destination maps, a dense destination, reject-on-invalid-source, word-aligned destinations, and word-sized root allocations. Keep bool canonicalization, signed/dynamic packed maps, fill, arithmetic, conversion, 64-bit storage, overlap, and non-global memory outside the profile. | Raw 8-bit storage is additional layout and dtype breadth, not value semantics. A separate profile prevents silent widening of packed16 or word32 consumers. Word ownership reuses the same race-free address/lowering seam without atomics, a byte-shaped executor, or optional WebGPU features. |
| D-209 | 2026-07-30 | accepted | Add `browsergrad.view-copy.positive-affine-rank2-rank3-word64@1` for exact same-dtype f64/i64/u64 storage copies. Retain every word32, packed8, and packed16 profile identity plus the shared verifier, CPU evaluator, specialization, index lowering, runtime binding, and evidence path. Generalize the raw byte-address seam while assigning one static invocation to each logical 64-bit destination element; copy its two adjacent u32 words directly so writes are disjoint and require no native 64-bit WGSL type. Require positive-affine source/destination maps, a dense destination, reject-on-invalid-source, aligned views, and word-sized root allocations. Keep signed/dynamic word64 maps, fill, arithmetic, conversion, rank 1 or 4+, overlap, and non-global memory outside the profile. | Raw 64-bit storage preservation is dtype/layout breadth, not f64 or 64-bit integer numerical support. A distinct profile avoids widening word32 or packed identities, and the two-word schedule reuses the canonical address/lowering path without optional GPU features or read-modify-write. |
| D-210 | 2026-07-30 | accepted | Add separately named `browsergrad.view-copy.signed-affine-rank2-rank3-packed8@1`, `browsergrad.view-copy.signed-affine-rank2-rank3-packed16@1`, and `browsergrad.view-copy.signed-affine-rank2-rank3-word64@1` source profiles while retaining every positive-affine and word32 identity. A nonnegative view byte offset must rebase the complete proved signed source range into its root allocation; the destination remains positive affine, dense, and injective. Reuse the canonical verifier, CPU evaluator, specialization, raw byte-address lowerer, width-owned static schedules, runtime binding, and evidence path. Keep signed destinations, dynamic launch, fill, arithmetic, conversion, rank 1 or 4+, overlap, and non-global memory outside these profiles. | Signed source strides are layout breadth, not value semantics. Distinct semantic identities make the widening explicit while the existing interval-proved i32 byte-address seam prevents a second source-shaped executor, unsigned reinterpretation, clamping, or per-dtype lowering. |
| D-211 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-1 packed8, packed16, and word64 profiles while retaining every rank-2/rank-3 and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, and width-owned schedules. Keep rank 4+, dynamic maps, fill, arithmetic/conversion, signed destinations, overlap, and non-global memory outside these profiles. | Rank-1 storage views are the same exact movement semantics over a smaller coordinate domain. Distinct profile identities expose the broader layout contract without silently widening prior consumers or creating a rank-shaped executor. |
| D-212 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-4 packed8, packed16, and word64 profiles while retaining every lower-rank and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, rank-4 coordinate reconstruction, and width-owned schedules. Keep rank 5+, dynamic maps, fill, arithmetic/conversion, signed destinations, overlap, and non-global memory outside these profiles. | Rank-4 exact storage movement needs broader coordinate admission, not a new operation or backend. Distinct identities preserve every previous contract while the shared rank-4 lowerer prevents a storage-width-specific or source-shaped execution path. |
| D-213 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-5 packed8, packed16, and word64 profiles while retaining every lower-rank and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, rank-5 coordinate reconstruction, and width-owned schedules. Keep rank 6+, dynamic maps, fill, arithmetic/conversion, signed destinations, overlap, and non-global memory outside these profiles. | Rank-5 exact storage movement is the same semantic operation over the existing generic coordinate path. Distinct identities preserve prior contracts while preventing a storage-width-specific or source-shaped executor. |
| D-214 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-6 packed8, packed16, and word64 profiles while retaining every lower-rank and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, rank-6 coordinate reconstruction, and width-owned schedules. Keep rank 7+, dynamic maps, fill, arithmetic/conversion, signed destinations, overlap, and non-global memory outside these profiles. | Rank-6 exact storage movement is the same semantic operation over the existing generic coordinate path. Distinct identities preserve prior contracts while preventing a storage-width-specific or source-shaped executor. |
| D-215 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-7 packed8, packed16, and word64 profiles while retaining every lower-rank and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, rank-7 coordinate reconstruction, and width-owned schedules. Keep rank 8+, dynamic maps, fill, arithmetic/conversion, signed destinations, overlap, and non-global memory outside these profiles. | Rank-7 completes the current static exact-storage rank envelope through the same canonical operation. Distinct identities preserve prior contracts while preventing a storage-width-specific or source-shaped executor. |
| D-216 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-8 word32 profiles while retaining every lower-rank and non-word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 address lowerer, and generic static mixed-radix coordinate reconstruction. Keep rank 9+, non-word32 rank 8+, dynamic rank-8 maps, signed destinations, division/modulo, overlap, and non-global memory outside these profiles. | Rank-8 word32 layout breadth extends the same static semantic operation without widening an older identity. The generic coordinate lowerer avoids a rank-shaped or source-shaped executor while leaving host-graph and non-word32 schedules independently bounded. |
| D-217 | 2026-07-30 | accepted | Add separately named positive-affine and signed-affine rank-8 packed8, packed16, and word64 profiles while retaining every lower-rank and word32 identity. Reuse the canonical verifier, CPU evaluator, signed-i32 raw byte-address lowerer, generic rank-8 mixed-radix coordinate reconstruction, and width-owned schedules. Keep rank 9+, dynamic rank-8 maps, fill, arithmetic/conversion, signed destinations, division/modulo, overlap, and non-global memory outside these profiles. | Rank-8 completes the current static exact-storage rank envelope through one canonical operation and generic coordinate path. Distinct identities preserve prior contracts while preventing a storage-width-specific, rank-shaped, or source-shaped executor. |
| D-218 | 2026-07-30 | accepted | Extend only request-time `runtime-u32-rectangular-prefix` to rank 8 as host-graph program version 1.22 while retaining exact 1.0-1.21 decoding and the version-1.21 produced-resource rank-7 limit. Bind eight positive bounded controls to the distinct rank-8 view-copy profile. CPU reuses the generic dense-rectangle evaluator. Portable WebGPU maps axes 7/6 to x/y, flattens axes 0 through 5 into z, and reconstructs them through the shared generic mixed-radix lowerer behind the existing 32-byte eight-u32 uniform. | The existing control ABI already carries exactly eight extents, so rank 8 changes verified domain admission without changing the operation, uniform layout, CPU evaluator, WGSL lowerer, pipeline authority, snapshots, budgets, or failure model. A separately versioned contract preserves older hashes and prevents request-time support from silently widening produced-resource rank. |
| D-219 | 2026-07-30 | accepted | Extend `resource-u32-rectangular-prefix` to rank 8 as host-graph program version 1.23 while retaining exact 1.0-1.22 decoding. Bind eight distinct ordered rank-local, aligned, zero-filled four-byte temporary u32 extent sources under the graph-wide at-most-one produced-resource feedback-node bound. CPU reads all eight private extents after their writers. Portable WebGPU submits all producers in one resident-buffer prefix stage, validates eight values together, then reuses the version-1.22 generic flattened-z mapping, 32-byte guard, maximum program, and exact suffix slots. | Reusing the established aggregate feedback stage and generic rank-8 program preserves writer dominance, private storage, exact byte accounting, lifecycle ownership, stable pipeline identity, and fail-stop publication without adding a rank-shaped execution path or packed control ABI. |
| D-220 | 2026-07-30 | accepted | Add exact shared-count resource launch fanout as host-graph program version 1.24 while retaining exact 1.0-1.23 decoding and their graph-wide one-feedback-node limit. Admit exactly two top-level `resource-u32-prefix-elements` dispatches only when both name the same ordered temporary-u32 source, rank, mode, and artifact maximum; forbid conditional/repeat combinations, rectangular sources, different bounds, and a third node. CPU reads the source for each semantic dispatch from private state. Portable WebGPU reads it once after one resident-buffer prefix stage, validates it against the shared maximum, substitutes both exact prepared dispatch slots, and executes the suffix over resident buffers. | This is one produced selection with two independently bound semantic consumers, not repeated or device-side feedback. One aggregate stage preserves exact work, source dominance, pipeline identity, snapshot/failure behavior, and one synchronization point without adding a callback, recompilation path, or packed control ABI. |
| D-221 | 2026-07-30 | accepted | Extend exact produced-selection fanout to `resource-u32-rectangular-prefix` as host-graph program version 1.25 while retaining exact 1.0-1.24 decoding and every older feedback limit. Admit exactly two top-level rectangular resource dispatches only when both have the same rank, mode, ordered axis/resource/rank/mode source tuples, and artifact maxima; forbid linear/rectangular mixing, conditional/repeat combinations, mismatched sources or bounds, and a third node. CPU reads each source from private state for both semantic consumers. Portable WebGPU reads the ordered extent set once after one resident-buffer prefix stage, validates one shared rectangle, substitutes both exact prepared dispatch slots, and executes the suffix over resident buffers. | A shared produced rectangle is the rectangular form of the version-1.24 one-selection/two-consumer contract, not repeated or device-side feedback. Reusing one aggregate stage and the generic rank-8 mapping preserves source dominance, exact work and byte accounting, stable pipeline identity, snapshot/failure behavior, and one synchronization point without a callback, recompilation path, packed control ABI, or rank-shaped executor. |
...
| D-228 | 2026-07-30 | accepted | Extend required one-shot Worker evidence with signed rank-8 i8 and f64 semantic dispatches, and share one bounded fixture constructor plus exact graph builder between the in-realm and Worker lanes. Keep the transport protocol and backend version unchanged. | Raw-u8 and identity-f32 proved realm transfer and device ownership but not that canonical signed packed and two-word layouts survive byte export, realm-local re-verification, preparation, and execution. Shared fixture construction removes a duplicated hand-built graph path while two storage extremes add this evidence without implying a worker mesh or every dtype/layout cross-product. |
| D-229 | 2026-07-30 | accepted | Replace the Worker lane's ad hoc success-only log with one validated `browsergrad.execution-evidence@1` terminal. Bind the exact four-case manifest, producer versions, explicit bit-exact comparison policy, browser/Worker-device environment identity, per-case artifact/output/device digests, one-shot transport invariants, partial failure prefix, stage, and diagnostics. | A required test result without a closed terminal record is difficult to audit and can lose failure context. Reusing the shared terminal contract aligns the Worker lane with the other WebGPU gates, while a Worker-specific validator prevents a generic schema-valid record from claiming success with missing, reordered, cross-device, or transport-incomplete cases. |
| D-230 | 2026-07-30 | accepted | Extend the exact sequential produced-resource profile to four stages as host-graph program version 1.28 while retaining exact 1.0-1.27 decoding and every older feedback limit. Admit exactly four top-level `resource-u32-prefix-elements` dispatches with distinct source resources only when three semantic producer relations form one connected root-to-leaf chain; reject disconnected degree-equivalent cycles, missing, forked, cyclic, extra, duplicate-source, mixed-mode, and pre-version profiles. CPU and portable WebGPU reuse the generic staged executors under the same resident-buffer and fail-stop lifecycle. | A separately bounded fourth stage tests that the canonical verifier and executors scale by semantic graph shape rather than another hard-coded source spelling. Exact connectivity, versioned ceilings, prewarmed slots, and explicit stage evidence deepen the existing contract without claiming arbitrary graphs, callbacks, device-side loops, or an unbounded feedback cycle. |
| D-231 | 2026-07-30 | accepted | Add one exact aggregate mixed-feedback profile as host-graph program version 1.29 while retaining exact 1.0-1.28 decoding and every older feedback limit. Admit exactly one `resource-u32-branch-sequential` conditional and one `resource-u32-count-sequential` repeat only when both name the same temporary-u32 source and rank. Execute the ordered producer, read the four-byte value once, then select both prewarmed consumers under one resident-buffer and fail-stop lifecycle. Reject different sources/ranks, a third consumer, pre-version use, and every other mixed profile. | Shared selection across two distinct semantic consumers is the smallest honest mixed-feedback capability. One exact aggregate stage avoids redundant readback and proves conditional/repeat composition without claiming sequential mixed feedback, arbitrary control graphs, callbacks, recompilation, or device-side control flow. |
| D-232 | 2026-07-30 | accepted | Add one exact sequential mixed-feedback profile as host-graph program version 1.30 while retaining exact 1.0-1.29 decoding and every older feedback limit. Admit exactly one produced-resource conditional followed by one produced-resource bounded repeat when their sources are distinct and both conditional branches guarantee-write the repeat-count resource. Execute the predicate producer, read and select one prewarmed branch, execute that branch, read its distinct count, and select the prewarmed repeat under one resident-buffer and fail-stop lifecycle. Reject a missing branch write, shared sources under this profile, a third consumer, pre-version use, and every other mixed profile. | Branch-produced work selection is the smallest honest sequential conditional/repeat capability. Guaranteed writes and existing dependency/effect checks make the second read semantically available, while two explicit stages expose synchronization cost without claiming arbitrary control graphs, callbacks, recompilation, or device-side flow. |
| D-233 | 2026-07-30 | accepted | Add the exact sequential conditional-to-linear-dispatch profile as host-graph program version 1.31 while retaining exact 1.0-1.30 decoding and every older feedback limit. Admit one produced-resource conditional followed by one `resource-u32-prefix-elements` dispatch when their sources are distinct and both branches guarantee-write the positive bounded launch-count resource. Reuse one conditional-prefix selector for standalone and sequential consumers; then read, validate, and specialize the maximum-prewarmed dispatch under one resident-buffer and fail-stop lifecycle. Reject rectangular launch, missing branch writes, shared sources, a third consumer, pre-version use, and every other mixed profile. | This deepens the same branch-produced selection seam without another source-shaped executor. Shared prefix selection, exact effect proof, two explicit stages, private resident buffers, and prewarmed slots preserve correctness and expose synchronization cost without claiming arbitrary geometry, callbacks, recompilation, or device-side flow. |
...
| D-238 | 2026-07-30 | accepted | Require exact separation of duties at the final production-release transition. The distribution reviewer identity must differ from the build-producer identity, and the reviewer P-256 public-key hash must differ from the producer key. Bind both successful checks into the version-2 release-authority identity and host observation. Keep external control and real organizational independence outside the software claim. | Two valid signatures under different statement types do not prove independent approval when one principal or key can occupy both roles. Enforcing the exact coordinates at the deepest composition seam prevents accidental or malicious self-approval without overstating what string and key inequality can establish. |
...
### 2026-07-26 — Exact browser-payload CPU/WebGPU convergence

- Commit `8d7f27eb` adds the bounded exact-distribution convergence lane. Host
  preflight rehashes the package-pinned 25-file private root, exact-decodes the
  profile, manifest, build lock, local producer policy/trust store, and DSSE
  envelope, independently re-verifies the local producer, and serves only the
  admitted bytes through no-follow, no-store routes. Host paths and all signing
  material stay outside the browser input.
- The first real execution reached the shared authorization boundary and
  exposed a latent identity defect: the browser compile profile named the Wasm
  binary label as its logical extractor, while native Artifact V3 correctly
  named `browsergrad-tools/cpp-cute-frontend`. Every real browser artifact was
  therefore impossible to authorize. The profile now uses the logical
  extractor identity and keeps Wasm binary/build identities separate.
- That correction invalidated the zero-import compiler Worker and all dependent
  identities. The Worker was deterministically regenerated at SHA-256
  `9c9591e725fca512d10a366bdec38b0067366f3d8ebdef50c29a5ebb0134def5`
  and 584,894 bytes. A fresh strict eight-case browser matrix passed and was
  repinned as the 26,213-byte resource
  `38fcfae4d0b9c11314ec90a50f3bbb17b34c46a2c407ca475b9f8b44311833e1`
  at source `8d7f27eb9a249d8277def3b401377c42e961b6c7`.
- Two fresh exact roots were independently materialized and finalized. Each has
  25 files and 103,637,695 bytes. Their 24 deterministic subjects are
  byte-identical; both detached envelopes bind build subject
  `bg.cpp.browser-build-subject.sha256.ed6344d3d2ffef5745f92e0ee53d4839e6c6ed7e193a56ca700c61853eb2da98`.
  The package-pinned full-distribution identity is
  `bg.cpp.browser-full-distribution-reproducibility.sha256.64cc7401523b6026aba9430e2f081d708bc62cfcbe6fc343bf58ec0798aeec7b`;
  its 8,951-byte resource is
  `285a73cc67ec4196104389f127f6728e531aa41c44fba8a79d61e89533f2b76e`.
- Headed Chromium on Apple Metal 3 then ran all eight exact distribution cases
  sequentially. Each case re-fetched and hashed the exact profile and nine
  assets, installed five packs/5,788 files, compiled unchanged C++17/CuTe in
  the exact package Worker, re-verified the local producer in the browser
  module realm, prepared a unique candidate, authorized it through the shared
  seam, lowered canonical layout/kernel artifacts, and matched an independent
  CPU oracle against actual WebGPU over every destination bit while preserving
  nonzero-offset canaries. The matrix identity is
  `bg.cpp.browser-exact-distribution-convergence.sha256.12665a3d1f38689f9439f0aba2a6b4c87f42d28e021c5804a528b294f8f7fe31`.
- The compact 13,224-byte package observation at SHA-256
  `c60b3dd770b3267633cda4ad6fe10995b6a237b83b4e9bd72fb0d1c5796759ea`
  rebinds the current full-distribution authority, exact case/source
  inventory, local producer boundary, unique artifact/candidate/authorization/
  execution lineages, CPU/WebGPU hashes, and one real device profile. Its
  verifier issues opaque copy-resistant local-engineering convergence
  authority only. External producer trust, license review, distribution
  approval, production backend authority, and release readiness remain false.
- Focused package evidence passes six files/28 tests. The current fast gate
  passes 98 files/795 tests in 20.14 seconds. Complete compiler verification
  passes root and compiler architecture checks, strict typecheck/lint/build,
  Docker-shell 2 files/96 tests, native/build-plan 55 files/289 tests with nine
  explicit skips in 67.05 seconds, and the compiler suite at 104 files/1,649
  tests in 28.17 seconds. Both architecture scans report zero cycles or leaks.

### 2026-07-27 — Distinct production backend composition

- One new opaque backend transition accepts only verifier-issued producer,
  complete-distribution reproducibility, and exact-payload convergence
  authorities. Structural copies, hostile options, and cancellation fail
  before authority issuance.
- The first focused run correctly rejected the older generic producer fixture:
  its synthetic asset graph has a different build subject. The exact-payload
  fixture now reconstructs the current nine-asset payload under an independent
  test trust root and proves that trust-root migration preserves the
  cycle-free payload build subject while changing producer profile, manifest,
  and asset-set identities.
- Composition therefore binds build subject, build lock, Worker, exact output
  set, and the eight-case CPU/required-WebGPU matrix. It records producer and
  execution control-plane identities separately and grants backend execution
  only. License review, distribution authorization, and release readiness stay
  false.
- The focused transition and adjacent producer/reproducibility/convergence
  tests pass four files/12 tests. This cryptographic fixture is not a
  package-controlled production policy, external builder key, or current
  production authority.
- The fast browser-build gate passes 99 files/798 tests in 18.63 seconds, and
  the complete compiler suite passes 105 files/1,652 tests in 16.76 seconds.
  Strict typecheck, lint, build, compiler architecture, and root semantic
  architecture also pass with zero cycles or leaks.

### 2026-07-27 — Distinct final release composition

- A second opaque transition accepts only the production backend authority and
  independently verified distribution approval. It cross-binds the current
  build lock plus the exact header-subset reproducibility and output-verification
  identities retained by the complete distribution.
- The release result retains both prerequisite authorities and their producer,
  backend, reviewer, approval, reproducibility, convergence, and build-subject
  identities. Only this final transition can set `releaseReady: true`; copied
  prerequisites/results, hostile options, and cancellation fail closed.
- Focused backend/release verification passes two files/six tests. Both signer
  and reviewer use ephemeral cryptographic fixtures, so current production
  trust, approval, backend issuance, and release status remain false.
- The fast browser-build gate passes 100 files/801 tests in 15.97 seconds, and
  the complete compiler suite passes 106 files/1,655 tests in 16.37 seconds.
  Strict typecheck, lint, build, and both architecture checks pass.
- The existing exchange operations intentionally discard opaque authority at
  process exit. A unified host operation must still verify both returned
  statements and invoke both compositions before serialized observations can
  be excluded from the actual release decision.

### 2026-07-27 — Unified in-process production release

- The existing hardened exchange now owns `verify-production-release`. It
  accepts twelve distinct immutable producer/reviewer inputs, rederives both
  exact requests, re-verifies both returned DSSE envelopes, admits the
  package-pinned complete-distribution and exact-convergence resources, and
  invokes backend plus final-release composition in one process.
- Its exclusive read-only output records the in-process release identity and
  successful transition facts, but explicitly serializes no reusable
  producer, approval, backend, or final-release authority and keeps
  `releaseReady: false`. Private keys remain outside the interface.
- Focused exchange verification passes one file/10 tests. The fast browser
  build gate passes 100 files/802 tests with a 16.52-second Vitest phase, and
  the complete compiler suite passes 106 files/1,655 tests in 21.89 seconds.
  Strict typecheck, focused lint, build, compiler architecture, and root
  semantic architecture pass with zero cycles or leaks.
- Running the clean-build fast gate concurrently with a raw full-suite process
  exposed one known harness constraint: the build lane deletes shared `dist`
  while rebuilding. The isolated full-suite rerun passed. Independent lanes
  may run concurrently only when they do not mutate the same package output,
  or when they use separate worktrees/output roots.
- The fixture signer and reviewer are ephemeral and test-only. Gate 3 remains
  `in-progress` solely because package-controlled production policies,
...
### 2026-07-27 — Gate 7 authority-bound CPU reference

- Semantic-core adds `browsergrad.host-graph.cpu-reference@1`. Preparation
  requires exact verifier-issued graph authority and exact opaque
  kernel/layout artifacts, rebuilds each view-copy specialization under a
  whole-graph preparation deadline, and retains no caller-declared effects or
  resource geometry.
- Execution accepts only complete rank-local input bindings. It rejects
  accessors, sparse/named arrays, duplicates, wrong ranks/resources/lengths,
  misalignment, typed-array subclasses, shared memory, and copied authority.
  Inputs are snapshotted before execution; temporaries and outputs are private;
  no caller destination exists; result buffers become observable only after
  the complete graph succeeds.
- The CPU profile executes dispatches in verified topological order and
  supports f32/i32/u32 all-reduce. F32 is finite-only, reduces in ascending
  participant rank, and rounds after each sum. Signed/unsigned sums wrap at 32
  bits; integer min/max is exact. Non-finite f32 inputs or overflow fail before
  output commit.
- Working storage includes all rank-local resources plus the largest
  collective scratch buffer. Separate ceilings bound total element
  operations, preparation time, execution time, and native cancellation.
- Seven focused CPU tests pass alongside the nine graph-verifier tests.
  Semantic-core passes 23 files/192 tests, strict typecheck, lint, and build.
  The packed fresh-consumer gate imports the public CPU API and executes the
  compiler-owned two-dispatch graph exactly.
- This is deterministic CPU reference evidence only. It grants no WebGPU
  execution, transport/topology, worker-mesh, pipeline, performance, or
  release authority.

### 2026-07-27 — Gate 7 deterministic resource initialization

- Host-graph resources now close their initial byte state: inputs require
  `external-input`; temporaries and outputs require `zero-fill`.
- The verifier rejects missing, unknown, or role-incompatible initialization.
  Compiler construction derives it and neither CPU nor future WebGPU callers
  can restate it.
- This correction makes partial-view and zero-element behavior deterministic
  before a second executor exists. It adds no backend or execution claim.

### 2026-07-27 — Gate 7 portable WebGPU static-DAG execution

- Kernels adds the explicit `/semantic_host_graph` entry point and
  `browsergrad.host-graph.webgpu@1` backend. Preparation accepts only exact
  verifier-issued graph authority plus the exact opaque kernel/layout
  artifacts named by that graph. The root architecture guard now permits
  kernels to consume semantic-core `/graph` while continuing to reject the
  semantic-core root, invented host subpaths, compiler imports, and all
  implementation-path deep imports.
- Each verified view-copy node expands through the existing canonical WGSL
  lowerer once per rank. Each all-reduce expands in canonical participant order
  into pairwise accumulator/operand steps followed by raw-u32 replication.
  This supports arbitrary admitted rank counts with at most three storage
  bindings per step instead of requiring a device binding for every rank.
- Execution snapshots every complete direct/unshared input before its first
  await, creates private rank/resource storage, applies the graph's exact
  external-input or zero-fill initialization, and publishes only fresh output
  copies after every dispatch and readback succeeds. Expanded steps, generated
  WGSL, preparation/execution time, complete private host/GPU storage, device
  allocation/binding/workgroup/dispatch limits, and one active graph per device
  are bounded before submission.
- F32 collective WGSL checks operands and each result through exact exponent
  bits and atomically records non-finite input or overflow in a separately read
  status word. Rank-ordered sum matches stepwise f32 rounding; min/max selects
  signed zero exactly like the CPU oracle. Signed sum wraps through explicit
  u32 bitcasts; unsigned sum and integer min/max remain exact. Reduction and
  replication preserve the declared participant order without host callbacks.
- The shared WebGPU diagnostic harness now supports promise-returning issue
  phases while popping all validation/OOM/internal scopes synchronously before
  the first await, races scope/operation settlement against device loss, and
  destroys late values after loss. Generic WGSL sequence preparation creates
  every resource, shader/pipeline request, and bind group for all steps before
  that first await, so later steps cannot escape diagnostic ownership. Graph
  timeout/cancellation suppresses stale results while the owned background
  operation completes cleanup; device loss invalidates both generic WGSL and
  kernel-device caches.
- Focused Node verification covers adversarial authority, accessor,
  sparse/duplicate/shared/misaligned input, revoked proxies, resource,
  cancellation, WGSL numerical-policy, and error-scope cases. Strict kernels
  and browser typechecks, build, lint, 21 files/174 tests, root architecture,
  and the 30-test compiler architecture suite pass. The focused graph/WGSL,
  sequence-issue, diagnostic-scope, and publish-gate slice passes five
  files/42 tests. The
  release gate passes 19 archive and 35 Node security tests plus packed and
  fresh-consumer checks.
- Required headed Chromium 148 on Apple Metal 3 passes four complete-output,
  bit-exact CPU/WebGPU cases: rank-ordered finite f32 sum, signed-zero f32 min,
  wrapping i32 sum, and exact u32 max. A fifth fail-stop check rejects a
  non-finite f32 collective; a fresh device is then destroyed and the adapter
  refuses subsequent execution as device loss. The terminal artifact hash is
  `b6b6bd3c137ec74fff89788e885602514ba43f41714b40efc6d36d372e24c2cd`;
  the device profile hash is
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- This closes portable execution for the current static graph profile only.
  Gate 7 remains `in progress`: readback/event/materialization semantic nodes,
  bounded conditionals/repetition, pipeline authority, transport/topology,
  worker meshes, native companions, and graph performance evidence remain
  open.

### 2026-07-27 — Gate 7 whole-allocation per-rank copy nodes

- Host-graph program version 1.1 additively introduces one `copy` node with
  `whole-allocation-bytes-per-rank` meaning. Artifact envelope and program
  minors must match; version 1.0 graphs retain their exact construction and
  decoding path, and copy under 1.0 fails as an unsupported profile.
- Verification requires distinct source/destination resources with identical
  dtype and allocation byte length. It derives a source read plus destination
  write and applies the existing input-immutability, read-before-write,
  dependency, and unordered-conflict checks. The node cannot smuggle effects,
  reinterpret dtype, self-copy, or select a partial range.
- The CPU reference copies all allocation bytes for every rank in private
  storage and charges each copied byte to the existing operation ceiling.
  Focused evidence preserves odd-sized u8 allocations exactly, snapshots
  caller inputs, requires no fake kernel/layout artifacts, and rejects an
  operation budget one byte below the complete graph requirement.
- Kernels backend version 1.1.0 expands each admitted copy to one existing
  raw-u32 replication program per rank. This preserves semantic dtype bits,
  including u8, without a new source-shaped kernel path. Whole-word allocation
  size is the portable-profile boundary; odd-sized copies fail explicitly
  before device access while the CPU oracle remains more general.
- Required headed Chromium 148 on Apple Metal 3 adds an exact complete-output
  u8 copy after caller mutation to the prior four collective cases. The
  five-case terminal artifact hash is
  `7055352309af1fe00fd49877a75c9b6df7ce37edb385104af305586c36ec31c6`;
  non-finite f32 and actual lost-device refusals remain required.
- Semantic-core passes 23 files/195 tests and kernels passes 21 files/175
  tests with strict typecheck/lint/build. Packed and fresh-consumer gates
  construct, CPU-execute, and WebGPU-prepare the version-1.1 raw copy directly
  from the public graph and kernels subpaths.
- Gate 7 remains `in progress`: readback/event/materialization nodes, bounded
  conditionals/repetition, pipeline authority, transport/topology, worker
  meshes, native companions, and graph performance evidence remain open.

### 2026-07-28 — Gate 7 explicit terminal materialization

- Host-graph program version 1.2 introduces
  `host-readback-after-graph-success` materialization while preserving exact
  version-1.0/1.1 decoding and construction identity. Every declared output
  requires exactly one materialization; older versions retain their implicit
  output behavior.
- Verification derives a read effect for the selected output and therefore
  requires an ordered final writer. Materialization cannot expose an input or
  temporary resource, cannot be duplicated or forged under an older minor,
  and is terminal: no later node may depend on it. This makes readback
  selection explicit without permitting partial publication or hidden
  snapshot work.
- The CPU reference includes the materialization in its executed-node trace,
  charges zero element operations for it, and publishes only the verifier's
  selected resources after every graph node succeeds.
- Kernels backend version 1.2.0 uses the same verified output selection for
  final asynchronous readback. Materialization adds no GPU step or allocation,
  and preparation fails internally if a nonempty selected output has no
  complete portable producer.
- Required headed Chromium 148 on Apple Metal 3 crosses version-1.2
  materialization in the exact u8 case and preserves the prior four collective
  cases, non-finite refusal, and actual lost-device refusal. The five-case
  terminal artifact hash is
  `f02d04572b4556bdf095a16d38fe4c18bcceae073dffa73152027322bc3633d7`;
  the device profile hash remains
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Semantic-core passes 23 files/198 tests and kernels passes 21 files/176
  tests. Packed and fresh-consumer gates construct, CPU-execute, and
  WebGPU-prepare explicit version-1.2 materialization through public subpaths.
- Gate 7 remains `in progress`: event nodes, bounded
  conditionals/repetition, pipeline authority, transport/topology, worker
  meshes, native companions, and graph performance evidence remain open.

### 2026-07-28 — Gate 7 dependency-ordered completion events

- Host-graph program version 1.3 introduces unique
  `completion-after-dependencies` events while preserving exact version
  1.0–1.2 decoding. Event IDs use the closed graph identifier grammar and are
  canonicalized independently from node IDs.
- Events have no resource effect, timestamp, queue identity, external wait, or
  transport authority. They are zero-work dependency milestones: later nodes
  may depend on them, and their completion order is the verified topological
  order.
- The CPU reference includes event nodes in the executed-node trace, charges
  zero operations, and returns completed event IDs only with the successful
  whole-graph result. A later failure therefore cannot publish an earlier
  milestone.
- Kernels backend version 1.3.0 preserves the same event order in the terminal
  WebGPU trace without adding a command, allocation, shader, or pipeline.
  Stronger timestamp, queue-fence, external-wait, and cross-worker event
  profiles remain explicitly unsupported.
- Required headed Chromium 148 on Apple Metal 3 crosses the
  `copy-complete` event before terminal u8 materialization and preserves the
  prior collective, non-finite, and device-loss evidence. The five-case
  terminal artifact hash is
  `3c745e9847bea82221b36bb66849596b844193cf8889c6a2d923aaf0101dc58a`;
  the device profile hash remains
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Semantic-core passes 23 files/201 tests and kernels passes 21 files/177
  tests. Packed and fresh-consumer gates preserve the version-1.3 event through
  construction, CPU execution, and WebGPU preparation.
- Gate 7 remains `in progress`: bounded conditionals/repetition, pipeline
  authority, transport/topology, worker meshes, native companions, graph
  performance, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 bounded fixed-count repetition

- Host-graph program version 1.4 introduces
  `fixed-count-sequential` repetition while preserving exact version 1.0–1.3
  decoding. Each node has a positive artifact-fixed count and a nonempty
  linear body containing only dispatch, all-reduce, and whole-allocation copy.
  Nested control, events, materialization, runtime counts, and conditionals
  fail closed.
- Verification requires top-level and repeat-body node IDs to be globally
  unique, aggregates the body's canonical resource effects without inventing
  caller-declared access, and caps the body at 64 nodes, count at 1,024, and
  graph expansion at 16,384 executable/terminal nodes. Read-before-write,
  input immutability, and unordered-hazard checks remain on the canonical graph
  seam.
- The CPU reference admits expanded element/copy work before execution, checks
  cancellation/time at every iteration and body node, and reports the repeat
  record only with successful whole-graph publication. Three repeated rank-2
  f32 sums transform rank-local `[1,2]` and `[3,4]` into `[16,24]`.
- Kernels backend version 1.4.0 lowers each body definition once through the
  existing dispatch, copy, collective-reduction, and replication lowerers,
  then reuses frozen step templates for the exact iteration count. It checks
  preparation cancellation/time and the step ceiling after every body node; it
  grants no runtime branch, loop-count, or dynamic-launch authority.
- Required headed Chromium 148 on Apple Metal 3 bit-matches the CPU oracle for
  the new three-iteration f32 case and preserves all five previous cases,
  non-finite refusal, and actual device-loss refusal. The six-case terminal
  artifact hash is
  `aeecac2cf5c8b369e656e1470af074273ffcf4cf2533f2c5f85b12b916eec263`;
  the device profile hash remains
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Semantic-core passes 23 files/204 tests and kernels passes 21 files/178
  tests. Packed and fresh-consumer gates preserve version-1.4 repetition
  through construction, CPU execution, and WebGPU preparation.
- Gate 7 remains `in progress`: bounded conditionals/runtime-derived control,
  pipeline authority, transport/topology, worker meshes, native companions,
  graph performance, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 bounded captured-input conditional

- Host-graph program version 1.5 introduces
  `input-u32-branch-sequential` while preserving exact version 1.0–1.4
  decoding. The predicate is one fixed-rank, aligned four-byte external-input
  u32; zero selects else and nonzero selects then. Both branches are nonempty
  bounded linear dispatch/all-reduce/copy sequences with equal node-kind
  structure. Nested control, events, materialization, GPU/backend-derived
  predicates, and mid-graph feedback fail closed.
- Verification keeps top-level and both branch-body IDs in one namespace,
  merges every possible read/write for dependency hazards, and treats a write
  as guaranteed only when both branches write. A conditional branch can
  therefore never become an implicit writer for a later read or terminal
  materialization.
- The CPU reference requires equal branch element-operation counts, charges
  exactly one branch before execution, selects only from its private predicate
  snapshot, checks cancellation/time at each selected body node, and reports
  the selected branch/body IDs only with successful whole-graph publication.
- Kernels backend version 1.5.0 pre-lowers both branches through the existing
  dispatch, copy, collective-reduction, and replication paths. Equal lowered
  execution shapes preserve exact public counts; preparation conservatively
  binds the union of branch resources/modules, and execution selects from the
  captured input before any device work. The specialization hash and terminal
  trace bind the selected branch.
- Required headed Chromium 148 on Apple Metal 3 bit-matches the CPU oracle for
  both then and else u8-copy branches after caller mutation, preserves the six
  earlier cases, and retains non-finite plus actual device-loss refusal. The
  eight-case terminal artifact hash is
  `e9cef83e998e36e1e3a07cff1c7f815de33b4f6b323c686f26ca99c6612414a7`;
  the device profile hash remains
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Semantic-core passes 23 files/208 tests and kernels passes 21 files/179
  tests. Packed and fresh-consumer gates preserve version-1.5 conditional,
  repetition, event, and materialization meaning through construction, CPU
  execution, and WebGPU preparation.
- Gate 7 remains `in progress`: runtime/backend-derived control, pipeline
  authority, transport/topology, worker meshes, native companions, graph
  performance, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 separate host-graph performance evidence

- Added a dedicated required headed-browser performance suite that compares the
  version-1.4 fixed-repeat graph with an equal-work version-1.2 unrolled graph.
  Both execute one whole-allocation copy, three rank-ordered f32 all-reduces,
  and one terminal materialization over two ranks and 65,536 elements, lowering
  to the same eight WebGPU steps.
- Fixture preparation proves equal CPU element-operation counts and bit-exact
  CPU outputs before device acquisition. An untimed WebGPU preflight then
  bit-matches both paths to that CPU result before any timing sample is
  accepted.
- The measurement wraps the complete production host API through output
  readback and queue drain, with eight warmups and twelve alternating paired
  `performance.now` samples per implementation. The terminal record retains
  raw samples, exact lifecycle/workload, prepared graph and WGSL identities,
  producer versions, browser/device profile, and uncaptured GPU errors.
- Required Chromium 148 on Apple Metal 3 observed candidate median/p95
  1.90/2.80 ms and baseline median/p95 2.60/4.30 ms. The record intentionally
  asserts no superiority or regression threshold. Terminal artifact
  `a6660fc3a64390d2b753f8e9b1b2473e317dc2bfe1a64a70df185e8e0c799aa2`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- CI isolates correctness and performance in independent jobs. Release and
  publish workflows retain the performance log by exact commit and staging
  refuses a missing or stale host-graph performance evidence marker.
- Gate 7 remains `in progress`: runtime/backend-derived control, pipeline
  authority, transport/topology, worker meshes, native companions, and
  stronger event profiles remain open.

### 2026-07-28 — Gate 7 bounded request-time runtime control

- Host-graph program version 1.6 introduces
  `runtime-u32-branch-sequential` while preserving exact version 1.0–1.5
  decoding. Its predicate names one bounded host execution control; zero
  selects else and any nonzero admitted `u32` selects then.
- The semantic verifier reuses version-1.5's nonempty equal-kind linear
  branches, conservative union effects, guaranteed-write intersection, exact
  one-branch work, and global node-ID namespace. It separately limits unique
  control IDs and keeps controls outside the graph resource model.
- CPU and portable WebGPU execution require exactly the graph's unique control
  set, reject missing, duplicate, unknown, and greater-than-u32 values, and
  admit controls before copying inputs. Both capture the admitted set with
  caller inputs before their first await or device access, reuse the existing
  verified branch paths, and publish selection evidence only after whole-graph
  success.
- Required Chromium 148 on Apple Metal 3 adds then and else runtime-control
  u8-copy cases after caller control mutation while preserving the prior eight
  complete-output cases, non-finite refusal, and actual device-loss refusal.
  The ten-case terminal artifact is
  `15bf0e5ab5d766d6fe33b61adfc241e1623ae0d1e9c36c3381554a3c3b52807a`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Semantic-core passes 23 files/212 tests and kernels passes 21 files/181
  tests. Packed and fresh-consumer gates preserve version-1.6 construction,
  CPU execution, control discovery, and WebGPU preparation.
- The separate required performance lane was refreshed under WebGPU backend
  1.6.0 on the same device profile: fixed-repeat median/p95 1.60/2.80 ms and
  unrolled median/p95 1.90/3.90 ms, with no superiority claim. Its prepared
  artifact remains
  `a6660fc3a64390d2b753f8e9b1b2473e317dc2bfe1a64a70df185e8e0c799aa2`.
- The broader compiler verifier exposed unrelated local toolchain drift rather
  than a Gate 7 failure. Homebrew Clang 22's default config names an absent
  macOS 26 SDK; with the available Xcode 14.5 SDK injected explicitly, all 13
  native files pass in 95.46 seconds. The later surface lane then correctly
  refuses the current Darwin `bsdtar` because it differs from the reviewed
  builder identity. No SDK or archive-builder identity was silently repinned.
- Gate 7 remains `in progress`: pipeline authority, GPU/backend-derived or
  mid-graph control, runtime loop counts/launches, transport/topology, worker
  meshes, native companions, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 device-bound pipeline authority

- Added the generic opaque `browsergrad.wgsl.pipeline-set@1` contract. It
  prewarms every unique program admitted at each exact sequence slot, supports
  explicit per-slot alternatives, reuses the resulting pipelines across
  request-specific bind groups, and rejects structural copies, destroyed
  authority, cross-device use, reordering, or unadmitted programs.
- Added `browsergrad.host-graph.webgpu-pipeline@1` as the graph-owned L5
  authority. It binds the exact prepared graph and WebGPU backend to one
  admitted `GPUDevice`, prewarms both sides of every bounded conditional, and
  includes graph, WGSL-module, workgroup-schedule, negotiated-feature,
  relevant-limit, and explicit numerical-policy facts in both its
  `pipelineIdentityHash` and low-level cache namespace. A caller may lower the
  maximum below the fixed 128-pipeline portable ceiling; overflow fails before
  shader or request-resource creation.
- `runSemanticHostGraphWebGpuPipeline` reuses an explicit hot authority while
  allocating fresh private request buffers. The existing
  `runSemanticHostGraphWebGpu` convenience API now captures caller inputs and
  runtime controls synchronously, then creates and destroys an ephemeral
  authority through the same execution implementation; no second graph path
  was added. Device loss and explicit destruction invalidate further use.
- Required Chromium 148 on Apple Metal 3 passes all ten complete-output
  CPU/WebGPU cases through explicit pipeline authority. Then/else pairs share
  request-independent pipeline identity while retaining distinct
  selected-branch specialization identities. Non-finite refusal, actual
  device loss, copied authority, destroyed authority, generic cross-device
  use, and generic program mismatch are separately exercised. The correctness
  artifact remains
  `15bf0e5ab5d766d6fe33b61adfc241e1623ae0d1e9c36c3381554a3c3b52807a`;
  the backend version is 1.7.0 and the device profile remains
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- The separate required performance lane now prewarms both authorities outside
  measurement and records their identities. Its two-rank, 65,536-element f32
  fixed-repeat/unrolled observation reports median/p95 1.60/1.90 ms and
  1.70/2.60 ms respectively, with no superiority claim. Terminal artifact
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`.
- Pipeline authority is no longer a Gate 7 gap. GPU/backend-derived or
  mid-graph control, runtime loop counts/launches, transport/topology, worker
  meshes, native companions, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 produced-resource conditional

- Added `resource-u32-branch-sequential` as host-graph version 1.7. One
  conditional may read an exact aligned four-byte rank-local temporary u32
  only after a dominating ordered writer. External-input/output predicates,
  unordered or read-before-write use, a second resource conditional, nested
  control, dynamic launch, and runtime loop counts fail closed.
- The CPU oracle reads the predicate from private graph storage after producer
  execution. The portable WebGPU adapter allocates and uploads private graph
  buffers once, runs the exact prefix over resident buffers, reads back only
  the four-byte predicate, and resumes the selected already-prewarmed branch
  plus suffix over those same buffers. One timeout, cancellation signal,
  per-device owner, device-loss path, cleanup path, numerical-status check, and
  terminal publication contract spans both submissions.
- Generic `browsergrad.wgsl.pipeline-set@1` authority now admits only exact
  contiguous slot ranges at validated offsets in addition to complete
  sequences. Wrong offsets, reordered programs, structural copies,
  cross-device use, destroyed authority, and unadmitted alternatives still
  fail closed.
- Required Chromium 148 on Apple Metal 3 passes twelve complete-output
  CPU/WebGPU parity cases through explicit pipeline authority. Both produced-
  resource branches preserve caller-input snapshot isolation, share one
  request-independent pipeline identity, have distinct specialization
  identities, and report exactly one mid-graph feedback stage. Correctness
  artifact `ddfc889b8af1acbc305f25e33e7b67daa77a991519e37f0c3482898d240c3ff4`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.8.0.
- The separate observational performance lane remains semantically unchanged
  and passed under backend 1.8.0. Its latest fixed-repeat median/p95 is
  1.70/3.10 ms and unrolled median/p95 is 1.60/5.50 ms; artifact
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`,
  with no superiority or regression claim.
- One bounded GPU-produced u32 feedback point is no longer a Gate 7 gap.
  General runtime-derived loop/launch control, nested/device-side branching,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-28 — Gate 7 bounded request-time repetition

- Added `runtime-u32-count-sequential` as host-graph version 1.8. Each node
  names one required request-time `u32-count` control and a positive artifact
  maximum under the existing 1,024-iteration ceiling. Zero through the maximum
  are exact; larger values fail before input copying or device access.
- Semantic verification charges the artifact maximum, preserves the existing
  linear-body/global-ID/effect/hazard rules, and marks runtime-repeat writes
  non-guaranteed because zero iterations perform no write. The CPU oracle
  reserves maximum work, executes the captured count with cancellation/time
  checks at every iteration/body boundary, and publishes the actual completion
  and actual operation total only after whole-graph success.
- Portable WebGPU lowers and prewarms the complete maximum schedule once.
  Generic `browsergrad.wgsl.pipeline-set@1` authority now validates strictly
  increasing exact step-index selections in addition to complete/contiguous
  ranges. Runtime selection can omit only preauthorized repeat slots; duplicate
  or reordered indices, copied/cross-device/destroyed authority, and program
  substitution still fail closed before request storage.
- Required Chromium 148 on Apple Metal 3 passes fourteen complete-output
  CPU/WebGPU parity cases through explicit pipeline authority. Zero and two
  runtime repetitions share one request-independent pipeline identity, retain
  distinct selected-slot specialization identities, and report exact submitted
  steps, collective counts, and repeat completion. Correctness artifact
  `1aa397fa49b314ac2de6dcf55b48fbe1950fe91d27fe2c418e392444df67574e`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.9.0.
- Semantic-core passes 23 files/219 tests; kernels passes 21 files/184 tests;
  generic real-browser WGSL coverage passes 18 tests including gapped exact
  slots and non-increasing-index refusal. The separate performance record
  remains artifact
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  its latest backend-1.9.0 fixed-repeat median/p95 is 1.70/2.70 ms and unrolled
  median/p95 is 1.90/2.80 ms, with no superiority or regression claim.
- Bounded request-time loop counts are no longer a Gate 7 gap. GPU/backend-
  derived loops, dynamic launches, nested/device-side control,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-28 — Gate 7 bounded request-time dynamic dispatch

- Added top-level `dynamic-dispatch` as host-graph version 1.9. Each node binds
  one required request-time `u32-prefix-element-count` and a positive artifact
  maximum within the verified semantic view-copy element domain. Only one
  through that maximum are admitted, before input copying or device access.
- Semantic verification derives the same resource bindings, launch domain,
  effects, dependencies, and hazards as static dispatch, charges maximum work,
  and rejects dynamic dispatch inside repeat or conditional bodies. The CPU
  oracle plans the canonical maximum view copy, executes the exact prefix for
  every rank, and publishes actual completion and rank-multiplied operation
  counts only after whole-graph success.
- Portable WebGPU prewarms the artifact maximum through the existing canonical
  view-copy WGSL program and exact device-bound pipeline slot. The initial
  profile requires one invocation per workgroup so the admitted logical prefix
  is the exact x-workgroup count; wider workgroups fail explicitly. Runtime
  selection changes only launch geometry and binds the actual completion into
  backend specialization and terminal trace evidence.
- The one-lane restriction follows a real-device correctness failure: reducing
  logical invocation count with a 64-lane workgroup still launches a complete
  workgroup, so inactive-but-unmasked lanes executed beyond the selected
  prefix. The implementation does not claim a masked static dispatch as
  dynamic work and does not compile or substitute a program per request.
- Required Chromium 148 on Apple Metal 3 passes sixteen complete-output
  CPU/WebGPU parity cases through explicit pipeline authority. One- and
  two-element f32 dynamic dispatches retain the same graph and pipeline
  identity, distinct backend specializations, exact two-rank completion
  counts, and exact outputs. Correctness artifact
  `5201eacee1bddc2930694656dabe157f716c2e48e6aef4c2e4541a78d3fd1ce6`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.10.0.
- At this checkpoint semantic-core passed 23 files/223 tests and kernels passed
  21 files/186 tests. The separate performance record remained artifact
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  its backend-1.10.0 fixed-repeat median/p95 was 1.70/3.00 ms and unrolled
  median/p95 was 1.60/4.50 ms, with no superiority or regression
  claim.
- Bounded positive request-time one-lane dynamic dispatch is no longer a Gate
  7 gap. GPU/backend-derived loop or launch control, broader dynamic schedules,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 bounded produced-resource repetition

- Added `resource-u32-count-sequential` as host-graph version 1.10 while
  retaining exact 1.0 through 1.9 decoding. One exact rank-local, aligned,
  four-byte, zero-filled temporary u32 with an ordered graph writer selects
  zero through a positive artifact maximum under the existing 1,024-iteration
  ceiling. The body cannot access the count resource, its writes are
  non-guaranteed, and a graph admits at most one produced-resource conditional
  or repeat feedback node.
- The CPU oracle reads the count from private graph storage after the producer,
  rejects an above-bound value without result publication, reserves maximum
  work, executes the actual count with existing cancellation/time checks, and
  publishes actual completion and element-operation totals only after
  whole-graph success.
- Portable WebGPU prewarms the complete maximum schedule, submits the exact
  producer prefix through the device-bound pipeline authority, reads back only
  the four-byte count, and submits the selected zero-through-maximum body slots
  plus suffix over the same resident private buffers. A zero-count repeat with
  no executable suffix reads outputs directly from the resident buffers. One
  timeout, cancellation signal, device owner, device-loss path, cleanup path,
  numerical-status check, and terminal-publication contract spans both stages.
- Required Chromium 148 on Apple Metal 3 passes eighteen complete-output
  CPU/WebGPU parity cases through explicit pipeline authority. Zero- and
  two-iteration produced-resource repeats retain one graph and pipeline
  identity with distinct specializations, exact completion/work records, and
  bit-exact outputs. A separate actual-device case rejects produced count four
  above artifact maximum three without output publication. Correctness
  artifact
  `29660d12e95945ca4004c1be5c8bec7af4d322aac63370624ae68ac79241fc25`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.11.0.
- Semantic-core passes 23 files/226 tests and kernels passes 21 files/187
  tests. The separate observational performance artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  its backend-1.11.0 fixed-repeat median/p95 is 1.90/6.60 ms and unrolled
  median/p95 is 1.90/6.20 ms, with no superiority or regression claim.
- One bounded GPU-produced repeat count is no longer a Gate 7 gap.
  Repeated/device-side feedback, GPU-produced dynamic launch control, broader
  dynamic schedules, nested/device-side control, transport/topology, worker
  meshes, native companions, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 bounded produced-resource dynamic dispatch

- Added `resource-u32-prefix-elements` as host-graph version 1.11 while
  retaining exact 1.0 through 1.10 decoding. One exact rank-local, aligned,
  four-byte, zero-filled temporary u32 with an ordered graph writer selects one
  through a positive artifact maximum within the verified semantic element
  domain. The dispatch cannot bind the count as data, maximum work is charged,
  and the at-most-one produced-resource feedback bound spans conditional,
  repeat, and dynamic-dispatch control.
- The CPU oracle reads the count from private graph storage after the producer,
  rejects zero and above-bound values without result publication, executes the
  exact canonical prefix for every rank, and publishes actual completion and
  rank-multiplied element-operation totals only after whole-graph success.
- Portable WebGPU prewarms the artifact-maximum launch once through the
  existing one-invocation-per-workgroup program and exact device-bound pipeline
  slots. Execution submits the producer prefix over resident private buffers,
  reads back only the four-byte count, substitutes the exact positive
  x-workgroup count in those already-authorized rank slots, and submits the
  dispatch plus suffix. One timeout, cancellation signal, device owner,
  device-loss path, cleanup path, numerical-status check, and terminal-
  publication contract spans both stages.
- Required Chromium 148 on Apple Metal 3 passes twenty complete-output
  CPU/WebGPU parity cases through explicit pipeline authority. One- and
  two-element produced-resource launches retain one graph and pipeline identity
  with distinct specializations, exact completion records, and bit-exact
  outputs. Separate actual-device cases reject produced count zero and three
  above artifact maximum two without output publication. Correctness artifact
  `c4f69a91cd057efec786459d922213b264031023bbd48e7c550b8d9097406872`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.12.0.
- Semantic-core passes 23 files/229 tests and kernels passes 21 files/188
  tests. The separate observational performance artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  its backend-1.12.0 fixed-repeat median/p95 is 1.30/1.50 ms and unrolled
  median/p95 is 1.30/1.40 ms, with no superiority or regression claim.
- One bounded positive GPU-produced one-lane launch prefix is no longer a Gate
  7 gap. Repeated/device-side feedback, broader dynamic schedules,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-28 — Gate 7 exact workgroup-aligned dynamic schedules

- WebGPU backend 1.13 widens the existing version-1.9 request-time and
  version-1.11 produced-resource dynamic-dispatch paths without changing graph
  semantics. The artifact maximum and selected positive logical-prefix count
  must both be exact multiples of the prepared workgroup size.
- The backend preserves the canonical logical invocation-count launch
  contract. The sequence runner therefore submits exactly
  `elementCount / workgroupSize` x workgroups through the maximum-prewarmed
  program and the same exact device-bound pipeline slots. There is no
  inactive-lane mask, static overdispatch, recompilation, or source-shaped
  execution path.
- Unaligned artifact maxima fail during preparation. An unaligned request-time
  count fails before device work; an unaligned produced-resource count fails
  after the single bounded four-byte feedback read but before its dispatch and
  without output publication.
- Required Chromium 148 on Apple Metal 3 passes twenty-four complete-output
  CPU/WebGPU parity cases. The four added cases execute 64- and 128-element
  prefixes at workgroup size 64 for both request-time and produced-resource
  control. Correctness artifact
  `48a4bf766981d702bc33c3c61eaa4f77ddb84eb9d9cfd7d2550950fc0717fdbd`;
  device profile
  `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`;
  WebGPU backend 1.13.0.
- Kernels passes 21 files/188 tests, with 24 focused host-graph preparation
  tests. The separate observational performance artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  its backend-1.13.0 fixed-repeat median/p95 is 3.00/10.10 ms and unrolled
  median/p95 is 2.50/5.90 ms, with no superiority or regression claim.
- Exact aligned wider-workgroup schedules are no longer a Gate 7 gap.
  Arbitrary unaligned or multidimensional dynamic schedules,
  repeated/device-side feedback, nested/device-side control,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-29 — Gate 7 arbitrary one-dimensional dynamic prefixes

- WebGPU backend 1.14 widens the existing version-1.9 request-time and
  version-1.11 produced-resource dynamic-dispatch paths to every positive
  bounded one-dimensional prefix without changing graph semantics or the CPU
  oracle.
- The canonical view-copy WGSL preparation seam emits a four-byte
  runtime-prefix uniform and checks it before coordinate, address, or resource
  evaluation. Execution changes only the validated logical dispatch count and
  matching uniform in the same maximum-prewarmed program and exact pipeline
  slot. The sequence runner submits `ceil(elementCount / workgroupSize)` x
  workgroups, and tail invocations return without semantic work.
- Every dynamic rank step charges 16 aligned GPU bytes plus four host bytes.
  Pipeline admission derives maximum total, storage, and uniform binding
  counts across all reachable programs and rejects insufficient
  `maxUniformBuffersPerShaderStage` before resource creation.
- Required Chromium 148 on Apple Metal 3 passes twenty-eight complete-output
  CPU/WebGPU parity cases. The added cases execute unaligned 65- and
  127-element prefixes at workgroup size 64 for both request-time and
  produced-resource control, while the existing 1/2 and aligned 64/128
  families retain exact outputs, graph/pipeline identity, and actual work
  records. Correctness artifact
  `1626d85876432578d833411e0f198cdd97f946204cf0256312531299d9aaf931`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`;
  WebGPU backend 1.14.0.
- Kernels passes 21 files/190 tests. A focused admission test rejects zero
  uniform capacity, and required actual-device evidence retains produced
  launch-count, device-loss, copied-authority, and destroyed-authority
  refusals.
- The separate observational performance artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.14.0 fixed-repeat median/p95 is 1.40/2.80 ms and unrolled
  median/p95 is 1.50/2.60 ms, with no superiority or regression claim.
- Arbitrary positive one-dimensional dynamic prefixes are no longer a Gate 7
  gap. Multidimensional dynamic schedules, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-29 — Gate 7 rank-2/rank-3 rectangular dynamic launch

- Host-graph program version 1.12 adds request-time
  `runtime-u32-rectangular-prefix` dispatch with one positive artifact-bounded
  u32 extent per semantic axis. Exact 1.0-1.11 decoding remains unchanged.
  Verification rejects missing/duplicate axes or control IDs, rank mismatch,
  above-shape maxima, rank-1/rank-4 domains, and pre-version use; portable
  preparation separately rejects maximum products above its view-copy ceiling.
- The CPU oracle maps each selected dense rank-2/rank-3 coordinate into the
  full row-major semantic domain, returns exact sparse rectangle effects, and
  reports logical extents plus their product only after whole-graph success.
  Runtime controls are captured and bounded before input observation.
- WebGPU backend 1.15 reuses one maximum-prewarmed canonical view-copy program
  and exact pipeline slot. One 16-byte uniform guards every logical axis before
  coordinate/address/resource evaluation; x/y/z map to trailing-to-leading
  axes and execution changes only true 2D/3D geometry plus uniform bytes.
  Device admission now checks x, y, and z workgroup counts independently.
- Required Chromium 148 on Apple Metal 3 passes 32 complete-output CPU/WebGPU
  parity cases. Small/full rank-2 `[3,4]` and rank-3 `[2,3,4]` pairs retain
  their respective graph/pipeline identities while recording distinct
  specializations and exact completions. Correctness artifact
  `d396265cee8b4d90a70334b5325c182efc4cd30f21f807dcc55a22ddba02bde3`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Semantic-core passes 23 files/235 tests; kernels passes 21 files/195 tests,
  strict typecheck/lint/build and browser typecheck; packed fresh-consumer
  execution passes under Node 25 and pinned pnpm 10.34.5; root architecture
  passes. The separate performance artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.15.0 fixed-repeat median/p95 is 1.30/3.10 ms and unrolled
  median/p95 is 1.30/3.40 ms, with no superiority or regression claim.
- Generic native compiler discovery now requires a warning-clean syntax probe
  and skips a Homebrew Clang with a stale configured sysroot; the LLVM-22
  Clang-pass lane binds the installed Xcode SDK explicitly. The complete
  13-file native matrix passes 35 tests with 9 platform skips. The broader
  compiler verifier then stops independently at its exact reviewed-bsdtar
  identity check because the installed builder has drifted; no trust pin was
  silently changed.
- Request-time rank-2/rank-3 rectangular schedules are no longer a Gate 7 gap.
  GPU-produced multidimensional launch, rank-4-or-higher dynamic domains,
  repeated/device-side feedback, nested/device-side control, transport,
  worker meshes, native companions, and stronger event profiles remain open.

### 2026-07-29 — Gate 7 produced-resource rectangular launch

- Host-graph program version 1.13 adds one
  `resource-u32-rectangular-prefix` dispatch with a distinct ordered
  rank-local temporary u32 source per rank-2/rank-3 axis. Exact 1.0-1.12
  decoding remains unchanged. Verification rejects duplicate axes/resources,
  malformed source storage, data-binding alias, missing writer dominance,
  above-shape maxima, and pre-version use.
- CPU and WebGPU reuse the version-1.12 rectangular coordinate semantics,
  maximum-product admission, completion records, canonical WGSL, and exact
  pipeline slots. WebGPU executes all extent producers in one resident-buffer
  prefix stage, reads exactly two or three four-byte values, rejects zero or
  above-bound extents, and submits the guarded rectangular dispatch plus
  suffix without exposing intermediate resources.
- Required Chromium 148 on Apple Metal 3 passes 36 complete-output CPU/WebGPU
  parity cases. Four produced small/full rank-2/rank-3 cases retain pairwise
  pipeline identity, record distinct specializations and exact completions,
  and prove one feedback stage plus zero/above-bound fail-stop refusal.
  Correctness artifact
  `4b74011f0f1c1308723d458ec02140ac467299b0196ea3560d316eb67e5e2b15`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Semantic-core passes 23 files/239 tests and kernels passes 21 files/196
  tests with strict package and browser typechecks. The separate performance
  artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.16.0 fixed-repeat median/p95 is 1.60/1.80 ms and unrolled
  median/p95 is 1.30/1.70 ms, with no superiority or regression claim.
- GPU-produced rank-2/rank-3 rectangles are no longer a Gate 7 gap.
  Rank-4-or-higher dynamic domains, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-29 — Gate 7 request-time rank-4 rectangular launch

- Host-graph program version 1.14 extends
  `runtime-u32-rectangular-prefix` to rank 4 while preserving exact
  1.0-1.13 decoding and the rank-2/rank-3 produced-resource profile.
- CPU executes the exact four-axis dense rectangle. WebGPU reuses the
  maximum-prewarmed canonical view-copy program and 16-byte extent uniform,
  maps logical axes 3/2 to physical x/y, flattens selected axes 0/1 into
  physical z, then reconstructs those leading coordinates by quotient and
  remainder after the dynamic-product guard.
- Required Chromium 148 on Apple Metal 3 passes 38 complete-output CPU/WebGPU
  parity cases. Rank-4 small/full `[2,2,3,4]` cases retain pipeline identity,
  record distinct specializations and exact completions, and exercise
  flattened-z device admission. Correctness artifact
  `a377d5deb9a32c28e84ca54b2b955d7ad3549389616928bab52cc1d985c56c15`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Semantic-core passes 23 files/239 tests and kernels passes 21 files/196
  tests with strict package and browser typechecks. The separate performance
  artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.17.0 fixed-repeat median/p95 is 1.70/2.70 ms and unrolled
  median/p95 is 1.60/1.90 ms, with no superiority or regression claim.
- Request-time rank-4 rectangles are no longer a Gate 7 gap. Produced-resource
  rank 4, rank-5-or-higher dynamic domains, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-29 — Gate 2 signed-affine rank-1 view copy

- Semantic-core adds the distinct
  `browsergrad.view-copy.signed-affine-rank1-word32@1` profile without
  widening either existing signed identity or any positive-affine identity.
  Signed source-coordinate scales now cover every portable rank from 1 through
  5 while the destination remains positive-affine and dense.
- CPU executes exact f32 rank-1 reversal through the same prepared accessor
  proof. Semantic view-copy WebGPU backend 2.4 executes exact u32 reversal
  through the same signed-i32 interval/address lowerer.
- Required Chromium 148 on Apple Metal 3 passes 20 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  ...
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/197
  tests with strict package/browser typecheck, lint, and build gates.
- Rank 6, signed destinations, packed/16-bit/bf16/f64 storage, overlap,
  non-global memory, integer division/modulo, and frontend-produced signed
  source compatibility remain separate capabilities.

### 2026-07-29 — Gate 2 signed-affine rank-4/rank-5 view copy

- Semantic-core adds the distinct
  `browsergrad.view-copy.signed-affine-rank4-rank5-word32@1` profile without
  widening the rank-2/rank-3 signed identity or any positive-affine identity.
  The destination remains positive-affine and dense; signed rank 1, signed
  destinations, and integer division/modulo remain refused.
- CPU executes exact f32/i32 rank-4 and rank-5 reversals through the same prepared
  accessor proof. Semantic view-copy WebGPU backend 2.3 composes the existing
  signed-i32 interval/address lowerer with the existing rank-4/rank-5
  mixed-radix coordinate path.
- Required Chromium 148 on Apple Metal 3 passes 19 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
...
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/197
  tests with strict package/browser typecheck, lint, and build gates.
- Signed rank 1, packed/16-bit/bf16/f64 storage, overlap, non-global memory,
  signed destinations, integer division/modulo, and frontend-produced signed
  source compatibility remain separate capabilities.

### 2026-07-29 — Gate 2 signed-affine rank-2/rank-3 view copy

- Semantic-core adds the distinct
  `browsergrad.view-copy.signed-affine-rank2-rank3-word32@1` profile for
  negative source-coordinate scales without widening any positive-affine
  identity. The destination remains positive-affine and dense; signed rank
  1/4/5, signed destinations, and integer division/modulo remain refused.
- CPU executes exact f32/i32/u32 source reversals through the existing prepared
  accessor proof. Semantic view-copy WebGPU backend 2.2 reuses the canonical
  signed-i32 interval/address lowerer and converts to u32 only after the proved
  view-byte-offset rebase and source guard.
- Required Chromium 148 on Apple Metal 3 passes 17 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
...
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/197
  tests with strict package/browser typecheck, lint, and build gates.
- This closes negative source strides at ranks 2 and 3 through the existing
  semantic and backend seams. Packed/16-bit/bf16/f64 storage, overlap,
  non-global memory, signed rank 1/4/5, signed destinations, integer
  division/modulo, and frontend-produced signed source compatibility remain
  separate capabilities.

### 2026-07-29 — Gate 2 portable rank-5 view copy

- Semantic-core adds the distinct
  `browsergrad.view-copy.positive-affine-rank5-word32@1` profile without
  widening any rank-1-through-rank-4 identity. It retains same-dtype
  f32/i32/u32 word storage, positive-affine global-memory indexing, exact
  effects/alias rules, and guarded source semantics; rank 6 remains refused.
- The CPU oracle executes exact rank-5 permutations and rectangular prefixes.
  Semantic view-copy WebGPU backend 2.1 lowers the same five-axis row-major
  domain through mixed-radix coordinate reconstruction in the canonical WGSL
  path.
- Required Chromium 148 on Apple Metal 3 passes 14 complete CPU/WebGPU
  view-copy parity cases. Correctness artifact
  `50afa87acf5f404a3f6b5c515349a426237c2425ad36d15599e5693cfb4d5a41`;
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/196
  tests with strict typecheck, browser typecheck, lint, and build gates.
- Signed rank-1/rank-4/rank-5 maps and signed destinations,
  packed/16-bit/bf16/f64 storage, overlap, non-global memory, dynamic rank-5
  host graphs, and frontend-produced rank-5 source compatibility remain
  separate capabilities.

### 2026-07-29 — Gate 7 produced-resource rank-5 rectangular launch

- Host-graph program version 1.17 extends
  `resource-u32-rectangular-prefix` to rank 5 while preserving exact
  1.0-1.16 decoding and all older rank limits. Five distinct ordered
  rank-local temporary u32 resources remain under the graph-wide
  one-feedback-node bound.
- CPU reads the five private extents after their producers and executes the
  exact dense rectangle. WebGPU submits every producer in one resident-buffer
  prefix stage, reads and validates the five values together, then reuses the
  version-1.16 flattened-z mapping, guarded 32-byte uniform,
  maximum-prewarmed canonical view-copy program, and exact pipeline slots.
- Required Chromium 148 on Apple Metal 3 passes 44 complete-output CPU/WebGPU
  parity cases. Produced rank-5 small/full `[2,2,2,3,4]` cases retain pipeline
  identity, record distinct specializations and exact completions, and retain
  one feedback stage. Correctness artifact
...
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/196
  ...
  artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.20.0 fixed-repeat median/p95 is 2.10/6.40 ms and unrolled
  median/p95 is 2.10/6.20 ms, with no superiority or regression claim.
- Produced-resource rank-5 rectangles are no longer a Gate 7 gap.
  Rank-6-and-higher dynamic domains, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-29 — Gate 7 request-time rank-5 rectangular launch

- Host-graph program version 1.16 extends
  `runtime-u32-rectangular-prefix` to rank 5 under the distinct rank-5
  view-copy profile while preserving exact 1.0-1.15 decoding and the
  produced-resource rank-4 limit.
- CPU reuses the generic exact dense-rectangle evaluator. WebGPU carries five
  extents in a 32-byte eight-u32 uniform, maps axes 4/3 to x/y, flattens axes
  0 through 2 into z, and reconstructs the leading coordinates by guarded
  mixed radix before semantic address evaluation.
- Required Chromium 148 on Apple Metal 3 passes 42 complete-output CPU/WebGPU
  parity cases. Rank-5 small/full `[2,2,2,3,4]` request-time cases retain
  pipeline identity, distinct specialization, exact completion, and
  input/control snapshot isolation. Correctness artifact
  `8ee4167935d6bf30d7ec1ab7425e6fcfb8788b656ddf8800e56742afa112a685`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Semantic-core passes 23 files/240 tests and kernels passes 21 files/196
  tests with strict package and browser typechecks. The separate performance
  artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.19.0 fixed-repeat median/p95 is 1.90/5.40 ms and unrolled
  median/p95 is 1.80/3.50 ms, with no superiority or regression claim.
- Request-time rank-5 rectangles are no longer a Gate 7 gap.
  Produced-resource rank 5, rank-6-and-higher dynamic domains,
  repeated/device-side feedback, nested/device-side control,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-29 — Gate 7 produced-resource rank-4 rectangular launch

- Host-graph program version 1.15 extends
  `resource-u32-rectangular-prefix` to rank 4 while preserving exact
  1.0-1.14 decoding and all older rank limits. Four distinct ordered
  rank-local temporary u32 resources remain under the graph-wide
  one-feedback-node bound.
- CPU reads the four private extents after their producers and executes the
  exact dense rectangle. WebGPU submits every producer in one resident-buffer
  prefix stage, reads and validates the four values together, then reuses the
  version-1.14 flattened-z mapping, guarded 16-byte uniform,
  maximum-prewarmed canonical view-copy program, and exact pipeline slots.
- Required Chromium 148 on Apple Metal 3 passes 40 complete-output CPU/WebGPU
  parity cases. Produced rank-4 small/full `[2,2,3,4]` cases retain pipeline
  identity, record distinct specializations and exact completions, and retain
  one feedback stage. Correctness artifact
  `4dde8ca9e0f6183dcf7a2ccb4d7acd7fd3f192f198c2d6c093f4cfd754d656cf`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Semantic-core passes 23 files/239 tests and kernels passes 21 files/196
  tests with strict package and browser typechecks. The separate performance
  artifact remains
  `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`;
  backend-1.18.0 fixed-repeat median/p95 is 1.60/2.80 ms and unrolled
  median/p95 is 1.70/3.30 ms, with no superiority or regression claim.
- Produced-resource rank-4 rectangles are no longer a Gate 7 gap.
  Rank-5-or-higher dynamic domains, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-30 — Closure-aligned signed exact-distribution convergence

- The package-owned unchanged signed-rank-2 CuTe source now joins the strict
  browser matrix. All nine cases compile through the real verifier/compiler
  Workers, produce accepted Artifact V3 payloads, enter the existing canonical
  authorization/lowering seam, and pass bit-exact CPU/required-WebGPU
  convergence on one Apple Metal device profile.
- A closed exact-convergence authoring command accepts only bounded canonical
  non-symlink input, rederives the current full-distribution binding, rejects
  stale package identities and known software adapters, validates exact
  nine-case coverage, and writes/checks the typed resource deterministically.
- The first fast self-check caught a stale reproducibility identity embedded in
  the compiler Worker bundle. The Worker, asset/profile/build-subject chain,
  two-root 25-output distribution, strict observation, and convergence resource
  were repinned together. The corrected 31,841,971-byte Wasm itself remained
  unchanged, so no cache-free compiler rebuild was required.
- Current identities are Worker
  ...
  strict resource
  `78cd399dbd660224377eb16a0388cbd5688440e0bf9849165346a0ceaed665b0`,
  and convergence matrix
  `bg.cpp.browser-exact-distribution-convergence.sha256.3e3c66fe9de9f74d8f6d0cc608cc996340a3b2aeaf66a8a580a82fe32f5f8ba3`.
- Fast verification passes 104 files/823 tests; the complete package passes
  106 files/1,660 tests; the evidence-input suite passes 19 files/177 tests;
  strict typecheck, lint, and whitespace checks pass. Strict browser compile
  times are 20.129-22.938 seconds per case and total case times are
  22.840-25.861 seconds. The full and evidence-input suites run concurrently.
- This closes the declared portable Gate 3 implementation exit. Gate 3 remains
  `in-progress` because the local engineering signer grants no production
...
- Producer-neutral C++/CuTe view-copy lowering version 2 accepts verified
  static signed source strides at Artifact V3 ranks 1 through 4 while keeping
  the destination positive-affine and dense.
- Storage admission now derives exact minimum and maximum affine element
  offsets. The explicit host view offset must rebase the minimum to a
  nonnegative root address, and the explicit allocation length must contain
  the maximum word endpoint. CuTe `cosize` remains outside storage authority.
- Four f32 reversal fixtures reach the existing semantic-core constructor,
  canonical CPU oracle, and kernels-owned WGSL preparation path. Complete
  source and destination canaries remain unchanged; under-rebased and short
  allocations fail at distinct request paths.
- Focused compiler coverage passes ten cases; four adjacent candidate,
  authorization, artifact, and lowering files pass 54 tests; the complete
  compiler suite passes 106 files/1,659 tests. Strict typecheck, source lint,
  semantic architecture, and compiler architecture pass with only existing
  generated/resource warnings.
- Required Chromium 148 on Apple Metal 3 independently reconfirms all 20
  semantic view-copy backend cases, including signed ranks 1 through 5, at
  correctness artifact
  `803dc85c4732ac6d38fe42dc175f56542663918f231d4c13b6aae596af1f2b8f`.
  This validates the shared backend tier, not a real signed C++ source compile.
- This is Artifact V3-to-backend convergence. The strict real-browser matrix
...
- A full compiler gate exposed five native test timeouts when two Clang-heavy
  Vitest files ran inside one of four already-parallel verifier lanes. The
  fixed 60-second `spawnSync` limit killed only the immediate compiler driver;
  descendants retaining its pipes amplified a bounded timeout into a
  409-second minimized failure and a 459.75-second native-lane failure.
- The verifier now performs type/lint validation and all 12 native files as
  serial prerequisites before admitting the four read-only parallel lanes.
  The 43 non-native Clang-Wasm surface files remain parallel inside the
  compiler-test lane. Plan validation rejects moving the native command back
  into a concurrent lane or moving the surface command out of its owner lane.
- All behavioral native compiler and executable invocations now use the
  existing no-shell build-process boundary. It enforces absolute executables,
  exact argv/cwd/environment, an 8 MiB per-stream ceiling, process-group
  termination, and a five-second TERM-to-KILL grace. A nested-descendant
  regression proves a 25-millisecond timeout settles in under two seconds. No
  test timeout was widened.
- Commit `66a3a279` passes strict build-plan typecheck/lint, the 43-file/261-test
  surface suite, the 12-file/30-test native suite with nine platform skips in
  71.82 seconds, the 106-file/1,658-test compiler suite, both architecture
  checks, Docker-shell tests, and every fixture/CLI lane. The complete
  `verify:compiler` gate passes in 114.23 seconds; the prior 6–7-minute
  timeout amplification is removed while independent work remains parallel.
- A later full gate under concurrent host build load exposed that the
  process-tree regression itself still ran in the parallel surface lane and
  could exhaust Vitest's five-second test budget there despite passing alone
  in 0.50 seconds. It now runs once beside the native child-process tests.
  The isolated lane passes 13 files/34 tests with nine skips in 78.87 seconds;
  the parallel surface lane passes 42 files/257 tests in 17.92 seconds; and the
  complete contention-affected verifier passes in 129.30 seconds.

### 2026-07-30 — Canonical rank-6 view-copy execution

- Distinct positive- and signed-affine rank-6 word32 profiles extend the same
  verified operation, CPU reference, specialization, signed address proof, and
  sole WGSL lowerer without changing rank-1-through-rank-5 identities.
- Backend 2.5 adds exact six-axis mixed-radix coordinate reconstruction and
  rectangular launch lowering. Rank 7+, signed destinations, packed storage,
  overlap, and non-global memory remain explicit refusals.
- Semantic-core passes 23 files/240 tests; kernels passes 21 files/197 tests,
  build, strict typecheck, browser view-copy typecheck, and required headed
  Chromium on Apple Metal 3. The 22-case matrix records correctness artifact
  `ee0b7da1e98c2dd49f5a78caa7217bdf6303014e214631d1af8b7a00fe0dd650`,
  case set `7f7b8ec4fe8327babf872eb25b15cdbc464fce262241bcf5410f69f168446527`,
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Capability commit `7d5d3075` is pushed to `origin/main`.

### 2026-07-30 — Gate 7 request-time rank-6 rectangular launch

- Host-graph program version 1.18 extends only
  `runtime-u32-rectangular-prefix` to rank 6 while preserving exact 1.0-1.17
  decoding and the produced-resource rank-5 limit.
- CPU reuses the generic exact rectangle evaluator. WebGPU maps axes 5/4 to
  x/y, flattens axes 0 through 3 into z, reconstructs them behind the 32-byte
  guard, and preserves one maximum-prewarmed canonical program and exact
  pipeline slot.
- Focused semantic-core graph coverage passes 2 files/62 tests and kernels
  host-graph coverage passes 1 file/29 tests with strict package typechecks.
  Required headed Chromium on Apple Metal 3 passes 46 complete-output
  CPU/WebGPU parity cases under backend 1.21.0. Correctness artifact
  `39d99833cfa0e11597cb7a30f91121ed4e4fac4ad5296962c2ba05f7b2e8103a`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Produced-resource rank 6, rank 7+, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-30 — Gate 7 produced-resource rank-6 rectangular launch

- Host-graph program version 1.19 extends
  `resource-u32-rectangular-prefix` to rank 6 while preserving exact 1.0-1.18
  decoding and every older rank limit. Six distinct ordered rank-local
  temporary u32 resources remain under the graph-wide one-feedback-node bound.
- CPU reads all six private extents after their producers and executes the
  exact dense rectangle. WebGPU submits every producer in one resident-buffer
  prefix stage, reads and validates the six values together, then reuses the
  version-1.18 flattened-z mapping, guarded 32-byte uniform,
  maximum-prewarmed canonical view-copy program, and exact suffix slots.
- Focused semantic-core graph coverage passes 2 files/62 tests and kernels
  host-graph coverage passes 1 file/29 tests with strict package and browser
  typechecks. Required headed Chromium on Apple Metal 3 passes 48
  complete-output CPU/WebGPU parity cases under backend 1.22.0. Correctness
  artifact
  `bd02e60f56a6e7fc4c90d3f5b35ad594694ad41ff70d547494f7cb9a75814c05`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Rank 7+, repeated/device-side feedback, nested/device-side control,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-30 — Canonical rank-7 view-copy execution

- Distinct positive- and signed-affine rank-7 word32 profiles extend the same
  verified operation, CPU reference, specialization, signed address proof, and
  sole WGSL lowerer without changing rank-1-through-rank-6 identities.
- Backend 2.6 adds exact seven-axis mixed-radix coordinate reconstruction and
  rectangular launch lowering under the existing eight-u32 dynamic uniform.
  Rank 8+, signed destinations, packed storage, overlap, and non-global memory
  remain explicit refusals.
- Focused semantic-core and kernels view-copy suites pass 17 and 13 tests with
  strict package/browser typechecks. Required headed Chromium on Apple Metal 3
  passes the 24-case matrix with correctness artifact
  `37fb0aa3f0041e0de0a4f6536cd7046b2b7192bc5027fd80bf13148dc7aa3e31`,
  case set `251c3411d23b594c1e31313da709b2f0522dd9cec4315468477901fa5a9f975f`,
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Rank-7 host-graph admission stays separately versioned: request-time follows
  in version 1.20, while produced-resource rank 7 remains future work.

### 2026-07-30 — Gate 7 request-time rank-7 rectangular launch

- Host-graph program version 1.20 extends
  `runtime-u32-rectangular-prefix` to rank 7 while preserving exact 1.0-1.19
  decoding and the produced-resource rank-6 limit.
- CPU reuses the generic dense-rectangle evaluator. WebGPU backend 1.23 maps
  axes 6/5 to x/y and uses shared generic launch and mixed-radix lowering for
  the five leading axes behind the existing 32-byte guard and exact
  device-bound pipeline authority.
- Semantic-core passes 23 files/241 tests and kernels passes 21 files/197 tests
  with strict package/browser typechecks. Required headed Chromium on Apple
  Metal 3 passes 50 complete-output CPU/WebGPU parity cases with correctness
  artifact
  `433edea8c4017b908328a2c915c6c8c0b900fbe23c86d5201fa33abb67c5ffd9`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Produced-resource rank 7, rank 8+, repeated/device-side feedback,
  nested/device-side control, transport/topology, worker meshes, native
  companions, and stronger event profiles remain open.

### 2026-07-30 — Gate 7 produced-resource rank-7 rectangular launch

- Host-graph program version 1.21 extends
  `resource-u32-rectangular-prefix` to rank 7 while preserving exact 1.0-1.20
  decoding and every older rank limit. Seven ordered private u32 extent
  resources remain under the graph-wide one-feedback-node bound.
- CPU reads all seven extents after their producers. WebGPU backend 1.24
  submits the producers in one resident-buffer prefix stage, validates the
  values together, then reuses the version-1.20 generic mapping, 32-byte guard,
  maximum-prewarmed program, and exact device-bound suffix slots.
- Semantic-core passes 23 files/241 tests and kernels passes 21 files/197 tests
  with strict package/browser typechecks. Required headed Chromium on Apple
  Metal 3 passes 52 complete-output CPU/WebGPU parity cases with correctness
  artifact
  `1a44aedfda7b53bddee3d526299e2f255dfff410a1efe0123f448dd5aaade157`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Rank 8+, repeated/device-side feedback, nested/device-side control,
  transport/topology, worker meshes, native companions, and stronger event
  profiles remain open.

### 2026-07-30 — Exact packed16 view-copy execution

- Semantic-core adds the distinct
  `browsergrad.view-copy.positive-affine-rank2-rank3-packed16@1` profile for
  exact same-dtype i16/u16/f16/bf16 storage at ranks 2 and 3. The operation,
  verifier, specialization, CPU evaluator, effects, and alias rules remain
  shared with word32 view copy.
- Kernels backend 2.7 adds one raw-u32 packed schedule. Each invocation owns
  two adjacent destination halfwords, preventing cross-invocation
  read-modify-write races; an odd final element preserves the unrelated upper
  half. Positive-affine maps, a dense destination, and
  reject-on-invalid-source remain semantic profile guards. Static launch,
  word-aligned destinations, and word-sized root allocations are explicit
  WebGPU backend guards.
- Semantic-core passes 23 files/242 tests and kernels passes 21 files/198
  tests with strict package/browser typechecks. Required headed Chromium 148
  on Apple Metal 3 passes 28 complete-destination CPU/WebGPU parity cases,
  including a halfword-shifted source, nonzero word-aligned destination, and
  unchanged prefix/suffix canaries.
  Correctness artifact
  ...
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- This is bit-preserving storage movement only. No f16/bf16 arithmetic,
  conversion, `shader-f16`, signed/dynamic packed map, or frontend source
  compatibility claim is implied.
- The unchanged 52-case host-graph CPU/WebGPU lane also passes after the
  view-copy backend rotation, with correctness artifact
  `6a169f1f257fbeb9b91988b2ac622e24b13720a9b403e09a3cfb0c3b60930b1b`.

### 2026-07-30 — Exact packed8 view-copy execution

- Semantic-core adds the distinct
  `browsergrad.view-copy.positive-affine-rank2-rank3-packed8@1` profile for
  exact same-dtype bool/i8/u8 storage at ranks 2 and 3. It retains the shared
  operation, verifier, specialization, CPU evaluator, effects, and alias
  rules; bool bytes remain raw storage and are not canonicalized.
- Kernels backend 2.8 generalizes the raw-u32 packed schedule. Each invocation
  owns four adjacent destination bytes, preventing cross-invocation
  read-modify-write races, while a partial final word preserves unrelated
  bytes. Static launch, positive-affine maps, dense destinations,
  reject-on-invalid-source, word-aligned destinations, and word-sized root
  allocations remain explicit guards.
- Semantic-core passes 23 files/243 tests and kernels passes 21 files/199
  tests with strict package/browser typechecks. Required headed Chromium 148
  on Apple Metal 3 passes 31 complete-destination CPU/WebGPU parity cases,
  including raw noncanonical bool bytes, byte-shifted sources, odd tails, and
  unchanged prefix/suffix canaries.
  Correctness artifact
  ...
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- This is bit-preserving storage movement only. It does not claim bool
  normalization, packed arithmetic or conversion, signed/dynamic packed maps,
  64-bit storage, or frontend source compatibility.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `318fef4a4f06095c0371607ebf50135f0779dc3fa4134c3022e82d6d8af9564b`.

### 2026-07-30 — Exact word64 view-copy execution

- Semantic-core adds the distinct
  `browsergrad.view-copy.positive-affine-rank2-rank3-word64@1` profile for
  exact same-dtype f64/i64/u64 storage at ranks 2 and 3. It retains the shared
  operation, verifier, specialization, CPU evaluator, effects, and alias rules
  while keeping 64-bit value semantics outside the claim.
- Kernels backend 2.9 extends the shared raw byte-address seam with a word64
  schedule. Each static invocation copies one logical element as two adjacent
  u32 words, so destination writes are disjoint and no native 64-bit WGSL type
  is required. Positive-affine maps, a dense destination,
  reject-on-invalid-source, aligned views, and word-sized roots remain
  explicit guards.
- Semantic-core passes 23 files/244 tests and kernels passes 21 files/200
  tests with strict package/browser typechecks. Required headed Chromium 148
  on Apple Metal 3 passes 34 complete-destination CPU/WebGPU parity cases,
  including f64 NaN payloads and signed zero, i64 byte-unit broadcast, u64
  rank-3 permutation, nonzero offsets, and unchanged destination canaries.
  Correctness artifact
  ...
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- This is bit-preserving storage movement only. It does not claim f64 or
  64-bit integer arithmetic, conversion, signed/dynamic word64 maps, native
  64-bit WGSL types, or frontend source compatibility.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `ffa7204a1dc59b2ccaaf9d22d8e5128a41cb3fc629d0875b1ea9e2ed5578027a`.

### 2026-07-30 — Signed non-word32 view-copy execution

- Semantic-core adds distinct signed-affine rank-2/rank-3 packed8, packed16,
  and word64 source profiles without widening any existing positive-affine or
  word32 identity. The canonical coordinate proof requires the nonnegative
  view byte offset to rebase the complete signed source range into its root;
  destinations remain positive affine, dense, and injective.
- Kernels backend 3.0 reuses the existing interval-proved signed-i32 raw
  byte-address lowerer and the same width-owned static schedules. There is no
  second executor, unsigned reinterpretation, clamping, source-shaped kernel,
  arithmetic, or conversion path.
- Semantic-core passes 23 files/244 tests and kernels passes 21 files/201
  tests. Required headed Chromium 148 on Apple Metal 3 passes 37
  complete-destination CPU/WebGPU parity cases, including reversed i8, f16,
  and f64 storage with exact destination-tail/canary preservation. Correctness
  artifact
...
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `b1968ed67331174c694e936718259bb8cf385a8582c6c017a7befd55ae995ad6`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles prove signed source layout addressing and raw storage movement
  only. Signed destinations, dynamic launch, padding fill, non-word32
  arithmetic/conversion, rank 1 or 4+, overlap, and non-global memory remain
  outside the claim.

### 2026-07-30 — Rank-1 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-1
  packed8, packed16, and word64 profiles without widening any rank-2/rank-3 or
  word32 identity. The existing verifier and CPU evaluator execute strided and
  reversed same-dtype storage views through the canonical coordinate proof.
- Kernels backend 3.1 reuses the width-owned packed-word and two-word schedules
  plus the signed-i32 raw byte-address lowerer. Rank 1 changes only coordinate
  reconstruction; no rank-shaped executor, native 64-bit type, arithmetic, or
  conversion path was added.
- Semantic-core passes 23 files/245 tests and kernels passes 21 files/202 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 43 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  `9e164f5635f68a039df7ad74a9c80a6bf3976427cb7ef973d77e09f9f02c44ad`;
  case set
  `a54bc08198e1881680e271c02db728c198921770706d67c228eebc9f78fd859b`;
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `5071213f7904d4988cab589d62a5ec60135084fe5d47aa2a60442f4da4c14eee`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles prove exact raw storage movement only. Signed destinations,
  dynamic maps, fill, non-word32 arithmetic/conversion, rank 4+, overlap, and
  non-global memory remain outside the claim.

### 2026-07-30 — Rank-4 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-4
  packed8, packed16, and word64 profiles without widening any lower-rank or
  word32 identity. The canonical CPU evaluator proves exact permutation and
  reversal of each storage width.
- Kernels backend 3.2 reuses the existing rank-4 coordinate reconstruction,
  signed-i32 raw byte-address lowerer, and width-owned packed-word or two-word
  schedules. No storage-specific coordinate path, arithmetic, conversion, or
  native 64-bit type was added.
- Semantic-core passes 23 files/246 tests and kernels passes 21 files/203 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 49 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  `e45a38b74648f0fa78a42cdd629527462057b19ba72f37d93ede27ffa8be8993`;
  case set
  `2a4ae1c056851af475fd7f1459b19ff68027bcb6984f66863c91e45edb385497`;
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `7851313fee7532fd098e71da50de680fc9d288f6ab55ec4ab1d8f563f5599735`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles prove exact raw storage movement only. Signed destinations,
  dynamic maps, fill, non-word32 arithmetic/conversion, rank 5+, overlap, and
  non-global memory remain outside the claim.

### 2026-07-30 — Rank-5 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-5
  packed8, packed16, and word64 profiles without widening any lower-rank or
  word32 identity. The canonical CPU evaluator proves exact permutation and
  reversal of each storage width.
- Kernels backend 3.3 reuses the existing rank-5 coordinate reconstruction,
  signed-i32 raw byte-address lowerer, and width-owned packed-word or two-word
  schedules. No storage-specific coordinate path, arithmetic, conversion, or
  native 64-bit type was added.
- Semantic-core passes 23 files/247 tests and kernels passes 21 files/204 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 55 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  `02fa8e42c657364383b624fc87a0d7b7f2a3618a8fc3421689b145c4aeab14b2`;
  case set
  `28e2c5e5cae79ff525606c5c8edcd702b6f26084015f55ef4e75c21f7288d48d`;
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `f040846ce84304741c182f9d6b1edad20b492328fe4b9481979c3520fdceb144`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles prove exact raw storage movement only. Signed destinations,
  dynamic maps, fill, non-word32 arithmetic/conversion, rank 6+, overlap, and
  non-global memory remain outside the claim.

### 2026-07-30 — Rank-6 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-6
  packed8, packed16, and word64 profiles without widening any lower-rank or
  word32 identity. The canonical CPU evaluator proves exact permutation and
  reversal of each storage width.
- Kernels backend 3.4 reuses the existing rank-6 coordinate reconstruction,
  signed-i32 raw byte-address lowerer, and width-owned packed-word or two-word
  schedules. No storage-specific coordinate path, arithmetic, conversion, or
  native 64-bit type was added.
- Semantic-core passes 23 files/248 tests and kernels passes 21 files/205 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 61 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  `e784ebcafba6c7742ed076a3be8429f9709759e00580fc6c8a2c194fbb61b6e1`;
  case set
  `f311a94dcf2456b6fb96cb81ad7fa8aa1fa270ae3b9db9fdc53021da42363912`;
  device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `7c5819d42d3c0d770ed07ce76d68f6ce7b6956ca7341d8f40ab7092cc2f08bcc`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles prove exact raw storage movement only. Signed destinations,
  dynamic maps, fill, non-word32 arithmetic/conversion, rank 7+, overlap, and
  non-global memory remain outside the claim.

### 2026-07-30 — Rank-7 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-7
  packed8, packed16, and word64 profiles without widening any lower-rank or
  word32 identity. The canonical CPU evaluator proves exact permutation and
  reversal of each storage width.
- Kernels backend 3.5 reuses the existing rank-7 coordinate reconstruction,
  signed-i32 raw byte-address lowerer, and width-owned packed-word or two-word
  schedules. No storage-specific coordinate path, arithmetic, conversion, or
  native 64-bit type was added.
- Semantic-core passes 23 files/249 tests and kernels passes 21 files/206 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 67 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
  `7e24609c34f885ad00c3d6121a8201a4b7d3558c595e676d2d66779c971de9a8`;
  case set
...
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `e38d97a66565f394041cc93bb51d5ae6c211fe6cc9e1b61fb985cf94b5f9568c`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles complete the current static exact-storage rank envelope.
  Signed destinations, dynamic maps, fill, non-word32 arithmetic/conversion,
  rank 8+, overlap, and non-global memory remain outside the claim.

### 2026-07-30 — Rank-8 word32 view-copy execution

- Semantic-core adds distinct positive-affine and signed-affine rank-8 word32
  profiles without widening any lower-rank or non-word32 identity. The
  canonical CPU evaluator proves exact f32 permutation and u32 reversal.
- Kernels backend 3.6 extends the generic static mixed-radix coordinate lowerer
  to rank 8 and reuses the signed-i32 address proof and one-word schedule. Host
  graphs and non-word32 storage retain their existing independent rank limits.
- Semantic-core passes 23 files/249 tests and kernels passes 21 files/206 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 69 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
...
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `0a4f3e7bfce00b33f99aa2c5a50eb7f87e6a37d15bbc02fb0ff0a88d3027c6ed`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Rank 9+, non-word32 rank 8+, signed destinations, dynamic rank-8 maps,
  division/modulo, overlap, and non-global memory remain outside the claim.

### 2026-07-30 — Rank-8 non-word32 view-copy execution

- Semantic-core adds six distinct positive-affine and signed-affine rank-8
  packed8, packed16, and word64 profiles without widening any lower-rank or
  word32 identity. The canonical CPU evaluator proves exact permutation and
  reversal of each storage width.
- Kernels backend 3.7 reuses the generic rank-8 mixed-radix coordinate
  reconstruction, signed-i32 raw byte-address lowerer, and width-owned
  packed-word or two-word schedules. No storage-specific coordinate path,
  arithmetic, conversion, or native 64-bit type was added.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 75 complete-destination
  CPU/WebGPU parity cases. Correctness artifact
...
- The unchanged 52-case host-graph CPU/WebGPU lane passes after the backend
  rotation, with correctness artifact
  `21fe32a22b9aa7cae04c11c511d5fe6c48ed2cc0cf439628f79418c761073937`
  and device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- These profiles complete exact raw-storage movement through rank 8. Rank 9+,
  signed destinations, dynamic rank-8 maps, fill, non-word32
  arithmetic/conversion, division/modulo, overlap, and non-global memory
  remain outside the claim.

### 2026-07-30 — Gate 7 request-time rank-8 rectangular launch

- Host-graph program version 1.22 extends
  `runtime-u32-rectangular-prefix` to rank 8 while preserving exact 1.0-1.21
  decoding and the version-1.21 produced-resource rank-7 limit.
- CPU reuses the generic dense-rectangle evaluator. WebGPU backend 1.25 maps
  axes 7/6 to x/y, flattens axes 0 through 5 into z, and reuses the existing
  32-byte eight-u32 uniform, generic mixed-radix coordinate lowerer,
  maximum-prewarmed canonical view-copy program, and exact pipeline slots.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 54 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `a1cdf5e1d70c7a579ff6d48978d52278cdce80712baa4650749bbf1c194577d9`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Produced-resource rank 8, repeated/device-side feedback, nested/device-side
  branching, transport/topology, native companions, and stronger event
  profiles remain separate capabilities.

### 2026-07-30 — Gate 7 produced-resource rank-8 rectangular launch

- Host-graph program version 1.23 extends
  `resource-u32-rectangular-prefix` to rank 8 while preserving exact 1.0-1.22
  decoding and every older source-specific rank limit.
- CPU reads eight private extents after their writers. WebGPU backend 1.26
  validates all eight values after one resident-buffer prefix stage and reuses
  the version-1.22 mapping, 32-byte guard, maximum program, and exact suffix
  pipeline slots.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 56 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `86ce39dcb21e3845fa0ba7b818b8f9996bc981df93f4cd9cbe75d1f48b45229b`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Repeated/device-side feedback, nested/device-side branching,
  transport/topology, native companions, and stronger event profiles remain
  separate capabilities.

### 2026-07-30 — Gate 7 shared produced-count dispatch fanout

- Host-graph program version 1.24 admits exactly two top-level linear resource
  dispatches sharing one source, rank, mode, and maximum while preserving exact
  1.0-1.23 decoding and their one-feedback-node limit.
- CPU reports both exact semantic completions. WebGPU backend 1.27 reads and
  validates the count once, substitutes both prewarmed dispatch slots, and
  executes one resident-buffer suffix without recompilation or a second stage.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 58 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `7686a372f0d7859b52822ff9bf7343e94ef036869c7b6a35a4844bcb81bf2040`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- Repeated/device-side feedback, nested/device-side branching,
  transport/topology, native companions, and stronger event profiles remain
...
### 2026-07-30 — Gate 7 shared produced-rectangle dispatch fanout

- Host-graph program version 1.25 admits exactly two top-level rectangular
  resource dispatches sharing the exact ordered sources and artifact maxima
  while preserving exact 1.0-1.24 decoding and every older feedback limit.
- CPU reports both exact semantic completions. WebGPU backend 1.28 reads and
  validates eight produced extents once, substitutes both maximum-prewarmed
  rank-8 dispatch slots, and executes one resident-buffer suffix without
  recompilation or a second feedback stage.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 60 complete-output
  CPU/WebGPU parity cases. Correctness artifact
...
### 2026-07-30 — Gate 7 sequential produced-resource feedback

- Host-graph program version 1.26 admits exactly two top-level linear resource
  dispatches with distinct sources only when one dispatch writes the other's
  source, while preserving exact 1.0-1.25 decoding and older feedback limits.
- CPU reads each private source at its ordered dispatch. WebGPU backend 1.29
  executes the first selected dispatch between two bounded readback stages,
  then substitutes the dependent prewarmed slot and executes the suffix over
  the same resident buffers without recompilation.
- Preparation, trace, and pipeline identity now distinguish feedback-node count
  from actual host-mediated stage count: shared fanout reports two/one, while
  the sequential profile reports two/two.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 62 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `959a6ada1766957d9d259e18aae6307ae02658b818761147993324983378bdb7`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- A third stage, repeated conditional/repeat/rectangular feedback, device-side
  feedback, nested/device-side branching, transport/topology, native
  companions, and stronger event profiles remain separate capabilities.

### 2026-07-30 — Gate 7 three-stage produced-resource feedback

- Host-graph program version 1.27 admits exactly three top-level linear
  resource dispatches only when their three distinct sources and two semantic
  producer relations form one chain; it preserves exact 1.0-1.26 decoding and
  older feedback limits.
- CPU reads each private source at its ordered dispatch. WebGPU backend 1.30
  reuses the generic staged executor for three bounded readback/selection
  points over one resident-buffer and fail-stop lifecycle without recompilation.
- Preparation, trace, and pipeline identity report three feedback nodes and
  three actual host-mediated stages; shared fanout remains two nodes/one stage.
- Semantic-core passes 23 files/250 tests and kernels passes 21 files/207 tests.
  Required headed Chromium 148 on Apple Metal 3 passes 64 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `3f08099ff57fe925c35bb0cc37770612061be5bf0397bed79119a086802ecf0f`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
- A fourth or mixed feedback stage, device-side feedback,
...
### 2026-07-30 — Gate 7 signed Worker layout transport

- Worker conformance v2 adds signed rank-8 i8 and f64 semantic dispatches to
  the existing raw-u8 and verified-f32 cases. Four independent one-shot Workers
  re-verify the canonical bytes, own their WebGPU devices, preserve caller
  input snapshots, and publish only complete materialized outputs.
- The in-realm and Worker lanes now share one bounded signed-layout artifact
  constructor, exact single-dispatch graph builder, deterministic byte-pattern
  generator, and independent element-reversal oracle. The helper rejects
  out-of-profile ranks, resource sizes, rank counts, and malformed element
  widths before artifact work.
- Required headed Chromium 148 on Apple Metal 3 passes all four Worker cases
  under backend 1.31. Worker correctness artifact
  `86ec30c8fbe73b451dda627950fcf41efb7c2bcf2be563785bec102b34b6bc77`;
  device profile
  `0436693df95430f4195cb27222e8ebca104c64b3b99ac27c24a5be5ec0f5bc06`.
  All four independent Workers report that same device profile. The lane now
  emits exactly one validated `browsergrad.execution-evidence@1` terminal with
  a closed planned/completed case set, browser and Worker-device environment
  identity, producer versions, explicit complete-output comparison policy,
  output digests, transport invariants, and fail/not-run diagnostics. The
  kernels package passes 24 files/221 tests including five fixture-boundary
  tests and four Worker-evidence mutation tests.
  This remains one Worker per request, not a mesh or cross-worker transport.

### 2026-07-30 — Gate 7 four-stage produced-resource feedback

- Host-graph program version 1.28 admits exactly four top-level linear resource
  dispatches only when four distinct sources and three semantic producer
  relations form one connected chain. It preserves exact 1.0-1.27 decoding
  and rejects disconnected degree-equivalent cycles in addition to every
  existing missing, forked, cyclic, extra, duplicate-source, mixed-mode, and
  pre-version refusal.
- CPU and WebGPU backend 1.32 reuse their generic staged executors for four
  bounded readback/selection points over one resident-buffer and fail-stop
  lifecycle. Preparation, trace, and pipeline identity report four feedback
  nodes and four actual host-mediated stages.
- Semantic-core passes 24 files/252 tests and kernels passes 24 files/221
  tests. Required headed Chromium 148 on Apple Metal 3 passes 80 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `e27040de4badadc0f8facfd2c862524f160249627a0cbe989c0f7261c0a5d30f`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.32 with
  correctness artifact
  `3e0d6275ceee27546f85e49618f00ac42fd8ae8f4fa991ba230f3d8561f28afa`.
- The first required actual-device run exposed one remaining executor check
  that admitted only two or three stages. Generalizing that final check to the
  artifact-owned four-stage ceiling made the same fixtures pass; no alternate
  execution path was added.
- A fifth or mixed feedback stage, device-side feedback,
  nested/device-side branching, worker meshes, native companions, and stronger
  event profiles remain separate capabilities.

### 2026-07-30 — Gate 7 shared conditional/repeat feedback

- Host-graph program version 1.29 admits exactly one produced-u32 conditional
  and one produced-u32 bounded repeat only when both name the same source and
  rank. Different selections, a third consumer, pre-version use, and every
  other mixed profile fail closed.
- CPU reads the private source at each ordered semantic node. WebGPU backend
  1.33 executes the common producer, reads the source once, and applies zero
  to the else branch plus zero iterations or a positive bounded value to the
  then branch plus that repeat count. Both use one resident-buffer, one
  aggregate feedback stage, prewarmed pipeline authority, and fail-stop
  publication.
- The kernels Node and browser lanes share one bounded program fixture, which
  covers two ranks, f32 all-reduce, raw-u8 branch copies, two materialized
  outputs, and zero/two selection. Semantic-core passes 24 files/253 tests and
  kernels passes 24 files/222 tests.
- Required headed Chromium 148 on Apple Metal 3 passes 82 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `7c4c42d6aaa502373d3f2e24f144d1d8f6bb0fb466ffa41249ccefe20aa6ad19`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.33 with
  correctness artifact
  `63451914d93adcf5583838155bf3d72ba4fa44ca5de7654bf906ba4097b30206`.
- Sequential conditional/repeat feedback, a fifth linear stage, other mixed
  profiles, device-side feedback, nested/device-side branching, worker meshes,
  native companions, and stronger event profiles remain separate capabilities.

### 2026-07-30 — Gate 7 sequential conditional-to-repeat feedback

- Host-graph program version 1.30 admits exactly one produced-u32 conditional
  followed by one bounded repeat when both branches guarantee-write its
  distinct count source. Missing branch writes, pre-version use, a third
  consumer, and every other mixed profile fail closed.
- CPU reads the private predicate, executes one branch, then reads that
  branch's count. WebGPU backend 1.34 executes the same semantics through two
  host-mediated feedback stages, prewarmed exact slots, one resident-buffer
  lifecycle, and fail-stop publication.
- Semantic-core passes 24 files/254 tests. Kernels passes 24 files/223 tests,
  both typechecks, and required real-device execution.
- Required headed Chromium 148 on Apple Metal 3 passes 84 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `1960f84defe847f3ee35b925d18ff28d3561ff8d6322e5b72445b09ea53972f1`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.34 with
  correctness artifact
  `7015a08e8b3de8f4ceb57e1b0b41658cd9b0e540c40aee255ca55b578693616e`.
- A fifth linear stage, broader mixed profiles, device-side feedback,
  nested/device-side branching, worker meshes, native companions, and stronger
  event profiles remain separate capabilities.

### 2026-07-30 — Gate 7 sequential conditional-to-dispatch feedback

- Host-graph program version 1.31 admits one produced-u32 conditional followed
  by one positive bounded linear resource dispatch when both branches
  guarantee-write its distinct launch-count source. Rectangular, missing-write,
  pre-version, third-consumer, and other mixed profiles fail closed.
- CPU executes the branch before reading the private launch count. WebGPU
  backend 1.35 shares one conditional-prefix selector across standalone and
  sequential consumers, then specializes the maximum-prewarmed dispatch through
  the second feedback stage under one resident-buffer and fail-stop lifecycle.
- Semantic-core passes 24 files/255 tests. Kernels passes 24 files/224 tests,
  both typechecks, and required real-device execution.
- Required headed Chromium 148 on Apple Metal 3 passes 86 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `76c9d52ef7eec1601b120fb23fd24f058f2dc1795b4a453959415a30bb5081d8`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.35 with
  correctness artifact
...
### 2026-07-30 — Gate 7 sequential conditional-to-rank-2-rectangle feedback

- Host-graph program version 1.32 admits one produced-u32 conditional followed
  by one positive bounded rank-2 rectangular resource dispatch when both
  branches guarantee-write both distinct extent sources after selecting from a
  third predicate resource. Rank 3 or higher, missing-write, shared-source,
  pre-version, third-consumer, and other mixed profiles fail closed.
- CPU executes the branch before reading both private extents. WebGPU backend
  1.36 reuses the shared conditional-prefix selector plus generic resource
  dispatch readback, validation, specialization, and completion under one
  resident-buffer and fail-stop lifecycle.
- Semantic-core passes 24 files/256 tests. Kernels passes 24 files/225 tests,
  typecheck, and required real-device execution.
- Required headed Chromium 148 on Apple Metal 3 passes 88 complete-output
  CPU/WebGPU parity cases. Correctness artifact
  `bb7e5caadb9261cd424314a295aff20dfb0184e711fbe0a73e99848356d7cbc3`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.36 with
  correctness artifact
  `96758e8b7e8042ec036bcb8ace65ad23c9fda25b6d304a408e543ba60589b22f`.
- Broader mixed profiles, device-side feedback, nested/device-side branching,
  worker meshes, native companions, and stronger event profiles remain separate.

### 2026-07-30 — Gate 7 maximum-rank conditional rectangle feedback

- Host-graph program version 1.33 extends the exact branch-produced rectangle
  profile through ranks 3 to 8. Both branches must guarantee-write every
  pairwise-distinct positive bounded extent after selection from a distinct
  predicate resource; pre-version, missing-write, shared-source,
  third-consumer, and other mixed profiles fail closed.
- CPU and WebGPU backend 1.37 reuse the rank-generic rectangle verifier,
  shared conditional-prefix selector, and generic resource-dispatch readback,
  validation, specialization, and completion path under one resident-buffer
  and fail-stop lifecycle.
- Semantic-core passes 24 files/257 tests. Kernels passes 24 files/225 tests,
  typecheck, and required real-device execution.
- Required headed Chromium 148 on Apple Metal 3 passes 90 complete-output
  CPU/WebGPU parity cases. Small/full rank-8 cases prove exact extent and output
  selection through two feedback stages. Correctness artifact
  `7aed748d3ebda3ecd49a522311fa2856fc0962cc7e52027b19fa397e465ddf77`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.37 with
  correctness artifact
  `0160d1919bac516edd4da6818092327a2c1d232a96cc0e4e2a8459b66e8cf2c0`.
- Broader mixed profiles, device-side feedback, nested/device-side branching,
  worker meshes, native companions, and stronger event profiles remain separate.

### 2026-07-30 — Gate 7 generic eight-stage resource feedback

- Host-graph program version 1.34 raises the exact connected linear
  producer-chain ceiling from four to eight feedback nodes. Five through eight
  distinct launch sources must form one root-to-leaf semantic producer chain;
  pre-version, missing, forked, cyclic, extra, duplicate-source, mixed-mode,
  and ninth-consumer profiles fail closed.
- CPU and WebGPU backend 1.38 reuse their generic ordered selection loops.
  WebGPU reads, validates, and specializes each maximum-prewarmed dispatch
  through one resident-buffer and fail-stop lifecycle; no stage-shaped executor
  or callback was added.
- Semantic-core passes 24 files/257 tests. Kernels passes 24 files/225 tests,
  typecheck, and required real-device execution.
- Required headed Chromium 148 on Apple Metal 3 passes 92 complete-output
  CPU/WebGPU parity cases. One/two-element eight-stage cases prove stable
  pipeline identity, distinct specialization, eight ordered completions, and
  eight reported feedback stages. Correctness artifact
  `55c2ea89c19329ef4c522b46666402a65671abbb6b38555e09c0446a22808206`;
  device profile
  `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`.
  The separately required four-case Worker lane re-pins backend 1.38 with
  correctness artifact
...
### 2026-07-30 — Production signer/reviewer separation of duties

- Final release authority is now version 2 and refuses composition when the
  exact build-producer and distribution-reviewer identities match or when both
  statements use the same P-256 public-key hash.
- Successful authority and host-observation projections retain two explicit
  separation facts. They do not claim external key control or organizational
  independence; only actual production policies and statements can establish
  those remaining facts.
- The focused production-release authority passes three tests. The hardened
  exchange passes 12 cases, including valid distinct principals plus exact
  same-identity and same-key refusals. The fast browser-build lane passes 104
  files/825 tests in 15.13 seconds, the complete compiler suite passes 106
  files/1,660 tests in 11.03 seconds, and packed-release checks pass.

### 2026-08-17 — No-private-key production-policy handoff

- `author:browser-production-policies` accepts only one private canonical root
  plus distinct canonical HTTPS producer and reviewer identities. The root must
  initially contain exactly two owned, immutable, single-link P-256 SPKI DER
  files at fixed producer/reviewer names; no caller-selected key path, private
  key, signature, or statement enters the interface.
- The boundary validates each exact public key through Web Crypto, derives its
  SHA-256 key identity, rejects exact identity or key reuse, prepares separate
  trust stores, derives and admits the producer and distribution-approval
  policies, and exclusively materializes those four canonical files plus one
  handoff record. Stable immutable inputs, exact tree closure, independent
  rereads, private directories, no-follow creation, no clobber, synchronization,
  and owned cleanup reuse the distribution output-file boundary.
- The handoff records exact policy, trust-store, public-key, and authored-output
  identities while keeping external key control, signatures, producer trust,
...
while provisioning the distinct externally controlled keys into the fixed
handoff root, authoring their exact policies through the implemented command,
and obtaining the externally issued exact-review statement. Neither
...
observation carries that authority. The next checkpoint is external: provision
the actual public keys, use `author:browser-production-policies`, materialize
the policy-bound distribution, issue the exact immutable request, sign only
through the externally controlled key, and verify the returned envelope in the
same process. Fixture or package-generated signatures remain synthetic and
cannot establish current production status.
...
current nine-case f32/i32/u32 rank-1-through-rank-4 matrix, including unchanged
signed-rank-2 CuTe source, from the promoted reproducible extractor. The exact
distribution convergence lane compiles, authorizes, lowers, and executes that
lineage through the shared CPU and required real-WebGPU seam without a
source-shaped bypass. Its authoring check rederives the current distribution
closure and rejects stale package or software-adapter evidence. The distinct
backend composition seam binds that result and current reproducibility to an
exact independently admitted producer through the stable payload build subject.
The final release seam separately binds that backend result to the approved
current header subset. Both fixtures are synthetic; actual authority issuance
still requires the package-controlled external policies and signed statements.
...
passes 75 layout cases, including canonical positive and signed-affine source
maps for packed8, packed16, word32, and word64 storage at ranks 1 through 8,
without minting source-producer, arithmetic/conversion,
distribution, or release authority.
The exact nine-case distribution lane now
supplies real Worker compilation and source-to-backend convergence, while its
local engineering producer deliberately does not substitute for externally
rooted production authority. Gate 4 now owns the first frontend-neutral
...
reference provides fail-stop whole-graph oracle execution, and the
kernels-owned portable WebGPU adapter now matches it on required actual-device
f32/i32/u32 dispatch/all-reduce evidence without partial output commit.
Whole-allocation copies and terminal materialization are separately versioned
and verified. Dependency-ordered completion events and bounded fixed-count
repetition are also verified. The version-1.5 conditional slice selects from a
captured external-input u32; version 1.6 separately admits an exact
request-time runtime u32 control set captured before execution. Version 1.7
adds one ordered produced-resource u32 predicate and executes it through an
explicit four-byte feedback readback plus resubmission over persistent private
buffers. Version 1.8 adds zero-through-artifact-maximum request-time u32-count
repetition with maximum admission and actual completion/work evidence. Version
1.9 adds positive request-time prefix dynamic dispatch under an artifact
maximum, executes exact prefix work through the CPU reference, and permits one
arbitrary positive one-dimensional prefixes through one runtime-uniform-guarded
maximum-prewarmed program and exact pipeline slot.
Version 1.10 adds one ordered temporary-resource u32 repeat count and executes
zero through the artifact maximum through the same bounded feedback authority,
resident private buffers, and exact prewarmed pipeline slots. Version 1.11
adds a positive produced-resource prefix element count and substitutes only
the validated logical count and matching four-byte uniform through those same
prewarmed slots. Version 1.12 adds positive request-time rank-2/rank-3
rectangular extents and substitutes only true 2D/3D geometry plus a guarded
16-byte uniform through one maximum-prewarmed canonical program and those same
exact slots. Version 1.13 reads one ordered rank-local produced u32 extent per
axis through a single resident-buffer feedback stage and reuses the same
rectangular program, geometry, uniform, and exact slots. Version 1.14 extends
request-time rectangles to rank 4 by mapping the two trailing logical axes to
physical x/y and flattening the two leading selected axes into physical z,
with guarded quotient/remainder reconstruction in the same canonical WGSL
program. Version 1.15 reads all four ordered produced extents through the same
single resident-buffer feedback stage and reuses that rank-4 mapping, program,
uniform, and exact slots. Version 1.16 extends request-time rectangles to rank
5 under the distinct rank-5 view-copy profile, carries five extents in a
32-byte uniform, maps axes 4/3 to x/y, and flattens axes 0 through 2 into z
before guarded mixed-radix reconstruction. Version 1.17 reads all five ordered
produced extents through the existing single resident-buffer feedback stage
and reuses that rank-5 mapping, uniform, program, and exact slots. Version 1.18
extends request-time rectangles to rank 6, maps axes 5/4 to x/y, flattens axes
0 through 3 into z, and reconstructs them behind the same 32-byte guard.
Version 1.19 reads all six ordered produced extents through the existing single
feedback stage and reuses that rank-6 mapping, guard, program, and exact slots.
Version 1.20 extends request-time rectangles to rank 7, maps axes 6/5 to x/y,
and generically flattens axes 0 through 4 into z behind the same 32-byte guard
and exact pipeline authority while retaining the produced-resource rank-6
limit.
Version 1.21 reads all seven ordered produced extents through the existing
single feedback stage and reuses that generic rank-7 mapping, guard, program,
and exact slots.
Version 1.22 extends request-time rectangles to rank 8, maps axes 7/6 to x/y,
and flattens axes 0 through 5 into z through the existing 32-byte eight-u32
uniform, generic coordinate lowerer, maximum program, and exact slots while
retaining the produced-resource rank-7 limit.
Version 1.23 reads all eight ordered produced extents through the same single
feedback stage and reuses that rank-8 mapping, guard, program, and exact slots.
Version 1.24 reads one ordered produced count once and selects two independently
bound linear dispatch slots with the same source and maximum.
Version 1.25 reads one ordered produced rectangle once and selects two
independently bound rectangular dispatch slots with the same sources and maxima.
Version 1.26 executes one selected linear dispatch before reading the distinct
produced source for its dependent dispatch and reports two feedback stages.
Version 1.27 extends the exact producer chain to three distinct sources,
dispatches, and reported feedback stages.
Version 1.28 extends the exact producer chain to four distinct sources,
dispatches, and reported feedback stages, and requires that all three producer
relations form one connected chain.
Version 1.29 separately shares one exact produced `u32` and rank between one
conditional and one bounded repeat, applying both selections after one
aggregate feedback stage.
Version 1.30 executes one produced-resource conditional before reading its
guaranteed distinct branch-produced bounded-repeat count, reporting two
feedback stages under the same resident-buffer and fail-stop lifecycle.
Version 1.31 applies the same exact two-stage contract to one positive bounded
branch-produced linear launch count through the maximum-prewarmed dispatch.
Version 1.32 applies it to two distinct positive bounded rank-2 extent sources
through the same generic maximum-prewarmed resource-dispatch specialization.
Version 1.33 extends that exact specialization through rank 8 while requiring
both branches to guarantee-write every pairwise-distinct extent source.
Version 1.34 raises the exact connected linear-chain ceiling from four to
eight feedback stages through the same generic ordered CPU/WebGPU executors.
The separate device-bound pipeline authority prewarms every exact
step-slot program and conditional alternative under
graph/backend/WGSL/schedule/feature/limit/numerical-policy identity, then
supports exact contiguous ranges, strictly increasing exact-slot selection,
...
The unified host operation now re-verifies both returned statements, rejects
exact producer/reviewer identity or key reuse, and invokes the implemented
backend and final-release compositions before process exit.
The next checkpoint is external: install the package-controlled policies and
externally issued exact statements, then run that operation against those
immutable responses. Externally rooted producer trust, external legal approval,
and actual production authority issuance remain separate prerequisites;
serialized observations, local execution, or synthetic fixtures cannot satisfy
them.
