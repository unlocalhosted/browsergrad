...
| Gate 6 — framework convergence | `in-progress` | Twenty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1 loss now owns strict same-shape floating promotion, bounded reductions, both-input closure/symbolic derivatives, per-example vmap, and resolved ONNX decomposition shared with Grad. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 13 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0024; architecture freeze narrowed from 36/39 to 11/13 while preserving the exact original-ID partition. Current full verification passes JIT 45 files/307 tests, Grad 50 files/342 tests, compiler 96 files/1,602 tests, packed release, and the seven-package Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.l1_loss` are retired
...
registry generates all twenty-six public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `smooth_l1_loss`, reusing the closed loss-reduction resource seam
while adding an exact beta and piecewise quadratic/linear derivative contract. Each
