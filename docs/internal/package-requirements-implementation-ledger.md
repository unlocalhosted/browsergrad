...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, and the fixed fail-stop/no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. The authority-bound CPU reference snapshots all inputs, executes dispatches and explicit collectives in private rank-local storage under memory/operation/time/cancellation ceilings, and exposes outputs only after whole-graph success. | Portable WebGPU graph execution; copy/readback/event/materialization nodes; bounded conditional/repetition control; pipeline authority; browser-worker transport/topology/failure evidence; named native companion profiles; actual portable-device execution and performance evidence. | Semantic-core 23 files/192 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; strict typecheck/lint/build; packed fresh-consumer CPU execution; root/compiler architecture checks with zero cycles/leaks. |
...
reference now provides fail-stop whole-graph oracle execution without becoming
portable-device evidence. The next software checkpoint is a WebGPU graph
executor that consumes exact prepared graph/kernel authority, preserves the
same resource and ordering meaning, handles device loss/cancellation without
partial result commit, and records actual-device evidence separately. Bounded
control, pipeline authority, transport/topology, worker meshes, and native
companion evidence remain later follow-on capabilities.
