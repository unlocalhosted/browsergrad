...
schedule preservation. Gate 5 is the next software workstream. Its first slice
must define a frontend-neutral tiled-attention semantic contract and keep the
existing row-wise online-softmax implementation explicitly named as a baseline.
...
portable-WebGPU execution, and honest re-legalized preservation reporting. The
next software transition is Gate 5's frontend-neutral tiled-attention semantic
contract. It must keep the row-wise online-softmax path named as a baseline,
separate logical Q/K/V and online-softmax meaning from physical staging and
schedule, and define causal masking and numerical policy before backend work.
