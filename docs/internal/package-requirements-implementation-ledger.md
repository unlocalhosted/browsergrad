...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile, producer-neutral request, asset-manifest, compilation-identity, and common-authorization authorities exist. Request v1 binds only compilation semantics, exact copied source snapshots/descriptors, layout-variable or view-copy-function anchor, expected artifact version, and shared semantic/input/output ceilings; deployment identity, Git provenance, conformance hashes, process limits, expected output, and opened closure stay detached. Available dependency/header manifests are distinct from request-observed opened inputs. Runtime ABI pins wasm32/Worker/VFS/memory boundaries. Closed manifest-bound VFS packs supply deterministic files. Docker may reproduce release assets but is not browser runtime. All current identities/assets remain fixtures. | Close CUDA dual-pass and build-provenance/license contracts; compose retained AOT metadata around the common request. Then implement verified fetch/cache/mount, worker invocation/evidence/browser authorization, real build assets, and one unmodified browser-local fixture. | 11 common-request tests plus profile/runtime-ABI/asset/VFS/authorization contracts; zero asset fetches, mounts, worker spawns, or browser-local C++ producer executions. |
| Gate 3 Clang-WASM build and distribution | `audit` | Read-only build audit found no extractor source, CMake/Emscripten build, production VFS-pack writer, Worker bundle, acquisition/mount host, build-provenance/license authority, release asset export, or required browser lane. Candidate LLVM 22.1.8, emsdk/Emscripten 6.0.3, and CUTLASS 3.7.0 revisions remain feasibility inputs only. Current fixtures mix synthetic Clang/libc++ versions and invented byte/feature identities. Local machine lacks Ninja, Emscripten, Binaryen, and TableGen tools; Docker daemon is unavailable. | Before build: model exact CUDA host/device semantic passes and artifact domains; add build-provenance plus file-level license allowlist authority. Then add pinned toolchain lock, extractor/C API, reproducible cross-build, production pack writer, package worker build/assets/notices, and strict browser/release proof. | Repository/source/package/workflow inventory only. No build command, binary, license closure, reproducibility result, or browser execution exists. |
...
Close both pre-build P0 contracts before adding more Docker producer work.
Profile and compilation identity must require explicit CUDA host and device
semantic passes; artifacts/evidence must identify each pass domain and define
deterministic merge ownership. Add strict build-provenance/license authority
behind `buildProvenanceLockSha256`, including exact source/tool/builder locks,
file-level redistributed-header allowlist/notices, build flags, outputs, and
two-clean-build reproducibility evidence. Compose retained optional AOT
source/provenance/conformance metadata around the common prepared request
instead of retaining a second source-intent authority.
