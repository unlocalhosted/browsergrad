...
- **Current implementation slice:** Gate 6 Grad construction/module device convergence
...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0041 classify the original JIT baseline and revised Grad contracts without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad rejects fake bfloat16 before allocation, Linear/Embedding enforce exact float32 parameters, `contiguous()` conditionally preserves identity or materializes a dtype-preserving C-order copy, `detach()` shares exact storage/dtype/layout while severing autograd, floating `to()` casts retain a source-dtype VJP, and tensor device requests never fake a transfer. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention, bfloat16-refusal, contiguous, detach, cast, and tensor-device integration; full package and actual-device evidence remain separately scoped. |
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 rejects fake bfloat16 before allocation, parameter constructors enforce their exact float32 profile, `contiguous()` owns conditional identity/C-order-copy semantics with dtype-preserving backward, `detach()` is a zero-copy storage/dtype/layout-preserving autograd boundary, float16/32/64 `to()` casts retain graph connectivity plus source-dtype VJPs, and tensor CPU/device requests are explicit without silent substitution. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Reconcile `torch.tensor(device=...)`, `nn.Module.to(...)`, and NumPy-interop fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0041; architecture freeze preserves the exact JIT partition and revised Grad contracts. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 102.62 seconds, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 24-test mutation suite pass. |
...
The next Gate 6 software transition is the remaining
`torch.tensor(device=...)` and `nn.Module.to(...)` device boundary, followed
by NumPy interop and generated runtime/profile support consumption. Support
must not be inferred from method presence or duplicated in hand-maintained
tables.
