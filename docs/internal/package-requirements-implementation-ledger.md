...
- **Current implementation slice:** Gate 7 produced-resource rank-5 dynamic domain
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.12 request-time rank-2/rank-3 rectangles, version-1.13 produced-resource rank-2/rank-3 rectangles, version-1.14 request-time rank-4 rectangles, version-1.15 produced-resource rank-4 rectangles, and version-1.16 request-time rank-5 rectangles. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Rank-5 WebGPU keeps axes 4/3 on x/y, flattens axes 0 through 2 into z, and reconstructs them behind a 32-byte eight-u32 guard in one maximum-prewarmed canonical view-copy slot. | Produced-resource rank 5, rank-6-and-higher dynamic domains, repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Semantic-core 23 files/240 tests; kernels 21 files/196 tests plus strict browser/build gates; packed fresh-consumer execution; 42 complete-output CPU/WebGPU parity cases through explicit pipeline authority on Apple Metal 3; correctness artifact `8ee4167935d6bf30d7ec1ab7425e6fcfb8788b656ddf8800e56742afa112a685`; separate backend-1.19 prewarmed performance observation; root/compiler architecture checks with zero cycles/leaks. The full compiler verifier's native matrix passes after commit `c5c3799b`; its independent exact reviewed-bsdtar identity check currently refuses installed tool drift and was not repinned. |
...
repeated/device-side feedback, produced-resource rank-5 or rank-6-and-higher
dynamic domains,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Version 1.12 adds request-time rank-2/rank-3 rectangles, version 1.13 adds produced-resource rank-2/rank-3 rectangles, version 1.14 adds request-time rank 4, version 1.15 adds produced-resource rank 4, and version 1.16 adds request-time rank 5 while preserving all older exact rank limits. All retain maximum-product admission, canonical axis normalization, exact older-version decoding, and logical-extent completion. | Produced-resource rank 5, rank-6-and-higher dynamic domains, repeated/device-side feedback, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/240 tests, including exact request-time rank-2-through-rank-5 and produced-resource rank-2-through-rank-4 CPU rectangles, pre-version/rank/axis/control/source/shape/bound rejection, and hostile extent admission. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.12 through 1.16 execute request-time rank-2-through-rank-5 or produced-resource rank-2-through-rank-4 dense rectangles against the full semantic row-major domain, reserve the artifact-maximum product, and report exact logical extents, product, and rank-multiplied actual work. | None for the current CPU-reference profile; portable WebGPU remains separate. Produced-resource rank 5 and rank-6-and-higher dynamic domains are not inferred. | Focused CPU coverage includes exact partial/full `[3,4]`, `[2,3,4]`, `[2,2,3,4]`, and `[2,2,2,3,4]` request-time rectangles, produced-resource rank-2-through-rank-4 rectangles, and zero/above-bound produced-source refusal before output publication. Semantic-core passes 23 files/240 tests and strict typecheck. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.19 adds version-1.16 request-time rank-5 rectangles through the same maximum-prewarmed canonical view-copy seam. One 32-byte eight-u32 uniform carries five extents; x/y map axes 4/3, z flattens axes 0 through 2, and guarded mixed-radix quotient/remainder reconstructs their coordinates before semantic evaluation. Device admission validates the flattened z product independently. | Produced-resource rank 5, rank-6-and-higher domains, repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Forty-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added small/full request-time rank-5 cases retain pipeline identity, distinct specialization, exact completion, and input/control snapshot isolation. Correctness artifact `8ee4167935d6bf30d7ec1ab7425e6fcfb8788b656ddf8800e56742afa112a685`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. Kernels passes 21 files/196 tests plus strict typecheck and browser typecheck. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.90/5.40 ms and baseline median/p95 1.80/3.50 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.19.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
before guarded mixed-radix reconstruction. The separate device-bound pipeline
authority prewarms every exact
...
a separately bounded produced-resource rank-5 dynamic domain,
rank-6-and-higher domain, or repeated/device-side feedback;
