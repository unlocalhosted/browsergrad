...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict producer, runtime ABI 1.8, exact reproducible factory, zero-import package Worker, production controller, and bounded evidence authorities remain intact. The ordinary harness stays in the low-thirty-second range and direct seven-archive distribution materialization is about 22.4 seconds. The package-pinned normalization path extracts 5,769 source files plus eight exact license/copyright evidence files, emits five independently inspected VFS packs containing 5,768 files/67,470,214 bytes, admits the exact CUDA 12.6.3 redistribution index, rechecks all ten package notices, and writes one canonical per-file review input. The configured header universe and engineering review input are complete; external license/distribution approval, full-distribution reproducibility, signed provenance, valid browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Obtain external approval for the exact prepared file map, CUDA index, and Linux/CUDA evidence; materialize the remaining declared notice outputs; bind approved pack identities into the asset/provenance chain; then prove one unmodified browser-local source through authenticated Worker execution, shared semantics, and real WebGPU. | Capability commits `3acf5b65` through `b371db1a`. The fast gate passes 47 files/431 tests; its focused test phase is 23.55 seconds. Exact pipeline ID: `bg.cpp.browser-header-pack-pipeline.sha256.fbc8e39bb2bf49a5124a10bce434e121a5638d42d60ed7df6075086842a9a154`. The 1,203,103-byte review input has ID `bg.cpp.header-distribution-review-input.sha256.28346afa0239011e988de9cd40818b9eff6bbeeecc6e7e4bf508137697f4dc82` and SHA-256 `0c809a69554731e3b78acc8c0717c1c0939883b8df811637120a067e28be97b7`. Zero valid production Worker compiles or browser-local C++ executions. |
...
| Gate 3 deterministic header-pack selection and assembly | `partial` | The exact plan binds seven archives, eight header subtrees, and eight upstream license/copyright files. Admission streams all 252,406,685 archive bytes. The release-shaped pipeline accepts only the package-reviewed Darwin arm64 `/usr/bin/bsdtar` and Node 25.9.0/Zstandard closure, preserves case-distinct Linux paths in collision-free storage, verifies the configured Clang resource-header output, and writes five canonical packs containing 5,768 files/67,470,214 bytes. It also admits the exact 49,142-byte CUDA index and writes the deterministic `license-inventory.json` that maps every distributed file to exact content, pack, component, package notice, and source evidence. Policy remains non-authoritative: the prepared map still requires external review, and full-distribution reproducibility/release facts remain false. | Obtain external review of the exact prepared input, materialize the remaining declared distribution notice outputs, bind approved pack identities into the asset/provenance chain, then mount only those exact packs inside the package Worker. | Capability commits `6351bcd4` through `b371db1a`; fast gate 47 files/431 tests with strict typecheck/lint/architecture. Direct exact materialization completed twice in about 22.4 seconds with stable pipeline/review identities. Pack hashes remain `fd7fb977...`, `f795494a...`, `4f1c39b7...`, `f66f1284...`, and `d04a460d...`. No license approval, production Worker compile, Docker, or browser execution occurred. |
...
the package-reviewed normalization environment, and the deterministic complete
header review input are now independent narrow authorities. Preserve them
while closing the complete distribution/license closure. External approval,
signed provenance, and exact notice materialization remain separate
authorities; neither reproducible bytes nor a package-generated file map can
substitute for them.
...
Submit the exact prepared file map, CUDA index, and source evidence for external
review. In parallel, materialize the remaining build-lock-declared component
license and aggregate third-party-notice outputs. Once approval exists, bind
the independently reread approved output identities into the asset/provenance
chain and mount only those exact packs in the package Worker.
