...
| Gate 6 — framework convergence | `in-progress` | Twenty-three public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`, and paired `torch.topk` values/indices. Top-k now owns one exact axis/k/largest/sorted contract, a single dtype-safe partial CPU selection, owning values/int64 indices, selected-permutation scatter closure and symbolic VJP, vmap, ONNX selected-k `TopK`/`GatherElements`, and bounded output/workspace/refusal semantics shared with Grad. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 16 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0021; architecture freeze narrowed from 36/39 to 13/16 while preserving the exact original-ID partition. Current full verification passes JIT 42 files/298 tests, Grad 47 files/337 tests, compiler 96 files/1,602 tests, packed release, and the seven-package Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.topk` outputs are retired
...
registry generates all twenty-three public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.scatter`, extending the typed indexing seam with exact
axis, index bounds, duplicate-write/reduction, dtype, derivative, transform,
export, and backend decisions. Each
