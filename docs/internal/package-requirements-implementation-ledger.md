...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. The canonical runtime ABI, asset/build binding, bounded raw-Wasm inspector, BrowserGrad-owned LibTooling tracer source/build integration, exact offline header-pack selection/assembly, Worker-owned VFS, and host-controller lifecycle boundary now exist as closed design inputs. The tracer cannot return `artifact-ready` before VFS/CUDA dual-pass/artifact-v3 integration. The controller enforces byte identity, nonce, first-terminal ownership, absolute wall deadline, cancellation, and terminate/revoke cleanup, but production is unconditionally capability-blocked before caller/platform access while the self-contained package Worker and transferred Emscripten factory are missing. Test execution returns only a disjoint non-authoritative simulation. Header-pack output, license, build, reproducibility, and release authority remain false; current empty first-build projections still prevent production Wasm conformance. Docker/native stays optional build/CI/reference infrastructure outside the portable graph. | Acquire and externally review the exact header/notice bytes; produce the first real extractor build; implement VFS, device-first/host-second tracing, artifact-v3 output, and a self-contained internally pinned package Worker module/factory bundle; capture native browser intrinsics before enabling production; independently repin observed Wasm projections; emit a canonical artifact from unmodified source in-browser; then prove semantic convergence, dynamic tensor copy, and required real WebGPU execution. | Header/VFS assembly: 2 files/19 focused tests. Extractor source/build-plan: compiler build, 29 planner/source tests, and 24 lock tests. Controller/protocol: 2 files/28 tests; focused controller 19 tests. Current Worker/VFS/ABI/profile baseline: 9 files/167 tests; latest full compiler baseline 59 files/1,316 tests. Zero real Clang-Wasm builds, Worker spawns, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock now pins the exact BrowserGrad-owned LibTooling tracer source set and deterministic Emscripten integration. It produces an internal ES-module factory plus Wasm sidecar; separate release blockers require executor-enforced source hash/length verification, deterministic hash-verified sidecar materialization, and bundling the exact factory into the package-owned Worker with host-supplied Wasm instantiation. The tracer has a real closed-VFS review action and exact eight-export state machine, but keeps `artifact-ready` unreachable. Asset manifest 1.1 binds the ABI identity and a standards-aware raw inspector exists. First-build projections, built bytes, header-pack license closure, and reproducibility evidence remain absent. | Implement the source-verifying build executor and distributed-output materializer, runtime VFS bridge, device-first/host-second CUDA actions, artifact-v3 writer, and Worker bundle; perform two locked builds; review and repin complete observed Wasm projections; resolve CUDA/sysroot file-level licensing; authenticate build provenance. | Compiler build; planner/source 2 files/29 tests; lock 1 file/24 tests; exact checked-in source hashes. No executor-enforced source verification, distributed output, build process, output binary, license closure, reproducibility proof, or release authority exists. |
...
| Gate 3 Worker invocation/result protocol | `partial` | Exact profile/asset/VFS/request/ABI/raw-Wasm identities bind one single-use invocation. Pure result validation remains `caller-frame-consistency-only`. The new controller checks package-module bytes before Blob construction, owns nonce/event source, enforces the absolute prepared deadline independently of timer ordering, accepts one terminal event, and cleans timer/listeners/Worker/Blob before any issuer. Production issuance is unconditionally capability-blocked while runtime bytes are missing. Test platforms return only `test-platform-simulation` with `workerExecutionObserved: false`; production/test types, brands, WeakMaps, and unwrap paths are disjoint. | Build and internally pin the self-contained Worker/Emscripten factory module, capture/brand-check native Worker platform intrinsics, then replace the explicit production gate only after real asset and first-build Wasm review. Only that reviewed controller may later mint execution authority; lowering remains separate. | Controller plus protocol 2 files/28 tests, strict typecheck/focused lint, independent final review with no P0/P1. No production issuer is reachable and no Worker was spawned. |
...
lock release-blocked until redistribution/license closure, executor-enforced
source verification/output materialization, complete
independently reviewed first-build interface allowlists, and two distinct clean
builds produce authenticated evidence.

The pure Worker protocol and Worker-owned VFS session are now closed bounded
components. The host-controller lifecycle is also closed, but its production
entrypoint is deliberately capability-blocked and therefore observes no
execution. Next build and internally pin the exact self-contained package
Worker plus transferred Emscripten factory, integrate the VFS session, and
capture/brand-check native browser intrinsics. Then deliberately replace the
production gate and prove terminate-and-replace behavior in a real browser.
Actual Wasm page/allocator instrumentation stays separate from logical live-
open reservations. Only that reviewed production controller may mint browser
execution evidence or feed common lowering authorization; caller-frame
validation remains non-authoritative.

Complete the checked-in LibTooling tracer with the closed VFS bridge, explicit
device-first/host-second CUDA actions, and canonical artifact-v3 writer. Then
produce the first real extractor build from the pinned inputs. The offline
