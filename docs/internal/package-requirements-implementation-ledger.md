...
| Gate 2 — multi-frontend, multi-backend view slice | `partial` | Semantic-core `0.2.0` owns verified view-copy meaning, shared specialization, and the sole canonical frontend construction sink. Kernels `0.2.0` passes the full nine-case CPU/actual-WebGPU bit-exact matrix on Apple Metal 3. The compiler prepares and lowers guarded read-only flat-logical-index bindings without changing frozen IR schemas and passes a six-case CPU/actual-WebGPU matrix over the supported non-padded rank-2/3 profile on the same adapter tier. JIT now emits a separate closed typed permutation-construction request from the same post-fusion traversal without widening the frozen plan. | Make kernels consume JIT requests into constructor-owned layout/kernel artifacts and hashes; route JIT execution through those artifacts; add structured compiler padding only through the L2 guard/fill contract. Re-run strict lanes on the exact release commit. Registry publication remains separate. | Semantic-core 95 tests; kernels 14 files/89 tests; compiler 30 files/966 tests; JIT focused 31 tests; packed bare-import runtime/typecheck; architecture guard; validated kernels nine-case and compiler six-case `passed` terminal records on Apple Metal 3. No two-frontend execution or compiler-padding claim yet. |
...
Strictly parse the emitted JIT side table in kernels, call the semantic-core
dense-permutation constructor, keep `valueId` out of artifact identity, and
route eligible residents through the canonical prepared WGSL without readback
or offset reconstruction. Prove the first rank-2 transpose layout/kernel hashes
against the compiler adapter before expanding the matrix. Re-run retained
strict lanes for the exact release commit; current local evidence does not prove
a future publish.
