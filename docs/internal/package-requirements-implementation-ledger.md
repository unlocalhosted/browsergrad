...
- **Current implementation slice:** Gate 7 higher-rank dynamic domains
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.12 request-time rank-2/rank-3 rectangular dispatch, and version-1.13 produced-resource rank-2/rank-3 rectangular dispatch. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Rectangular WebGPU changes only true 2D/3D launch geometry plus a 16-byte guarded uniform in one maximum-prewarmed canonical view-copy slot; produced extents use one bounded resident-buffer prefix/readback stage and every axis guard precedes semantic evaluation. | Rank-4-or-higher dynamic domains, repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Semantic-core 23 files/239 tests; kernels 21 files/196 tests plus strict browser/build gates; packed fresh-consumer execution; 36 complete-output CPU/WebGPU parity cases through explicit pipeline authority on Apple Metal 3; correctness artifact `4b74011f0f1c1308723d458ec02140ac467299b0196ea3560d316eb67e5e2b15`; separate backend-1.16 prewarmed performance observation; root/compiler architecture checks with zero cycles/leaks. The full compiler verifier's native matrix passes after commit `c5c3799b`; its independent exact reviewed-bsdtar identity check currently refuses installed tool drift and was not repinned. |
...
repeated/device-side feedback, rank-4-or-higher dynamic domains,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Version 1.12 adds request-time rank-2/rank-3 rectangular dispatch; version 1.13 adds one produced-resource rectangle through a distinct ordered rank-local temporary-u32 source per axis under the existing graph-wide feedback bound. Both retain maximum-product admission, canonical axis normalization, exact older-version decoding, and logical-extent completion. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | Rank-4-or-higher dynamic domains, repeated/device-side feedback, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/239 tests, including request-time and produced rank-2/rank-3 normalization/execution, pre-version/rank/axis/control/source/shape/bound rejection, and hostile extent admission. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.12 and 1.13 execute request-time or produced rank-2/rank-3 dense rectangles against the full semantic row-major domain, reserve the artifact-maximum product, and report exact logical extents, product, and rank-multiplied actual work. | None for the current CPU-reference profile; portable WebGPU remains separate. Rank-4-or-higher dynamic domains are not inferred. | Focused CPU coverage includes exact partial/full `[3,4]` and `[2,3,4]` request-time and produced rectangles plus zero/above-bound produced-source refusal before output publication. Semantic-core passes 23 files/239 tests and strict typecheck. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.16 adds version-1.13 produced rank-2/rank-3 rectangles to the backend-1.15 maximum-prewarmed canonical view-copy program. All ordered extent producers execute in one resident-buffer prefix stage; exactly one four-byte u32 per axis is read back and validated before only true 2D/3D geometry plus the existing guarded 16-byte uniform changes in the suffix. Transient budgets charge exact per-axis feedback bytes, axis guards precede semantic evaluation, and device admission covers x/y/z plus total/storage/uniform bindings. | Rank-4-or-higher dynamic domains, repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Thirty-six complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Four added small/full produced rank-2/rank-3 cases retain pairwise pipeline identity, distinct specialization, exact extent/product completion, one feedback stage, and zero/above-bound fail-stop refusal. Correctness artifact `4b74011f0f1c1308723d458ec02140ac467299b0196ea3560d316eb67e5e2b15`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. Kernels passes 21 files/196 tests plus strict typecheck and browser typecheck. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.60/1.80 ms and baseline median/p95 1.30/1.70 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.16.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
rectangular program, geometry, uniform, and exact slots. The
...
a separately bounded higher-rank dynamic domain or repeated/device-side
feedback; transport/topology, worker meshes, native companion execution, and
stronger event profiles remain later independent capabilities.
