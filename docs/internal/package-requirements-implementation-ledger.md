...
`Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.gather`, `Tensor.prod`, `Tensor.repeat`, and
`Tensor.repeat_interleave` are retired
...
registry generates all eleven public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.var`, with canonical correction/axis semantics, stable
reduction VJP, eager conformance, and explicit transform/export/device
