...
- **Current implementation slice:** Gate 5 separate attention performance evidence
...
| Gate 5 — tiled attention flagship | `partial` | Semantic-core owns the closed frontend-neutral `browsergrad.kernel.attention-forward@1` artifact, separate `browsergrad.schedule.attention-online-kv-tile@1` artifact, schedule-independent CPU oracle, and authority-bound schedule specialization. Logical meaning binds static rank-4 f32 Q/K/V/output views, exact scale, masks, stable-softmax phases, finite-domain requirements, comparison policy, effects, and VJP refusal. The schedule independently binds physical tiles, cooperative K/V staging, increasing traversal, tile-wise rescaling, uniform barriers, scalar vectors, and pre-state-update masks. The CPU path snapshots admitted inputs, rounds every declared f32 step, fails atomically, and implements the comparator. Schedule specialization derives bounded workgroup, staging, private-state, key-tile, and dispatch geometry from the exact logical/schedule authorities. Kernels prepares and executes exact schedule-bound scalar WGSL with cooperative K/V staging, private Q/output state, causal/tail exclusion before recurrence, uniform barriers, retained finite host snapshots, device-limit admission, scoped diagnostics, and bounded publication. | Add a separate named-device/browser performance record against the row-wise online-softmax baseline without converting an observation into an unsupported superiority claim; require the new correctness lane in exact-commit release evidence. | Semantic-core 19 files/166 tests. Kernels passes 19 files/154 tests, build, Node/browser typecheck, lint, architecture, hostile binding/domain/authority/resource/device tests, packed-release consumption, and a required headed-Chromium Apple Metal 3 lane. The required lane executes causal/non-causal `(B=1,H=2,Sq=9,Sk=11,D=4,Dv=6)` through 8x8 and 8x16 schedules; all four complete outputs pass the declared CPU and same-mask cross-schedule comparator. Commits `4cfacd75`, `1ccd541e`, `de80103d`, `fb438082`, and `e2ce169d` passed CI runs `29820268289`, `29820944140`, `29822236114`, `29822619976`, and `29823343925`. No performance, FlashAttention-v2, frontend convergence, or Gate 5 exit claim. |
...
schedule preservation. Gate 5 is now partial. Its first capability defines the
...
are implemented, with required causal/non-causal two-schedule correctness on a
named device/browser. The current slice is separate performance evidence
against the frozen row-wise baseline.
...
- Required headed Chromium on Apple Metal 3 executes causal and non-causal
  ...
  device lane. Performance remains a separate next capability; no
  FlashAttention-v2 or superiority claim is made.
...
Gate 5 now owns the separate frontend-neutral attention-forward semantic
contract and keeps the row-wise online-softmax path named as a baseline. It
also owns a separate verified tiled-attention schedule bound to the exact
logical hash, with physical query/key tiling, cooperative K/V staging, boundary
participation, uniform barriers, and tile-wise online-softmax recurrence. The
schedule-independent CPU oracle and comparison policy now prove causal and
non-causal logical meaning without reconstructing semantics from that physical
schedule. The next software transition is the bounded schedule specialization
and block-tiled WebGPU lowering consuming the exact prepared logical and
schedule authorities; execution and preservation tiers remain false until
