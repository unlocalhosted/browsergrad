...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. A closed, opaque, resource-bounded AOT profile verifier now separates extractor, compiler, dependency, VFS, target, supported-feature, typed-unsupported-family, and detached-provenance policy. No real-source compatibility claim has been made. | Complete the frontend artifact and provenance verifiers; pin real source/toolchain/header inputs; prove shared layout semantics, CPU, and WebGPU; preserve typed target intrinsics; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; 13 focused hostile profile tests; existing compiler corpus and `verify:real-world-cuda` gates remain audit inputs, not Gate 3 proof. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Audit selected an ahead-of-time resolved frontend artifact as the first deployment mode. Its profile verifier is closed and authority-bound; raw flags are replaced by ordered typed options, source sets stay artifact-bound rather than creating one toolchain profile per source, extractor identity is distinct from compiler identity, and provenance remains a detached allowlisted-attestation policy rather than a self-asserted trust bit. | Closed artifact and attestation verifiers, hermetic producer, layout tracer, dynamic tensor copy, exact Gate 2 hash equality, required WebGPU evidence. | 13 focused hostile profile tests; compiler typecheck/lint; three independent read-only audits agree current regex/motif handling is not CUTE-002 evidence. |
...
Implement the closed AOT C++/CuTe frontend artifact and detached provenance
verifiers, then lower a pinned layout-only resolved fact into semantic-core. Keep the frozen
