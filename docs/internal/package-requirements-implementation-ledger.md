...
- **Current implementation slice:** Gate 7 sequential conditional-to-dispatch feedback
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exact shared-selection fanout; versions 1.26/1.27/1.28 permit exact two-/three-/four-stage linear chains; version 1.29 shares one conditional/repeat source; versions 1.30/1.31 require both conditional branches to guarantee-write one distinct repeat or linear-launch source. Earlier versions retain their feedback bounds and rectangle-rank limits. | Broader mixed or device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/65 tests, including one/two conditional-produced linear dispatch, zero/two conditional-produced repeat, pre-version and missing-branch-write refusal, shared fanout, producer chains, and rank-2-through-rank-8 rectangles. Full semantic-core passes 24 files/255 tests. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed ceilings, and publishes only after success. Versions 1.30/1.31 read the predicate, execute the selected branch, then read its guaranteed distinct repeat or positive linear-launch count. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves conditional-produced repeat/launch selection plus shared fanout, two-/three-/four-stage linear chains, refusal, and rank-2-through-rank-8 rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.35 adds version 1.31's conditional-to-linear-dispatch profile and shares one conditional-prefix selector across standalone and sequential consumers. It reads the predicate, executes one prewarmed branch, reads its distinct launch count, and specializes the maximum-prewarmed dispatch over the same resident buffers without recompilation. | Broader mixed or device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Eighty-six complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. One/two branch-produced launch cases retain pipeline identity, distinct specialization, exact branch/launch completions, snapshot isolation, two feedback nodes, and two stages. Correctness artifact `76c9d52ef7eec1601b120fb23fd24f058f2dc1795b4a453959415a30bb5081d8`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
