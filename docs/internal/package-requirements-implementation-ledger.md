...
- **Current implementation slice:** Gate 7 sequential produced-resource feedback
...
These profiles do not claim timestamps, external waits, a fourth or mixed
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exactly two top-level produced-resource linear/rectangular dispatches only for an exact shared selection. Version 1.26 permits two distinct linear sources only when one dispatch writes the other's source; version 1.27 permits three distinct linear sources only when two semantic producer relations form one exact chain. Earlier versions retain their feedback bounds, and versions 1.12 through 1.23 preserve every older exact rectangle-rank limit. | Broader mixed/device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact shared-count/shared-rectangle fanout, one/two-element two- and three-stage producer-chain CPU execution, pre-version, missing-producer, duplicate-source, and mismatched-source/maximum refusal, plus request-time/produced-resource rank-2-through-rank-8 rectangles. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.24/1.25 execute both dispatches from one shared selection; versions 1.26/1.27 read every distinct selection only after its preceding selected dispatch executes and report two or three exact completions. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves single dispatch, exact shared-count/shared-rectangle fanout, one/two-element two- and three-stage sequential feedback, invalid selection refusal, and partial/full rank-2-through-rank-8 rectangles from both control sources. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.30 extends sequential linear feedback to version 1.27: it reads, validates, specializes, and executes each exact prepared dispatch before reading the distinct source that dispatch produced for its dependent, all over the same resident buffers without recompilation. Public preparation/trace evidence separately reports feedback nodes and actual host-mediated stages. | Broader mixed/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Sixty-four complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two- and three-stage one/two-element cases retain pipeline identity, distinct specialization, exact completions, snapshot isolation, and matching feedback-node/stage counts; shared fanouts report two nodes but one stage. Correctness artifact `3f08099ff57fe925c35bb0cc37770612061be5bf0397bed79119a086802ecf0f`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
transport profile or a separately bounded fourth/mixed feedback-stage contract;
transport/topology, worker meshes, native companion execution, and stronger
event profiles remain later independent capabilities.
