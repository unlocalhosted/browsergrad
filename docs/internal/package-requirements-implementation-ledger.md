...
- **Current implementation slice:** Gate 7 shared conditional/repeat feedback
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, shared selection fanout, rank-2-through-rank-8 request-time/produced-resource rectangles, shared rank-8 rectangle fanout, exact two-, three-, and four-stage producer chains, and one exact shared produced-u32 conditional/repeat profile. CPU and portable WebGPU execute through private storage and one device-bound pipeline authority. Exact same-dtype dispatch covers all 13 built-in storage dtypes, with signed rank-8 composition across all four storage widths. The one-shot dedicated-browser-Worker transport re-verifies canonical bytes, owns its device, and preserves the same graph executor and fail-stop terminal contract. | Broader/sequential mixed or device-side feedback, nested/device-side branching, worker meshes/cross-worker synchronization, native companion profiles, broader collective arithmetic, and stronger event synchronization remain open. | Required headed Chromium passes 82 complete-output CPU/WebGPU parity cases under backend 1.33 with correctness artifact `7c4c42d6aaa502373d3f2e24f144d1d8f6bb0fb466ffa41249ccefe20aa6ad19`. Separate Worker conformance v2 passes raw-u8, f32, signed-rank-8 i8, and signed-rank-8 f64 cases with artifact `63451914d93adcf5583838155bf3d72ba4fa44ca5de7654bf906ba4097b30206`. Its strict execution-evidence terminal closes the case manifest, environment identity, producer versions, comparison policy, output digests, per-Worker device agreement, and failure/not-run diagnostics; hostile lifecycle tests and the backend-1.20 prewarmed performance observation remain separate. |
...
These profiles do not claim timestamps, external waits, a fifth, sequential
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Versions 1.24/1.25 permit exactly two top-level produced-resource linear/rectangular dispatches only for an exact shared selection. Version 1.26 permits two distinct linear sources only when one dispatch writes the other's source; version 1.27 permits three distinct linear sources only when two semantic producer relations form one exact chain; version 1.28 permits four distinct linear sources only when three relations form one connected exact chain. Version 1.29 permits exactly one produced-u32 conditional and one repeat only when both share the exact source and rank. Earlier versions retain their feedback bounds, and versions 1.12 through 1.23 preserve every older exact rectangle-rank limit. | Broader/sequential mixed or device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/63 tests, including exact shared-count/shared-rectangle fanout, one/two-element two-, three-, and four-stage producer-chain CPU execution, zero/two shared conditional/repeat execution, pre-version, missing-producer, disconnected-cycle, duplicate-source, and mismatched-source/rank/maximum refusal, plus request-time/produced-resource rank-2-through-rank-8 rectangles. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.24/1.25 execute both dispatches from one shared selection; versions 1.26/1.27/1.28 read every distinct selection only after its preceding selected dispatch executes and report two, three, or four exact completions; version 1.29 reads one private selection for both its conditional and repeat. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves single dispatch, exact shared-count/shared-rectangle fanout, one/two-element two-, three-, and four-stage sequential feedback, zero/two shared conditional/repeat selection, invalid selection refusal, and partial/full rank-2-through-rank-8 rectangles from both control sources. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.33 adds version 1.29's shared conditional/repeat profile: it executes the ordered producer, reads one private u32 once, and applies the selection to both prewarmed consumers over the same resident buffers without recompilation. Public preparation/trace evidence separately reports feedback nodes and actual host-mediated stages. | Broader/sequential mixed or device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Eighty-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Zero/two shared conditional/repeat cases retain pipeline identity, distinct specialization, exact branch/repeat completions, snapshot isolation, two feedback nodes, and one feedback stage. Two-, three-, and four-stage linear cases retain matching node/stage counts. Correctness artifact `7c4c42d6aaa502373d3f2e24f144d1d8f6bb0fb466ffa41249ccefe20aa6ad19`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
separately bounded sequential mixed-feedback profile or worker-mesh/native
