...
- **Current implementation slice:** Gate 3 authorized static CuTe layout tracer and hermetic producer boundary
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers preserve exact resolved facts. Detached provenance uses standard DSSE PAE around an in-toto Statement v1 with a BrowserGrad-specific predicate, profile-pinned trust root, exact runner/container/invocation projections, authenticated subject/policy checks, and an instance-bound authorized wrapper. A compiler lowerer now accepts only that wrapper and one explicit selected layout entry, rechecks unique root/fact ownership, folds bounded static CuTe integer algebra, preserves nested hierarchy through explicit coordinate composition, and prepares the allocation-free semantic-core layout. Pure semantic hashes exclude provenance; compiler origin hashes retain exact artifact/profile/attestation/span/macro lineage. No real producer, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists yet. | Pin a real producer profile/source/header closure; implement and sandbox the hermetic producer; bind dynamic tensor/storage meaning; prove exact shared-semantic CPU/WebGPU parity; add dynamic tensor binding; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; 47 focused profile/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 39 files/1053 tests plus typecheck/lint and architecture/corpus gates; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
Pin the first real C++/CuTe producer profile, source fixture, virtual header
closure, compiler resource directory, extractor binary, container manifest,
runner, and sandbox contract. Implement a hermetic producer that emits the
already-verified artifact/provenance formats without adding a second browser
parser or widening the frozen CUDA-lite normalizers. Keep the three stale
direct-publish guidance edits isolated because their files contain overlapping
user-owned work.
