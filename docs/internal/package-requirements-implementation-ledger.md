...
- **Current implementation slice:** Gate 7 dynamic launch contract
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, whole-allocation copies, terminal fail-stop materialization, dependency-ordered completion events, bounded fixed-count and request-time u32-count repetition, captured-input and request-time runtime-control u32 conditionals, one ordered produced-resource u32 conditional, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors run dispatches, copies, collectives, fixed/runtime repeats, and one selected branch in private rank-local storage under explicit ceilings, report actual selected work only with whole-graph success, and expose only verified materialized outputs. The WebGPU adapter pre-lowers every branch and artifact-maximum repeat schedule, selects input/runtime controls before device work, and executes the produced-resource predicate through one exact prefix submission, four-byte readback, and selected branch-plus-suffix submission over persistent private buffers. Its separate device-bound pipeline authority prewarms every exact step-slot program and alternative, authorizes complete, contiguous-range, or strictly increasing exact-slot selections, binds graph/backend/WGSL/schedule/features/limits/numerical-policy identity into the cache namespace, rejects forged/destroyed/cross-device/invalid-offset/index/mismatched use, and is reused by explicit hot execution and the convenience path. Complete f32/i32/u32 collective, fixed/runtime repeated f32 sum, event-marked raw-u8 materialization, and both branches of all three predicate sources have actual-device parity. A separate required real-device record compares prewarmed fixed repetition with an equal-work, bit-exact statically unrolled graph without asserting superiority. | GPU/backend-derived loop control, dynamic launch, and nested/device-side branching; browser-worker transport/topology/failure evidence; named native companion profiles. Event timestamps, external waits, and cross-queue/cross-worker synchronization require separately named future profiles. | Semantic-core 23 files/219 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/184 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; fourteen complete-output CPU/WebGPU parity cases through explicit pipeline authority plus non-finite refusal and actual device-loss/authority-lifecycle refusal on Apple Metal 3; generic real-device contiguous-range and gapped exact-slot authority coverage; correctness artifact `1aa397fa49b314ac2de6dcf55b48fbe1950fe91d27fe2c418e392444df67574e`; separate prewarmed two-rank 65,536-element f32 authority-reuse performance observation with 8 warmups and 12 paired samples on the same device; root/compiler architecture checks with zero cycles/leaks. |
...
prewarmed pipeline slots while reporting actual work. These profiles do not
claim timestamps, external waits, GPU/backend-derived loops, dynamic launch,
nested/device-side branching, or transport. Separate device-bound pipeline authority is
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies. Versions 1.5 and 1.6 add bounded captured-input and request-time runtime-control u32 conditionals. Version 1.7 adds at most one produced-resource u32 conditional whose exact rank-local aligned four-byte temporary predicate requires a dominating ordered writer. Version 1.8 adds zero-through-artifact-maximum request-time u32-count repetition, charges the maximum expanded envelope, and treats body writes as non-guaranteed because zero iterations perform no write. All control profiles retain globally unique node IDs, canonical effects/hazards, bounded work, and exact older-version decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. GPU/backend-derived loops, dynamic launch, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/219 tests, including canonical older-version retention plus repeat and conditional version/mode/predicate/control/identity/body/shape/effect/order/count rejection, zero-iteration guaranteed-write refusal, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and required runtime control before its first await, runs topologically ordered view-copy dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, exact fixed/runtime repeats, and one input-, runtime-control-, or produced-resource-selected conditional branch with cancellation/time checks at every admitted boundary. Resource predicates are read from private graph storage only after ordered producer execution. Missing, duplicate, unknown, greater-than-u32, or above-artifact-repeat-bound controls fail before input copying or execution. Preparation admits maximum repeat work; successful results report the actual captured repeat count and actual executed operation total. Completed event IDs, repeat records, conditional branch records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, maximum repeated or exact one-branch element/copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller input/control mutation isolation, strict runtime-control admission, copied-byte budget refusal, terminal output publication, successful completion reporting, exact fixed three-iteration f32 sum, zero/two-iteration request-time f32 sum with actual work counts, both branches from all three predicate sources, ordered resource production, and pre-admission maximum-operation refusal. Semantic-core passes 23 files/219 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed/runtime-repeat/conditional execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and fixed or artifact-maximum runtime repeat bodies into bounded canonical WGSL sequences, and pre-lowers every conditional branch through those same paths. `browsergrad.host-graph.webgpu-pipeline@1` separately binds the exact prepared graph to one admitted device, prewarms every unique program at each exact step slot including conditional alternatives, and records graph/backend/WGSL/schedule/features/limits/numerical-policy identity. Explicit hot execution reuses that authority; the convenience API captures caller inputs/controls and then delegates through the same ephemeral path. Complete sequences, validated contiguous ranges, and strictly increasing exact-slot selections reject copied, destroyed, cross-device, reordered/duplicate, invalid-offset/index, or unadmitted program use before request resource allocation. Request-time controls select before input copying or device work. Runtime repeats omit only artifact-prewarmed slots, retain maximum prepared budgets, and report actual submitted steps/counts/completion in specialization and terminal trace. The version-1.7 resource predicate keeps private graph buffers resident across an exact prefix submission, four-byte predicate readback, and selected branch-plus-suffix submission; the same timeout, cancellation, in-flight owner, device-loss, cleanup, numerical-status, and terminal-publication contract spans both stages. Events and materialization contribute no GPU step. | None for the current portable DAG/fixed/runtime-repeat/three-predicate/pipeline-authority profile. Non-word-sized copies fail explicitly; GPU/backend-derived loops, dynamic launch, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Fourteen complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities and cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions plus zero/two request-time repetitions of f32 sum, and both branches of captured-input, runtime-control, and produced-resource predicates after caller mutation. Runtime-repeat pairs retain one request-independent pipeline identity, distinct exact-slot specialization identities, and actual trace counts; resource branches report one feedback stage. Correctness artifact `1aa397fa49b314ac2de6dcf55b48fbe1950fe91d27fe2c418e392444df67574e`. Separate checks reject non-finite f32, a lost device, copied authority, and post-destroy reuse. Generic real-browser coverage also rejects cross-device, wrong-offset, non-increasing indices, and program-mismatched pipeline-set use. Kernels passes 21 files/184 tests; packed/fresh-consumer coverage prepares and executes graph control through the public package seam. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.70/2.70 ms and baseline median/p95 1.90/2.80 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.9.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
repetition with maximum admission and actual completion/work evidence. The
separate device-bound pipeline authority prewarms every exact
...
dynamic launch or GPU/backend-derived looping beyond the bounded resource
predicate and request-time count; transport/topology, worker meshes, native companion execution, and
