...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is now the primary portable-product producer. Existing source-spelling CuTe adapters remain frozen. Only the canonical artifact/resource verifier and semantic/static-layout lowering are producer-neutral today; profile, request, authorization, and provenance remain AOT-shaped. No tracked Clang-WASM build, browser worker, packaged compiler/header assets, browser-local profile/request/common authorization, browser-local producer, real CuTe artifact, dynamic tensor/storage meaning, or browser-local source-compatibility evidence exists yet. Docker/native execution-environment, plan, OCI, observation, receipt, and lifecycle contracts are synthetic optional CI/reference-producer infrastructure only; they are not browser runtime or Gate 3 exit evidence. | Pin a browser-capable Clang-WASM distribution and exact header closure; define its browser-local profile, request, worker/VFS/resource, and common authorization contracts; emit and verify the canonical artifact from unmodified source in-browser; then prove dynamic tensor copy, exact Gate 2 semantic convergence, and required real WebGPU execution. If the native/AOT producer is retained, prove parity separately without placing it on the portable-product critical path. | Gate 2 exact-source baseline at `aa605421`; producer-neutral artifact and static-layout verifier tests; no browser-local C++ producer test or artifact exists. Existing Docker shell/lifecycle tests prove only synthetic optional-adapter contracts. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile and asset-manifest authorities now exist. The browser profile pins one package-owned dedicated worker contract, one content-identical monolithic Clang/extractor WASM executable, deployment and producer-neutral compilation identities, build-provenance/asset-set locks, VFS closure, and profile-owned resource ceilings. The asset manifest admits only same-origin root-relative content-addressed WASM/data packs; remote loader JavaScript is impossible by schema. Artifact semantic identity now binds the producer-neutral compilation contract, and the lowerer consumes a common producer-authorization authority; the existing AOT attestation is only one producer adapter. All current browser identities and assets are synthetic fixtures: no tracked Clang-WASM build, fetched/verified asset authority, unpacker, worker request/result, browser authorization adapter, browser-produced artifact, or browser execution exists. | Select and license-audit the exact compiler distribution and build recipe; replace fixture hashes with release-derived assets and build evidence; implement safe fetch/cache/unpack/VFS collision handling; define and implement the closed worker request/result/resource/cancellation authority plus browser authorization adapter; compile one unmodified layout fixture locally in a supported browser and verify artifact/diagnostic/source-map closure. | Closed profile/asset/compilation-contract/common-authorization contracts and focused tests only; zero asset fetches and zero browser-local C++ producer executions. |
...
- Current hashes remain synthetic fixtures. No Clang-WASM distribution,
  license/build recipe, real header pack, safe archive loader, worker request,
  browser producer, or browser execution has been implemented.
...
Implement the browser acquisition and worker evidence boundary before adding
more Docker producer work. Select the exact Clang-WASM distribution and build
recipe; record license, compiler/WASM/worker hashes, CUDA/CuTe header closure,
target ABI, and browser support floor. Replace fixture asset identities with
release-derived bytes and authenticated build evidence. Fetch into a
content-addressed cache, verify before use, and unpack VFS archives with entry,
path, type, size, collision, symlink, and decompression ceilings.
