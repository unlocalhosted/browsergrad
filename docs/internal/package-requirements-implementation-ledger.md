...
required real-WebGPU seam without a source-shaped bypass. This package-pinned
result remains local-engineering observation authority; production still
requires externally rooted producer admission plus a distinct backend
execution authority.
...
required real-WebGPU convergence is already satisfied and package-pinned for
the local-engineering build. Production composition must re-admit that exact
build under the external trust root, consume the current convergence
authority, and issue a separate production backend authority. The external
file-level redistribution decision has a software admission seam but no
production evidence. Externally rooted producer trust, external legal approval,
production backend authority, and final release composition remain separate
prerequisites; reproducibility or local execution cannot satisfy them.
