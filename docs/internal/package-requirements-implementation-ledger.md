...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The current extractor is package-pinned from two byte-identical cache-free builds, and the strict eight-case Chromium matrix now uses one package-owned exact compile profile rather than a synthetic fixture. The five-pack/5,788-file header universe, Runtime ABI 1.17, both zero-import Workers, unchanged C++17/CuTe f32/i32/u32 rank-1-through-rank-4 sources, accepted Artifact V3 outputs, and distinct semantic view-copy candidates are content-bound. CPU and actual WebGPU independently share the canonical word32 view-copy seam. A package-owned verifier covers the exact 24-path complete distribution plan while separating 23 deterministic subjects from its one detached DSSE envelope. The producer path now issues exact policy-scoped format-only signing material for the current build. A separate policy-rooted verifier admits an externally signed distribution decision only for the exact current 17-output header-review subject. Docker stays outside the portable runtime graph. | Admit the exact observed candidates through externally rooted producer trust and shared CPU/required real-WebGPU execution. Install the production producer and distribution-approval policies, externally controlled keys, and externally issued exact-build and exact-header-distribution statements. Run the complete-output verifier over two live full distributions and issue final release authority without collapsing those authorities. | Capability commits through `e722434c`; current lock `bg.cpp.browser-build-input-lock.sha256.5a96def9bac1db052108142dfe4c82e729f4b41f450d459406a4f3c5227daad7`; ABI manifest `bg.cpp.browser-runtime-abi.sha256.73131ff8285e0e66e4c7d74786b85c5cc565c937acb6923c1b1a91df5ae4137a`. Workflow `30069614333` produced identical 31,841,008-byte Wasm at `19edd5622461b2308e83f10fb90f9f029241a5ba706e4c1741b194cb52a82138`. The 26,213-byte strict resource is `4d8b956050834e550405b15f1c7d52b16927ee3e8d4bc7b7da4035d430edc80a` at source `e9062c20f2a070774743e4d839c275c05df47225`. Fast gate 92 files/771 tests in 14.23 seconds; complete compiler 101 files/1,640 tests in 14.03 seconds; architecture zero cycles/leaks. Production producer trust, production license/distribution approval, lowering authority, backend execution for the exact observed payload, live full-distribution reproducibility, and `releaseReady` remain false. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the extractor closure, LLVM 22.1.8, Emscripten image, recipe, ABI, and exact 24-path distribution plan. Cached diagnostics remain untrusted and separate from clean/release authority. The exact reviewer rejects interface drift after preserving bounded evidence. The current extractor is package-pinned from two distinct-path cache-free builds; the exact 17-file header subset is independently rehashed across two roots and package-pinned. The complete-output verifier rehashes two exact private trees, requires all 23 deterministic subjects to be byte-identical, and permits the sole detached DSSE bytes to differ only when strict-decoded build subjects and build-lock references match. Its opaque authority keeps signing, licensing, distribution, trust, execution, lowering, backend, and release false. The distribution-approval verifier separately binds an exact external decision to the current header-review subject without upgrading full-output reproducibility or release. | Install the package-controlled production approval policy, externally controlled reviewer key, and externally issued exact-review statement. Produce two live complete distributions, pass the 24-output verifier, and retain both authorities separately from final release approval. | Commits `8eea3660` and `b1caf988`; current lock `bg.cpp.browser-build-input-lock.sha256.5a96def9bac1db052108142dfe4c82e729f4b41f450d459406a4f3c5227daad7`; runtime ABI manifest `bg.cpp.browser-runtime-abi.sha256.73131ff8285e0e66e4c7d74786b85c5cc565c937acb6923c1b1a91df5ae4137a`; header reproducibility ID `bg.cpp.browser-header-distribution-reproducibility.sha256.4d4c054fd4c93dbdbdef9581eeac52b037af3425e6a1c7eff8acc585abce1e55`; green reproducibility workflow `30069614333`; Wasm SHA-256 `19edd5622461b2308e83f10fb90f9f029241a5ba706e4c1741b194cb52a82138`. The complete-output verifier passes four focused adversarial cases and the approval verifier passes seven, but neither live full-distribution nor production external-approval authority exists yet. |
...
| Gate 3 external header-distribution approval | `partial` | One bounded host-only policy admits an exact policy ID, trust-store hash, reviewer allowlist, and key allowlist. The verifier reauthenticates the current package-pinned 17-output distribution, derives the exact current review subject including `license-inventory.json`, verifies one canonical DSSE/in-toto P-256 P1363 decision, and mints an opaque authority that upgrades only the external legal-review and distribution claims for that policy and subject. Signing material is format-only; structural copies, stale subjects, wrong roots, malformed policies, hostile accessors, unsafe buffers, and cancellation fail closed. | Replace the synthetic fixture with a package-controlled production policy, externally controlled reviewer key, and externally issued statement over the exact current subject. Retain this authority separately from full-distribution reproducibility, producer trust, execution, backend, and release. | Commit `b1caf988`; seven focused adversarial tests, strict typecheck/lint, 91-file/765-test fast gate, 100-file/1,634-test complete compiler suite, and zero-cycle/zero-leak compiler and workspace architecture checks pass. No production policy, external key, or external decision is installed. |
...
Its signing request is format-only, and the synthetic test fixture establishes
no production approval. Preserve these authorities while installing the
package-controlled production policy, externally controlled reviewer key, and
externally issued exact-review statement. Neither reproducible bytes nor a
package-generated file map, notice bundle, signing request, or signature can
substitute for independent approval.

The manifest-pinned build-signature verifier, exact policy-scoped signing
request, and separate host-only trust-policy transition are implemented.
Producer authority requires the exact opaque signature binding plus an
independently admitted predicate, trust-store hash, builder ID, key ID, and
policy version; neither the request nor signature binding claims trust. The
next checkpoint is operational: install the package-controlled production
policy, then sign through an externally controlled key and verify the issued
statement over the exact current build subject. Fixture or package-generated
signatures remain synthetic and cannot establish current production status.
...
Install the production distribution-approval policy and have the external
reviewer sign the exact prepared subject containing the file map, CUDA index,
source evidence, notice identities, and license inventory. Then verify that
statement and bind the independently reread approved output identities into
the asset/provenance chain before mounting those exact packs in the package
Worker.
...
production evidence. It and a live two-root run of the complete 24-output
reproducibility verifier remain separate release prerequisites.
