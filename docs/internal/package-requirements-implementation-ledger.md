...
- **Current implementation slice:** Gate 6 generated Grad platform-support source
...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. JIT now projects the same executable registry into detached JavaScript and framework-neutral platform records. Grad schema v2 has no remaining compatibility-debt behavior: it owns a closed twelve-dtype registry, separates direct and torch constructor inference/ownership, implements zero-stride expand views, rejects fake bfloat16, fixes contiguous/detach/cast semantics, keeps all eager placement CPU-only and fail-closed, zero-copy wraps every supported writable NumPy storage dtype/layout, and emits only owning NumPy snapshots. Runtime generates all 53 requirement definitions from the architecture vocabulary, creates complete provider-bound environments, accepts them directly in run-plan/preflight/matrix/profile-runner APIs, and preserves relevant canonical records through plans, reports, handoffs, external requests, and benchmark rows. Semantic-core and runtime distinguish static capability/backend definitions from program/artifact-scoped lowering decisions; runtime support views can be constructed only from actual decisions for one exact subject. The new dependency-safe platform view retains requirement, program, and framework facts independently and accepts multiple framework sources without importing them. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Generate Grad's framework-neutral source from its frozen executable compatibility contracts; keep terminal execution evidence separate. | ADR-0002 and ADR-0004 through ADR-0050; architecture freeze preserves the exact JIT partition, revised Grad contracts, byte-exact generated requirement and program-capability registries, six direct resolution consumers, five resolution carriers, and architecture implementation/declaration parity. Node 25.9.0 passes the seven-package build in 8.33 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 99.46 seconds, JIT unit tests pass 5 files/27 tests, semantic-core passes 21 files/174 tests, runtime passes 14 files/137 tests, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 28-test mutation suite pass. |
...
`Tensor.var`, top-level `cat` and `stack`, `F.pad`, and both `torch.sort` and
`torch.topk` outputs, overwrite `torch.scatter`, general `torch.einsum`, and
...
`torch.nn.functional.cross_entropy`, `torch.nn.functional.dropout`, and
`nn.BatchNorm1d`, `torch.nn.functional.interpolate`, and
`bg.kernels.attention_forward` are retired from the opaque callback inventory
and have typed CPU, VJP/refusal, transform/refusal, export/refusal, backend, and
residency decisions. The executable v1 framework-operation registry generates
all thirty-six public support records from their validator bindings and
preserves the original opaque-ID partition. No framework-owned NumPy callback
or advertised framework operation remains opaque; only the intentional
user-authored WGSL extension boundary remains. The constructor-only
`webnn_matmul` and `transformer_block` surfaces are retained only as
unsupported-surface identities in the frozen original partition; narrow
`navigator.ml` presence detection remains without an execution claim.
The next Gate 6 software transition is a Grad framework-neutral source derived
from the frozen executable compatibility inventory and fixtures. The runtime
view and generated JIT source are implemented; Grad must map only its proved
eager contracts into the same ten decision categories, with explicit
not-applicable/refusal records where lazy transforms or backends do not exist.
Support must not be inferred from method presence, static backend registration,
requirement availability, or duplicated in hand-maintained tables.
