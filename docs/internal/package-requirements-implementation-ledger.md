...
- **Current implementation slice:** Gate 7 request-time rank-8 rectangular launch
...
rank, mode, and maximum; one readback selects both exact prepared slots.
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Version 1.24 permits exactly two top-level produced-resource linear dispatches only when they share one exact source, rank, mode, and maximum; earlier versions retain one feedback node. Versions 1.12 through 1.23 cover request-time/produced-resource rectangles through rank 8 while preserving every older exact rank limit. | Repeated/device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact shared-count two-dispatch CPU fanout, pre-version and mismatched-maximum refusal, and request-time/produced-resource rank-2-through-rank-8 rectangles. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Version 1.24 executes both independently bound dispatches from one produced count and reports each exact completion; versions 1.12 through 1.23 execute request-time or produced-resource rank-2-through-rank-8 dense rectangles. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves one/two-element single dispatch, exact shared-count fanout, invalid selection refusal, and partial/full rank-2-through-rank-8 rectangles from both control sources. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.27 adds version-1.24 shared-count two-dispatch linear fanout: it reads one ordered u32 after one resident-buffer prefix stage, validates it against the shared maximum, substitutes both prepared dispatch slots, and runs the suffix without recompilation or another feedback stage. | Repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Fifty-eight complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added one/two-element fanout cases retain pipeline identity, distinct specialization, two exact completions, two materialized outputs, and snapshot isolation. Correctness artifact `7686a372f0d7859b52822ff9bf7343e94ef036869c7b6a35a4844bcb81bf2040`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
separately bounded repeated/device-side feedback contract or a transport profile;
