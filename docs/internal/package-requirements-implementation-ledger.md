...
| Gate 6 — framework convergence | `in-progress` | Twenty-seven public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, loss, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. L1 and Smooth L1 now own strict same-shape floating promotion, bounded reductions, both-input closure/symbolic derivatives, nested per-example vmap, and exact ONNX decompositions shared with Grad. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 12 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0025; architecture freeze narrowed from 36/39 to 11/12 while preserving the exact original-ID partition. Current full verification passes JIT 46 files/310 tests, Grad 51 files/344 tests, compiler 96 files/1,602 tests, release packages, and the seven-package Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.nn.functional.l1_loss` plus `torch.nn.functional.smooth_l1_loss` are retired
...
registry generates all twenty-seven public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `binary_cross_entropy`, reusing the closed loss-reduction resource
seam while adding an exact probability-domain and numerical-policy contract. Each
