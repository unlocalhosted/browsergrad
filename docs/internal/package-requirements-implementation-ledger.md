...
- **Current implementation slice:** Gate 7 multidimensional dynamic launch schedules
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core `browsergrad.host-graph@1` verifies bounded multi-dispatch DAGs, per-rank resource multiplicity with exact dtype/bytes/alignment, roles, external-input versus zero-fill initialization, exact dimension bindings, derived view-copy effects and geometry, dependency/lifetime hazards, explicit f32/i32/u32 all-reduce meaning, whole-allocation copies, terminal fail-stop materialization, dependency-ordered completion events, bounded fixed-count, request-time u32-count, and one produced-resource u32-count repetition, bounded positive request-time or produced-resource prefix dynamic dispatch, captured-input and request-time runtime-control u32 conditionals, at most one ordered produced-resource conditional, repeat, or dynamic-dispatch feedback node, and the fixed no-partial-output-commit failure model. Compiler composes opaque prepared view-copy bindings into a bounded linear pipeline. Separately authorized CPU and portable-WebGPU executors run dispatches, bounded dynamic dispatches, copies, collectives, fixed/runtime/produced-resource repeats, and one selected branch in private rank-local storage under explicit ceilings, report actual selected work only with whole-graph success, and expose only verified materialized outputs. The WebGPU adapter pre-lowers every branch, artifact-maximum repeat schedule, and artifact-maximum dynamic launch, selects input/runtime controls before device work, and executes the one admitted produced-resource conditional, repeat, or dynamic dispatch through one exact prefix submission, four-byte readback, and selected branch, repeat, or launch plus suffix submission over persistent private buffers. Its dynamic profile lowers one canonical runtime-prefix uniform guard, reuses the maximum-prewarmed program and exact pipeline slot, changes only validated logical launch geometry plus the four-byte guard, and returns tail invocations before coordinate, address, or resource evaluation. Dynamic uniform memory and total/storage/uniform binding limits are admitted before resource creation. Its separate device-bound pipeline authority prewarms every exact step-slot program and alternative, authorizes complete, contiguous-range, or strictly increasing exact-slot selections, binds graph/backend/WGSL/schedule/features/limits/numerical-policy identity into the cache namespace, rejects forged/destroyed/cross-device/invalid-offset/index/mismatched use, and is reused by explicit hot execution and the convenience path. Complete f32/i32/u32 collective, fixed/runtime/produced-resource repeated f32 sum, event-marked raw-u8 materialization, both branches of all three predicate sources, and 1/2-, aligned 64/128-, and unaligned 65/127-element workgroup-64 request-time and produced-resource f32 dynamic dispatch have actual-device parity. A separate required real-device record compares prewarmed fixed repetition with an equal-work, bit-exact statically unrolled graph without asserting superiority. | Repeated/device-side feedback, multidimensional dynamic launch schedules, and nested/device-side branching; browser-worker transport/topology/failure evidence; named native companion profiles. Event timestamps, external waits, and cross-queue/cross-worker synchronization require separately named future profiles. | Semantic-core 23 files/229 tests; compiler 106 files/1,658 tests plus focused 14-test construction/dynamic-binding coverage; kernels 21 files/190 tests plus strict browser/build gates; packed fresh-consumer CPU execution and WebGPU preparation; twenty-eight complete-output CPU/WebGPU parity cases through explicit pipeline authority plus produced repeat/launch-count limit refusal, uniform-limit refusal, non-finite refusal, and actual device-loss/authority-lifecycle refusal on Apple Metal 3; generic real-device contiguous-range and gapped exact-slot authority coverage; correctness artifact `1626d85876432578d833411e0f198cdd97f946204cf0256312531299d9aaf931`; separate prewarmed two-rank 65,536-element f32 authority-reuse performance observation with 8 warmups and 12 paired samples on the same device; root/compiler architecture checks with zero cycles/leaks. |
...
repeated/device-side feedback, multidimensional dynamic schedules,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies. Versions 1.5 and 1.6 add bounded captured-input and request-time runtime-control u32 conditionals. Version 1.7 adds at most one produced-resource u32 conditional whose exact rank-local aligned four-byte temporary predicate requires a dominating ordered writer. Version 1.8 adds zero-through-artifact-maximum request-time u32-count repetition, charges the maximum expanded envelope, and treats body writes as non-guaranteed because zero iterations perform no write. Version 1.9 adds top-level positive request-time prefix dynamic dispatch under an artifact maximum no larger than the verified semantic element domain, while deriving the same geometry, effects, resources, hazards, and maximum work as static dispatch. Version 1.10 adds `resource-u32-count-sequential`: one exact rank-local aligned four-byte zero-filled temporary u32 with an ordered writer selects zero through a positive artifact maximum; the body cannot access the count resource and body writes are non-guaranteed. Version 1.11 adds `resource-u32-prefix-elements`: the same exact temporary-u32 class selects one through the artifact maximum for canonical prefix dispatch, cannot also be a dispatch data binding, and extends the at-most-one produced-resource feedback bound across conditional, repeat, and dynamic-dispatch control. All control profiles retain globally unique node IDs, canonical effects/hazards, bounded work, and exact older-version decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. Repeated/device-side feedback, broader dynamic schedules, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/229 tests, including canonical older-version retention plus repeat, conditional, resource-feedback, and dynamic-dispatch version/mode/control/bound/identity/effect/order/count rejection, zero-iteration guaranteed-write refusal, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and required runtime control before its first await, runs topologically ordered view-copy dispatches, bounded request-time or produced-resource prefix dynamic dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, exact fixed/runtime/produced-resource repeats, and one input-, runtime-control-, or produced-resource-selected conditional branch with cancellation/time checks at every admitted boundary. Produced-resource predicates, repeat counts, and launch prefixes are read from private graph storage only after ordered producer execution. Missing, duplicate, unknown, greater-than-u32, above-artifact-repeat-bound, zero-dynamic-dispatch, or above-artifact-dispatch-bound controls fail before input copying or execution; zero or above-bound produced launch counts and above-bound produced repeat counts fail before any result publication. Preparation admits maximum repeat and dynamic-dispatch work; successful results report actual captured counts and actual executed operation totals. Completed event IDs, repeat records, dynamic-dispatch records, conditional branch records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, maximum repeated, dynamic, or exact one-branch element/copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller input/control mutation isolation, strict runtime-control admission, copied-byte budget refusal, terminal output publication, successful completion reporting, exact fixed three-iteration f32 sum, zero/two-iteration request-time and produced-resource f32 sum with actual work counts, exact one/two-element request-time and produced-resource dynamic view-copy prefixes with actual work counts, both branches from all three predicate sources, ordered resource production, produced-count refusal, and pre-admission maximum-operation refusal. Semantic-core passes 23 files/229 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified static and bounded dynamic view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and fixed or artifact-maximum runtime/produced-resource repeat bodies into bounded canonical WGSL sequences, and pre-lowers every conditional branch through those same paths. `browsergrad.host-graph.webgpu-pipeline@1` separately binds the exact prepared graph to one admitted device, prewarms every unique program at each exact step slot including conditional alternatives, and records graph/backend/WGSL/schedule/features/limits/numerical-policy identity. Explicit hot execution reuses that authority; the convenience API captures caller inputs/controls and then delegates through the same ephemeral path. Complete sequences, validated contiguous ranges, and strictly increasing exact-slot selections reject copied, destroyed, cross-device, reordered/duplicate, invalid-offset/index, or unadmitted program use before request resource allocation. Request-time controls select before input copying or device work. Runtime repeats omit only artifact-prewarmed slots, retain maximum prepared budgets, and report actual submitted steps/counts/completion in specialization and terminal trace. Request-time and produced-resource dynamic dispatch prewarm one canonical view-copy program with a four-byte runtime-prefix uniform guard before coordinate/address/resource evaluation. Execution changes only the validated logical dispatch count and matching uniform in the same exact slot; the sequence runner submits `ceil(elementCount / workgroupSize)` x workgroups and tail invocations return before semantic work. Every dynamic rank step charges 16 GPU bytes and four host bytes, while pipeline admission derives the actual maximum total, storage, and uniform binding counts across all reachable programs. The version-1.7 resource predicate, version-1.10 resource repeat count, and version-1.11 resource launch prefix each use the one admitted feedback boundary: private graph buffers remain resident across an exact prefix submission, four-byte readback, and selected branch, repeat, or launch plus suffix submission through exact authorized slots. A zero-count repeat with no executable suffix reads outputs directly from the resident graph buffers. The same timeout, cancellation, in-flight owner, device-loss, cleanup, numerical-status, and terminal-publication contract spans both stages. Events and materialization contribute no GPU step. | None for the current portable DAG/fixed/runtime/resource-repeat/request-time-or-produced-prefix-dynamic/three-predicate/pipeline-authority profile. Non-word-sized copies fail explicitly; repeated/device-side feedback, multidimensional dynamic schedules, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Twenty-eight complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities and cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions plus zero/two request-time and produced-resource repetitions of f32 sum, 1/2-, aligned 64/128-, and unaligned 65/127-element workgroup-64 request-time and produced-resource f32 dynamic view-copy dispatch, and both branches of captured-input, runtime-control, and produced-resource predicates after caller mutation. Runtime-repeat, resource-repeat, and all dynamic-dispatch pairs retain their respective graph/pipeline identity while recording distinct specializations and actual trace counts. Resource branches, repeats, and dynamic dispatches report one feedback stage. Correctness artifact `1626d85876432578d833411e0f198cdd97f946204cf0256312531299d9aaf931`. Separate checks reject zero/above-bound produced launch counts and an above-bound produced repeat count without output publication, insufficient uniform capacity, non-finite f32, a lost device, copied authority, and post-destroy reuse. Generic real-browser coverage also rejects cross-device, wrong-offset, non-increasing indices, and program-mismatched pipeline-set use. Kernels passes 21 files/190 tests; packed/fresh-consumer coverage prepares and executes graph control through the public package seam. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.40/2.80 ms and baseline median/p95 1.50/2.60 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.14.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
prewarmed slots. The
separate device-bound pipeline authority prewarms every exact
...
multidimensional dynamic launch schedules beyond the bounded resource
conditional, resource repeat count, request-time count, and request-time or
produced-resource arbitrary one-dimensional dynamic prefix;
repeated/device-side feedback, transport/topology, worker meshes, native
companion execution, and stronger event profiles remain later independent
capabilities.
