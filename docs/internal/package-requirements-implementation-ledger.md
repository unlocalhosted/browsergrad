...
| Gate 6 — framework convergence | `in-progress` | Public `Tensor.expand`, `Tensor.abs`, `Tensor.sign`, `Tensor.sin`, `Tensor.cos`, `Tensor.clamp`, `Tensor.flip`, `Tensor.gather`, `Tensor.masked_fill`, `Tensor.prod`, `Tensor.repeat`, `Tensor.repeat_interleave`, `Tensor.tril`, and `Tensor.var` are retired from `CUSTOM` and emit typed `BROADCAST_TO`, `ABS`, `SIGN`, `SIN`, `COS`, `CLAMP`, `FLIP`, `INDEX`, masked-fill `WHERE`, `PROD`, `REPEAT`, `REPEAT_INTERLEAVE`, `TRIL`, and `VAR`. Expand retains shared eager/lazy conformance and materializing/resident plan routes. Abs/sign preserve real-numeric shape/dtype; sin/cos preserve floating shape/dtype and reject integer drift. Clamp owns finite normalized optional bounds, floating dtype preservation, an inclusive-mask VJP, vmap, and ONNX `Clip`. Flip owns strict single-axis reversal, involutive VJP, batch-axis shifting, and ONNX `Slice`. Gather owns exact axis/int64-index shape and bounds semantics, deterministic scatter-add VJP, paired batch-safe vmap, ONNX `GatherElements`, and shared eager/lazy conformance. Masked fill owns bool-mask source-shape broadcasting, exact source-dtype scalar fill, mask-complement closure/symbolic VJP, batch-safe vmap, ONNX `Where`, and shared eager/lazy conformance. Triangular selection owns rank/dtype/diagonal validation, owning CPU results, idempotent closure/symbolic VJP, batch-safe vmap, ONNX `Trilu`, and shared eager/lazy conformance. Grad indexing/reshape/transpose/permute now preserve float16, bool, and integer storage dtype plus compatible aliasing. Product owns canonical static axes, owning dtype-preserving scalar/tensor results, zero-aware closure/symbolic VJP, batch-safe vmap, ONNX `ReduceProd`, and shared eager/lazy conformance. Repeat owns bounded tile multipliers, block-sum VJP, batch-safe vmap, ONNX `Tile`, and shared eager/lazy dtype conformance. Repeat-interleave owns strict axis/count semantics, selected-axis block-sum VJP, batch-safe vmap, exact ONNX decomposition, and shared eager/lazy dtype conformance. Variance owns canonical axes/correction, owning floating results, centered closure/symbolic VJP, batch-safe vmap, an exact float32 ONNX decomposition, and shared eager/lazy conformance. Host-only typed operations explicitly refuse tensor-plan/WebGPU. JIT's bounded v1 executable operation registry binds all fourteen typed records one-to-one with validators and generates detached public support reporting from those same records. | Migrate the remaining advertised opaque operations through the registry; reconcile the remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate the cross-framework/platform support views from executable contracts. | ADR-0002 through ADR-0014; focused JIT operation/registry/VJP tests; architecture freeze narrowed from 36/39 to 22/25 while preserving the exact original-ID partition. Full regression counts are recorded in the dated verification entries. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`Tensor.repeat_interleave`, `Tensor.tril`, and `Tensor.var` are retired
...
registry generates all fourteen public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `Tensor.triu`, with typed upper-triangular-selection semantics,
stable selection VJP, eager conformance, and explicit
transform/export/device decisions. Each operation must enter the registry with
