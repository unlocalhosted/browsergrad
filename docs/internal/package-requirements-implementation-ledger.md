...
- **Current implementation slice:** Gate 3 fail-closed Docker container lifecycle
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, logical execution plan, artifact, canonical artifact resource, runner receipt, provenance, authorization, static lowering, cacheable OCI metadata, and plan-specific OCI authorization authorities are distinct. The offline-runner input boundary synchronously snapshots exact source bytes; its bounded frame strict-decodes canonical artifact then receipt bytes. A private Node shell now derives three fixed Docker probes from policy, attests the exact client/engine/API/store tuple, cross-binds the selected local `linux/amd64` manifest/config/rootfs projection to authorized OCI metadata, and mints a temporal observation only after bounded child completion and verified private-directory cleanup. The production and fake-test issuers are disjoint. This is fake-adapter contract evidence only: no live daemon/image execution, container enforcement, real producer, real receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Implement the fixed create/inspect/start/wait/result/terminal-inspect/remove state machine and consume the same observation session once; then pin/build and execute the real native extractor, image, environment, source/header closure, and external signing workflow before dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; focused 10 files/105 tests; Docker shell 1 file/76 tests; compiler full gate 44 files/1088 tests plus build/typecheck/lint/architecture/corpus; release-package and whitespace gates; all Docker evidence remains synthetic until a pinned live environment is exercised. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Ahead-of-time profile, pre-run producer request, structural artifact verification, canonical artifact-resource identity, receipt contract, receipt-authenticated provenance, offline source/frame authority, and the bounded local Docker runtime/image observation shell are implemented as separate instance-authorized records. The request pins exact source Git identity, source blobs, VFS paths, declaration anchor/expected entry, and output closure before execution. The one-TU artifact preserves resolved source/type/template/layout/intrinsic facts. Its semantic hash excludes producer metadata; canonical bytes retain independent resource identity. | Container lifecycle and result capture, real native extractor/image/profile/fixture/signing workflow, dynamic tensor copy, exact Gate 2 hash equality, and required WebGPU evidence. | Focused compiler contracts and 76 Docker-shell tests; pinned synthetic request/profile/job/invocation/receipt/raw/semantic identities; compiler typecheck/lint and architecture gates. |
...
| Gate 3 logical AOT execution plan | `verified` | One checked-in deeply immutable policy defines the output-independent Docker contract: one exact local Unix endpoint, authorized-manifest-reference creation with pull forbidden, fixed native supervisor and source root, non-root identity, zero image/override/effective environment, no network or IPC, private remaining namespaces, read-only root/snapshots, no added capabilities/new privileges, no image volumes/devices/socket, bounded mode-1777 tmpfs/result/stderr, profile-derived limits, complete artifact/receipt/OCI/Docker-observation budgets, and external attestation after receipt. Plan hashing re-verifies the current built-in policy identity. Profile and receipt also bind the OCI image-config digest and closed execution-environment manifest. | Container creation must use the authorized manifest reference and prove its actual config/container/mount/argv state implements this logical plan. CPU/peak resource facts require the real pinned in-image supervisor/cgroup observation; fake Docker tests cannot establish them. | Checked-in JSON/TypeScript equality, canonical policy hash `b8c08d81712865a1e7ad1b2e1abc4ea043bc9692291ffc71a8e6e5d8c3ea2816`, execution plan `3dd711a8ed71e1f068fb0d5e2c37930e378afe2986f96467d334ee31633c5ede`, nested immutability, runtime/image probe mutations, receipt/provenance plan/environment/config mutations, strict typecheck/lint. |
| Gate 3 offline-runner I/O authority | `verified` | The only caller inputs are one prepared-job authority plus exact `{ fileId, bytes }` blobs. Captured data descriptors establish membership, kind, and exact length before allocation; intrinsic typed-array checks reject proxies, shared/detached buffers, and prototype-disguised non-byte arrays. Bytes are synchronously copied before the first await, checked by digest and declaration-token anchor, then retained privately. The output boundary accepts exactly one magic/versioned frame with two big-endian u64 lengths, enforces independent artifact/receipt and aggregate ceilings, rejects truncation/trailing output, and strict-decodes the canonical artifact before the receipt using plan-bound complete decode limits. Exposed byte accessors return disposable copies without authority. | The container shell must stage only these snapshots, capture only this frame, remove all run state before returning authority, and prove actual created/terminal container state. | 10 focused runner-plan tests; caller-buffer TOCTOU, descriptor/proxy/species resistance, unknown/duplicate/pre-copy length rejection, typed-array kind/shared/accessor rejection, anchor recheck, accepted/rejected outcome preservation, bounded/noncanonical frame mutations, abort, structural-copy rejection, and copy isolation; compiler full gate. |
| Gate 3 OCI manifest/config metadata authority | `verified` | One cacheable opaque authority snapshots and strict-decodes exact raw OCI manifest/config bytes, derives their SHA-256 descriptors, requires a closed OCI leaf manifest and self-contained distributable layer descriptors, binds the embedded config descriptor to exact supplied bytes, requires `linux/amd64`, exact rootfs layer/diff-ID correspondence, valid bounded history/annotations, and absent/null/empty image execution config. A second opaque authority binds that metadata to one exact prepared plan. A private shell can now create the third temporal authority only from its own fixed, bounded Docker version/info/image-inspect probes. Layer blob bytes and actual live execution remain outside synthetic evidence. | Execute the contract against the exact pinned live environment and recheck the created container's image/config/rootfs plus every effective runtime field before start. | OCI contract tests plus 76 Docker-shell lifecycle/strict-decode/adversarial tests; compiler full gate. No live-daemon claim. |
| Gate 3 local Docker runtime/image observation contract | `verified` | Fixed `/usr/bin/docker` probes run in strict order `version -> info -> image-inspect` against one Unix socket with a closed environment, private empty mode-0700 CLI config/home, `--platform=linux/amd64`, exact Go templates, and independent byte/time ceilings. Exact Docker 29.6.1 client/engine/API compatibility and the containerd image store are required before image inspection. The shell verifies selected manifest identity, repo digest, descriptor/config/rootfs/platform closure, empty image config, child `close`, and run-root removal before minting an instance-bound observation. Production and fake-test issuers are disjoint. | A real invocation must provide live evidence. Container creation must consume the exact same private observation session once and recheck created state; binary provenance/hash is intentionally not claimed. | 76 fake-adapter and real-child-process tests: exact/cap-plus-one streams, simultaneous drain, timeout/abort/PID reaping, hostile request/output objects, stage mutation matrices, cleanup ordering/failure, issuer separation, and structural-copy rejection. |
| Gate 3 canonical artifact resource | `verified` | The normalized artifact envelope has one authority-bound canonical byte representation with an independent raw SHA-256 and wire-u64 length. Strict byte decode snapshots input and rejects noncanonical producer representations. The receipt and in-toto subject bind the raw resource; the predicate and authorization retain raw, transport, and semantic identities without conflation. | None for resource identity or authenticated contract binding. Real byte emission remains producer evidence. | 14 artifact, 7 receipt, and 11 provenance tests; pinned canonical byte SHA-256/length; strict authority and signed mutation matrices. |
| Gate 3 AOT runner receipt contract | `verified` | `browsergrad.compiler.cpp-cute.aot-runner-receipt@1.0` is a closed content-addressed report binding exact prepared job/profile, deterministic invocation, deterministic logical execution plan, OCI manifest/config, execution environment, opened job inputs, resolved or rejected selection, exact output, and all 24 extraction/process observations. `userProducedNativeExecution: forbidden` replaces the inaccurate blanket native-execution wording because the pinned compiler/supervisor are native. Artifact-visible counts are independently recomputed. Structural and strict-byte authorities remain separate. | The real runner must prove actual container state implements the logical plan, snapshot/hash/mount the same source blobs, and decode the same bounded output/receipt buffers. | 7 receipt and 11 provenance tests plus policy coverage; plan/environment/config and all prior identity/resource mutations. |
| Gate 3 detached provenance authorization | `verified` | A profile pins extractor, runner, resolved `linux/amd64` OCI manifest, sandbox policy, allowed builders, and exact P-256 SPKI trust store. Verification authenticates one canonical DSSE envelope, then requires one strict-decoded receipt resource. Artifact/profile/job/source/toolchain/sandbox/run/output facts derive from that receipt and are compared against the signed in-toto predicate. Authorization no longer accepts caller artifact/profile objects; it uses the exact receipt-owned artifact, rejects negative frontend outcomes, and retains receipt lineage in compiler origin. Structural receipt objects/copies, unpinned inputs, hostile trust stores, malformed signatures, noncanonical payloads, and signed receipt/resource/policy drift fail closed. The predicate is explicitly not SLSA or Sigstore. | Real producer and Sigstore-backed external evidence remain separate and are not claimed. | 11 provenance tests plus mutation matrices; pinned trust/profile/job/invocation/receipt/raw/semantic/source identities; authorized-only lowering and receipt-lineage origin coverage; strict typecheck/lint. |
...
Implement the fixed container lifecycle without claiming a real producer.
Consume one production runtime/image observation session exactly once; stage
only the opaque plan's source snapshots; derive container name, authorized
manifest reference, create options, mounts, argv, environment, and limits from
policy plus prepared job. Require
`create -> inspect-created -> start -> wait/result -> inspect-terminal -> remove`
with no attach race, prove every effective created-container field before
start, and kill/remove by exact container ID on every exit, timeout, abort,
overflow, parse failure, or inspection mismatch. Result capture must accept
only the existing bounded frame and cleanup must finish before any authority is
returned. Cover the complete transition/failure matrix through the disjoint
test adapter. Keep live toolchain/image/extractor/signing and truthful
cgroup-supervisor evidence unclaimed until separately exercised.
