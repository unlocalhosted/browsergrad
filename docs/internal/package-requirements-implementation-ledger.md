...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The strict native producer and pinned four-step no-shell build execute in a networkless, read-only, capability-free container with bounded immutable evidence. The cached diagnostic lane is stable near five minutes and the canonical local harness loop is 21 to 27 seconds. Forbidden generated imports were closed before review; the current 58-function import surface, 29 support functions, one dispatch table, memory, features, and exports are independently hash-pinned. ABI 1.7 separates browser-required features from inspector-only opcode markers. Production run `29674599138` built a 31,307,826-byte module with zero raw-Wasm ABI mismatches and exact interface conformance. Cached diagnostics still grant neither Worker nor release authority. Header licensing, clean-build execution, two-build reproducibility, factory bundling, provenance, browser-local C++ execution, and release authority remain false. Docker stays outside the portable runtime graph. | Complete and inspect one uncached clean build; run the two-clean-build lane; bundle the reviewed factory into package-owned Worker bytes; acquire and license exact header packs; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Capability commits are on `main` through `348d7373`. Local `verify:browser-clang-wasm:fast` passes 266 tests in measured runs of 21.74 and 26.37 seconds. The broader local native gate passes 170 tests with 9 intentional platform skips in 91.91 seconds. Cross-runtime CI run `29674595640` passes the Node 20, 24, and 25 source/harness jobs. Cold run `29658164083` spent 97 minutes 5 seconds before failing closed; warm production runs remain in the four-to-five-minute range with the same untrusted content-addressed cache key. The uploaded run `29674599138` completed in 4 minutes 44 seconds; its report records SHA-256 `b7a5daf6d121c306a2d07b5d3c14c00a664aaa2ff4ae3357a8b389326eeeb06f`, raw-Wasm verification, exact interface conformance, 58 imports, and zero mismatches. Zero completed pinned clean Clang-Wasm builds, valid Worker compile launches, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact BrowserGrad LibTooling extractor closure, LLVM 22.1.8 archive, Emscripten image manifest/config, four-step recipe, ABI, and distribution plan. The executor stages only locked snapshots, executes exact argv/cwd/environment without a shell, enforces per-step time/output/cancellation limits, binds inputs against inode-version drift, hashes native TableGen tools, seals the factory/Wasm/link map and logs, validates UTF-8/WebAssembly, and atomically materializes only the Wasm sidecar. The manual workflow verifies acquisition and image identities, then runs with no network, read-only root/input mounts, zero capabilities, no-new-privileges, and a private writable work mount. Clean runs additionally seal the exact Node runtime closure; evidence v2 records it and the two-build comparator requires it to match. Cached validation restores only the exact content-addressed native-TableGen and Wasm-LLVM/Clang directories, treats them as untrusted, and emits no clean/reproducibility authority. Link-only policy is excluded from that expensive cache key, while compiler flags, CMake, selected libraries, LLVM, Emscripten, and the recipe remain bound. The fast lane covers the ABI/profile/asset identity chain before container execution; clean/reproducibility and ordinary CI retain the full native sanitizer and platform-libc boundary. Every admitted build runs the bounded raw-Wasm reviewer against an independently pinned, least-authority projection. Temporary legacy/broad cache migration paths have been removed. | Finish and review one clean build, then run the two-job clean-build lane under the exact runtime boundary. Preserve ABI conformance as a separate authority before any Worker or distribution authority. | Build lock `bg.cpp.browser-build-input-lock.sha256.f6f6dddccd58b61433ed19d530c6d98e29b4aa55d8a86fa91a7072b594bae733`; diagnostic toolchain key `ba2867dd05bdc99485e0de219510c5f87ec5c5a459d2ee607bca40be549de41a`. Run `29674599138` at `348d7373` used the primary cache key and completed in 4 minutes 44 seconds, uploading a report for the 31,307,826-byte Wasm with 58 imports, raw-Wasm verification, exact interface conformance, and zero mismatches. Local `verify:browser-clang-wasm:fast` passes 266 tests in 21.74 to 26.37 seconds; full compiler unit passes 1,446 tests; the broader local build-plan gate passes 170 with 9 intentional skips; optional Docker contracts pass 96. Cross-runtime CI run `29674595640` passes the Node 20, 24, and 25 source/harness jobs, including the Node 24 native boundary. No completed clean build, licensed distribution, authenticated provenance, Worker execution, or release authority yet. |
...
Run clean validation without a restored diagnostic cache and inspect its
