...
| Gate 6 — framework convergence | `in-progress` | Thirty-six public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. No advertised framework operation remains opaque; only the intentional user-authored WGSL extension stays opaque. Grad schema v2 owns a closed twelve-dtype registry, separates direct and torch constructor inference/ownership, rejects fake bfloat16, fixes contiguous/detach/cast semantics, keeps all eager placement CPU-only and fail-closed, zero-copy wraps every supported writable NumPy storage dtype/layout, and emits only owning NumPy snapshots. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Reconcile owning expand materialization; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002 and ADR-0004 through ADR-0045; architecture freeze preserves the exact JIT partition and revised Grad contracts. Node 25.9.0 passes the seven-package build in 15.49 seconds; full JIT integration passes 55 files/336 tests, full Grad integration passes 59 files/362 tests in 99.60 seconds, the full compiler gate passes 96 files/1,602 tests plus parallel lanes, and semantic architecture plus its 24-test mutation suite pass. |
...
The next Gate 6 software transition is owning expand materialization, followed
by generated runtime/profile support consumption. Support must not be inferred
