...
|---|---|---|---|---|
| Gate 0 — freeze and inventory | `verified` | Workspace direction and all six legacy adapters are machine-frozen. Stable diagnostic/capability/backend/requirement vocabularies are separated. Compiler pointer/scalar, runtime requirements, Grad behavior, and the exact remaining JIT one-constructor/one-operation intentional user-kernel opaque boundary have pinned inventories and executable contracts. ADR-0002 and ADR-0004 through ADR-0048 classify the original JIT baseline and revised Grad/runtime contracts without widening opaque callers or silently substituting semantics. The JIT set is 36 typed retirements, one intentional opaque identity, and two removed unsupported surfaces. Grad owns a closed twelve-dtype eager registry, separates direct and torch constructor semantics, implements zero-stride expand views, rejects fake bfloat16 before allocation, fixes contiguous/detach/cast semantics, keeps every eager placement entrypoint CPU-only and fail-closed, zero-copy wraps all twelve supported writable NumPy storage dtypes, and exports only owning NumPy snapshots. Runtime generates all 53 registered assignment definitions, requires provider-bound resolution before availability, and preserves relevant records through every public readiness carrier. No Grad behavior remains marked compatibility debt. | None. Any baseline change requires the accepted-ADR exception path. | Architecture check and mutation suite; JIT and Grad Gate 0 contracts; focused typed-attention, dtype/view/cast/device, expand-view, NumPy integration, and runtime requirement-resolution tests; full package and actual-device evidence remain separately scoped. |
...
The next Gate 6 software transition is generated runtime/profile support
consumption and cross-framework/platform views. Support must not be inferred
from method presence or duplicated in hand-maintained tables.
