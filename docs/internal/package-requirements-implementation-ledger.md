...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, logical execution plan, artifact, canonical artifact resource, runner receipt, provenance, authorization, and static lowering authorities are distinct. The offline-runner input boundary synchronously snapshots and verifies exact source bytes behind opaque authority; its single bounded result frame strict-decodes canonical artifact bytes before receipt bytes and retains neither mutable caller buffers nor caller execution controls. The deeply immutable policy pins complete decoder budgets, no IPC namespace, and zero image/override/effective environment. The profile pins both OCI platform-manifest and image-config digests plus a closed execution-environment manifest. Detached DSSE/in-toto provenance authenticates the strict receipt and plan/environment lineage. No real producer, real receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Implement OCI evidence verification and fixed Docker create/inspect/start/cleanup; then pin/build the real native extractor, image, environment, source/header closure, and external signing workflow before dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 71 focused policy/profile/job/receipt/artifact/provenance/layout/runner tests; semantic-core 12 files/107 tests; compiler 43 files/1077 tests plus typecheck/lint and architecture/corpus gates; synthetic evidence proves contracts only. |
...
Implement the Docker shell without claiming a real producer. It must verify raw
OCI platform-manifest and config evidence for an already-local digest-pinned
`linux/amd64` image, stage only the opaque plan's source snapshots, derive every
create option/mount/argv/environment value from the built-in policy and prepared
job, inspect the created container before start, capture one bounded result,
inspect terminal state, and kill/remove by exact container ID on every exit,
timeout, abort, overflow, or parse failure. Cover effects with an adversarial
fake-process adapter. Keep real toolchain/image/extractor/signing and truthful
cgroup-supervisor evidence blocked.
