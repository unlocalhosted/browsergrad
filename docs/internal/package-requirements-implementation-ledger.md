...
- **Current implementation slice:** Gate 3 browser-WASM build, header-pack, and Worker integration
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. The canonical runtime ABI, asset/build binding, bounded raw-Wasm inspector, BrowserGrad-owned LibTooling tracer source/build integration, exact offline header-pack selection/assembly, lock-derived source-snapshot/sidecar materialization observations, Worker-owned VFS, memoized canonical input frame, complete artifact/decode verification contract, host-controller lifecycle boundary, source-level C++ imported-VFS adapter, local Wasm memory/allocator metrics observer, and package-owned same-realm runtime binding now exist as bounded components. Runtime ABI 1.1 now pins one exact nonzero stable metrics pointer and 72-byte record, including zero-byte and realloc accounting; the current C++ extractor remains ABI 1.0 and is explicitly blocked until it implements and behaviorally proves the producer. The C++ adapter is lazy, fail-stop, and exact-close, but has not been compiled. The current runtime binding cannot cross a Worker realm because its opaque VFS session is already bound to one exact Wasm memory; production must reconstruct a memory-independent mount in the Worker and bind it once to exported memory. The tracer still cannot return `artifact-ready` before CUDA dual-pass/artifact-v3 integration. Header-pack licensing, build execution, output identity, ABI conformance, reproducibility, provenance, browser execution, and release authority remain false. Docker/native stays optional build/CI/reference infrastructure outside the portable graph. | Split the extractor into runtime, VFS, Clang-action, artifact, and metrics translation units; implement and behaviorally prove the ABI 1.1 metrics producer; add Worker-local VFS mount reconstruction and one-time memory binding; implement device-first/host-second tracing and artifact-v3 output; acquire/review exact headers and notices; build and internally pin the Worker/factory; produce and inspect two builds; then prove one unmodified browser-local source through shared semantics and real WebGPU. | Metrics ABI/profile/asset/build focused boundary: 6 files/144 tests. Build-plan/source/VFS: 3 files/49 tests plus lock 24 tests. Full compiler: 62 files/1,349 tests plus optional Docker 96 tests and build-plan 49 tests. Release gate: 19 Python and 35 Node security tests plus packed/fresh consumers. Zero real Clang-Wasm builds, Worker spawns, reviewed distributed header packs, or browser-local C++ executions. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.4, producer-neutral request, artifact v3, request binding, compilation contract 1.0, common lowering, explicit CUDA-pass authorities, exact ABI asset binding, raw-Wasm inspection, Worker frames/VFS, host-controller lifecycle, and one package-owned runtime input binding now share one closed contract. The runtime binding copies and rehashes the exact frame and Wasm bytes, reserves one opaque invocation, and terminalizes its VFS/invocation before capability-failing. It intentionally has no loader/factory/network seam and cannot instantiate until package-owned Worker bytes, the generated factory, and first-build projections exist. Caller frames remain non-authoritative; production and test execution facts remain false. | Add the self-contained internally pinned Worker/Emscripten factory bundle and native-intrinsic platform adapter; build/review the first real Wasm module and licensed packs; then deliberately enable the production issuer and execute one unmodified browser-local fixture. | Worker runtime 1 file/11 tests; related Worker/ABI/VFS boundary 9 files/136 tests; zero Worker spawns or browser-local C++ executions. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the exact BrowserGrad-owned LibTooling tracer and deterministic Emscripten plan. The Node executor verifies and stages its exact source closure and can atomically install generated Wasm bytes. The checked-in C++ extractor implements the exact six-import closed VFS, but remains ABI 1.0. Runtime ABI 1.1 and the build lock now explicitly block conformance until the ninth metrics export and its zero-size/reallocation producer behavior exist. The current locked command remains internally buildable instead of naming an undefined export. The monolithic C++ file must be split before frame decoding, pass coordination, metrics hooks, and artifact serialization enlarge it. No pinned LLVM/Emscripten compile has occurred. | Split the extractor into bounded translation units; implement the ABI 1.1 metrics producer and behavior fixtures; then add frame decode, dual CUDA passes, artifact-v3 output, two locked builds, observed projection review, licensing, and provenance. | Metrics ABI focused 6 files/144 tests; build-plan/source 3 files/49 tests plus lock 24 tests; full compiler 62 files/1,349 tests. No process, build output authority, licensed distribution, reproducibility proof, or release authority exists. |
| Gate 3 Worker-owned VFS session | `partial` | One exact unshared Wasm memory serves canonical source snapshots and verified pack ranges through the six runtime-ABI imports. The bounded VFS implementation and source-level C++ adapter are individually closed. Audit found the current preparation boundary is same-realm only: it accepts host opaque authorities and the exact `WebAssembly.Memory` before a dedicated Worker exists, so neither that session nor its WeakMap authorities can be transferred into production. Production must reconstruct verified manifest/ABI/request/pack authorities inside the Worker, prepare a memory-independent mount, and bind it exactly once to the module-exported memory. Neither side has run together. | Split VFS preparation into Worker-local verified mount reconstruction and one-time exported-memory binding; transfer only exact canonical bytes and ArrayBuffers; then compare artifact `openedFileIds` with terminal VFS observations. | VFS session 1 file/8 tests; extractor/build-plan source suite 3 files/49 tests; read-only realm-boundary audit. No Worker or semantic-contribution evidence. |
| Gate 3 Wasm runtime metrics | `partial` | Runtime ABI 1.1 now owns the exact ninth pointer export, nonzero/stable/read-only pointer contract, 72-byte little-endian record, synchronous snapshot rules, requested-byte equations, exclusions, overflow behavior, and exact malloc/free/realloc/zero-byte event semantics. The local observer derives its layout from that authority, records exact pages/capacity and local phase time, and claims neither Worker evidence nor JS heap/resident/CPU/process metrics. The current ABI 1.0 C++ extractor has no export or instrumented producer, so the build lock keeps a behavioral-conformance blocker. | Implement module-global allocator hooks and the ABI 1.1 export in the split C++ metrics unit; add zero-byte/realloc behavioral fixtures; validate between synchronous calls in an executed Worker. | Focused 6 files/144 tests; full compiler 62 files/1,349 tests; final review closed ABI-version, impossible-linker, and realloc/zero-byte P1s. No producer evidence. |
| Gate 3 Worker invocation/result protocol | `partial` | Exact profile/asset/VFS/request/ABI/raw-Wasm identities bind one single-use invocation. The artifact/decode and frame contracts remain closed. The current package runtime binding rehashes and reserves an invocation/VFS/frame/Wasm tuple, but it is a same-realm ownership/cleanup proof, not a transferable production launch contract, because its opaque invocation/VFS authorities cannot cross into the Worker. It capability-fails before instantiation. Pure result validation remains `caller-frame-consistency-only`; production execution and lowering authority remain false. | Define the canonical transferable launch record and Worker-local authority reconstruction; then bundle the factory, capture native intrinsics, and enable the controller only after the real asset/build review. | Runtime binding 1 file/11 tests; related boundary 9 files/136 tests; no production issuer, Worker spawn, C++ ABI execution, or lowering authority exists. |
...
| D-094 | 2026-07-17 | accepted | Split the BrowserGrad C++ extractor before adding frame parsing, CUDA pass coordination, metrics hooks, and artifact serialization. Use separate runtime/lifecycle, imported-VFS, Clang-action/instrumentation, artifact-v3, and metrics translation units; keep the top-level extractor as composition only. Isolate LLVM-version-sensitive CUDA/tooling APIs in the Clang-action unit and repin the complete source-set closure atomically. | The current source already exceeds a reviewable composition boundary. Adding several independent state machines and canonical serialization to one file would create hidden coupling between ABI lifetime, VFS failure, Clang pass ownership, allocator accounting, and artifact identity—the exact kind of three-month refactor risk this work is intended to avoid. |
...
contract, Worker-owned VFS session, package runtime input binding, and local
Wasm metrics observer are closed bounded components. The source-level C++ VFS
bridge now implements the exact imports, but it has not compiled or executed.
The host-controller and runtime entrypoints remain deliberately
capability-blocked and therefore observe no execution.

Runtime ABI 1.1 and every dependent profile/asset/build identity now pin the
exact metrics record, but the C++ producer is deliberately missing. Next split
the monolithic extractor into runtime/lifecycle, imported-VFS,
Clang-action/instrumentation, artifact-v3, and metrics units. Implement the
metrics export and module-global allocator behavior first, with exact
zero-byte/realloc conformance fixtures; only then clear its build blocker.

In parallel, replace the same-realm VFS/runtime composition with a canonical
transfer record and Worker-local authority reconstruction. Prepare the verified
mount without memory, instantiate only host-verified Clang-Wasm, and bind the
mount exactly once to its exported memory. Do not bundle the generated factory
before this ownership seam is closed. Afterward add device-first/host-second
CUDA actions and the canonical artifact-v3 writer, then build and internally
pin the self-contained package Worker/factory and capture native browser
intrinsics. Only an independently inspected first build may unlock production
controller evidence. Wasm page/allocator observations remain separate from
logical VFS reservations and browser-process memory.

Run the lock-derived executor for the first real extractor build from pinned
inputs after the metrics ABI, CUDA passes, and artifact writer close. The offline
header-pack selector/writer is now closed, but it is not acquisition or release
