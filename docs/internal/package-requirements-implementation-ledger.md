...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-Wasm remains the primary portable-product producer. The five-pack/5,788-file header universe is reproducible under a narrow stable header-input projection. The native producer now emits a complete source-derived pointer/Tensor/`cute::copy` Artifact V3 graph. Runtime ABI 1.14 pins the observed 75-function import surface and 15,304-entry table; detached inspection reports exact conformance with zero mismatches. The package Worker and verifier bundles were regenerated deterministically. The broader Linux boundary caught and rejected one stale native runtime-ABI resource binding; it is now corrected and covered by a fast cross-language source assertion. The prior layout-only Chromium observation remains historical and is deliberately rejected by the current exact-Worker verifier. Docker stays outside the portable runtime graph. | Build and obtain two-clean/detached package evidence for the corrected current source and lock, then run the exact source-derived view-copy payload through the current real browser Worker. Admit that exact observed candidate through externally rooted producer trust, shared lowering, and required real WebGPU. Independently obtain external approval for the exact file map/notices and an externally issued exact-build statement under the production signer policy. Broader dtype/layout profiles remain separate capabilities. | Diagnostic build `30053246445` completed isolated compile/link in about 3 minutes 18 seconds and produced a 31,835,141-byte Wasm with SHA-256 `7939cb244b1d02346dbd511c61b4bdaf4d58c9321dfd62dba20bf7c57558cc8f`; ABI 1.14 detached review reports zero mismatches. Reproducibility run `30054873849` exposed the stale native binding before certification. The corrected fast harness passes 85 files/740 tests and the complete build-plan boundary passes 49 files/262 tests. The historical strict observation remains pinned but no longer mints current authority; `producerTrusted`, license approval, lowering, backend execution, and `releaseReady` remain false. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile 2.6, producer-neutral request, Artifact V3, compilation contract 1.2, CUDA-pass authorities, exact semantic-adapter binding, separate raw-Wasm verifier/compiler Workers, closed VFS, canonical transfer, one-shot entry, and typed terminal protocol previously completed one strict package-pinned browser layout compile. The native producer now also extracts exact pointer/Tensor declarations, `cute::make_tensor`, and `cute::copy` from unchanged C++/CuTe source and emits the complete source-derived view-copy Artifact V3 graph; native host/device parity and spoof rejection pass. ABI 1.14 exactly admits the three additional Emscripten exception-dispatch wrappers and 15,304-element table with no ambient-capability widening. Native profile admission now pins the same exact ABI resource identity and the fast harness enforces that cross-language seam. | Obtain current two-clean-build evidence, then run the exact source-derived view-copy payload in the regenerated real browser Worker. Keep external producer trust, distribution approval, shared lowering, backend execution, and release as separate false authorities until that payload crosses each boundary. | Diagnostic Wasm SHA-256 `7939cb244b1d02346dbd511c61b4bdaf4d58c9321dfd62dba20bf7c57558cc8f`, 31,835,141 bytes; detached ABI 1.14 review has zero mismatches. Reproducibility run `30054873849` correctly rejected the stale native ABI-resource binding; the corrected complete local build-plan gate passes. The prior strict Chromium layout observation is historical and rejected against the new Worker bundle, so no new browser authority has been minted. |
| Gate 3 Clang-WASM build and distribution | `partial` | The lock pins the extractor closure, LLVM 22.1.8, Emscripten image, recipe, ABI, and distribution plan. Cached diagnostics remain untrusted and separate from clean/release authority. The independent reviewer remains fail-closed. The exact 17-file header subset is independently rehashed across two roots and package-pinned. Its stable identity is derived from the narrow header-input projection, while every live run still reauthenticates the complete current build lock; ABI/extractor lock churn cannot invalidate unchanged header bytes or be hidden by a permissive verifier. | Obtain two-clean-build and detached verification evidence for the current lock, then close external file-level license approval, full distributed-output reproducibility, and production policy/external signer evidence independently. | Current lock `bg.cpp.browser-build-input-lock.sha256.2ba2736bfd84e2243f6f019bbabc0c7b9ce486df714cef9b6b2ba9035ee48b22`; runtime ABI manifest `bg.cpp.browser-runtime-abi.sha256.d2a18b132528aa3dcde74effc007b732c9083c4ffdf6ae06da18f39e2cbd68de`; header projection `bg.cpp.browser-header-input-projection.sha256.48490ddb7b2fe655ec36824e276b90122e2f548a77b768f5978029a31129c5b7`; header reproducibility ID `bg.cpp.browser-header-distribution-reproducibility.sha256.4d4c054fd4c93dbdbdef9581eeac52b037af3425e6a1c7eff8acc585abce1e55`; 17 outputs total 71,114,743 bytes. The focused fast harness passes 85 files/739 tests; the full compiler suite passes 97 files/1,620 tests. |
