...
- **Current implementation slice:** Gate 2 signed rank-1 view-copy profile design
...
| L2 materializing view-copy contract and CPU reference | `verified` | `view-copy@1.0` owns effects, exact reject/fill bits, and forbid-overlap semantics. Generic L2 verification stays backend-neutral; positive-affine profiles cover f32/i32/u32 ranks 1 through 5, and distinct signed-affine profiles cover negative source-coordinate scales at ranks 2 through 5 without admitting a signed destination. Prepared CPU execution compiles maps once, proves guarded reads and dense destination writes, caches source offsets, derives binding-sensitive specialization hashes, and enforces element/step/scratch/wall-time budgets, cooperative browser yielding and abort, plus native buffer-slot, length, alignment, overlap, and shared-memory checks. |
| Kernels-owned WGSL view-copy lowering | `verified` | The lowerer consumes authority-bound immutable backend-neutral specializations, preserves whole-root 32-bit words through u32 storage, interval-proves signed i32 arithmetic, lowers canonical source/destination maps, emits structured guarded fill loads, validates device and transient-working-set limits, and derives semantic plus device-specific hashes. Backend 2.3 admits the rank-4/rank-5 signed-affine source profile through the same address and mixed-radix paths as the lower-rank profile. One-in-flight ownership, timeout/abort stale-result suppression, exact scope drainage, distinct error stages, and device-loss invalidation have deterministic fake-device coverage. The required headed lane emits one validated `passed` terminal record for all 19 bit-exact CPU/WebGPU cases on Apple Metal 3; headless absence remains a truthful failed environment record. |
...
passes 19 word32 layout cases, including canonical rank-5 positive-affine and
rank-2-through-rank-5 signed-affine source maps, without minting source-producer,
distribution, or release authority.
