...
- **Current implementation slice:** Gate 7 shared produced-rectangle dispatch fanout
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.24 exact shared-count two-dispatch linear fanout, version-1.12/1.13 request-time/produced-resource rank-2/rank-3 rectangles, version-1.14/1.15 rank-4 rectangles, version-1.16/1.17 rank-5 rectangles, version-1.18/1.19 rank-6 rectangles, version-1.20/1.21 rank-7 rectangles, version-1.22/1.23 request-time/produced-resource rank-8 rectangles, and version-1.25 exact shared-rectangle two-dispatch fanout. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Each fanout reads one ordered produced selection once, validates its exact shared source and maximum contract, and substitutes both prepared dispatch slots in one feedback lifecycle. | Repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Focused semantic-core 2 files/62 tests and kernels 1 file/29 tests pass with strict typechecks; 60 complete-output CPU/WebGPU parity cases execute through explicit pipeline authority on Apple Metal 3; correctness artifact `1f876dbee0ddecac913ed6b67028a85b6355410f6222fc9fd3ba8678862aad99` under backend 1.28; separate backend-1.20 prewarmed performance observation. |
...
These profiles do not claim timestamps, external waits,
repeated/device-side feedback, rank-9-and-higher domains,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix and rectangular dynamic dispatch. Version 1.24 permits exactly two top-level produced-resource linear dispatches only when they share one exact source, rank, mode, and maximum; version 1.25 permits exactly two rectangular dispatches only when their ordered sources and maxima match exactly. Earlier versions retain one feedback node, and versions 1.12 through 1.23 preserve every older exact rectangle-rank limit. | Repeated/device-side feedback and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact shared-count and shared-rectangle two-dispatch CPU fanout, pre-version and mismatched-source/maximum refusal, and request-time/produced-resource rank-2-through-rank-8 rectangles. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.24 and 1.25 execute both independently bound dispatches from one produced linear or rectangular selection and report each exact completion; versions 1.12 through 1.23 execute request-time or produced-resource rank-2-through-rank-8 dense rectangles. | None for the current CPU-reference profile; portable WebGPU remains separate. | Focused CPU coverage proves one/two-element single dispatch, exact shared-count and shared-rectangle fanout, invalid selection refusal, and partial/full rank-2-through-rank-8 rectangles from both control sources. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.28 adds version-1.25 shared-rectangle two-dispatch fanout: it reads eight ordered u32 extents after one resident-buffer prefix stage, validates them against the exact shared maxima, substitutes both prepared dispatch slots, and runs the suffix without recompilation or another feedback stage. | Repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Sixty complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added partial/full rank-8 fanout cases retain pipeline identity, distinct specialization, two exact completions, two materialized outputs, and snapshot isolation. Correctness artifact `1f876dbee0ddecac913ed6b67028a85b6355410f6222fc9fd3ba8678862aad99`; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
separately bounded repeated host-mediated feedback contract or a transport profile;
