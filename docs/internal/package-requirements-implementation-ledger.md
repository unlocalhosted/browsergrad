...
- **Current implementation slice:** Gate 5 attention CPU reference and numerical conformance
...
| Gate 5 — tiled attention flagship | `partial` | Semantic-core owns the closed frontend-neutral `browsergrad.kernel.attention-forward@1` artifact and a separate closed `browsergrad.schedule.attention-online-kv-tile@1` artifact. Logical meaning binds static rank-4 f32 Q/K/V/output views, exact scale, non-causal or upper-left causal masks, stable-softmax phases, finite-score/state preconditions, comparison policy, disjoint effects, and VJP refusal. The schedule independently binds physical query/key tiles, cooperative K/V staging, increasing traversal, tile-wise online rescaling, uniform barriers, scalar vectors, and pre-state-update masks without logical or backend fields. | Implement the CPU oracle, schedule specialization, block-tiled K/V-staged WebGPU lowering, causal/non-causal numerical evidence, truthful baseline naming, and correctness/performance records on named devices/browsers. | Semantic-core 18 files/155 tests; build, typecheck, lint, architecture, hostile mutation, canonical round-trip, two-schedule separation, causal/non-causal binding, and packed-release consumer checks pass locally. Semantic-only commit `4cfacd75` passed CI `29820268289`. No CPU, WebGPU, performance, FlashAttention, frontend convergence, or Gate 5 exit claim. |
...
slice is the CPU reference and numerical conformance contract.
...
next software transition is the schedule-independent CPU reference and
comparison-policy implementation; it must prove causal and non-causal logical
meaning without reconstructing semantics from the physical schedule.
