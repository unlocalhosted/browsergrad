...
| Gate 6 — framework convergence | `in-progress` | Twenty-five public operation records are retired from `CUSTOM` into typed IR with executable registry decisions. The set includes elementwise, broadcast, indexing, reduction, replication, triangular, scan, variadic concat/stack, constant pad, paired `torch.sort`/`torch.topk` values and indices, overwrite `torch.scatter`, and general string-equation `torch.einsum`. Einsum now owns bounded parsing/inference, repeated-label diagonals, broadcasting, PyTorch ellipsis reduction, promotion with float32 half accumulation, general closure/symbolic VJP, captured-safe vmap, and resolved ONNX `Einsum` shared with Grad. Host-only typed operations explicitly refuse tensor-plan/WebGPU until their canonical portable lowerings exist. | Migrate the remaining 14 advertised opaque operations through the registry; reconcile remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate cross-framework/platform support views from executable contracts. | ADR-0002, ADR-0004 through ADR-0023; architecture freeze narrowed from 36/39 to 11/14 while preserving the exact original-ID partition. Current full verification passes JIT 43 files/301 tests, Grad 48 files/338 tests, compiler 96 files/1,602 tests, packed release, and the seven-package Node 25 build. The existing kernels actual-device lane proves `BROADCAST_TO`; host-only typed operations make no actual-device claim. |
...
`torch.topk` outputs, overwrite `torch.scatter`, and general `torch.einsum` are retired
...
registry generates all twenty-five public support records from their validator
bindings and preserves the original opaque-ID partition. The next software
transition is `l1_loss`, replacing the smallest remaining pure loss callback
with typed elementwise absolute difference, exact reduction, dtype,
derivative, transform, export, resource, and backend decisions. Each
