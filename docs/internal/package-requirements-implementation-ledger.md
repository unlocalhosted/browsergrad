...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers preserve exact resolved facts. Detached provenance now uses standard DSSE PAE around an in-toto Statement v1 with a BrowserGrad-specific predicate, profile-pinned trust root, exact runner/container/invocation projections, authenticated subject/policy checks, and an instance-bound authorized wrapper. This is not SLSA/Sigstore evidence, no lowerer consumes the wrapper yet, and no real-source compatibility claim has been made. | Pin a real producer profile/source/header closure; implement and sandbox the hermetic producer; make layout lowering consume only authorized authority; prove exact shared-semantic CPU/WebGPU parity; add dynamic tensor binding; assess native-companion evidence. | Gate 2 exact-source baseline at `aa605421`; 38 focused profile/artifact/provenance tests; compiler typecheck/lint and architecture guards; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
Implement detached AOT provenance verification and the authorized artifact
wrapper, then lower a pinned layout-only resolved fact into semantic-core. Keep the frozen
CUDA-lite CuTe normalizers unchanged. Keep the three stale direct-publish
guidance edits isolated because their files contain overlapping user-owned work.
