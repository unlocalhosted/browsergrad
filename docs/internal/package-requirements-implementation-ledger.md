...
- **Current implementation slice:** Gate 7 produced-resource dynamic launch control
...
maximum through the same explicit bounded feedback authority. These profiles
do not claim timestamps, external waits, repeated/device-side feedback,
GPU-produced dynamic launch control, broader dynamic schedules,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with per-rank resource multiplicity, exact geometry/roles, external-input versus zero-fill initialization, opaque kernel/layout references, dimension bindings, derived effects, dependency/lifetime hazards, explicit all-reduce meaning, and fail-stop/no-partial-output-commit failure semantics. Version 1.1 additively admits exact whole-allocation copies. Version 1.2 requires exactly one terminal ordered materialization read for every declared output. Version 1.3 adds unique dependency-ordered completion events with no resource effect, timestamp, queue identity, or external wait authority. Version 1.4 adds positive fixed-count sequential repetition over bounded linear dispatch/all-reduce/copy bodies. Versions 1.5 and 1.6 add bounded captured-input and request-time runtime-control u32 conditionals. Version 1.7 adds at most one produced-resource u32 conditional whose exact rank-local aligned four-byte temporary predicate requires a dominating ordered writer. Version 1.8 adds zero-through-artifact-maximum request-time u32-count repetition, charges the maximum expanded envelope, and treats body writes as non-guaranteed because zero iterations perform no write. Version 1.9 adds top-level positive request-time prefix dynamic dispatch under an artifact maximum no larger than the verified semantic element domain, while deriving the same geometry, effects, resources, hazards, and maximum work as static dispatch. Version 1.10 adds `resource-u32-count-sequential`: one exact rank-local aligned four-byte zero-filled temporary u32 with an ordered writer selects zero through a positive artifact maximum; the body cannot access the count resource, body writes are non-guaranteed, and at most one produced-resource conditional or repeat feedback node may appear in a graph. All control profiles retain globally unique node IDs, canonical effects/hazards, bounded work, and exact older-version decoding. Compiler's `createCudaLiteViewCopyHostGraph` remains the first concrete cross-package producer. | None for the current semantic/construction profile. Repeated/device-side feedback, GPU-produced dynamic launch control, broader dynamic schedules, and nested/device-side branching require new versioned contracts. | Semantic-core passes 23 files/226 tests, including canonical older-version retention plus repeat, conditional, resource-feedback, and dynamic-dispatch version/mode/control/bound/identity/effect/order/count rejection, zero-iteration guaranteed-write refusal, and packed fresh-consumer construction. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and required runtime control before its first await, runs topologically ordered view-copy dispatches, bounded prefix dynamic dispatches, arbitrary-byte whole-allocation copies, finite rank-ordered f32 or wrapping/exact i32/u32 all-reduces, zero-work events/materialization, exact fixed/runtime/produced-resource repeats, and one input-, runtime-control-, or produced-resource-selected conditional branch with cancellation/time checks at every admitted boundary. Produced-resource predicates and repeat counts are read from private graph storage only after ordered producer execution. Missing, duplicate, unknown, greater-than-u32, above-artifact-repeat-bound, zero-dynamic-dispatch, or above-artifact-dispatch-bound controls fail before input copying or execution; an above-bound produced count fails before any result publication. Preparation admits maximum repeat and dynamic-dispatch work; successful results report actual captured counts and actual executed operation totals. Completed event IDs, repeat records, dynamic-dispatch records, conditional branch records, and selected outputs become observable only with the successful whole-graph result. Fixed configurable ceilings bound working memory including collective scratch, maximum repeated, dynamic, or exact one-branch element/copied-byte operations, preparation time, execution time, and native cancellation. | None for the current CPU-reference profile; the portable WebGPU executor is separately verified below. | Focused CPU coverage includes odd-sized u8 exact copy, caller input/control mutation isolation, strict runtime-control admission, copied-byte budget refusal, terminal output publication, successful completion reporting, exact fixed three-iteration f32 sum, zero/two-iteration request-time and produced-resource f32 sum with actual work counts, exact one/two-element dynamic view-copy prefixes with actual work counts, both branches from all three predicate sources, ordered resource production, above-bound produced-count refusal, and pre-admission maximum-operation refusal. Semantic-core passes 23 files/226 tests, strict typecheck/lint/build, and packed fresh-consumer execution. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-repeat/conditional execution | `verified` | `browsergrad.host-graph.webgpu@1` consumes exact graph/kernel/layout authority, expands verified static and bounded dynamic view-copy dispatches, whole-allocation word copies, arbitrary-participant collectives, and fixed or artifact-maximum runtime/produced-resource repeat bodies into bounded canonical WGSL sequences, and pre-lowers every conditional branch through those same paths. `browsergrad.host-graph.webgpu-pipeline@1` separately binds the exact prepared graph to one admitted device, prewarms every unique program at each exact step slot including conditional alternatives, and records graph/backend/WGSL/schedule/features/limits/numerical-policy identity. Explicit hot execution reuses that authority; the convenience API captures caller inputs/controls and then delegates through the same ephemeral path. Complete sequences, validated contiguous ranges, and strictly increasing exact-slot selections reject copied, destroyed, cross-device, reordered/duplicate, invalid-offset/index, or unadmitted program use before request resource allocation. Request-time controls select before input copying or device work. Runtime repeats omit only artifact-prewarmed slots, retain maximum prepared budgets, and report actual submitted steps/counts/completion in specialization and terminal trace. Dynamic dispatch prewarms the artifact maximum through the same canonical program and exact pipeline slot; its initial portable profile requires one invocation per workgroup and substitutes only the admitted positive x-workgroup count, while wider workgroups fail explicitly. The version-1.7 resource predicate and version-1.10 resource repeat count each use the one admitted feedback boundary: private graph buffers remain resident across an exact prefix submission, four-byte readback, and selected branch-or-zero-through-maximum-repeat-plus-suffix submission through exact authorized slots. A zero-count repeat with no executable suffix reads outputs directly from the resident graph buffers. The same timeout, cancellation, in-flight owner, device-loss, cleanup, numerical-status, and terminal-publication contract spans both stages. Events and materialization contribute no GPU step. | None for the current portable DAG/fixed/runtime/resource-repeat/bounded-prefix-dynamic/three-predicate/pipeline-authority profile. Non-word-sized copies fail explicitly; repeated/device-side feedback, GPU-produced dynamic launch control, broader dynamic schedules, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Eighteen complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities and cover f32 rank-order sum and signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 copy, three fixed repetitions plus zero/two request-time and produced-resource repetitions of f32 sum, one/two-element f32 dynamic view-copy dispatch, and both branches of captured-input, runtime-control, and produced-resource predicates after caller mutation. Runtime-repeat, resource-repeat, and dynamic-dispatch pairs retain their respective graph/pipeline identity while recording distinct specializations and actual trace counts. Resource branches and repeats report one feedback stage. Correctness artifact `29660d12e95945ca4004c1be5c8bec7af4d322aac63370624ae68ac79241fc25`. Separate checks reject an above-bound produced repeat count without output publication, non-finite f32, a lost device, copied authority, post-destroy reuse, and wider-workgroup dynamic dispatch. Generic real-browser coverage also rejects cross-device, wrong-offset, non-increasing indices, and program-mismatched pipeline-set use. Kernels passes 21 files/187 tests; packed/fresh-consumer coverage prepares and executes graph control through the public package seam. |
| Gate 7 host-graph performance observation | `verified` | `browsergrad.kernels.semantic-host-graph.webgpu-performance@1` compares version-1.4 `fixed-count-sequential` control with a version-1.2 statically unrolled equivalent after proving bit-exact CPU outputs, equal element-operation counts, equal eight-step WebGPU expansion, and an untimed actual-device correctness preflight. Both exact device-bound pipeline authorities are prewarmed outside measurement; the lane then measures authority-bound execution through complete readback and queue drain with eight warmups and twelve alternating paired samples. | The record is observational only. It establishes neither a superiority/regression threshold nor runtime-control semantics, transport/topology, native, or general workload performance. | Required headed Chromium 148 on Apple Metal 3 records candidate median/p95 1.90/6.60 ms and baseline median/p95 1.90/6.20 ms for two ranks, 65,536 f32 elements, and three reductions under WebGPU backend 1.11.0. Terminal artifact `03c775dd373034ee90b0fed381554848e584da45a51429408ba4de5da17657db`; device profile `3b8e2fd6e3849d1436fc27243d1d0d708d4665ca4de6a4c3deb87ee05ca615b6`. CI, release, and publish retain exact-commit evidence separately from correctness. |
...
resident private buffers, and exact prewarmed pipeline slots. The
...
GPU-produced dynamic launch control beyond the bounded resource conditional,
resource repeat count, request-time count, and one-lane dynamic prefix;
repeated/device-side feedback, broader dynamic schedules, transport/topology,
