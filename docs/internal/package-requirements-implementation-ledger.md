...
| Gate 3 browser-local Clang-WASM producer | `audit` | Product direction is accepted, but repository has no tracked Clang-WASM build, loader, dedicated compiler worker, packaged CUDA/CuTe header closure, browser-local deployment profile/request/common authorization, or browser-produced frontend artifact. Only the canonical artifact/resource verifier and semantic/static-layout lowering are reusable integration targets today; existing profile/request/attestation contracts belong to the optional native lane. | Select and license-audit exact compiler distribution; pin hashes and build provenance; define browser profile/request/common authorization plus worker/VFS/cancellation/memory/output ceilings; compile one unmodified layout fixture locally in a supported browser; verify artifact and diagnostic/source-map closure. | Direction and producer-neutral artifact acceptance contract only; zero browser-local producer executions. |
