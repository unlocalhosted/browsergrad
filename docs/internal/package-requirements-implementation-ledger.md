...
- **Current implementation slice:** Gate 3 offline fail-closed AOT runner boundary
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, logical execution plan, artifact, canonical artifact resource, runner receipt, provenance, authorization, static lowering, cacheable OCI metadata, and plan-specific OCI authorization authorities are distinct. The offline-runner input boundary synchronously snapshots exact source bytes; its bounded frame strict-decodes canonical artifact then receipt bytes. Exact raw OCI manifest/config bytes now prove a closed leaf manifest, `linux/amd64` config, empty image execution config, descriptor/diff-ID closure, and bounded aggregate layer metadata without claiming unseen layer blobs or local Docker state. The deeply immutable policy pins all decoder/resource budgets, exact local endpoint, no IPC, and zero image/override/effective environment. Detached DSSE/in-toto provenance authenticates the strict receipt and plan/environment lineage. No live Docker observation, container enforcement, real producer, real receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Implement the separate shell-owned live Docker image observation plus fixed create/inspect/start/cleanup state machine; then pin/build the real native extractor, image, environment, source/header closure, and external signing workflow before dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 81 focused policy/profile/job/receipt/artifact/provenance/layout/runner/OCI tests; semantic-core 12 files/107 tests; compiler 44 files/1087 tests plus typecheck/lint and architecture/corpus gates; synthetic evidence proves contracts only. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Ahead-of-time profile, pre-run producer request, structural artifact verification, canonical artifact-resource identity, receipt contract, and receipt-authenticated provenance are implemented as separate instance-authorized records. The request pins exact source Git identity, source blobs, VFS paths, declaration anchor/expected entry, and output closure before execution. The one-TU artifact preserves resolved source/type/template/layout/intrinsic facts. Its semantic hash excludes producer metadata; canonical bytes retain independent resource identity. | Offline sandbox runner, real native extractor/image/profile/fixture/signing workflow, dynamic tensor copy, exact Gate 2 hash equality, and required WebGPU evidence. | 60 focused Gate 3 tests; pinned synthetic request/profile/job/invocation/receipt/raw/semantic identities; compiler typecheck/lint and architecture gates. |
...
| Gate 3 logical AOT execution plan | `verified` | One checked-in deeply immutable policy defines the output-independent Docker contract: one exact local Unix endpoint, config-digest creation with pull forbidden, fixed native supervisor and source root, non-root identity, zero image/override/effective environment, no network or IPC, private remaining namespaces, read-only root/snapshots, no added capabilities/new privileges, no image volumes/devices/socket, bounded mode-1777 tmpfs/result/stderr, profile-derived limits, complete artifact/receipt/OCI decoder and image-metadata budgets, and external attestation after receipt. Plan hashing re-verifies the current built-in policy identity. Profile and receipt also bind the OCI image-config digest and closed execution-environment manifest. | Node runner must prove its actual image/config/container/mount/argv state implements this logical plan. CPU/peak resource facts require the real pinned in-image supervisor/cgroup observation; fake Docker tests cannot establish them. | Checked-in JSON/TypeScript equality, canonical policy hash `c8bf6bdc...`, execution plan `636f1c9f...`, nested immutability, receipt/provenance plan/environment/config mutation tests, strict typecheck/lint. |
| Gate 3 offline-runner I/O authority | `verified` | The only caller inputs are one prepared-job authority plus exact `{ fileId, bytes }` blobs. Captured data descriptors establish membership, kind, and exact length before allocation; intrinsic typed-array checks reject proxies, shared/detached buffers, and prototype-disguised non-byte arrays. Bytes are synchronously copied before the first await, checked by digest and declaration-token anchor, then retained privately. The output boundary accepts exactly one magic/versioned frame with two big-endian u64 lengths, enforces independent artifact/receipt and aggregate ceilings, rejects truncation/trailing output, and strict-decodes the canonical artifact before the receipt using plan-bound complete decode limits. Exposed byte accessors return disposable copies without authority. | The Node shell must stage only these snapshots, capture only this frame, clean up before returning authority, and prove actual container state. | 10 focused runner-plan tests; caller-buffer TOCTOU, descriptor/proxy/species resistance, unknown/duplicate/pre-copy length rejection, typed-array kind/shared/accessor rejection, anchor recheck, accepted/rejected outcome preservation, bounded/noncanonical frame mutations, abort, structural-copy rejection, and copy isolation; compiler full gate. |
| Gate 3 OCI manifest/config metadata authority | `verified` | One cacheable opaque authority snapshots and strict-decodes exact raw OCI manifest/config bytes, derives their SHA-256 descriptors, requires a closed OCI leaf manifest and self-contained distributable layer descriptors, binds the embedded config descriptor to exact supplied bytes, requires `linux/amd64`, exact rootfs layer/diff-ID correspondence, valid bounded history/annotations, and absent/null/empty image execution config. A second opaque authority binds that metadata to one exact prepared plan. Layer blob bytes, acquisition, registry state, and local daemon state are explicitly outside both authorities. | The Node shell must mint a distinct temporal live-image observation from its own bounded fixed Docker process result, cross-bind it to this plan authorization, and recheck created-container state before start. | 10 OCI tests covering plan-neutral reuse, exact ordered/frozen closure, digest/size/media/platform/config/history/resource failures, strict JSON, hostile bytes/options, cancellation ordering, RFC 3339 calendar validity, aggregate layer bytes, and structural-copy rejection; focused 9 files/81 tests and compiler full gate. |
...
Implement the shell-owned live Docker observation and process state machine
without claiming a real producer. A fixed bounded image-inspect projection from
the exact pinned local Unix endpoint must cross-bind image ID, repo digest,
platform, rootfs diff IDs, descriptor lineage, and empty image config to the
authorized OCI metadata; caller bytes cannot mint this authority. Then stage
only the opaque plan's source snapshots, derive every create option,
mount/argv/environment value from policy and prepared job, inspect the created
container before start, capture one bounded result, inspect terminal state, and
kill/remove by exact container ID on every exit, timeout, abort, overflow, or
parse failure. Cover effects with an adversarial fake-process adapter. Keep real
toolchain/image/extractor/signing and truthful cgroup-supervisor evidence blocked.
Keep the three stale direct-publish guidance edits isolated because their files
contain overlapping user-owned work.
