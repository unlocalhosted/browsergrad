...
- **Current implementation slice:** Gate 7 broader dynamic launch schedules
...
exact prefix on CPU and one-invocation-per-workgroup portable WebGPU through
the same prewarmed program and pipeline slot. Version 1.10 admits one ordered
...
authority and substitutes only exact one-invocation-per-workgroup launch
geometry in the already-prewarmed pipeline slots. These profiles do not claim
timestamps, external waits, repeated/device-side feedback, broader dynamic schedules,
...
maximum, executes exact prefix work through the CPU reference, and requires
one invocation per workgroup so that prefix is the exact portable WebGPU
x-workgroup count through the same prewarmed program and pipeline slot.
...
exact one-invocation-per-workgroup launch geometry through those same
prewarmed slots. The
...
broader dynamic launch schedules beyond the bounded resource conditional,
resource repeat count, request-time count, and request-time or produced-
resource one-lane dynamic prefix; repeated/device-side feedback, transport/topology,
worker meshes, native companion execution, and
