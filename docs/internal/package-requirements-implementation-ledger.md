...
- **Current implementation slice:** Gate 7 unaligned/multidimensional dynamic launch schedules
...
exact prefix on CPU and one-invocation-per-workgroup or exact
workgroup-aligned portable WebGPU through the same prewarmed program and
pipeline slot. Version 1.10 admits one ordered
...
authority and substitutes only exact one-invocation-per-workgroup or
workgroup-aligned launch geometry in the already-prewarmed pipeline slots.
These profiles do not claim timestamps, external waits,
repeated/device-side feedback, unaligned/multidimensional dynamic schedules,
...
invocation per workgroup or exact workgroup-aligned prefixes through the same
prewarmed program and pipeline slot without inactive lanes.
...
exact one-invocation-per-workgroup or workgroup-aligned launch geometry through
those same prewarmed slots. The
...
arbitrary unaligned and multidimensional dynamic launch schedules beyond the
bounded resource conditional, resource repeat count, request-time count, and
request-time or produced-resource exact aligned dynamic prefix;
