...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Browser-local Clang-WASM is the primary portable-product producer. Closed AOT/browser profiles, deployment-versus-compilation identity, a producer-neutral local-source request, source-independent available-header profiles, a distinct compiler-runtime ABI, strict browser asset/VFS-pack authorities, common lowering authorization, and an explicit CUDA device-extraction/host-validation contract now exist. The device pass exclusively owns the canonical semantic graph; host ABI is keyed through content-derived canonical source-entity records and cannot reference device graph IDs. Successful passes must report the same nonempty content-derived selected-root projection; source domains derive from pass observations plus ABI ownership. Each pass owns its observed VFS closure and exact diagnostics. Existing source-spelling CuTe adapters remain frozen. All browser hashes/assets remain synthetic: no tracked Clang-WASM build, verified fetch/cache or mounted VFS authority, worker invocation/result/evidence, browser authorization adapter, browser-produced artifact, real CuTe source proof, dynamic tensor/storage meaning, or browser-local source-compatibility evidence exists. Docker/native contracts remain optional synthetic CI/reference infrastructure only. | Close exact build-provenance/license authority; adapt retained optional AOT metadata around the common request; build/pin the LibTooling extractor and full declared header packs; implement asset acquisition/mount and worker evidence; emit the canonical artifact from unmodified source in-browser; then prove Gate 2 semantic convergence, dynamic tensor copy, and required real WebGPU execution. | Gate 2 exact-source baseline at `aa605421`; final CUDA semantic-pass focused suite 6 files/89 tests, full compiler 53 files/1189 tests, and release-package gate; zero browser-local C++ producer executions. Existing Docker shell/lifecycle tests prove only optional synthetic-adapter contracts. |
...
| Gate 3 browser-local Clang-WASM producer | `partial` | Closed browser-local profile, producer-neutral request, asset-manifest, compilation-identity, common-authorization, and explicit CUDA semantic-pass authorities exist. Request v1 binds only compilation semantics, exact copied source snapshots/descriptors, layout-variable or view-copy-function anchor, expected artifact version, and shared semantic/input/output ceilings; deployment identity, Git provenance, conformance hashes, process limits, expected output, and opened closure stay detached. Available dependency/header manifests are distinct from request-observed opened inputs. Runtime ABI pins wasm32/Worker/VFS/memory boundaries. Closed manifest-bound VFS packs supply deterministic files. Device-first order is BrowserGrad extraction policy, not a claim about Clang driver order; each pass is a separate exact host-only/device-only invocation. Selected entry roots bind one payload-level source registry and must converge across successful passes; Docker may reproduce release assets but is not browser runtime. All current identities/assets remain fixtures. | Close build-provenance/license authority; compose retained AOT metadata around the common request. Then implement verified fetch/cache/mount, worker invocation/evidence/browser authorization, real build assets, and one unmodified browser-local fixture. | Final semantic-pass suite 6 files/89 tests; full compiler 53 files/1189 tests; request/profile/runtime-ABI/asset/VFS/authorization contracts; zero asset fetches, mounts, worker spawns, or browser-local C++ producer executions. |
| Gate 3 Clang-WASM build and distribution | `audit` | Read-only build audit found no extractor source, CMake/Emscripten build, production Worker bundle, acquisition/mount host, verified build-provenance/license authority, release asset export, or required browser lane. The canonical VFS-pack writer is landed: it emits the exact closed v1 format from immediately snapshotted regular-file bytes with deterministic path ordering and bounded allocation. Candidate LLVM 22.1.8, emsdk/Emscripten 6.0.3, and CUTLASS 3.7.0 revisions remain feasibility inputs only. Current fixtures mix synthetic Clang/libc++ versions and invented byte/feature identities. Local machine lacks Ninja, Emscripten, Binaryen, and TableGen tools; Docker daemon is unavailable. | Review and narrow the draft build-provenance contract to the exact selected build recipe and shipped license/notice files. Then add pinned toolchain lock, extractor/C API, reproducible cross-build, package worker build/assets/notices, real VFS packs, and strict browser/release proof. | Canonical writer plus reader focused suite: 2 files/15 tests; strict compiler typecheck and source lint. No real pack, build command, binary, license closure, reproducibility result, or browser execution exists. |
...
| Gate 3 pre-run AOT producer request | `verified` | Version 2.0 is a closed immutable request authority. It pins exact prepared profile, canonical HTTPS repository and tagged Git revision, sorted content-addressed source VFS files, exactly one main source, one bounded declaration-token anchor, and expected artifact schema/source/header/input closure. Request identity covers only declaration kind and anchor; it does not prescribe a producer's semantic entry ID or depend on expected output. Job identity still closes all source and expected-output fields. Source counts/bytes use profile ceilings; semantic decode budgets, cancellation, versioning, hashes, and instance authority fail closed. Compiler flags, commands, environment, host paths, include-root overrides, and output destinations are absent by schema. | Runner must open and hash actual blobs, resolve the anchor through the real Clang AST, record the actual resolved entry ID, and compare emitted artifact closure to every expected field. This authority proves a request contract only, not producer execution or trust. | Focused v2 job/receipt tests cover output-independent request IDs, output-sensitive job IDs, pinned identities, profile drift, header/project-header closure, VFS/main/anchor escape, operational fields, accessors, structural copies, budgets, version, and cancellation. |
...
| Gate 3 offline-runner I/O authority | `verified` | The only caller inputs are one prepared-job authority, its exact prepared execution-environment authority, and exact `{ fileId, bytes }` blobs. Captured descriptors establish membership/kind/length before allocation; hostile/shared/detached/non-byte arrays fail closed. Bytes are copied before the first await, checked by digest/anchor, and retained privately. The lifecycle stages canonical profile, job, and environment bytes plus exact source snapshots with exclusive no-follow files, read-only bind trees, readback hashes, and a closed path inventory. Output accepts one bounded framed canonical artifact+receipt only after container absence and staging removal. | No contract gap. Real producer output remains unproved until live execution. | Runner-plan tests plus lifecycle happy/failure staging checks; 0444 files, 0555 bind trees, descriptor/proxy/TOCTOU/frame/copy isolation coverage. |
...
| Gate 3 AOT runner receipt contract | `verified` | Receipt v2.0 is a closed content-addressed report binding exact prepared job/profile/environment authority, deterministic invocation, plan-v2 identity, OCI manifest/config, opened job inputs, resolved or rejected selection, exact output, and complete extraction/process accounting. Actual observed inputs, process measurements, emitted artifact counts, and configured upper bounds are separate typed categories. Resolved selection binds request anchor to the actual artifact entry ID rather than a preselected semantic ID. `userProducedNativeExecution: forbidden` remains explicit because the pinned compiler/supervisor are native. Structural and strict-byte authorities remain separate. | The real optional runner must prove actual container state implements the logical plan, snapshot/hash/mount the same control/source bytes, and decode the same bounded output/receipt buffers. | Focused v2 receipt/provenance tests plus policy coverage; exact environment instance, plan/config, actual-versus-ceiling categories, profile binding, and prior identity/resource mutations. |
...
| Work item | Status | Notes | Remaining | Evidence |
|---|---|---|---|---|
| Gate 3 canonical artifact resource | `verified` | Artifact v2 has one authority-bound canonical byte representation with independent raw SHA-256 and wire-u64 length. Exact input ownership distinguishes source, compiler resource, and pinned dependency files; forced compiler includes are profile-bound rather than producer-asserted; initializer expressions retain declaration meaning; diagnostics distinguish real source locations from explicitly locationless invocation/profile/compiler subjects. Strict byte decode snapshots input and rejects noncanonical producer representations. Optional AOT receipt and in-toto contracts bind the raw resource; a browser evidence contract remains missing. | None for v2 resource identity. Real browser-produced bytes and common producer authorization remain unproved. | Schema-v2 suite passes 10 files/108 tests; pinned canonical byte SHA-256/length; ownership, forced-include, root-containment, strict-authority, and signed-mutation matrices. |
...
| D-068 | 2026-07-16 | accepted | Before worker implementation, extract the unpublished AOT-named source job into a producer-neutral frontend request. Bind compilation-contract hash, exact source descriptors/snapshots, main path, source anchor, and expected artifact schema/version. Keep deployment profile/assets/worker/container, repository/revision, and pre-known output/header/input-closure hashes out. Source provenance and conformance expectations are detached. The profile pins the available VFS/header universe; evidence records the request-specific files actually opened. Add a distinct compiler-runtime ABI and separately coherent transfer/cache/VFS/WASM-memory/stack/output budgets. Host-verify exact self-contained worker bytes and length before Blob-module construction; no worker self-attestation or unverified URL fallback. | Mandatory Git provenance blocks ordinary editor buffers. Comparing a request's actually opened headers to one profile-wide constant rejects valid sources with different include closures. Source target ABI cannot describe the WASM compiler runtime, and current asset ceilings can exceed the worker heap without a declared host-backed versus in-WASM VFS model. Module workers have no SRI option, so a stored hash is not enforcement until the host verifies the exact bytes before spawn. Correcting these unpublished seams now avoids producer-specific requests, false closure checks, impossible memory profiles, and bundler-dependent worker identity. |
...
The CUDA semantic-pass prerequisite is closed. Review the draft
build-provenance/license authority before landing it: bind one exact selected
build recipe, every source/tool/builder input, actual shipped license/notice
files, build flags, outputs, and independently produced clean-build evidence.
Do not let a self-declared result or a generic inventory masquerade as build
proof. Compose retained optional AOT source/provenance/conformance metadata
around the common prepared request instead of retaining a second source-intent
authority.

Then implement browser acquisition and installation authority over the closed
v1 VFS packs: same-origin redirect-free fetch, exact hash/length verification,
rehash-before-use content cache, cross-pack mount collision checks, abort, and
typed failure. There is no general archive extraction or decompression in v1.
