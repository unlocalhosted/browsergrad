...
- **Current implementation slice:** Gate 3 observed AOT runner receipt authority
...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed profile, pre-run job, artifact, canonical artifact resource, runner receipt, provenance, authorization, and static lowering authorities are distinct. The receipt binds pre-run intent to reported opened inputs, deterministic invocation, pinned toolchain/container/sandbox facts, exact canonical output, resolved-or-rejected frontend outcome, and every extraction/process ceiling. Strict byte-origin wrappers prevent an in-memory object from impersonating emitted artifact or receipt bytes. Detached provenance does not yet require the receipt. No real producer, authenticated receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists. | Require the exact receipt-byte authority in provenance; implement the offline fail-closed container runner; pin/build the real native extractor, image, policy, profile, source/header closure, and external signing workflow; then prove dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 60 focused profile/job/receipt/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 41 files/1066 tests plus typecheck/lint and architecture/corpus gates; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
| Gate 3 AOT C++/CuTe frontend artifact | `in-progress` | Ahead-of-time profile, pre-run producer request, structural artifact verification, and canonical artifact-resource identity are implemented as separate instance-authorized records. The request is closed, content-addressed, profile/header-bound, and pins exact source Git identity, source blobs, VFS paths, selected declaration anchor/expected entry, and expected source/header/input/output closure before execution. The one-TU artifact preserves resolved source/type/template/layout/intrinsic facts. Its semantic hash excludes producer metadata; its canonical normalized bytes have a separate SHA-256 and byte length. | Observed runner receipt, offline sandbox runner, real native extractor/image/profile/fixture/signing workflow, dynamic tensor copy, exact Gate 2 hash equality, and required WebGPU evidence. | 33 focused profile/job/artifact tests; pinned synthetic request/profile/semantic/raw artifact identities; compiler typecheck/lint; compiler and semantic architecture checks. |
...
| Gate 3 canonical artifact resource | `verified` | The normalized artifact envelope has one authority-bound canonical byte representation with an independent raw SHA-256 and wire-u64 length. Strict byte decode snapshots input and rejects whitespace, key-order, duplicate-key, or other noncanonical producer representations instead of silently normalizing them. The in-toto subject digest and signed output manifest bind the raw resource; the predicate and authorization retain raw, transport, and semantic identities without conflation. | None for resource identity. The next authority must prove which runner observed and emitted it. | 14 artifact and 11 provenance tests; pinned canonical byte SHA-256/length; signed raw-digest/length/transport mutation cases; typecheck/lint and focused 53-test Gate 3 suite. |
| Gate 3 AOT runner receipt contract | `verified` | `browsergrad.compiler.cpp-cute.aot-runner-receipt@1.0` is a closed content-addressed report that binds exact prepared job/profile authority, deterministic output-independent invocation identity, runner/container/extractor/compiler/dependency/sandbox projections, exact opened job files and source/header/closure hashes, resolved or rejected selection, raw/transport/semantic output identity, and all 24 extraction/process observations. Artifact-visible counts and output bytes are independently recomputed; all profile ceilings fail closed. Structural object authority and strict byte-origin authority are separate for both artifact and receipt. | Detached provenance must require the exact receipt resource before this report is authenticated. The later runner must snapshot/hash/mount the same source blobs and decode the same bounded output/receipt buffers. | 7 receipt tests plus 14 artifact and 6 job tests; accepted/rejected outcomes, all ceilings, exact counters, byte snapshots, noncanonical encodings, instance forgery, identity/policy/input/selection/output/hash/version/outcome/operational mutations; typecheck/lint. |
| Gate 3 detached provenance authorization | `verified` | A profile pins extractor, runner, resolved `linux/amd64` OCI manifest, sandbox policy, allowed builders, and exact P-256 SPKI trust store. Verification authenticates one canonical DSSE envelope before evaluating its in-toto Statement v1 claims, binds the exact canonical artifact resource plus semantic identity, profile/toolchain/sandbox/runner/invocation/output identity, accepts tagged Git SHA-1/SHA-256 revisions, enforces source/include VFS roots, and mints opaque attestation and authorization authority. Rejected artifacts, structural copies, object-instance substitution, unpinned profiles/sources, hostile trust stores, malformed P1363 signatures, noncanonical payloads, and signed identity drift fail closed. The static layout lowerer consumes only this authority and retains raw plus semantic origin identity. The custom predicate is explicitly not SLSA or Sigstore. | Add and require the observed runner receipt before real producer evidence. Sigstore-backed external evidence remains separate and is not claimed. | 11 provenance tests plus mutation matrices; authorized-only lowering substitution tests; pinned trust/profile/raw/semantic artifact/source identities; strict typecheck and lint. |
...
- Provenance audit found the in-toto subject bound semantic identity rather than
  ...
  without an observed runner receipt. Canonical-byte authority is now fixed and
  mutation-tested. The observed receipt remains a pre-producer blocker; no real
  artifact may be accepted as producer evidence until receipt authority exists.
...
Require the exact strict-decoded receipt resource in detached provenance. Bind
receipt ID, raw digest/length, job, deterministic pre-run invocation, and output
manifest; derive duplicated toolchain/sandbox/run facts from the receipt and
reject structural-only receipt objects or cross-instance substitution. Only
then add the offline Docker runner shell.
