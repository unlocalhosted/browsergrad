...
| Gate 3 — real C++/CuTe frontend slice | `in-progress` | Existing source-spelling CuTe adapters are frozen. Closed, opaque, resource-bounded AOT profile and one-translation-unit frontend artifact verifiers preserve exact resolved facts. A separate pre-run job authority pins exact source identity/revision, content-addressed virtual files, one AST declaration-token anchor and expected entry, and profile/header/input/output closure without an operational escape hatch. Canonical normalized artifact bytes now have an independent raw SHA-256/length identity used by the in-toto subject and output manifest; semantic identity remains separate. Detached provenance mints an instance-bound authorized wrapper consumed by the static lowerer. No real producer, observed sandbox receipt, dynamic tensor/storage meaning, backend execution, SLSA/Sigstore evidence, or source-compatibility claim exists yet. | Bind provenance to an observed runner receipt; implement the offline fail-closed container runner; pin/build the real native extractor, image, policy, profile, source/header closure, and external signing workflow; then prove dynamic tensor/WebGPU tiers. | Gate 2 exact-source baseline at `aa605421`; 53 focused profile/job/artifact/provenance/layout tests; semantic-core 12 files/107 tests; compiler 40 files/1059 tests plus typecheck/lint and architecture/corpus gates; existing corpus gates remain audit inputs, not Gate 3 proof. |
...
Add a closed, versioned, instance-authorized runner receipt that binds the
pre-run job/request, observed input bytes, derived fixed invocation, resolved
container/toolchain/sandbox identities, exit/resource observations, and exact
canonical output bytes. Require its digest in provenance and mutation-test
request/receipt/artifact substitution. Only then add the offline Docker runner
shell.
