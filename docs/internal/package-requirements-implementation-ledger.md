...
- **Current implementation slice:** Gate 2 rank-8 exact-storage layout breadth
...
| Gate 7 — host graphs and optional systems expansion | `in-progress` | Semantic-core verifies the bounded DAG, explicit resources/effects/hazards, collectives, copies, events, fail-stop materialization, fixed/runtime/produced-resource repetition, three predicate sources, request-time or produced-resource linear-prefix dispatch, version-1.12/1.13 request-time/produced-resource rank-2/rank-3 rectangles, version-1.14/1.15 rank-4 rectangles, version-1.16/1.17 rank-5 rectangles, version-1.18/1.19 rank-6 rectangles, and version-1.20/1.21 request-time/produced-resource rank-7 rectangles. CPU and portable WebGPU execute through private storage and the same exact device-bound pipeline authority. Rank-7 WebGPU keeps axes 6/5 on x/y, generically flattens axes 0 through 4 into z, and reconstructs them behind the existing 32-byte eight-u32 guard in one maximum-prewarmed canonical view-copy slot; seven produced extents cross the existing single feedback stage. | Rank 8+, repeated/device-side feedback, nested/device-side branching, browser-worker transport/topology/failure evidence, native companion profiles, and stronger event synchronization remain open. | Focused semantic-core 2 files/62 tests and kernels 1 file/29 tests pass with strict typechecks; 52 complete-output CPU/WebGPU parity cases execute through explicit pipeline authority on Apple Metal 3; correctness artifact `21fe32a22b9aa7cae04c11c511d5fe6c48ed2cc0cf439628f79418c761073937` after the view-copy backend-3.7 rotation; separate backend-1.20 prewarmed performance observation. |
...
repeated/device-side feedback, rank-8-and-higher domains,
...
| Gate 7 host-graph semantic foundation | `verified` | The `/graph` subpath verifies bounded version-1 DAGs with exact resources, derived semantic geometry/effects, hazards, all-reduce meaning, fail-stop publication, copies, materialization, events, fixed/runtime/produced-resource repetition, captured/runtime/produced-resource conditionals, and request-time or produced-resource linear-prefix dynamic dispatch. Versions 1.12/1.13 add request-time/produced-resource rank-2/rank-3 rectangles, 1.14/1.15 add rank 4, 1.16/1.17 add rank 5, 1.18/1.19 add rank 6, and 1.20/1.21 add rank 7 while preserving every older exact rank limit. All retain maximum-product admission, canonical axis normalization, exact older-version decoding, and logical-extent completion. | Rank 8+, repeated/device-side feedback, and nested/device-side branching require new versioned contracts. | Semantic-core focused graph coverage passes 2 files/62 tests, including exact request-time or produced-resource rank-2-through-rank-7 CPU rectangles plus pre-version rank-7 refusal. |
| Gate 7 authority-bound CPU reference | `verified` | `browsergrad.host-graph.cpu-reference@1` consumes exact graph/kernel/layout authority, privately snapshots every rank-local input and runtime control before its first await, executes all admitted graph work under fixed memory/work/time/cancellation ceilings, and publishes completions and materialized outputs only after whole-graph success. Versions 1.12 through 1.21 execute request-time or produced-resource rank-2-through-rank-7 dense rectangles against the full semantic row-major domain, reserve the artifact-maximum product, and report exact logical extents, product, and rank-multiplied actual work. | None for the current CPU-reference profile; portable WebGPU remains separate. Rank 8+ is not inferred. | Focused CPU coverage includes exact partial/full `[3,4]`, `[2,3,4]`, `[2,2,3,4]`, `[2,2,2,3,4]`, `[2,2,2,2,3,4]`, and `[2,2,2,2,2,3,4]` rectangles. |
| Gate 7 portable WebGPU DAG/fixed/runtime/resource-feedback execution | `verified` | `browsergrad.host-graph.webgpu@1` and its separate device-bound pipeline authority retain the canonical bounded DAG, feedback, fail-stop, and exact-slot lifecycle. Backend 1.24 adds version-1.21 produced-resource rank-7 rectangles through the same maximum-prewarmed canonical view-copy seam. Seven ordered private u32 extents cross one resident-buffer feedback stage, are validated together, and reuse the version-1.20 mapping, 32-byte guard, maximum program, and exact suffix slots. | Rank 8+, repeated/device-side feedback, nested/device-side branching, transport/topology, native companions, and stronger event profiles require new contracts. | Fifty-two complete-output headed-Chromium CPU/WebGPU parity cases execute through separately prepared pipeline authorities. Two added small/full produced-resource rank-7 cases retain pipeline identity, distinct specialization, exact completion, snapshot isolation, and one feedback stage. Correctness artifact `21fe32a22b9aa7cae04c11c511d5fe6c48ed2cc0cf439628f79418c761073937` after the view-copy backend-3.7 rotation; device profile `a72951410740a4adee212bba13ee44da16fdb9d6644d3b58ec38e0623f2c7b48`. |
...
a separately bounded repeated/device-side feedback contract, rank-8 domain, or
transport profile;
