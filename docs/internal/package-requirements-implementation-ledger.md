...
| Gate 6 — framework convergence | `in-progress` | Public `Tensor.expand`, `Tensor.abs`, `Tensor.sign`, `Tensor.sin`, and `Tensor.cos` are retired from `CUSTOM` and emit typed `BROADCAST_TO`, `ABS`, `SIGN`, `SIN`, and `COS`. Expand retains its shared eager/lazy conformance and materializing/resident plan routes. Abs/sign preserve real-numeric shape/dtype; sin/cos preserve floating shape/dtype and reject integer drift. All four unary operations provide owning CPU realization, closure and symbolic VJP, functional grad, vmap, direct ONNX mappings, and explicit tensor-plan/WebGPU refusals. JIT's bounded v1 executable operation registry binds all five typed records one-to-one with validators and generates detached public support reporting from those same records. | Migrate the remaining advertised opaque operations through the registry; reconcile the remaining Grad dtype/view fixtures; make runtime/profile consumers use requirement resolutions and program-specific capability/lowering records; generate the cross-framework/platform support views from executable contracts. | ADR-0002 through ADR-0005; focused JIT unary/registry/VJP tests; architecture freeze narrowed from 36/39 to 31/34 while preserving the exact original-ID partition. Full regression counts are recorded in the dated verification entries. The existing kernels actual-device lane proves `BROADCAST_TO`; typed unary operations make no actual-device claim. |
...
and `Tensor.cos` are retired from the opaque callback inventory and have typed
CPU, VJP, transform, export, backend, and residency decisions. The executable
v1 framework-operation registry generates all five public support records from
their validator bindings and preserves the original opaque-ID partition. The
next software transition is `Tensor.clamp`, with a closed optional-bound and
piecewise-gradient contract. Each operation must enter the registry with
