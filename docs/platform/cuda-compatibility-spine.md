...
- Real-world no-regression gate:
  `NVIDIA/cuda-samples@b7c5481` must stay at `357` kernel definitions, `>=299`
  compile/codegen-runnable, and `<=57` hard gaps;
  ...
  compile/codegen-runnable, and `<=18` hard gaps. The aggregate gate also verifies
