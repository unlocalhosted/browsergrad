...
| JIT-009 | GPU-resident backward/optimizer steps. | Partial | backward roots and functional SGD/AdamW update IR can lower through tensor-plan WebGPU; default `.backward()` mutation, mutating `Optimizer.step()`, resident optimizer state, and memory planner remain |
...
- `bg.optim.sgd_update(...)` and `bg.optim.adamw_update(...)` emit primitive
  optimizer/update IR with NumPy handlers, vmap/ONNX refusals, tensor-plan
  lowering, and real WebGPU kernels. This is functional update IR, not yet
...
- `runTensorGpuPlan()` is the first generic tensor-plan executor: it consumes
  ...
  forward/backward, functional SGD/AdamW updates, and materializes only the root.
...
still mutates CPU `.grad` buffers, functional SGD/AdamW updates can run as
