...
The rank-1-through-rank-8 non-word32 profiles preserve exact bool/i8/u8,
...
destination. They grant no value semantics, arithmetic, conversion, or native 64-bit WGSL claim; Apple Metal 3 backend-3.7 evidence passes 75 complete CPU/WebGPU cases.
...
Rank 8+, signed destinations, dynamic non-word32 maps, non-word32
arithmetic/conversion, overlap, and other undeclared layouts
...
3. The separately named
`browsergrad.view-copy.signed-affine-rank2-rank3-packed8@1` and
`browsergrad.view-copy.signed-affine-rank2-rank3-packed16@1` profiles admit
negative coordinate scales in the source; a nonnegative view byte offset
rebases every proved source access into its root. The destination remains
positive affine, dense, and injective, and invalid source coordinates reject.
The current WebGPU
...
unrelated byte. The profiles do not admit signed destinations, padding fill,
boolean canonicalization, arithmetic, conversion, or a `shader-f16`
requirement.

The distinct positive-affine and signed-affine rank-1 packed8 and packed16
profiles apply the same exact storage, rebased-source, dense-destination, and
width-owned schedule rules to one-dimensional views. Their identities do not
widen the rank-2/rank-3 profiles.

The distinct positive-affine and signed-affine rank-4 packed8 and packed16
profiles apply those same rules to four-dimensional views without widening the
rank-1 or rank-2/rank-3 identities.

The distinct positive-affine and signed-affine rank-5 packed8 and packed16
profiles apply those same rules to five-dimensional views without widening any
lower-rank identities.

The additive
`browsergrad.view-copy.positive-affine-rank2-rank3-word64@1` profile admits
same-dtype f64/i64/u64 exact storage copies at ranks 2 and 3. The separately
named `browsergrad.view-copy.signed-affine-rank2-rank3-word64@1` profile admits
negative coordinate scales in the source under the same complete rebased byte
address proof. The destination remains positive affine, dense, and injective,
and invalid source coordinates reject. The current static WebGPU schedule
copies one logical element as two adjacent raw u32 words per invocation, so
writes are disjoint. It requires aligned views and word-sized root allocations,
uses no native 64-bit WGSL type, and grants no 64-bit arithmetic or conversion
claim.

The distinct positive-affine and signed-affine rank-1 word64 profiles apply
that same raw two-word schedule and rebased signed-source proof to
one-dimensional views without widening the rank-2/rank-3 word64 identities.

The distinct positive-affine and signed-affine rank-4 word64 profiles apply
that same schedule and proof to four-dimensional views without widening the
rank-1 or rank-2/rank-3 word64 identities.

The distinct positive-affine and signed-affine rank-5 word64 profiles apply
that same schedule and proof to five-dimensional views without widening any
lower-rank word64 identities.
...
Backend 2.9.0 adds the distinct word64 profile. It copies each logical value as
two adjacent raw u32 words under one invocation, requires no native 64-bit WGSL
type, and retains every older semantic and backend profile identity.
Backend 3.0.0 admits the separately named signed-affine packed8, packed16, and
word64 source profiles through the same interval-proved signed-i32 byte-address
lowerer. It adds no second executor, source-shaped kernel, clamping, arithmetic,
conversion, or unsigned address reinterpretation.
Backend 3.1.0 applies the same width-owned schedules and signed-i32 address path
to the separately named positive-affine and signed-affine rank-1 packed8,
packed16, and word64 profiles.
Backend 3.2.0 applies those schedules and the shared rank-4 coordinate lowerer
to separately named positive-affine and signed-affine rank-4 packed8, packed16,
and word64 profiles.
Backend 3.3.0 applies those schedules and the shared rank-5 coordinate lowerer
to separately named positive-affine and signed-affine rank-5 packed8, packed16,
and word64 profiles.
Backend 3.4.0 applies those schedules and the shared rank-6 coordinate lowerer
to separately named positive-affine and signed-affine rank-6 packed8, packed16,
and word64 profiles.
...
positive-affine and signed-affine rank-8 word32 profiles.
Backend 3.7.0 applies the same generic rank-8 coordinate lowerer and width-owned
schedules to separately named positive-affine and signed-affine rank-8 packed8,
packed16, and word64 profiles.
...
feedback boundary. Version 1.8 adds artifact-capped request-time u32-count
repetition. Version 1.9 adds artifact-capped positive request-time logical-
prefix dynamic dispatch. Version 1.10 adds one artifact-capped
produced-resource u32-count repeat through the explicit feedback boundary.
Version 1.11 adds artifact-capped positive produced-resource logical-prefix
dynamic dispatch through that same boundary. Version 1.12 adds artifact-capped
rank-2/rank-3 request-time rectangular prefix dispatch with one positive u32
extent per verified semantic axis. Version 1.13 adds one produced-resource
rank-2/rank-3 rectangle through distinct ordered rank-local temporary-u32
extent sources under the same graph-wide one-feedback-node bound. Cycles are
illegal. Version 1.14 extends request-time rectangular dispatch to rank 4
while versions 1.12 and 1.13 retain their exact rank-2/rank-3 limit.
Version 1.15 extends produced-resource rectangular dispatch to rank 4 through
four distinct ordered temporary-u32 sources under the same feedback bound.
Version 1.16 extends request-time rectangular dispatch to rank 5 under the
...
Version 1.19 extends produced-resource rectangular dispatch to rank 6 through
...
rectangular dispatch to rank 7 through seven ordered temporary-u32 sources under the same feedback bound.
Version 1.22 extends request-time rectangular dispatch to rank 8 under the
distinct rank-8 view-copy profile while preserving the version-1.21
produced-resource rank-7 limit. Version 1.23 extends produced-resource
rectangular dispatch to rank 8 through eight ordered temporary-u32 sources
under the same feedback bound. Repeated/device-side feedback,
rank-9-and-higher dynamic domains, or nested
...
and rank-2-through-rank-8 request-time and produced-resource rectangles are
...
- Add the distinct signed-affine rank-2/rank-3 word32 source profile while
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add the distinct signed-affine rank-4/rank-5 word32 source profile while
  preserving the lower-rank signed identity. Reuse the same CPU proof,
  signed-i32 WGSL address path, and canonical mixed-radix coordinates. The
  required 19-case Apple Metal 3 lane records correctness artifact
...
- Add a distinct positive-affine rank-2/rank-3 packed8 profile for exact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add a distinct positive-affine rank-2/rank-3 word64 profile for exact
  f64/i64/u64 storage only. Keep arithmetic, conversion, signed maps, padding
  fill, native 64-bit WGSL types, and dynamic launch out of scope. The required
  34-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct signed-affine rank-2/rank-3 packed8, packed16, and word64 source
  profiles while preserving every positive-affine and word32 identity. Reuse
  the canonical CPU proof and signed-i32 raw byte-address path. The required
  37-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-1 packed8, packed16, and
  word64 profiles while preserving every rank-2/rank-3 identity. The required
  43-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-4 packed8, packed16, and
  word64 profiles while preserving every lower-rank identity. The required
  49-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-5 packed8, packed16, and
  word64 profiles while preserving every lower-rank identity. The required
  55-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-6 packed8, packed16, and
  word64 profiles while preserving every lower-rank identity. The required
  61-case Apple Metal 3 lane records correctness artifact
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-7 packed8, packed16, and
  word64 profiles while preserving every lower-rank identity. The required
  67-case Apple Metal 3 lane records correctness artifact
...
- Add distinct positive-affine and signed-affine rank-8 word32 profiles while
  ...
  and device profile
  `9589abc8fafb412d83194febaf210f7f89da7a580bf20d3272e1eef9dcda2f66`.
- Add distinct positive-affine and signed-affine rank-8 packed8, packed16, and
  word64 profiles while preserving every lower-rank and word32 identity. The
  required 75-case Apple Metal 3 lane records correctness artifact
...
preservation at ranks 1 through 8, including signed-affine source addressing,
without claiming value semantics, arithmetic support, signed destinations, or
widening an older rank or word32 identity.
...
one-branch node counts. Program version 1.6 adds
`runtime-u32-branch-sequential` conditionals. Each predicate names one
required host execution control rather than a graph resource; zero selects
...
counts fail closed. Program version 1.8 adds
`runtime-u32-count-sequential` repetition. Each repeat names one required
request-time `u32-count` control and one positive artifact maximum within the
existing 1,024-iteration ceiling. Zero through that maximum execute the
nonempty bounded linear body exactly that many times; larger request values
fail before input capture or device access. Semantic verification charges the
maximum expanded node and operation envelope, shares the existing global
node-ID and body-hazard rules, and treats body writes as non-guaranteed because
zero iterations perform no write. This grants bounded host-known work
selection, not a GPU/backend-derived loop or dynamic launch. Program version
1.9 adds top-level `dynamic-dispatch` with
`runtime-u32-prefix-elements` mode. Each node names one required request-time
`u32-prefix-element-count` control and a positive artifact maximum no larger
than the verified semantic element domain. Only values from one through that
maximum are admitted, and admission completes before input capture or device
access. Verification derives the same dispatch geometry, effects, resource
bindings, and hazards as a static dispatch, charges the maximum work envelope,
and rejects nesting inside repeat or conditional bodies. This is bounded
request-selected prefix launch geometry, not a GPU/backend-derived launch,
arbitrary grid control, or a new source-shaped execution path. Program version
1.10 adds one `resource-u32-count-sequential` repeat. Its `iterationSource`
names an exact rank-local, aligned, four-byte, zero-filled temporary `u32`
with an ordered graph writer. Zero through a positive artifact maximum under
the existing 1,024-iteration ceiling execute the bounded linear body exactly
that many times. The body cannot access the captured count resource, its
writes are non-guaranteed because zero performs no body work, and the graph
admits only one produced-resource conditional or repeat feedback node. This is
one bounded host-mediated GPU-produced loop count, not a device-side loop,
repeated feedback, or opaque callback. Program version 1.11 adds
`resource-u32-prefix-elements` dynamic dispatch. Its
`launchSource` names the same exact rank-local ordered temporary-u32 class.
One through a positive artifact maximum no larger than the verified semantic
domain execute the exact logical prefix; zero and larger values fail closed.
The dispatch cannot bind the captured launch source as data, and versions
through 1.23 retain a graph-wide at-most-one produced-resource feedback bound
across conditionals, repeats, and dynamic dispatches. This is one bounded host-mediated
GPU-produced launch prefix, not a device-side loop, arbitrary grid, repeated
feedback, or source-shaped backend path. Program version 1.24 admits exactly
two top-level linear-prefix resource dispatches only when both name the same
ordered temporary-u32 source, rank, mode, and artifact maximum. One produced
count fans out to both independently bound semantic dispatches through one
aggregate feedback stage; conditionals, repeats, rectangular selectors,
different sources or bounds, a third feedback node, and pre-version fanout
fail closed. This is shared selection, not repeated or device-side feedback.
Program version 1.25 extends the same exact-two-consumer profile to rectangular
resource dispatches only when their ordered axis/source/rank/mode tuples and
...
Program version 1.27 admits exactly three linear resource dispatches with
three distinct launch-source resources only when their semantic writes form
one exact producer chain. Missing, forked, cyclic, or extra producer relations,
pre-version use, a fourth feedback node, and mixed feedback modes fail closed.
The three positive bounded selections execute through three ordered
host-mediated stages under one fail-stop lifecycle; this is not an unbounded
or device-side feedback cycle.
Program version 1.28 extends that contract to exactly four linear resource
dispatches with four distinct sources and three semantic producer relations.
The verifier requires one connected root-to-leaf chain and rejects disconnected
degree-equivalent cycles as well as missing, forked, cyclic, extra, duplicate-
source, mixed-mode, and pre-version profiles. The four positive bounded
selections execute through four ordered host-mediated stages under the same
resident-buffer and fail-stop lifecycle; this remains neither an arbitrary nor
a device-side feedback graph.
Program version 1.29 separately admits exactly one
`resource-u32-branch-sequential` conditional and one
`resource-u32-count-sequential` repeat only when both name the same produced
temporary-u32 resource and rank. The ordered producer executes once, the
backend reads the four-byte selection once, zero selects the else branch and
zero repeat iterations, and a positive value selects the then branch plus that
bounded repeat count. Different sources or ranks, a third feedback consumer,
...
Program version 1.31 admits the equivalent sequential conditional-to-linear-
dispatch profile. Both conditional branches must guarantee-write one distinct
temporary-u32 launch source consumed by one later
`resource-u32-prefix-elements` dispatch. The value must remain positive and
within the dispatch's artifact maximum. Program version 1.32 extends the same
...
Program version 1.34 raises only the exact linear producer-chain ceiling from
four to eight resource-controlled dispatches. Five through eight distinct
launch sources must form one connected root-to-leaf producer chain; missing,
forked, cyclic, extra, duplicate-source, mixed-mode, pre-version, and ninth-
consumer profiles fail closed. The bounded host-mediated staged executor
remains generic and preserves the same resident-buffer and fail-stop lifecycle.
The authority-bound
...
It requires equal conditional branch element-operation counts, rejects
...
For version 1.8 it admits the exact request control set and every runtime-repeat
bound before copying inputs, reserves the maximum operation envelope, executes
only the captured count with per-iteration and per-body cancellation/time
checks, and reports the actual count and actual executed operation total only
with successful whole-graph publication. For version 1.9 it reserves the
artifact-maximum dynamic-dispatch work, executes the exact positive captured
prefix against the canonical view-copy plan for every rank, and reports the
actual element count and rank-multiplied element-operation total only with
successful whole-graph publication. For version 1.10 it reads the count from
private rank-local storage after the ordered producer, rejects values above the
artifact maximum, reserves maximum work, executes the actual zero-through-
maximum count with per-iteration checks, and publishes actual completion/work
only with whole-graph success.
For version 1.11 it reads the positive prefix count from private rank-local
storage after the ordered producer, rejects zero and above-maximum values,
reserves maximum work, executes the exact canonical prefix for every rank, and
publishes actual completion/work only with whole-graph success.
...
publishes branch completion only after whole-graph success. Version-1.6
...
launch. Version-1.8 runtime repeats lower and prewarm the complete
artifact-maximum schedule through the same canonical body templates.
Execution admits the request count before input copying or device access and
selects the exact strictly increasing pipeline slots for zero through the
maximum iterations without compiling, reordering, or substituting a program.
The prepared report retains maximum resource/step/work counts; the terminal
trace and backend-specialization identity retain the selected slot set, actual
...
the suffix through the same exact pipeline slots. Above-bound produced counts
...
lifecycle contract spans both stages. Version-1.12 rectangular dynamic
dispatch prewarms one maximum-shape canonical view-copy program and exact
pipeline slot. Execution captures every positive bounded axis extent before
input copying, substitutes true 2D/3D launch geometry plus one 16-byte uniform,
and maps physical x/y/z to trailing-to-leading logical axes. Every physical
tail or out-of-rectangle invocation returns before coordinate, address, or
resource evaluation. Completion retains the logical extents and their product.
Version-1.13 produced-resource rectangular dispatch submits all ordered extent
producers through resident private buffers, reads back exactly one four-byte
rank-local u32 per axis, rejects zero or above-maximum values, and substitutes
only validated 2D/3D geometry plus the existing guarded 16-byte uniform in the
same maximum-prewarmed exact slots. One lifecycle contract spans both stages.
Version-1.14 request-time rank-4 dispatch keeps the trailing axes on x/y and
flattens the two leading selected axes into z. The guarded shader reconstructs
their logical coordinates with quotient/remainder before semantic evaluation;
the maximum program and pipeline slot remain stable and device admission checks
the flattened z product independently.
Version-1.15 produced-resource rank-4 dispatch reads all four ordered
rank-local temporary u32 extents after one resident-buffer prefix stage,
validates the complete selection, and reuses the same flattened-z mapping,
16-byte uniform, maximum program, and exact pipeline slots for the suffix.
Version-1.16 request-time rank-5 dispatch uses the distinct rank-5 view-copy
profile and one 32-byte, eight-u32 uniform whose first five words carry the
selected extents. Physical x/y map logical axes 4/3 while z flattens axes 0
through 2. The guard checks that selected leading product before mixed-radix
quotient/remainder reconstructs all three leading coordinates ahead of
semantic evaluation. The maximum program and exact pipeline slot remain stable
across selected rectangles. Version-1.17 produced-resource rank-5 dispatch
...
the same 32-byte guard before semantic evaluation. Version-1.19
produced-resource rank-6 dispatch reads all six ordered temporary-u32 extents
after one resident-buffer prefix stage, validates the complete selection, and
reuses that same mapping, guard, maximum program, and exact suffix slots.
Version-1.20 request-time rank-7 dispatch keeps axes 6/5 on physical x/y,
...
slots.
Version-1.22 request-time rank-8 dispatch keeps axes 7/6 on physical x/y,
flattens axes 0 through 5 into z, and reconstructs all six leading coordinates
behind the same 32-byte eight-u32 guard and maximum-prewarmed rank-8 view-copy
program.
Version-1.23 produced-resource rank-8 dispatch reads all eight ordered
temporary-u32 extents after one resident-buffer prefix stage, validates them
together, and reuses the version-1.22 mapping, guard, program, and exact slots.
Every dynamic rank step charges one aligned 16-byte GPU uniform allocation for
ranks 1 through 4 or 32 bytes for ranks 5 through 8, and either four linear
host bytes, 16 rank-2-through-rank-4 rectangular host bytes, or 32
rank-5-through-rank-8 rectangular host bytes to transient
...
workgroup counts independently, and rejects insufficient limits before
resource creation. F32 collectives use a separately
...
u8-copy branches, both produced-resource u8-copy branches, and zero/two
request-time repetitions of f32 sum, plus one/two-element f32 dynamic
dispatches and zero/two produced-resource repetitions of f32 sum through their
...
one/two-element shared-count two-dispatch fanout through one aggregate
feedback stage and stable graph/pipeline identity, and
small/full rank-2 `[3,4]` plus rank-3 `[2,3,4]` request-time rectangular
dispatches through stable graph/pipeline identities, plus matching small/full
produced-resource rank-2/rank-3 rectangles through one feedback stage, and
small/full request-time rank-4 rectangles through stable graph/pipeline
identity and distinct specialization, plus matching small/full
produced-resource rank-4 rectangles through the same one-feedback lifecycle,
and small/full request-time rank-5 rectangles through stable graph/pipeline
...
identity and distinct specialization, and
matching small/full produced-resource rank-6 rectangles through the same
one-feedback lifecycle, and
...
one-feedback lifecycle, and
small/full request-time rank-8 rectangles through stable graph/pipeline
identity and distinct specialization, and
matching small/full produced-resource rank-8 rectangles through the same
one-feedback lifecycle, and
...
one/two-element three-dispatch produced-resource chains through three reported
feedback stages, stable pipeline identity, distinct specialization, and three
exact completions, and
one/two-element four-dispatch produced-resource chains through four reported
feedback stages, stable pipeline identity, distinct specialization, and four
exact completions, and
one/two-element eight-dispatch produced-resource chains through eight reported
feedback stages, stable pipeline identity, distinct specialization, and eight
exact completions, and
...
one/two sequential conditional-to-linear-dispatch selections through two
feedback stages, stable pipeline identity, distinct specialization, exact
branch/launch completions, and one materialized output, and
small/full sequential conditional-to-rank-2-rectangle and maximum-rank
conditional-to-rank-8-rectangle selections through two
feedback stages, stable pipeline identity, distinct specialization, exact
branch/extent completions, and one materialized output, and
separately proves non-finite f32 and lost-device refusal.
The required lane completes 92 CPU/WebGPU parity cases under backend 1.38.0;
...
accepts only pipelines in the authorized step slot, exact contiguous slot
range, or strictly increasing exact slot selection; invalid offsets/indices,
duplicate or reordered slots, copied, destroyed, cross-device, or
...
has 2.10 ms fixed-repeat and 2.10 ms unrolled medians under backend 1.20.0.
...
count through two ordered stages. Version 1.31 reuses those stages for one
branch-produced positive linear launch count, and version 1.32 applies the
same generic dynamic-selection stage to two branch-produced positive rank-2
rectangle extents. Version 1.33 extends that exact profile through rank 8,
...
Portable WebGPU backend 1.38 stores graph resources as raw u32 words while
