...
export function checkWorkspaceImportSpecifier(packageName, file, specifier) {
  ...
  if (
    ...
    failures.push(`${file} imports ${specifier}; kernels may import semantic-core schema/layout/kernel/schedule/graph protocols only`);
  }
...
export function validateGradCompatibilityInventory(inventory, fixture, freeze, filename = "grad-compatibility-inventory.json") {
  ...
  exactKeys(`${filename}.dtypeResolution`, inventory.dtypeResolution, ["defaultTensorStorageDtype", "defaultTorchBooleanStorageDtype", "defaultTorchFloatingStorageDtype", "defaultTorchIntegerStorageDtype", "torchArrayLikeInputPolicy", "torchConstructorOwnership", "unknownStringPolicy", "nonStringPolicy", "storageByteWidths", "unsupportedDtypes", "aliases", "torchTokens"]);
  if (!isRecord(inventory.dtypeResolution)) return failures;
  if (inventory.dtypeResolution.defaultTensorStorageDtype !== "float32" || inventory.dtypeResolution.defaultTorchBooleanStorageDtype !== "bool" || inventory.dtypeResolution.defaultTorchFloatingStorageDtype !== "float32" || inventory.dtypeResolution.defaultTorchIntegerStorageDtype !== "int64" || inventory.dtypeResolution.torchArrayLikeInputPolicy !== "python-canonical-numpy-and-tensor-preserve" || inventory.dtypeResolution.torchConstructorOwnership !== "always-owning" || inventory.dtypeResolution.unknownStringPolicy !== "reject-outside-alias-registry" || inventory.dtypeResolution.nonStringPolicy !== "accept-supported-numpy-dtype-spec-otherwise-reject") {
  ...
  const classes = new Set(["conversion", "device-placement", "dtype-refusal", "dtype-resolution", "interop", "materialization", "view"]);
