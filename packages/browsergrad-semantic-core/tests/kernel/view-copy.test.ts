...
describe("verified materializing view-copy", () => {
  ...
  it("executes rank-8 exact storage through distinct positive and signed profiles", async () => {
    const cases = [
      {
        dtype: "i8" as const,
        dtypeBytes: 1,
    ...
    const shape = ["2", "2", "2", "2", "2", "2", "2", "2"] as const;
    const positiveOrder = bitReversedOrder(8);
    for (const testCase of cases) {
