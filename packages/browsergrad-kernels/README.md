...
| `prepareSemanticViewCopyWgsl` / `runSemanticViewCopyWebGpu` | Verified `view-copy@1.0` lowering over canonical layout/index artifacts, with exact raw 16-bit/32-bit storage and structured guarded padding | ✅ 28-case strict CPU/WebGPU parity on Apple Metal 3 across f32/i32/u32 ranks 1–7, i16/u16/f16/bf16 ranks 2–3, negative word32 source strides at every portable rank, guarded negative predicates, broadcast, offsets, odd packed destinations, and float padding; release evidence remains commit-scoped |
...
`browsergrad.webgpu.view-copy.word32@2` or the distinct
`browsergrad.webgpu.view-copy.packed16@1` profile. The word32 profile uses
signed-i32 address expressions while preserving same-dtype f32, i32, or u32
storage bit-for-bit. Positive-affine word32 profiles cover ranks 1 through 7.
The distinct
...
covers rank 1. Separate rank-6 and rank-7 identities complete signed source
coverage through rank 7. All retain a dense positive-affine destination and
the same exact address proof.

`browsergrad.view-copy.positive-affine-rank2-rank3-packed16@1` preserves
same-dtype i16, u16, f16, or bf16 bits at ranks 2 and 3. Two logical elements
are owned by one invocation, preventing concurrent read-modify-write races in
the raw `array<u32>` storage representation. An odd final element preserves
the destination word's unrelated upper half. The semantic profile requires a
dense destination, reject-on-invalid-source, and positive-affine source maps.
The current WebGPU backend additionally requires static launch, a word-aligned
destination, and word-sized root allocations. It performs no f16 or bf16
arithmetic, needs no `shader-f16` feature, and makes no conversion or
widened-arithmetic claim.
...
Required Chromium 148 on Apple Metal 3 passes 28 complete-destination cases
through backend 2.7.0. Correctness artifact
...
rank-2-through-rank-5 request-time or produced-resource rectangular prefix
dynamic dispatch,
...
not claim repeated/device-side feedback or rank-6-and-higher dynamic domains,
