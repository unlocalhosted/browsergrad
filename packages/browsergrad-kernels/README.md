...
| `prepareSemanticHostGraphWebGpu` / `prepareSemanticHostGraphWebGpuPipeline` / `runSemanticHostGraphWebGpuPipeline` | Authority-bound `browsergrad.host-graph@1` execution with a separately prepared exact device-bound pipeline authority, per-rank private storage, canonical view-copy dispatches, whole-allocation raw copies, dependency-ordered completion events, bounded fixed-count, request-time u32-count, and one produced-resource u32-count repetition, bounded positive request-time or produced-resource arbitrary one-dimensional prefix dispatch including exact shared-selection fanout and exact two-through-eight-stage producer-chain profiles, one exact shared produced-u32 conditional/repeat fanout plus sequential conditional-to-repeat, conditional-to-linear-dispatch, and conditional-to-rank-2-through-rank-8-rectangular-dispatch chains, rank-2-through-rank-8 request-time and produced-resource rectangular prefix dispatch including one exact shared-rectangle two-dispatch fanout, captured-input, runtime-control, and produced-resource u32 conditionals, terminal materialization, and ordered f32/i32/u32 all-reduce | ✅ required 92-case real-WebGPU complete-output bit parity with the CPU graph oracle, including exact same-dtype semantic copies for all 13 built-in storage dtypes and signed rank-8 reverse layouts across representative 8/16/32/64-bit storage, finite f32 sum/signed-zero min, wrapping i32 sum, exact u32 max, event-marked/materialized u8 allocation copy, fixed plus zero/two-iteration request-time and produced-resource f32 repetition, zero/two shared conditional/repeat selection through one feedback stage, zero/two branch-produced repeat selection, one/two branch-produced linear launch, and small/full branch-produced rank-2 and rank-8 rectangular launches through two stages, 1/2-, aligned 64/128-, and unaligned 65/127-element workgroup-64 request-time and produced-resource linear launch, shared-count two-dispatch fanout, one/two-element two-, three-, four-, and eight-stage produced launches, small/full request-time and produced-resource rank-2-through-rank-8 rectangular launch, shared rank-8 rectangular two-dispatch fanout, and both branches of all three conditional sources through prewarmed pipeline authority, plus a separate observational fixed-repeat/unrolled authority-reuse record; broader arithmetic/collectives, broader mixed or device-side feedback, worker meshes, and native companions remain separate |
| `executeSemanticHostGraphBrowserWorker` | Closed `browsergrad.host-graph.browser-worker-transport@1` one-shot dedicated-Worker controller. It sends private canonical artifact/input bytes, re-verifies artifacts in the Worker realm, uses a Worker-owned `GPUDevice` and the canonical graph executor, validates one authenticated terminal, and always terminates the Worker on success, failure, timeout, or cancellation. | ✅ required four-case real-browser raw-u8, verified-f32, signed-rank-8 i8, and signed-rank-8 f64 Worker/WebGPU execution with caller-mutation snapshots; a strict pass/fail/not-run execution-evidence terminal binds the closed case set, environment, outputs, device agreement, and lifecycle; exact byte/wall-time ceilings and hostile terminal/lifecycle tests. This is one Worker topology, not a mesh or distributed collective transport. |
...
} from "@unlocalhosted/browsergrad-kernels/semantic_host_graph_worker";
```
...
Version-1.30 composes one produced-resource conditional with one later
produced-resource repeat through two exact host-mediated stages. Preparation
prewarms both branches and the complete bounded repeat schedule. Execution
runs the predicate producer, reads only its private four-byte `u32`, executes
the selected branch over resident buffers, reads only the distinct
branch-produced repeat count, and executes the selected repeat plus suffix
through exact authorized slots. The semantic verifier requires both branches
to guarantee-write that distinct count source. One timeout, cancellation,
device-loss, cleanup, and fail-stop output contract spans all three
submissions; this is not arbitrary mixed feedback or device-side control flow.
Version-1.31 reuses the shared conditional-prefix stage for one later linear
resource dispatch. Both branches must guarantee-write one distinct positive
bounded launch-count source. Execution reads the predicate, runs one prewarmed
branch, reads its private count, and specializes the existing maximum-prewarmed
runtime-guarded dispatch without recompilation. Exact branch and launch
completions, two feedback stages, resident-buffer ownership, and fail-stop
publication remain explicit.
Version-1.32 applies that same executor to one rank-2 rectangular resource
dispatch. Both branches guarantee-write two distinct positive bounded extent
sources after selection from a third predicate resource. One second readback
validates both extents and specializes the existing maximum-prewarmed
rectangular dispatch; no new source-shaped or geometry-specific executor is
introduced.
...
conditional or repeat count, with exactly two compatible produced-resource
dispatch consumers in one aggregate feedback stage or one exact linear
producer chain across two through eight reported feedback stages, plus one
exact shared-source conditional/repeat pair in one aggregate stage and one
conditional-to-distinct-repeat, conditional-to-linear-dispatch, or
conditional-to-rank-2-through-rank-8-rectangle chain across two stages. It
does not claim a fifth or broader mixed conditional/repeat/rectangular feedback profile,
device-side feedback, or rank-9-and-higher dynamic domains,
