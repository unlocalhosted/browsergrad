...
| `prepareSemanticViewCopyWgsl` / `runSemanticViewCopyWebGpu` | Verified `view-copy@1.0` lowering over canonical layout/index artifacts, with exact 32-bit-word storage and structured guarded padding | ✅ 20-case strict CPU/WebGPU parity on Apple Metal 3 across f32/i32/u32, ranks 1–5, negative source strides at every portable rank, guarded negative predicates, broadcast, offsets, and float padding; release evidence remains commit-scoped |
...
covers rank 1. All retain a dense positive-affine destination and the same
exact address proof.
...
Required Chromium 148 on Apple Metal 3 passes 20 complete-destination cases
through backend 2.4.0. Correctness artifact
