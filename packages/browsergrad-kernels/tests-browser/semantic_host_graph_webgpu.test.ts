...
const CASE_IDS = Object.freeze([
  ...
  "f32-sequential-conditional-rank8-rectangle-small",
  "f32-sequential-conditional-rank8-rectangle-full",
...
it("executes multi-rank host graphs on a required real GPUDevice", async (context) => {
  ...
  try {
    ...
    const oneSequentialConditionalDispatch = completedCases.find(
      ({ caseId }) =>
        caseId === "f32-sequential-conditional-dispatch-one",
    );
    const twoSequentialConditionalDispatch = completedCases.find(
      ({ caseId }) =>
        caseId === "f32-sequential-conditional-dispatch-two",
    );
    expect(oneSequentialConditionalDispatch?.pipelineIdentityHash)
      .toBe(twoSequentialConditionalDispatch?.pipelineIdentityHash);
    expect(oneSequentialConditionalDispatch?.backendSpecializationHash)
      .not.toBe(twoSequentialConditionalDispatch?.backendSpecializationHash);
    expect(oneSequentialConditionalDispatch).toMatchObject({
      expandedStepCount: 3,
      dispatchStepCount: 1,
      copyStepCount: 2,
      materializationCount: 1,
    ...
    expect(twoSequentialConditionalDispatch).toMatchObject({
      expandedStepCount: 3,
      dispatchStepCount: 1,
      copyStepCount: 2,
      materializationCount: 1,
    ...
    const smallRank8ConditionalRectangle = completedCases.find(
      ({ caseId }) =>
        caseId === "f32-sequential-conditional-rank8-rectangle-small",
    );
    const fullRank8ConditionalRectangle = completedCases.find(
      ({ caseId }) =>
        caseId === "f32-sequential-conditional-rank8-rectangle-full",
    );
    expect(smallRank8ConditionalRectangle?.pipelineIdentityHash)
      .toBe(fullRank8ConditionalRectangle?.pipelineIdentityHash);
    expect(smallRank8ConditionalRectangle?.backendSpecializationHash)
      .not.toBe(fullRank8ConditionalRectangle?.backendSpecializationHash);
    expect(smallRank8ConditionalRectangle).toMatchObject({
      completedDynamicDispatches: [{
        nodeId: "copy-selected-rectangle",
        logicalExtents: [1, 2, 1, 2, 1, 2, 2, 3].map(wire),
        elementCount: wire(48),
      }],
      completedConditionals: [{
        nodeId: "choose-launch-extents",
        selectedBranch: "else",
    ...
    expect(fullRank8ConditionalRectangle).toMatchObject({
      completedDynamicDispatches: [{
        nodeId: "copy-selected-rectangle",
        logicalExtents: [2, 2, 2, 2, 2, 2, 3, 4].map(wire),
        elementCount: wire(768),
      }],
      completedConditionals: [{
        nodeId: "choose-launch-extents",
        selectedBranch: "then",
    ...
    const oneThreeStageFeedback = completedCases.find(({ caseId }) =>
      caseId === "f32-three-stage-resource-feedback-one");
    const twoThreeStageFeedback = completedCases.find(({ caseId }) =>
      caseId === "f32-three-stage-resource-feedback-two");
    expect(oneThreeStageFeedback?.pipelineIdentityHash)
      .toBe(twoThreeStageFeedback?.pipelineIdentityHash);
    expect(oneThreeStageFeedback?.backendSpecializationHash)
      .not.toBe(twoThreeStageFeedback?.backendSpecializationHash);
    expect(oneThreeStageFeedback).toMatchObject({
      expandedStepCount: 4,
      dispatchStepCount: 3,
      copyStepCount: 1,
      materializationCount: 1,
    ...
    expect(twoThreeStageFeedback).toMatchObject({
      expandedStepCount: 4,
      dispatchStepCount: 3,
      copyStepCount: 1,
      materializationCount: 1,
...
async function prepareSequentialConditionalDispatchCase(
  caseId:
    | "f32-sequential-conditional-dispatch-one"
    | "f32-sequential-conditional-dispatch-two",
  predicate: 0 | 1,
  ...
  const graph = (await createVerifiedHostGraphArtifact(
    sequentialConditionalDynamicDispatchProgram(artifacts),
    artifactOptions(artifacts),
  )).artifact;
  const prepared = await prepareSemanticHostGraphWebGpu(
    graph,
...
async function prepareSequentialConditionalRectangleCase(
  ...
  smallExtents: readonly number[] = [1, 2],
): Promise<PreparedCase> {
  if (
    smallExtents.length !== shape.length ||
    smallExtents.some((extent, axis) =>
      extent <= 0 || extent > (shape[axis] ?? 0))
  ) {
    throw new Error(
      "conditional rectangle case requires one positive bounded extent per axis",
    );
  }
  const artifacts = await createVerifiedDensePermutationViewCopyArtifacts({
  ...
  const inputs = Object.freeze([
    ...
    ...shape.map((extent, axis) =>
      namedInput(
        0,
        `then-extent-input-${axis}`,
        u32Bytes([extent]),
      )),
    ...smallExtents.map((extent, axis) =>
      namedInput(
        0,
        `else-extent-input-${axis}`,
        u32Bytes([extent]),
      )),
    namedInput(0, "value-input", f32Bytes(Array.from(
      {
        length: shape.reduce(
