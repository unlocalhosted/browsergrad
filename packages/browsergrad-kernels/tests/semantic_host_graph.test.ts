...
describe("semantic host-graph WebGPU preparation", () => {
  ...
  it("prewarms sequential conditional-to-repeat feedback stages", async () => {
    const graph = (await createVerifiedHostGraphArtifact(
      sequentialConditionalRepeatFeedbackProgram(),
      { kernelArtifacts: [], layoutArtifacts: [] },
    )).artifact;
  ...
  it("prewarms sequential conditional-to-dispatch feedback stages", async () => {
  ...
  it("prewarms one produced rank-2 through rank-8 rectangle with exact feedback budgets", async () => {
    for (const shape of [
      [3, 4],
      [2, 3, 4],
      [2, 2, 3, 4],
  ...
  it("prewarms produced dynamic dispatch, shared fanout, and sequential feedback", async () => {
    ...
    const threeStageGraph = (await createVerifiedHostGraphArtifact(
      sequentialResourceDynamicProgram(countArtifacts, artifacts, 3),
      sequentialOptions,
    )).artifact;
    const threeStage = await prepareSemanticHostGraphWebGpu(
      threeStageGraph,
      sequentialOptions,
    );
