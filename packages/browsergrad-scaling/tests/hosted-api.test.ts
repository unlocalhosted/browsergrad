...
describe("createHostedScalingApiMock", () => {
  it("matches the CS336 A3 budget, submit, duplicate, list, and final-submission contract", () => {
    const api = createHostedScalingApiMock({
    ...
    expect(second.experiment_id).toBe(2);
    ...
    expect(experiments.map((item) => item.experiment_id)).toEqual([1, 2]);
    ...
    expect(final).toEqual({
      training_config: trainingConfig(256, 15),
