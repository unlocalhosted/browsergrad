...
describe("BrowserGrad-owned Clang-WASM extractor source", () => {
  ...
  it("closes unsupported browser host services inside the Wasm module", async () => {
    ...
    for (const symbol of [
      "getenv", "__wasi_environ_sizes_get", "__wasi_environ_get", "_tzset_js",
      "__clock_gettime", "clock_gettime", "time", "__gmtime_r",
  ...
  it("owns frame validation and the bounded module-owned result lifecycle", async () => {
    ...
    expect(source).toContain("kRuntimeAbiVersion = 0x0001'0005U");
    ...
    expect(header).toContain("32U * 1024U * 1024U");
