...
with readback comparisons. Required fixture names currently cover `34` exact
...
matmul, normalization, vector-pack activations, and CuTe transpose cases.
`e2e:webgpu:corpus -- --require-webgpu` enforces this as a no-regression floor:
at least `34` total passing real WebGPU corpus fixtures, with per-corpus minimums
of CUDA-120 `2`, NVIDIA `cuda-samples` `4`, `llm.c` `10`, and LeetCUDA `18`.
