...
`__float2half_rn` / `_rz` / `_ru` / `_rd`, `__int2half_*`,
`__uint2half_*`, `__short2half_*`, `__ushort2half_*`,
`__half_as_short`, `__half_as_ushort`, `__short_as_half`,
`__ushort_as_half`, `__half2short_*`, `__half2ushort_*`, `hexp`,
`__hadd`, `__hsub`, `__hmul`, `__hdiv`, `__hneg`, `__hfma`,
...
Half2 helpers include arithmetic, lane extraction/packing (`__low2half`,
`__high2half`, `__halves2half2`, `__half2half2`, `__low2half2`,
`__high2half2`, `__lows2half2`, `__highs2half2`,
`__lowhigh2highlow`), unary math, ordered/unordered comparison vectors,
comparison masks, boolean reductions, `__hisnan2`, and NaN-propagating
...
`__hfma_relu`, `__hmin_nan`, `__hmax_nan`, ordered/unordered comparisons,
`__hisnan`, and `__hisinf`, also lower through the native f32-backed bf16 path
without requiring `shader-f16`.
...
matmul, normalization, vector-pack activations, direct and CuTe transpose cases,
RoPE pack/transposes, layernorm, scale/lowp conversions, and cuda-samples
...
that compare prepared median time against one-shot median time. Use
`--expect-prepared-readback-ratio-max N` to cap prepared readback overhead
against prepared resident dispatch. Pass `--benchmark-webgpu` to
`verify:real-world-cuda` to run these browser perf gates after each source/dist
real WebGPU fixture pass.
