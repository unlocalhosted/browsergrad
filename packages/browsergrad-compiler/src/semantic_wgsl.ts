...
function emitSemanticVectorOperand(
  ...
  const scalar = emitSemanticExpressionAs(expression, ir, names, "f32", options, textureSpecializations);
  return `vec${laneCount}<f32>(${Array.from({ length: laneCount }, () => `f32(${scalar})`).join(", ")})`;
}
