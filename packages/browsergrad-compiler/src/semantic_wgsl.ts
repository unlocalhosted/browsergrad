...
function emitSemanticFunction(
  ...
  const params = [
    ...fn.params.map((param) => `${nameFor(param.name, names)}: ${emitSemanticFunctionParamType(param)}`),
    "local_id: vec3<u32>",
...
function emitSemanticVoidFunctionCall(
  ...
  const args = operation.args.map((arg, index) => emitSemanticFunctionArg(arg, fn.params[index], ir, names, options, textureSpecializations));
...
function emitSemanticSurfaceRead(
  ...
  if (isSemanticWgslFloatVectorType(expression.valueType)) {
    ...
    const vector = `vec${lanes.length}<f32>(${lanes.join(", ")})`;
    return `select(vec${lanes.length}<f32>(), ${vector}, (${xBytes} >= 0 && (${xBytes} % 4) == 0))`;
  }
