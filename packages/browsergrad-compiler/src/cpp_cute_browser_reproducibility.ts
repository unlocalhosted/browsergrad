...
export async function verifyCppCuteBrowserReproducibilityResource(
  ...
  if (first.wasmSha256 !== CPP_CUTE_BROWSER_REPRODUCIBILITY_WASM_SHA256 ||
      first.wasmByteLength !== CPP_CUTE_BROWSER_REPRODUCIBILITY_WASM_BYTE_LENGTH) {
    fail(
      "BG-COMPILER-CPP-CUTE-BROWSER-REPRODUCIBILITY-EVIDENCE-HASH-MISMATCH",
      "$.builds[0].wasm",
      "built-in evidence differs from the reviewed package Wasm identity",
    );
  }
