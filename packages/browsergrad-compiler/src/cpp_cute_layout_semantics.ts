...
/** Non-authoritative preparation shared by candidate inspection and authorized lowering. */
...
export interface PreparedCppCuteLayoutSemantics {
  readonly preparedLayout: PreparedLayoutExpression;
  readonly entry: Extract<CppCuteFrontendEntryV1, { readonly kind: "layout" }>;
  readonly fact: CppCuteAffineLayoutFactV1;
  readonly origin: {
...
export interface NormalizedCppCuteLayoutOptions {
...
export async function prepareVerifiedCppCuteLayoutSemantics(
  ...
  const prepared = await prepareCppCuteLayoutSemantics(
...
export async function prepareCppCuteLayoutSemantics(
  payload: CppCuteFrontendPayloadV3,
  entryId: string,
  options: NormalizedCppCuteLayoutOptions,
): Promise<PreparedCppCuteLayoutSemantics> {
  ...
  try {
    preparedLayout = await prepareLayoutExpression({ symbols: [], constraints: [], layout }, {
  ...
  return Object.freeze({
    ...
    entry,
    fact,
    origin: collectOriginClosure(payload, fact.origin),
  });
}
  ...
  shapeByMode.forEach((mode, axis) => {
    ...
    for (const extent of mode) {
      const coordinate: IndexExpr = divisor === 1n
        ...
        : { kind: "floorDiv", value: { kind: "coordinate", axis }, divisor: indexConstant(divisor) };
      sourceCoordinates.push({ kind: "mod", value: coordinate, divisor: indexConstant(extent) });
      divisor *= extent;
    }
  });
  return { kind: "compose", source: leafLayout, shape: topShape.map(constant), sourceCoordinates };
}
...
function collectOriginClosure(
  ...
  return Object.freeze({ spans: Object.freeze(spans), macroExpansions: Object.freeze(macros) });
}

export function normalizeCppCuteLayoutOptions(
  options: LowerAuthorizedCppCuteLayoutEntryOptions,
): NormalizedCppCuteLayoutOptions {
  if (typeof options !== "object" || options === null || Array.isArray(options)) invalidRequest("$options", "options must be a plain object");
  ...
  for (const key of Reflect.ownKeys(options)) {
    ...
    if (descriptor === undefined || descriptor.enumerable !== true || !("value" in descriptor)) {
      invalidRequest(`$options.${String(key)}`, "options require enumerable data properties without accessors");
    }
  }
  ...
  return Object.freeze({ limits, ...(options.signal === undefined ? {} : { signal: options.signal }) });
}

export function validateCppCuteLayoutRequest(request: LowerAuthorizedCppCuteLayoutEntryRequest): string {
...
export function throwIfCppCuteLayoutAborted(signal: AbortSignal | undefined): void {
  if (signal?.aborted === true) failure("BG-COMPILER-CPP-CUTE-LAYOUT-CANCELLED", "$.signal", "authorized layout lowering was aborted");
}
