...
const BUILTIN_FEATURES: readonly CudaFeatureRecord[] = [
  ...
  feature("unsupported-surface", "texture", "Unsupported surface access", "unsupported", false, true, "Direct scalar/vector surface reads, half/half2 direct surface reads/writes when native shader-f16 is available, typed integer vector surface reads/writes, scalar/vector surface writes, scalar/vector surface helper params, and z-linearized 3D/layered surface paths lower through semantic IR; advanced surface forms use remaining modeled paths or are unsupported."),
