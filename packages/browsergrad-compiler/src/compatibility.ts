...
const BUILTIN_FEATURES: readonly CudaFeatureRecord[] = [
  ...
  feature("unsupported-vector-assignment", "frontend", "Unsupported CUDA vector assignment", "unsupported", false, true, "Float/int/uint vector storage element reads/writes, local values, constructors, helper value params, local lane writes, and dynamic lane reads lower through semantic IR; remaining vector assignments require a modeled vector lvalue and value."),
  feature("unsupported-vector-member", "frontend", "Unsupported CUDA vector member", "unsupported", false, true, "CUDA vector member must resolve to a modeled lane or read swizzle; xyzw, rgba, s0-s3, multi-lane xyzw/rgba, and s0123-style read swizzles are supported."),
