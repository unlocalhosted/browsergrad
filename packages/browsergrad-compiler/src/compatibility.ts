...
const BUILTIN_FEATURES: readonly CudaFeatureRecord[] = [
  ...
  feature("unsupported-local-pointer", "memory", "Local pointer aliases", "unsupported", false, true, "Fixed local-array decay, element aliases, straight-line assignments, conditionals, and +/- rebases are lowered into semantic IR; remaining local pointer forms need modeled alias rules."),
  ...
  feature("unsupported-pointer-pointer-comparison", "memory", "Unsupported pointer comparison", "unsupported", false, true, "Same-root fixed local-array pointer equality is lowered into semantic IR; remaining pointer comparison requires null checks or addresses in the same modeled address system."),
