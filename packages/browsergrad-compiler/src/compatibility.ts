...
const BUILTIN_FEATURES: readonly CudaFeatureRecord[] = [
  ...
  feature("unsupported-sincos-output", "math", "Unsupported sincos output target", "unsupported", false, true, "Storage-only sincos outputs lower through semantic IR; local and mixed storage outputs are supported by modeled statement lowering; remaining forms must target modeled float storage."),
