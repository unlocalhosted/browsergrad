...
const BUILTIN_FEATURES: readonly CudaFeatureRecord[] = [
  ...
  feature("unsupported-vector-argument", "frontend", "Unsupported CUDA vector argument", "unsupported", false, true, "Float vector constructors and float vector helper value params lower through semantic IR; remaining vector operations require compatible modeled value types."),
  feature("unsupported-vector-assignment", "frontend", "Unsupported CUDA vector assignment", "unsupported", false, true, "Float vector storage element reads/writes, local values, constructors, helper value params, local lane writes, and dynamic lane reads lower through semantic IR; remaining vector assignments require a modeled vector lvalue and value."),
