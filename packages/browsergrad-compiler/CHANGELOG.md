...
- Adds compiler-owned construction of bounded linear multi-dispatch view-copy
  ...
  dtype, byte length, alignment, and external-input/zero-fill initialization
  must agree exactly; graph effects, dimension bindings, semantic view IDs,
  and dependencies are derived rather than caller-declared. The result is a
  verified semantic-core host graph and makes no backend execution, transport,
  retry, or release claim.
...
- Scalarized POD-record argument expansion now also rewrites CUDA child-launch
  argument lists, so dynamic-parallelism parents and children keep matching
  flattened signatures instead of carrying stale record object args.
