/**
 * browsergrad_grad.utils.data — a tiny Dataset / DataLoader pair that mirrors
 * the public API of torch.utils.data for the subset that makes sense inside
 * a browser.
 *
 * Limits (and the reason for each):
 *   - num_workers must be 0. No process pool inside Pyodide; building a real
 *     Worker-backed loader would need a separate channel, out of scope for
 *     the educational use cases this targets.
 *   - No memory pinning, prefetching, or DistributedSampler.
 *   - We default collate to a NumPy stack (faster than the default torch
 *     collate, and Tensor-aware via np.asarray).
 *
 * The point is to be *enough* DataLoader that a typical lab notebook runs
 * unmodified — not to be a DataLoader clone.
 */
export const UTILS_DATA_PY = `
