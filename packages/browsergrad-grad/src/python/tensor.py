...
class Tensor:
    ...
    def triu(self, diagonal=0) -> "Tensor":
        return _triangular(self, diagonal, upper=True)
...
def _triangular(a: Tensor, diagonal=0, *, upper: bool) -> Tensor:
    operation = "triu" if upper else "tril"
    selector = np.triu if upper else np.tril
    if a.dtype not in _TRIANGULAR_DTYPES:
        raise ValueError(f"{operation}: dtype {a.dtype!r} is not supported")
    diagonal_i = _normalize_triangular_diagonal(diagonal, a.shape, upper=upper)
    out_data = selector(a.data, k=diagonal_i)
    out = Tensor(np.array(out_data, dtype=a.data.dtype, copy=True), dtype=a.dtype)
    def backward(g):
        gradient = selector(g.data, k=diagonal_i)
...
def triu(input, diagonal: int = 0) -> Tensor:
    ...
    return _triangular(_wrap(input), diagonal, upper=True)


def tril(input, diagonal: int = 0) -> Tensor:
    """Lower-triangular part of a matrix or batch of matrices."""
    return _triangular(_wrap(input), diagonal, upper=False)
