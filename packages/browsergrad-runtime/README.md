...
## Program capability support

Assignment requirement availability and program support are separate facts.
After a frontend or scheduler evaluates one concrete program or semantic
artifact, use `createProgramCapabilitySupportView()` to retain its lowering
decisions:
...
Definitions are generated byte-for-byte from the architecture vocabulary and
contain no support state. A view must contain at least one actual decision for
its exact program ID or artifact hash. Positive decisions name a registered
preservation level; conditional decisions retain their guards; refusals and
