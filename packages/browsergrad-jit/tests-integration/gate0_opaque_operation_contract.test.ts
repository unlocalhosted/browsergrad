...
x = bg.from_numpy(np.full((2, 2), 0.5, dtype=np.float32), requires_grad=True)
target = bg.from_numpy(np.array([[0.0, 1.0], [0.0, 1.0]], dtype=np.float32))
y = F.binary_cross_entropy(x, target, reduction="none")
...

{
    ...
    "functionalGradError": error_name(lambda: bg.func.grad(lambda value: F.binary_cross_entropy(value, target, reduction="none").sum())(x)),
