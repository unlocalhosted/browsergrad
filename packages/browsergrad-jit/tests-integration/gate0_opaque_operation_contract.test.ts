...
x = bg.from_numpy(np.array([-2.0, 2.0], dtype=np.float32), requires_grad=True)
y = x.clamp(-1.0, 1.0)
...

{
    ...
    "functionalGradError": error_name(lambda: bg.func.grad(lambda value: value.clamp(-1.0, 1.0).sum())(x)),
...
`);
  ...
  it("pins conditional backward and the six silently disconnected callbacks", async () => {
    const target = await getJitTarget();
    const result = await target.run<Record<string, unknown>>(`
import browsergrad_jit as bg
...
sort_values, sort_indices = bg.sort(x, dim=1)
topk_values, topk_indices = x.topk(1, dim=1)
einsum = bg.einsum("ij,jk->ik", x, bg.ones(2, 2))
forward_only = [einsum, scatter, sort_indices, sort_values, topk_indices, topk_values]
