...
@register_vmap(OP_L1_LOSS)
def _vmap_l1_loss(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
    ...
    mapped_contract = infer_l1_loss_contract(
        mapped_inputs,
        contract.reduction,
        contract.batch_rank + 1,
    )
...
@register_vmap(OP_L1_LOSS_VJP)
def _vmap_l1_loss_vjp(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
...
@register_vmap(OP_SMOOTH_L1_LOSS)
def _vmap_smooth_l1_loss(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
    ...
    mapped_contract = infer_smooth_l1_loss_contract(
        mapped_inputs,
        contract.beta,
        contract.reduction,
        contract.batch_rank + 1,
    ...
    return UOp(
        ...
        arg={
            "reduction": mapped_contract.reduction,
            "batch_rank": mapped_contract.batch_rank,
            "beta": mapped_contract.beta,
        },
    )


@register_vmap(OP_SMOOTH_L1_LOSS_VJP)
def _vmap_smooth_l1_loss_vjp(
    node: UOp,
    batched: Dict[int, UOp],
    B: int,
) -> UOp:
...
@register_vmap(OP_BINARY_CROSS_ENTROPY_WITH_LOGITS)
def _vmap_binary_cross_entropy_with_logits(
    node: UOp,
    batched: Dict[int, UOp],
    B: int,
) -> UOp:
    ...
    mapped_contract = infer_binary_cross_entropy_with_logits_contract(
        mapped_inputs,
        contract.reduction,
        contract.batch_rank + 1,
    )
...
@register_vmap(OP_BINARY_CROSS_ENTROPY_WITH_LOGITS_VJP)
def _vmap_binary_cross_entropy_with_logits_vjp(
    node: UOp,
    batched: Dict[int, UOp],
    B: int,
) -> UOp:
    ...
    if "vjp_of" in node.arg:
        arg["vjp_of"] = node.arg["vjp_of"]
    return UOp(
        op=OP_BINARY_CROSS_ENTROPY_WITH_LOGITS_VJP,
        inputs=mapped_inputs,
        shape=mapped_inputs[operand + 1].shape,
        dtype=mapped_inputs[operand + 1].dtype,
        arg=arg,
...
@register_vmap(OP_KL_DIV)
def _vmap_kl_div(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
    ...
    mapped_contract = infer_kl_div_contract(
        mapped_inputs,
        contract.reduction,
        contract.log_target,
        contract.batch_rank + 1,
    ...
    return UOp(
        ...
        arg={
            "reduction": mapped_contract.reduction,
            "batch_rank": mapped_contract.batch_rank,
            "log_target": mapped_contract.log_target,
        },
    )


@register_vmap(OP_KL_DIV_VJP)
def _vmap_kl_div_vjp(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
...
@register_vmap(OP_NLL_LOSS)
def _vmap_nll_loss(node: UOp, batched: Dict[int, UOp], B: int) -> UOp:
    ...
    mapped_contract = infer_nll_loss_contract(
        mapped_inputs,
        contract.reduction,
...
@register_vmap(OP_NLL_LOSS_VJP)
def _vmap_nll_loss_vjp(
    node: UOp,
    batched: Dict[int, UOp],
    B: int,
) -> UOp:
