...
def _normalize_triangular_diagonal(
    ...
    *,
    ...
    if len(shape) < 2:
        raise ShapeError(
            f"{operation}: input must have rank at least two, got {len(shape)}"
        )
    if type(value) not in _EXACT_INTEGER_SCALAR_TYPES:
        raise ShapeError(
            f"{operation}: diagonal must be a built-in or NumPy integer scalar, "
            f"got {type(value).__name__}"
        )
    rows, columns = shape[-2:]
    if rows == 0 or columns == 0:
        return 0
    minimum, maximum = (1 - rows, columns) if upper else (-rows, columns - 1)
...
class TensorProxy:
    ...
    def _triangular(self, diagonal: Any, *, upper: bool) -> "TensorProxy":
        operation = "triu" if upper else "tril"
        opcode = OP_TRIU if upper else OP_TRIL
        selector = np.triu if upper else np.tril
        if self.dtype not in _TRIANGULAR_DTYPES:
            raise ShapeError(f"{operation}: dtype {self.dtype!r} is not supported")
        diagonal_i = _normalize_triangular_diagonal(
            diagonal,
            self.shape,
            upper=upper,
        )
        ...
        def _bw(dy: np.ndarray, _ins) -> Tuple[Optional[np.ndarray], ...]:
            gradient = selector(dy, k=diagonal_i)
    ...
    def tril(self, diagonal: Any = 0) -> "TensorProxy":
        return self._triangular(diagonal, upper=False)

    def triu(self, diagonal: Any = 0) -> "TensorProxy":
        return self._triangular(diagonal, upper=True)
    ...
    def topk(
        ...
        return topk(self, k=k, dim=dim, largest=largest, sorted=sorted)
    ...
    def double(self) -> "TensorProxy":
        return self.cast("float64")

    def int(self) -> "TensorProxy":
    ...
    def to(self, *args: Any, **kwargs: Any) -> "TensorProxy":
        dtype_spec = kwargs.get("dtype") or (args[0] if args else None)
        if dtype_spec is None:
            return self
        try:
            dtype_name = np.dtype(dtype_spec).name
        except (TypeError, ValueError):
            return self
        return self.cast(dtype_name)

    def cpu(self) -> "TensorProxy":
        return self

    def cuda(self) -> "TensorProxy":
        return self
...
def topk(
    ...
    *,
    ...
    if type(x) is not TensorProxy:
        raise TypeError(f"topk: input must be a TensorProxy, got {type(x).__name__}")
    if out is not None:
        raise JitNotImplementedError(
            "topk: out= is not supported because lazy tensor mutation has no typed effect contract"
        )
    proxy = x
    axis, k_value, largest_flag, sorted_flag, output_shape = normalize_topk_request(
        proxy._uop,
        k,
        dim,
        largest,
...
def multinomial(input: Any, num_samples: int, replacement: bool = True) -> "TensorProxy":
    proxy = _to_proxy(input, None)
    probs = proxy._realize_array().astype(np.float32, copy=False)
    if probs.ndim == 1:
        total = probs.sum()
        if total <= 0:
        ...
        return from_numpy(idx.astype(np.int64), session=proxy._get_session())
    if probs.ndim == 2:
        rows = []
        for row in probs:
            total = row.sum()
            ...
            p /= p.sum()
            rows.append(np.random.choice(len(p), size=int(num_samples), replace=replacement, p=p))
        return from_numpy(np.array(rows, dtype=np.int64), session=proxy._get_session())
