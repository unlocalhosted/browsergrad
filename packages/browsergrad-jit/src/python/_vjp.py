...
@register_vjp(OP_SMOOTH_L1_LOSS)
def _vjp_smooth_l1_loss(
    output: UOp,
    inputs: Tuple[UOp, ...],
    dy: UOp,
) -> Tuple[Optional[UOp], ...]:
    contract = validate_smooth_l1_loss_contract(output)
    if inputs != output.inputs:
        raise ShapeError(
            "SMOOTH_L1_LOSS VJP inputs must equal the forward operands"
        )
    if dy.shape != output.shape or dy.dtype != output.dtype:
        raise ShapeError(
            "SMOOTH_L1_LOSS VJP upstream gradient must match the forward output"
        )
    gradients = []
    for operand, source in enumerate(inputs):
        gradients.append(_vjp_uop(
